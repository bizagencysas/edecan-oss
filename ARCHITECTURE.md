...
- **LLM no-IDE: Cloudflare Workers AI** mediante un adaptador
  OpenAI-compatible detrás de `LLMProvider`. El IDE conserva un runtime de
...
Interfaz única `LLMProvider` (§10.6). La composición activa usa
`WorkersAIProvider`, que delega el protocolo HTTP al adaptador
OpenAI-compatible de Cloudflare. `TaskRouter` decide automáticamente según
metadatos internos; ninguna persona o cliente selecciona proveedor o modelo.
Chat, voz, llamadas, herramientas ligeras y workers fuera del IDE usan
`@cf/zai-org/glm-4.7-flash`. El IDE queda fuera de esta frontera y una tarea
marcada como ingeniería se rechaza aquí en vez de degradarse al modelo de chat.
La factoría de proveedor es inyectable para reemplazar Cloudflare sin cambiar
agentes, telefonía o workers.
...
`WORKERS_AI_CHAT_MODEL` (default `@cf/zai-org/glm-4.7-flash`),
`WORKERS_AI_TIMEOUT_SECONDS` (default `60`), `EMBEDDINGS_MODEL`,
...
provider_factory=build_workers_ai_provider, provider=None, task_router=None)`
conserva `resolve(alias, tenant_flags)` para los agentes existentes, pero el
alias ya no permite seleccionar un modelo. `route(request, alias)` delega la
clasificación a `TaskRouter` y `complete(...)` reemplaza siempre
`request.model` por la decisión central.

`WorkersAIProvider` usa
`https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/v1`, autentica
con el token del host y fija
`chat_template_kwargs.enable_thinking=false`. Las credenciales de inferencia
no son credenciales por tenant y nunca se aceptan desde los clientes.
...
- `GET /v1/models/chat` (catálogo del selector del chat: `{default, esfuerzos, esfuerzo_default, modelos[]}`, leído de `config/modelos.yml` → `modelos_chat`) · `PUT /v1/conversations/{id}/model {model, effort}` → `{model, effort}` (el modelo es propiedad de la CONVERSACIÓN, no de la credencial del tenant — `PATCH /v1/credentials/llm/models` sigue en 409; `model: null` = automático, o sea la cadena `WORKERS_AI_CHAT_MODEL` → `MODELO_POR_DEFECTO` de siempre; `effort` se traduce al presupuesto de `max_tokens` por iteración del loop y solo aplica a los modelos con `soporta_esfuerzo`). `POST .../messages` acepta los mismos `model`/`effort` como override del turno que también persiste.
...
### 12.b Credenciales conectables por tenant (TokenVault)

Deepgram, ElevenLabs, imágenes, búsqueda y conectores continúan resolviéndose
por tenant con `edecan_db.vault.TokenVault` (§10.4). La inferencia LLM es la
excepción deliberada: desde la migración a Workers AI sus credenciales son
propiedad del host y no se leen ni escriben mediante el vault del tenant.
...
- `connector_key ∈ {"llm", "voice_stt", "voice_tts", "homeassistant", "whatsapp"}`
  ...
  varias cuentas por conector, aquí es singular). La clave histórica `"llm"`
  solo puede limpiarse por compatibilidad; nunca gobierna inferencia.
- `TokenBundle.access_token` (§10.5) guarda un **JSON serializado** con la
  configuración completa del proveedor para `"voice_stt"`/`"voice_tts"` —
  y el **token crudo** (string tal cual, sin envolver en
...
### 12.c Adaptadores alternativos y compatibilidad de datos

```python
@dataclass(frozen=True)
class LLMProviderConfig:
    kind: str  # adaptador alternativo o configuración histórica
...
`LLMProviderConfig` se conserva como tipo de datos para adaptadores opcionales
y lectura compatible de instalaciones antiguas. Ningún call site de primera
parte lo entrega al `LLMRouter` activo. La composición de producción se hace
exclusivamente mediante la `ProviderFactory` del host (§10.6).
...
si está fijado) + `<bin> --version` con timeout
...
(§12.a `setup`, fase v3) conserva esto como diagnóstico local compatible; las
aplicaciones de primera parte ya no lo presentan como selección de inferencia.
...
`"http://localhost:11434"`), `CLAUDE_CLI_PATH` (`str | None`, default
`None` = autodetectar en PATH), `LLM_CLI_TIMEOUT_SECONDS`
