...
- `get_redis`, `get_llm_router`, `get_vault`: construyen los colaboradores que
  entran en `ToolContext` (§10.7). `get_llm_router` entrega el router global
  administrado; el usuario no elige proveedor ni modelo.
- `rate_limit`: límite simple de 60 solicitudes/min por tenant (Redis INCR+EXPIRE).
- `ide_rate_limit`: bucket independiente de 300 solicitudes/min para el canal
  interactivo del IDE. Sus lectores por cursor hacen polling mientras una
  terminal o agente está vivo y no deben agotar el límite del resto de Edecán.
...
IDE_RATE_LIMIT_MAX_REQUESTS = 300
DELETED_USER_KEY_TTL_SECONDS = ACCESS_TOKEN_TTL_SECONDS + 24 * 60 * 60
...
def get_auth_redis(settings: Settings = Depends(get_settings)) -> redis_asyncio.Redis:
    """Cliente Redis para el denylist de cuentas eliminadas.

    Se declara antes de `get_current_user`; `_redis_client` se resuelve cuando
    FastAPI ejecuta la dependencia, después de cargar todo el módulo.
    """
...
async def _enforce_tenant_rate_limit(
    *,
    ...
    """Aplica una ventana fija aislada por tenant y por tipo de tráfico."""
    window = int(time.time()) // RATE_LIMIT_WINDOW_SECONDS
    key = f"ratelimit:{bucket}:{current_user.tenant_id}:{window}"
...
async def rate_limit(
    ...
    """Límite general por tenant: 60 solicitudes/min, ventana fija."""
...
async def ide_rate_limit(
    current_user: CurrentUser = Depends(get_current_user),
    redis_client: redis_asyncio.Redis = Depends(get_redis),
) -> None:
    """Límite independiente para streaming/polling y acciones del IDE."""
    await _enforce_tenant_rate_limit(
        current_user=current_user,
        redis_client=redis_client,
        bucket="ide",
        limit=IDE_RATE_LIMIT_MAX_REQUESTS,
    )
