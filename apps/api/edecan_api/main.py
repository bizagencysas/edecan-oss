...
# Prefixes de rutas que no requieren Bearer del túnel.  El endpoint
# ``/v1/files/public/`` se auto-autentica con firma HMAC-SHA256 (ver
# ``files.download_public_file``): Instagram, Facebook y Threads descargan
# el video/imagen desde esa URL cuando se publica en multi-red, y no
# llevan (ni pueden llevar) un token de dispositivo.
_TUNNEL_UNAUTHENTICATED_PREFIXES = ("/v1/files/public/",)

# Webhooks de Twilio: los llama Twilio, no un dispositivo emparejado, así que NUNCA traen
# `Authorization: Bearer`. Sin esta excepción el guardia del túnel los rechaza con 401 antes
# de que el router pueda mirarlos, y Twilio -- que solo ve "no pude obtener el guion" --
# cuelga a los pocos segundos: la llamada sale, suena, y se corta sin que nadie hable. Rompe
# por igual las salientes y las ENTRANTES (`/v1/phone/twilio/incoming`).
#
...
class LocalTunnelGuardMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if (
            ...
            # y firmado que emitió `authorize`. Sin esta exención el gate del
            # túnel lo tumba con 401 y la conexión de conectores nunca completa
            # detrás de Cloudflare.
            or (path.startswith("/v1/connectors/") and path.endswith("/callback"))
        ):
            return await call_next(request)
...
def create_app() -> FastAPI:
    ...
    def _companion_de_proceso(tenant_id: uuid.UUID):
        if not manager.is_connected(tenant_id):
            return None
        return functools.partial(manager.send_command, tenant_id)

    register_companion_factory(_companion_de_proceso)
    ...
    app.state.llm_router = LLMRouter(settings, on_usage=None)
    # Estado por proceso: evita insistir contra un proveedor caído y alimenta
    # el readiness/telemetría futura sin mezclar tenants ni credenciales.
    provider_health_store = None
    if settings.is_prod and settings.PROVIDER_HEALTH_PERSISTENCE:
        provider_health_store = ProviderHealthEventStore(get_session)
    app.state.provider_health_store = provider_health_store
    app.state.provider_health = ProviderHealth(
        event_sink=provider_health_store.enqueue if provider_health_store is not None else None
    )
    ...
    app.include_router(preparacion.router)
    ...
    if importlib.util.find_spec("edecan_premium") is not None:
        ...
        logger.info("edecan_premium detectado: rutas heredadas de telefonía Twilio montadas.")
    ...
    for name in V2_ROUTER_NAMES:
        ...
        if name == "remote":
            extra_realtime_router = getattr(mod, "realtime_router", None)
            if extra_realtime_router is not None:
                app.include_router(extra_realtime_router)
        logger.info("router v2 'edecan_api.routers.%s' montado.", name)

    # Always-on: identidad/configuración durable, sin ejecutar jobs.
    try:
        persistent_agents = importlib.import_module("edecan_api.routers.persistent_agents")
    except ImportError:
        logger.warning("router persistent_agents no disponible; se omite.")
    else:
    ...
    # Gimnasio (`/v1/gym`) — router SIN flag de plan (como `negocios`), montado
    # con el MISMO patrón defensivo que v2/v3/v4/v5/v6 para no acoplar su
    # aterrizaje al de los demás work packages.
    for name in ("gym",):
        try:
            mod = importlib.import_module(f"edecan_api.routers.{name}")
        except ImportError:
            logger.warning(
                "router 'edecan_api.routers.%s' no disponible todavía — se omite.",
                name,
            )
            continue
        app.include_router(mod.router)
