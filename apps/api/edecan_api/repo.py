...
MAIN_CONVERSATION_TITLE = "Actividad"
"""Título de la conversación "principal" (frente 5, paridad REFERENCIA): la que
recibe los eventos automáticos que el dueño no pidió -- llamada entrante,
automatización ejecutada, recordatorio disparado -- igual que el hilo de
avisos de REFERENCIA. `Repo.resolve_main_conversation` es el get-or-create único
para esa fila; ver su docstring en `SqlRepo` para la receta SQL exacta
(migración `0026_conversations_main`, columna `is_main` + índice único
parcial `uq_conversations_tenant_user_main`). Cualquier otro servicio con
acceso directo a la misma base (p. ej. `apps/worker/edecan_worker/repo.py`,
que ya usa el mismo patrón "buscar por título o crear" en
`handlers/send_reminder.py`) debe replicar EXACTAMENTE esa columna e índice
si necesita escribir en la misma conversación -- así dos procesos nunca
crean dos "principales" distintas por una carrera entre servicios.
"""
...
class Repo(Protocol):
    ...
    async def delete_user_account(self, user_id: uuid.UUID) -> dict[str, Any]: ...
    async def account_deletion_preflight(self, user_id: uuid.UUID) -> dict[str, Any]: ...
    async def list_user_missions(
        self, *, tenant_id: uuid.UUID, user_id: uuid.UUID, limit: int = 10_000
    ) -> list[Row]: ...
    async def list_user_automations(
        self, *, tenant_id: uuid.UUID, user_id: uuid.UUID, limit: int = 10_000
    ) -> list[Row]: ...
    async def list_user_devices(
        self, *, tenant_id: uuid.UUID, user_id: uuid.UUID, limit: int = 100
    ) -> list[Row]: ...
    ...
    async def search_conversations(
        self, *, tenant_id: uuid.UUID, user_id: uuid.UUID, query: str
    ) -> list[Row]: ...
    ...
    async def list_cross_chat_message_snippets(
        self,
        *,
        tenant_id: uuid.UUID,
        user_id: uuid.UUID,
        exclude_conversation_id: uuid.UUID,
        conversations_limit: int = 8,
    ...
    async def list_phone_call_events_since(
        self, *, tenant_id: uuid.UUID, call_id: uuid.UUID, after_id: uuid.UUID | None = None
    ) -> list[Row]: ...
    ...
    async def list_user_quality_feedback(
        self, *, tenant_id: uuid.UUID, user_id: uuid.UUID
    ) -> list[Row]: ...
    ...
    async def get_file_by_id(self, *, file_id: uuid.UUID) -> Row | None: ...
...
class SqlRepo:
    ...
    async def delete_user_account(self, user_id: uuid.UUID) -> dict[str, Any]:
        """Borra la identidad solo si no quedan dependencias externas sin limpiar."""
        preflight = await self.account_deletion_preflight(user_id)
        if preflight["blocked"]:
            return {"deleted": False, "blocked": preflight["blocked"]}
        deleted = await self._first(
            "DELETE FROM users WHERE id = :user_id RETURNING id", {"user_id": user_id}
        )
        return {"deleted": deleted is not None, "blocked": []}

    async def account_deletion_preflight(self, user_id: uuid.UUID) -> dict[str, Any]:
        """Inspecciona blockers sin mutar identidad, vault ni proveedores."""
        memberships = await self._all(
            """
            SELECT tenant_id, role,
                   COUNT(*) OVER (PARTITION BY tenant_id) AS member_count
        ...
        for membership in memberships:
            ...
            if await self._first(
                "SELECT 1 FROM files WHERE tenant_id = :tenant_id AND user_id = :user_id LIMIT 1",
                {"tenant_id": tenant_id, "user_id": user_id},
            ):
                blockers.add("files_s3")
            member_count = int(membership.get("member_count") or 0)
            if membership.get("role") == "owner" and member_count > 1:
                blockers.add("tenant_ownership")
            elif member_count <= 1:
                if await self._first(
                    "SELECT 1 FROM connector_accounts WHERE tenant_id = :tenant_id LIMIT 1",
                    {"tenant_id": tenant_id},
                ):
                    blockers.add("connector_credentials")
                if await self._first(
                    "SELECT 1 FROM subscriptions WHERE tenant_id = :tenant_id LIMIT 1",
                    {"tenant_id": tenant_id},
                ):
                    blockers.add("billing_subscription")
        return {"ready": not blockers, "blocked": sorted(blockers)}

    async def list_user_missions(
        self, *, tenant_id: uuid.UUID, user_id: uuid.UUID, limit: int = 10_000
    ) -> list[Row]:
        return await self._all(
            "SELECT id, tenant_id, user_id, objetivo, status, plan, resultado, presupuesto, "
            "error, archived_at, created_at, updated_at FROM agent_missions "
    ...
    async def list_user_automations(
        ...
        return await self._all(
            "SELECT id, tenant_id, user_id, nombre, descripcion, enabled, timezone, "
            "next_run_at, last_run_at, consecutive_failures, disabled_at, created_at, updated_at "
            "FROM automations WHERE tenant_id = :tenant_id AND user_id = :user_id "
            "ORDER BY created_at DESC LIMIT :limit",
            {"tenant_id": tenant_id, "user_id": user_id, "limit": limit},
        )
    ...
    async def list_user_devices(
        ...
        return await self._all(
            "SELECT id, tenant_id, user_id, nombre, plataforma, kind, status, last_seen_at, "
            "push_platform, paired_at, created_at, updated_at FROM devices "
            "WHERE tenant_id = :tenant_id AND user_id = :user_id "
            "ORDER BY created_at DESC LIMIT :limit",
            {"tenant_id": tenant_id, "user_id": user_id, "limit": limit},
        )
    ...
    async def get_main_conversation(
        ...
        return await self._first(
            ...
            WHERE tenant_id = :tenant_id AND user_id = :user_id AND is_main
            """,
            {"tenant_id": tenant_id, "user_id": user_id},
        )

    async def resolve_main_conversation(self, *, tenant_id: uuid.UUID, user_id: uuid.UUID) -> Row:
        """Get-or-create atómico de la conversación "principal" (ver
        `MAIN_CONVERSATION_TITLE`). Mismo idioma SQL que `set_local_owner`
        (arriba en este archivo): intenta el INSERT contra el índice único
        parcial `uq_conversations_tenant_user_main` (migración
        `0026_conversations_main`) con `ON CONFLICT ... DO NOTHING`. Si pierde
        la carrera -- ya existe una fila `is_main = true` para este
        tenant+usuario, insertada por otra request concurrente -- el INSERT
        no devuelve fila y se hace un SELECT de la que ya ganó. Nunca hay dos
        conversaciones principales para el mismo tenant+usuario ni una
        excepción de integridad sin manejar por la carrera.
        """
        row = await self._first(
            """
            INSERT INTO conversations (
                id, tenant_id, user_id, title, title_source, channel, is_main,
                created_at, updated_at
            )
            VALUES (
                :id, :tenant_id, :user_id, :title, 'auto', 'api', TRUE, :now, :now
            )
            ON CONFLICT (tenant_id, user_id) WHERE is_main
            DO NOTHING
            RETURNING *
            """,
            {
                ...
                "tenant_id": tenant_id,
                "user_id": user_id,
                "title": MAIN_CONVERSATION_TITLE,
                "now": _now(),
            },
        )
        if row is not None:
            return row
        existing = await self.get_main_conversation(tenant_id=tenant_id, user_id=user_id)
        assert existing is not None  # el índice único parcial garantiza que alguien ganó
        return existing
    ...
    async def search_conversations(
        ...
        Busca en títulos de conversaciones y contenido de mensajes
        usando ILIKE como fallback.
        """
        pattern = f"%{query}%"
        return await self._all(
            """
            ...
            WHERE c.tenant_id = :tenant_id
              ...
              AND (c.title ILIKE :pattern OR
                   m.content::text ILIKE :pattern)
            ORDER BY c.updated_at DESC
            LIMIT 20
            """,
            {"tenant_id": tenant_id, "user_id": user_id, "pattern": pattern},
        )
    ...
    async def update_conversation_model(
        ...
        *,
        ...
        """Fija (o limpia) la selección del selector de modelos del chat.

        Escribe las DOS columnas siempre: `model=None` es "volver a
        automático", un valor legítimo y no "no cambiar". Quien quiera
        conservar una de las dos manda el valor que ya tenía -- así el
        endpoint PUT es idempotente y no hay un tercer estado ambiguo.
        `updated_at` NO se toca: elegir modelo no es actividad de la
        conversación y no debería reordenar el historial del usuario.
        """

        return await self._first(
            """
            UPDATE conversations
            SET chat_model = :model, chat_effort = :effort
            WHERE tenant_id = :tenant_id AND user_id = :user_id AND id = :id
            RETURNING *
            """,
            {
    ...
    async def clear_conversation_context(
        ...
        """Comando local `/clear`: mueve el límite de contexto a AHORA.

        NO ES UN BORRADO -- ningún `Message` se toca. `list_messages(...,
        after=...)` es quien deja de devolver lo anterior al límite, tanto
        para `GET /{id}` (lo que ve la pantalla) como para el historial que
        arma el turno siguiente (`chat_context.build_contextual_history`).
        Repetir `/clear` es idempotente en efecto (mueve el límite de nuevo a
        AHORA, un poco más adelante) y nunca resucita mensajes ya excluidos.
        """
        return await self._first(
            """
            UPDATE conversations
            SET context_cleared_at = :now
            WHERE tenant_id = :tenant_id AND user_id = :user_id AND id = :id
            RETURNING *
            """,
            {
    ...
    async def update_message_content(
        ...
        *,
        ...
        return await self._first(
            """
            UPDATE messages
            SET content = :content ::jsonb, updated_at = :now
            WHERE tenant_id = :tenant_id
    ...
    async def list_messages(
        ...
        *,
        ...
        # El desempate por `id` no es decorativo desde que el cliente deriva del HILO si una
        # tarjeta de pregunta ya se contestó (`HiloDePreguntas` en iOS): una pregunta está
        # contestada si hay un mensaje del usuario DESPUÉS. Con dos filas de `created_at`
        # idéntico, Postgres puede devolver el par en cualquier orden, y si sale invertido
        # (respuesta, pregunta nueva) la pregunta recién llegada nace marcada como contestada
        # y el usuario no puede tocarla. Las otras dos consultas de mensajes de este archivo
        # ya desempataban así; esta se había quedado atrás.
        #
        # `after` (comando local `/clear`, `context_cleared_at` de la conversación):
        # el filtro va DENTRO del subselect, antes del `LIMIT`, para que el tope
        # siga contando "los últimos N tras el límite" y no "los últimos N de
        # siempre, filtrados después" -- lo segundo podría devolver menos de
        # `limit` filas útiles aunque el hilo tenga de sobra tras el corte.
        ...
        *,
        ...
        return await self._all(
            ...
            WITH recent_conversations AS (
                ...
                WHERE tenant_id = :tenant_id
                  AND user_id = :user_id
                  AND id <> :exclude_conversation_id
                ORDER BY updated_at DESC
            ...
            ranked_messages AS (
                ...
                FROM recent_conversations AS c
                JOIN messages AS m
            ...
            WHERE rn <= :messages_per_conversation
            ORDER BY conversation_updated_at DESC, conversation_id, created_at ASC
            """,
            {
                "tenant_id": tenant_id,
                "user_id": user_id,
    ...
    async def set_phone_call_summary_if_absent(
        ...
        *,
        ...
        """Guarda el primer resumen con contenido, o reemplaza uno vacío.

        Un StatusCallback prematuro puede dejar `transcript.available=false`.
        El cierre real de la llamada, ya con turnos, tiene que poder
        sustituirlo. Un resumen que SÍ tuvo transcripción no se toca.
        """
        return await self._first(
            """
            UPDATE phone_calls
    ...
    async def list_phone_call_events_since(
        ...
        """Eventos de una llamada más nuevos que `after_id` (frente 6b: cursor de
        polling barato para la transcripción en vivo -- ver `routers/phone.py
        get_call_transcript`).

        `id` es un UUID v4 (`_uuid()`), sin orden propio, así que la posición se ancla
        al par `(occurred_at, created_at)` del evento cursor -- el mismo criterio de
        orden que ya usa `list_phone_call_events` -- en vez de comparar `id`
        directamente. Si `after_id` es `None` o no pertenece a este tenant+llamada (p.ej.
        un cursor viejo de una llamada ya purgada), el `LEFT JOIN` no encuentra fila
        cursor y se cae al mismo contrato que `list_phone_call_events`: historial
        completo. Así el llamador nunca tiene que distinguir "sin cursor" de "cursor
        inválido" -- ambos devuelven algo usable en vez de una lista vacía engañosa.
        """
        if after_id is None:
            return await self.list_phone_call_events(tenant_id=tenant_id, call_id=call_id)
        return await self._all(
            """
            WITH cursor_event AS (
                SELECT occurred_at, created_at FROM phone_call_events
                WHERE tenant_id = :tenant_id AND call_id = :call_id AND id = :after_id
            )
            SELECT e.* FROM phone_call_events e
            LEFT JOIN cursor_event c ON TRUE
            WHERE e.tenant_id = :tenant_id AND e.call_id = :call_id
              AND (
                c.occurred_at IS NULL
                OR (e.occurred_at, e.created_at) > (c.occurred_at, c.created_at)
              )
            ORDER BY e.occurred_at ASC, e.created_at ASC
            """,
            {"tenant_id": tenant_id, "call_id": call_id, "after_id": after_id},
        )
    ...
    async def quality_snapshot_all_tenants_since(self, *, since: datetime) -> Row:
        """Agrega señales históricas durables sin leer contenido de usuarios.

        El dashboard de calidad debe distinguir métricas observadas de métricas
        aún no instrumentadas. Por eso esta consulta solo usa contadores,
        estados y tokens persistidos; nunca devuelve prompts, resultados ni
        argumentos de herramientas.
        """
        mission_rows = await self._all(
            """
            SELECT status, COUNT(*) AS total FROM agent_missions
            WHERE created_at >= :since GROUP BY status ORDER BY status
            """,
            {"since": since},
        )
        automation_rows = await self._all(
            """
            SELECT status, COUNT(*) AS total FROM automation_runs
            WHERE started_at >= :since GROUP BY status ORDER BY status
            """,
            {"since": since},
        )
        message_rows = await self._all(
            """
            SELECT role, COUNT(*) AS total,
                   COALESCE(SUM(tokens_in), 0) AS tokens_in,
                   COALESCE(SUM(tokens_out), 0) AS tokens_out
            FROM messages WHERE created_at >= :since GROUP BY role ORDER BY role
            """,
            {"since": since},
        )
        usage_rows = await self.sum_usage_all_tenants_since(since=since)
        cost_row = await self._first(
            """
            SELECT COALESCE(SUM(
                       CASE WHEN meta->>'cost_status' = 'known'
                            THEN (meta->>'estimated_cost_usd')::numeric
                            ELSE 0 END
                   ), 0) AS known_cost,
                   COUNT(*) FILTER (WHERE meta->>'cost_status' IS DISTINCT FROM 'known')
                       AS unknown_events
            FROM usage_events
            WHERE kind = 'llm_tokens' AND created_at >= :since
            """,
            {"since": since},
        )
        feedback_rows = await self._all(
            """
            SELECT meta->>'kind' AS kind, meta->>'category' AS category,
                   COUNT(*) AS total
            FROM audit_log
            WHERE action = 'quality.feedback_received' AND created_at >= :since
            GROUP BY meta->>'kind', meta->>'category'
            ORDER BY total DESC
            """,
            {"since": since},
        )
        ...
        missions = counts(mission_rows)
        automations = counts(automation_rows)
        messages = {
            str(row["role"]): {
                "total": int(row["total"]),
        ...
        terminal_tasks = sum(missions.get(status, 0) for status in ("done", "error", "cancelled"))
        successful_tasks = missions.get("done", 0)
        terminal_automations = sum(automations.get(status, 0) for status in ("done", "error"))
        successful_automations = automations.get("done", 0)
        return {
            "missions": {
                "by_status": missions,
                "success_rate": (successful_tasks / terminal_tasks if terminal_tasks else None),
            },
            "automations": {
                "by_status": automations,
                "success_rate": (
                    successful_automations / terminal_automations if terminal_automations else None
                ),
            },
            "messages": messages,
            "usage_by_tenant": usage_rows,
            "cost": {
                "status": (
                    "partial"
                    if int(cost_row["unknown_events"] or 0) > 0
                    else "known"
                    if cost_row and cost_row["known_cost"] is not None
                    else "unknown"
                ),
                "estimated_usd": (
                    float(cost_row["known_cost"])
                    if cost_row and cost_row["known_cost"] is not None
                    else None
                ),
                "unknown_events": int(cost_row["unknown_events"] or 0) if cost_row else 0,
            },
            "feedback": {
                "by_kind": {
                    str(row["kind"]): sum(
                        int(item["total"]) for item in feedback_rows if item["kind"] == row["kind"]
                    )
                    for row in feedback_rows
                },
                "by_category": {
                    str(row["category"]): sum(
                        int(item["total"])
    ...
    async def list_user_quality_feedback(
        ...
        return await self._all(
            """
            SELECT id, meta->>'kind' AS kind, meta->>'category' AS category,
                   meta->>'feedback_ref' AS feedback_ref, created_at
            FROM audit_log
            WHERE tenant_id = :tenant_id AND actor_user_id = :user_id
              AND action = 'quality.feedback_received'
