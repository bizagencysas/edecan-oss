...
def _public_run_detail(value: Any) -> dict[str, Any]:
    """Expone el trace de una corrida sin filtrar argumentos sensibles."""
    detail = _from_jsonb(value)
    public: dict[str, Any] = {}
    for key in ("status", "usage"):
        if key in detail:
            public[key] = detail[key]
    for key in ("resultado", "error"):
        if key in detail:
            public[key] = redact(str(detail[key]))
    pending = detail.get("pendiente")
    ...
    if isinstance(tool_log, list):
        public["tool_log"] = [
            {
                key: (
                    redact(str(event[key]))
                    if key in {"message", "text", "content"}
                    else event[key]
                )
                for key in ("type", "name", "status", "message", "text", "content")
                if key in event
            }
...
def _public_automation(row: dict[str, Any], *, settings: Settings) -> dict[str, Any]:
    ...
    elif trigger.get("kind") == "schedule":
        # `descripcion` es la `rrule` en cristiano ("Todos los días a las 4:00
        # UTC"), calculada acá y no en cada cliente: la frase lleva HORA, y
        # sólo este backend sabe en qué zona corre de verdad la regla (ver el
        # docstring de `edecan_automations.describe`). Puede venir `None`
        # cuando la regla no se puede traducir con certeza — las superficies
        # muestran la `rrule` cruda en ese caso, nunca una hora aproximada.
        #
        # `timezone=` es el huso REAL de esta fila (columna `automations.
        # timezone`, migración `0029_social_drafts_tz`): sin pasarlo, una
        # automatización sembrada como "09:00 America/Bogota" se rotularía
        # "a las 9:00 UTC" — la hora correcta con la zona equivocada, que es
        # justo el tipo de mentira a medias que este módulo existe para
        # evitar. `.get()` porque estas filas vienen de un `SELECT *` que
        # también corre contra una base sin la migración aplicada todavía;
        # ahí llega `None` y se rotula "UTC", que es el huso real de esa fila.
        trigger = {
            **trigger,
            "descripcion": describe_rrule(
...
async def list_automation_suggestions(
    ...
    for row in result.mappings().all():
        suggestion = suggest_automation_from_event(
            {
    ...
    return [
        {
            "id": str(row["id"]),
            "status": row["status"],
            "detalle": _public_run_detail(row.get("detalle")),
