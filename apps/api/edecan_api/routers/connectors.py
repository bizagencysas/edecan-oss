...
from edecan_connectors.social.linkedin import get_organization_urns
...
def _oauth_error_page(
    ...
    """Página HTML legible cuando el proveedor OAuth redirige al callback SIN
    `code`: el usuario canceló, o rechazó/no tiene los scopes pedidos — p. ej.
    LinkedIn manda `?error=unauthorized_scope_error` cuando la app no tiene
    aprobado el producto que otorga ese scope. Antes `code` era un query param
    obligatorio y FastAPI respondía un 422 "Field required" que no le explica
    NADA al usuario. La sirve el propio backend porque el web sólo lee `?ok`."""
    nombre = getattr(connector, "display_name", None) or "el conector"
    err = (error or "sin_codigo").strip()
    desc = (error_description or "").strip()

    pista = ""
    if err == "unauthorized_scope_error" or "scope" in err.lower():
        pista = (
            "Tu app de LinkedIn no tiene aprobado alguno de los permisos que Edecán "
            "pide para publicar en la página de empresa. Entra a "
            "<b>developer.linkedin.com/apps → tu app → Products</b> y agrega (esperando "
            "que queden aprobados): <b>Sign In with LinkedIn using OpenID Connect</b>, "
            "<b>Share on LinkedIn</b> y <b>Community Management API</b>. Cuando estén "
            "listos, vuelve a Edecán y reintenta la conexión."
        )
    elif err in {"user_cancelled_login", "user_cancelled_authorize"}:
        pista = "Cancelaste la autorización en LinkedIn. Puedes reintentar cuando quieras."

    detalle = (
        f"<p class='mono'>{err}{(' — ' + desc) if desc else ''}</p>" if (err or desc) else ""
    )
    pista_html = f"<p class='hint'>{pista}</p>" if pista else ""
    html = (
        "<!doctype html><html lang=\"es\"><head><meta charset=\"utf-8\">"
        "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">"
        f"<title>No se pudo conectar {nombre}</title><style>"
        ":root{color-scheme:light dark}"
        "body{font-family:-apple-system,system-ui,sans-serif;margin:0;display:grid;"
        "place-items:center;min-height:100vh;background:#0b0b0f;color:#e9e9ee}"
        ".card{max-width:520px;padding:2rem;text-align:center}"
        "h1{font-size:1.25rem;margin:0 0 .5rem}"
        ".mono{font-family:ui-monospace,monospace;font-size:.85rem;color:#ff9a9a;"
        "background:#1a1a22;padding:.5rem .75rem;border-radius:.5rem;word-break:break-word}"
        ".hint{color:#b9b9c6;line-height:1.5}b{color:#fff}"
        "</style></head><body><div class=\"card\">"
        f"<h1>No se pudo conectar {nombre}</h1>{detalle}{pista_html}"
        "<p class=\"hint\">Puedes cerrar esta ventana y volver a Edecán.</p>"
        "</div></body></html>"
    )
        ...
        try:
            async with httpx.AsyncClient(timeout=20.0) as http_client:
                ...
                if key == "linkedin":
                    ...
                    # Community Management: si el dueño autorizó los scopes de
                    # organización, capturamos los URN de las páginas que
                    # administra para poder publicar COMO la página. Se guardan
                    # en `bundle.extra` (cifrado con el resto). Best-effort: si
                    # no autorizó org o la API no devuelve nada, se publica como
                    # la persona (comportamiento previo), sin romper la conexión.
                    try:
                        org_urns = await get_organization_urns(http_client, bundle)
                        if org_urns:
                            bundle.extra["organization_urns"] = org_urns
                    except Exception:  # noqa: BLE001 - captura opcional
                        pass
        except ConnectorError as exc:
...
async def connect_twilio(
    ...
    async with httpx.AsyncClient(timeout=_TWILIO_VERIFY_TIMEOUT_SECONDS) as http_client:
        ...
        if phone_sid:
            # `twilio_webhook_base_url` y no `PUBLIC_BASE_URL`: esta URL queda
            # GUARDADA en la consola de Twilio y es la que él llamará cada vez
            # que entre una llamada. Tiene que apuntar al backend real, no a la
            # puerta siempre-viva que ve el teléfono del dueño (ver el porqué en
            # `Settings.twilio_webhook_base_url`).
            await _configure_twilio_incoming_webhook(
                account_sid,
                auth_token,
                phone_sid,
                f"{settings.twilio_webhook_base_url}/v1/phone/twilio/incoming",
                http_client=http_client,
                status_callback_url=f"{settings.twilio_webhook_base_url}/v1/phone/twilio/status",
            )
