...
# Id de destino de publicación: abierto (no `Literal["personal", "acme"]`), mismo
...
def _editorial_profile_key(platform: str, target: str | None) -> str:
    ...
    # con CUALQUIER id de destino de organización, no solo el literal "acme".
...
def _linkedin_account_matches_target(account: dict[str, Any], target: str) -> bool:
    return _resolve_destination(target).matches_connected_account(account)
...
async def create_social_content(
    ...
    if body.platform == "linkedin":
        # La tool no conoce "personal"/"acme": ese destino solo existe en
...
async def put_social_editorial_profile(
    ...
    profile_key = _editorial_profile_key(platform, target if platform == "linkedin" else None)
    saved = await save_editorial_profile(ctx, profile_key, body.model_dump())
...
async def publish_social_content(
    ...
    # `_linkedin_publish` de Aria (`features/acme_social.py`): si el destino
    ...
    if _resolve_destination(body.target).actor == "organization":
        ...
        # (`external_account_id`, p. ej. "urn:li:organization:119793977"), que
        # es como Aria lo tiene. Sin este fallback el post de la PÁGINA caía
...
async def _lock_social_draft(
    ...
    result = await session.execute(
        sql_text(
            "SELECT draft_id, user_id, platform, target, text, image_file_id, status, "
            "published_provider_id, verification FROM social_drafts "
            "WHERE tenant_id = :tenant_id ::uuid AND draft_id = :draft_id "
            "FOR UPDATE"
        ),
...
def _published_chat_text(
    ...
    """La frase que queda escrita en el chat. Portada de Aria (*"Publicado en
    tu X, usuario. ✅"*, `app.py:6060`) SIN el trato de "usuario": la formalidad es
    ...
    base = (
        f"Publicado en tu página de LinkedIn ({destination.label})."
        if destination.actor == "organization"
        else "Publicado en tu LinkedIn."
    )
    if verified == "confirmed":
        return f"{base} ✅"
    detail = f" ({verification_note})" if verification_note else ""
    return (
        f"{base} Ojo: no pude releerlo en LinkedIn para confirmar que quedó de verdad"
        f"{detail}. Entra a LinkedIn y revísalo tú mismo antes de darlo por publicado."
    )
...
async def publish_social_draft(
    ...
    # La confirmación se escribe en el chat, como hace Aria al colgar o al
