    ...
    *,
    ...
    if not response.get("ok", False):
        if response.get("lsp_no_disponible"):
            # ``LspOpencodeNoDisponibleError`` (``ide_opencode_lsp.py``, vía
            # ``ide_runtime.execute_ide_action``): la capacidad NO existe en la
            # superficie pública de opencode -- no es "esta vez falló" (422,
            # el resto de rechazos del companion), es "esto nunca va a
            # devolver datos hasta que opencode cambie". 501, no 500: el
            # servidor entendió la petición perfectamente, es la CAPACIDAD
            # remota la que falta, igual que exige la regla 4 del encargo.
            raise HTTPException(
                status_code=status.HTTP_501_NOT_IMPLEMENTED,
                detail=str(response.get("error") or "Esta capacidad de LSP no está disponible."),
            )
        raise HTTPException(
...
# ---------------------------------------------------------------------------
# Modo y esfuerzo de la sesión
#
# `ide_modos.py` ya tenía los stores (`EsfuerzoStore`, `ModoPlanificacionStore`)
# pero solo se podían tocar escribiendo `/effort` o `/plan` en el chat. Esto los
# expone para que la interfaz tenga su selector de modo y su control de esfuerzo.
#
# El esfuerzo NO es solo una preferencia de calidad: sube el `reasoning_effort`
# que se le pide al modelo Y el presupuesto de tokens en consecuencia. Sin lo
# segundo, subir el esfuerzo devuelve respuestas VACÍAS cobradas igual — el
# razonamiento se come el presupuesto de salida. Ver `max_tokens_para_esfuerzo`.
# ---------------------------------------------------------------------------


class ModoIn(BaseModel):
    """Ajustes de sesión. Todos opcionales; al menos uno tiene que venir.

    `modo` faltaba: el companion lleva tiempo aceptándolo (`ide_modo_set` ->
    `SessionManager.set_modo_agente`) y con opencode los cuatro modos frenan de
    verdad -- pero sin este campo no había forma de fijarlos desde la interfaz,
    así que el menú se mostraba deshabilitado. Otro caso del patrón que este
    proyecto arrastra: capacidad real en el motor, sin cable en la capa REST.
    """

    esfuerzo: Literal["bajo", "medio", "alto"] | None = None
    modo: Literal["manual", "aceptar_ediciones", "plan", "auto"] | None = None
    # `modo_plan` se conserva por compatibilidad con el motor viejo, donde
    # "plan" era un interruptor aparte y no uno de cuatro modos.
    modo_plan: bool | None = None
    motivo: str | None = Field(default=None, max_length=2000)


@advanced_router.get("/agents/{session_id}/modo")
async def get_modo(
    session_id: str,
    current_user: CurrentUser = Depends(get_current_user),
    companion_manager: ConnectionManager = Depends(get_companion_manager),
) -> dict[str, Any]:
    """Modo de planificación y nivel de esfuerzo vigentes en la sesión."""
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
        current_user.tenant_id,
...
@advanced_router.put("/agents/{session_id}/modo")
async def set_modo(
    session_id: str,
    body: ModoIn,
    current_user: CurrentUser = Depends(get_current_user),
    ...
    """Fija el esfuerzo, el modo de planificación, o ambos."""
    _require_companion_ide(current_user.tenant)
    if body.esfuerzo is None and body.modo is None and body.modo_plan is None:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT,
            detail="Indica «esfuerzo», «modo» o «modo_plan».",
        )
    return await _send_or_error(
        companion_manager,
        current_user.tenant_id,
        ...
        {
            "session_id": session_id,
            **({} if body.esfuerzo is None else {"esfuerzo": body.esfuerzo}),
            **({} if body.modo is None else {"modo": body.modo}),
            **({} if body.modo_plan is None else {"modo_plan": body.modo_plan}),
            **({} if body.motivo is None else {"motivo": body.motivo}),
        },
    )


# ---------------------------------------------------------------------------
# Modelo de la sesión (Hecho 2 del encargo)
#
# `SessionManager.set_modelo_agente` ya existía y aplicaba el modelo EN VIVO a
# una sesión de opencode viva -- pero sin ruta REST, el selector de la
# interfaz solo podía guardar la preferencia para la PRÓXIMA sesión. Este
# endpoint es el cable que faltaba, mismo patrón que `PUT .../modo` arriba.
# ---------------------------------------------------------------------------


class AgentModelIn(BaseModel):
    """`model` es obligatorio: a diferencia de `ModoIn` (donde cada campo es
    opcional porque el PUT puede tocar solo uno), esta ruta solo hace una
    cosa."""

    model: str = Field(min_length=1, max_length=200)


@advanced_router.post("/agents/{session_id}/model")
async def set_agent_model(
    ...
    """Cambia el modelo de la sesión. Si la sesión de opencode ya está viva,
    el companion lo aplica de inmediato (`SessionManager.set_modelo_agente`);
    si no, queda guardado para el arranque, igual que hoy.

    El catálogo permitido lo valida el companion (`modelo_ide_permitido`,
    dentro de `set_modelo_agente`) -- acá solo se valida aquí para el mismo
    caso que `post_agent` ya cubre: dar un 422 claro sin siquiera llamar al
    companion cuando el modelo no está en `modelos_ide_disponibles()`.
    """
    _require_companion_ide(current_user.tenant)
    if not modelo_ide_permitido(body.model):
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT,
            detail="Ese modelo no está habilitado para Forge Studio.",
        )
...
# ---------------------------------------------------------------------------
# Planes: aprobar, editar y rechazar
#
# El companion YA sabe ejecutarlos (`ide_sessions.approve_plan` reparte los
# pasos independientes entre sub-agentes vía `ide_reparto`/`ide_equipo`), pero
# hasta ahora no había endpoint para llamarlo: el botón "Aprobar" de la torre
# de control estaba deshabilitado con esa explicación a la vista, y un plan
# propuesto se quedaba colgado para siempre. Esto es ese cable.
# ---------------------------------------------------------------------------


class PlanEditIn(BaseModel):
    """Pasos corregidos por la persona ANTES de aprobar."""

    steps: list[str] = Field(min_length=1, max_length=60)
...
@advanced_router.get("/agents/{session_id}/plan")
async def get_active_plan(
    session_id: str,
    current_user: CurrentUser = Depends(get_current_user),
    companion_manager: ConnectionManager = Depends(get_companion_manager),
) -> dict[str, Any]:
    """El plan vivo de la sesión, si lo hay.

    Existía en el companion desde el principio con un docstring que decía
    literalmente "para que la UI sepa si debe mostrar la tarjeta de
    aprobación" — y nunca tuvo endpoint. Ese es el motivo de que un plan
    propuesto quedara invisible en la conversación: la interfaz no tenía cómo
    preguntar si había uno esperando.
    """
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
...
@advanced_router.post("/agents/{session_id}/plan/resume")
async def resume_plan(
    session_id: str,
    current_user: CurrentUser = Depends(get_current_user),
    companion_manager: ConnectionManager = Depends(get_companion_manager),
) -> dict[str, Any]:
    """Retoma un plan sin repetir los pasos que ya terminaron.

    Es la durabilidad del trabajo largo: si la sesión se corta a mitad de un
    plan de veinte pasos, retomar no rehace los quince que ya pasaron.
    """
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
...
@advanced_router.post("/agents/{session_id}/plan/{plan_id}/approve")
async def approve_plan(
    ...
    """Arranca la ejecución real del plan.

    No es una lectura: reparte los pasos entre sub-agentes que ESCRIBEN
    archivos, así que en el companion está en `_APPROVAL_ACTIONS` y pasa por
    el mismo gate humano que arrancar un agente.
    """
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
...
@advanced_router.post("/agents/{session_id}/plan/{plan_id}/edit")
async def edit_plan(
    session_id: str,
    plan_id: str,
    body: PlanEditIn,
    ...
    """Corrige el desglose mientras el plan sigue `proposed`."""
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
        current_user.tenant_id,
        "ide_plan_edit",
        {"session_id": session_id, "plan_id": plan_id, "steps": body.steps},
    )


@advanced_router.post("/agents/{session_id}/plan/{plan_id}/reject")
async def reject_plan(
    ...
    """Descarta el plan y deja la sesión libre para el siguiente mensaje."""
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
        current_user.tenant_id,
        "ide_plan_reject",
        {"session_id": session_id, "plan_id": plan_id, "reason": body.reason},
    )
...
# ---------------------------------------------------------------------------
# Preguntas y permisos del agente (motor opencode)
#
# El companion ya expone las cinco acciones (`ide_agent_question_list|answer|
# reject`, `ide_agent_permission_list|answer`) y la interfaz ya trae la tarjeta
# que las usa (`AgentThread.tsx::AgentQuestionCard`) -- pero NO había ninguna
# ruta REST entre las dos, así que pulsar "responder" daba 404. Es el mismo
# patrón que este proyecto arrastra: capacidad construida en los dos extremos,
# sin cable en medio. Estas rutas son ese cable.
#
# Sin ellas, el modo Manual es una trampa: opencode pausa esperando permiso, la
# persona ve el aviso, y no tiene forma de concederlo.
# ---------------------------------------------------------------------------


class QuestionReplyIn(BaseModel):
    """Una lista de respuestas por cada pregunta de la solicitud, en orden.

    Es lista de listas porque una sola solicitud de opencode puede traer varias
    preguntas, y cada una admitir varias respuestas (opción múltiple).
    """

    respuestas: list[list[str]] = Field(min_length=1, max_length=20)


class PermissionAnswerIn(BaseModel):
    conceder: bool
    # `recordar` lo puede rechazar el companion (`NoSePuedeRecordarError`) si la
    # acción es peligrosa o si opencode no la ofrece como recordable: se manda
    # tal cual y se deja que el que sabe decida, en vez de adivinar aquí.
    recordar: bool = False
    mensaje: str | None = Field(default=None, max_length=2000)


@advanced_router.get("/agents/{session_id}/question")
async def list_agent_questions(
    session_id: str,
    current_user: CurrentUser = Depends(get_current_user),
    companion_manager: ConnectionManager = Depends(get_companion_manager),
) -> dict[str, Any]:
    """Preguntas que el agente dejó esperando una respuesta humana."""
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
        current_user.tenant_id,
...
@advanced_router.post("/agents/{session_id}/question/{request_id}/reply")
async def reply_agent_question(
    session_id: str,
    request_id: str,
    body: QuestionReplyIn,
    current_user: CurrentUser = Depends(get_current_user),
    companion_manager: ConnectionManager = Depends(get_companion_manager),
) -> dict[str, Any]:
    """Responde la pregunta y deja que el turno siga desde donde se paró."""
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
        current_user.tenant_id,
...
@advanced_router.post("/agents/{session_id}/question/{request_id}/reject")
async def reject_agent_question(
    ...
    """Descarta la pregunta sin responderla."""
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
        current_user.tenant_id,
...
@advanced_router.get("/agents/{session_id}/permission")
async def list_agent_permissions(
    session_id: str,
    current_user: CurrentUser = Depends(get_current_user),
    companion_manager: ConnectionManager = Depends(get_companion_manager),
) -> dict[str, Any]:
    """Permisos que el turno dejó pendientes (modos Manual / Aceptar ediciones)."""
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
        current_user.tenant_id,
...
@advanced_router.post("/agents/{session_id}/permission/{request_id}/answer")
async def answer_agent_permission(
    session_id: str,
    request_id: str,
    body: PermissionAnswerIn,
    current_user: CurrentUser = Depends(get_current_user),
    companion_manager: ConnectionManager = Depends(get_companion_manager),
) -> dict[str, Any]:
    """Concede o deniega un permiso, y el MISMO turno continúa.

    Conceder deja al agente escribir archivos o correr comandos, así que es una
    acción con consecuencias reales -- por eso pasa por el mismo camino
    autenticado que el resto, no por una ruta de solo lectura.
    """
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
...
# ---------------------------------------------------------------------------
# LSP (motor opencode): símbolos, estado de servidores, definición, referencias
#
# `edecan_companion.ide_opencode_lsp.ClienteLspOpencode` ya envolvía `GET
# /find/symbol` y `GET /lsp` de opencode -- construido, verificado contra un
# `opencode serve` real, y SIN cable (mismo patrón que este proyecto lleva
# encontrando ronda tras ronda: capacidad construida, nadie la llama). Estas
# cuatro rutas son ese cable, todas GET porque las cuatro son lecturas.
#
# `definition`/`references` (LSP `textDocument/definition`/`references`) NO
# existen en la superficie pública de `opencode serve` 1.17.18 -- se enumeraron
# las ~150 rutas de `/doc` una por una y no aparecen bajo ninguna forma (ver el
# docstring de `ide_opencode_lsp.py`, puntos 3-4; ojo con `GET /api/reference`,
# que SÍ existe pero es un concepto no relacionado -- catálogo de documentos de
# referencia del proyecto, no referencias de código). Se dejan como rutas REST
# de todas formas porque el cliente ya expone un método que EXPLICA por qué no
# existen en vez de fallar mudo -- estas dos SIEMPRE responden 501 con ese
# mensaje real (ver `_send_or_error`), nunca datos inventados.
# ---------------------------------------------------------------------------


@advanced_router.get("/workspaces/{workspace_id}/lsp/status")
async def get_workspace_lsp_status(
    workspace_id: str,
    current_user: CurrentUser = Depends(get_current_user),
    companion_manager: ConnectionManager = Depends(get_companion_manager),
) -> dict[str, Any]:
    """Servidores de lenguaje que opencode reporta conectados para este workspace.

    Casi siempre vacío hoy, con o sin servidor de lenguaje instalado: la
    superficie HTTP pública de `opencode serve` no tiene ninguna ruta que le
    pida "abrí/indexá este archivo" antes de reportar conexiones (verificado
    con `pyright` funcionando de verdad, ver el docstring de
    `ide_opencode_lsp.py`) -- por eso la respuesta trae un campo `aviso`
    cuando `servidores` viene vacío, en vez de dejar que la interfaz lea `[]`
    como "no hay nada instalado".
    """
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
...
@advanced_router.get("/workspaces/{workspace_id}/lsp/symbols")
async def get_workspace_lsp_symbols(
    workspace_id: str,
    query: str = Query(..., min_length=1),
    current_user: CurrentUser = Depends(get_current_user),
    companion_manager: ConnectionManager = Depends(get_companion_manager),
) -> dict[str, Any]:
    """Símbolos del workspace cuyo nombre coincide con `query` (LSP
    `workspace/symbol`, vía `GET /find/symbol` de opencode).

    Misma advertencia que `get_workspace_lsp_status`: un resultado vacío no
    es evidencia de que el símbolo no exista -- ver el `aviso` que trae la
...
@advanced_router.get("/workspaces/{workspace_id}/lsp/definition")
async def get_workspace_lsp_definition(
    workspace_id: str,
    path: str = Query(..., min_length=1),
    line: int = Query(..., ge=0),
    ...
    """`textDocument/definition` de LSP -- NO existe en la superficie pública
    de `opencode serve` 1.17.18 (ver el docstring de `ide_opencode_lsp.py`,
    punto 3, para la verificación completa contra `/doc`). Esta ruta siempre
    responde `501`; no hay ningún workspace ni posición para los que devuelva
    una definición.
    """
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        ...
        current_user.tenant_id,
        "ide_lsp_definition",
        {"workspace_id": workspace_id, "path": path, "line": line, "character": character},
    )


@advanced_router.get("/workspaces/{workspace_id}/lsp/references")
async def get_workspace_lsp_references(
    workspace_id: str,
    path: str = Query(..., min_length=1),
    line: int = Query(..., ge=0),
    character: int = Query(..., ge=0),
    ...
    """`textDocument/references` de LSP -- NO existe en la superficie pública
    de `opencode serve` 1.17.18 (docstring de `ide_opencode_lsp.py`, punto 4:
    `GET /api/reference` EXISTE pero es un concepto distinto, no confundir).
    Esta ruta siempre responde `501`.
    """
    _require_companion_ide(current_user.tenant)
    return await _send_or_error(
        companion_manager,
