...
from edecan_api.routers.voice import _estimate_seconds_from_text, _stt_para_tenant, _tts_para_tenant
...
# -- susurro al oído (frente 6a, paridad REFERENCIA) -----------------------------------------
# REFERENCIA (Pipecat, tiempo real) inyecta el susurro apenas llega. Edecán conversa por
...
# Prefijo EXACTO que usa REFERENCIA (`voz_agent.py:641`) para que el modelo distinga una
# instrucción del dueño, inyectada a mitad de llamada, de algo que dijo el interlocutor.
PHONE_WHISPER_PREFIX = "[NOTA URGENTE DEL SEÑOR — incorpórala con naturalidad en tu respuesta]: "
...
def _wss_media_url(request: Request) -> str:
    return f"{https_to_wss(_base_para_twilio(request))}/v1/phone/twilio/media"


def _status_callback_url(request: Request) -> str:
    return f"{_base_para_twilio(request)}/v1/phone/twilio/status"


async def _puede_usar_realtime(
    request: Request,
    *,
    ...
    """Stream en vivo solo si Twilio puede abrir WSS y el tenant oye y habla de verdad."""
    settings = request.app.state.settings
    if not getattr(settings, "PHONE_REALTIME_ENABLED", True):
        return False
    if not _base_para_twilio(request).startswith("https://"):
        return False
    if vault is None:
        return False
    stt = await _stt_para_tenant(vault, repo, tenant_id, settings)
    tts = await _tts_para_tenant(vault, repo, tenant_id, settings)
    return isinstance(stt, DeepgramSTT) and isinstance(tts, ElevenLabsTTS)
...
def _transcript_events_from_messages(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Reconstruye turnos si los eventos aún no son visibles (otra transacción)."""
    events: list[dict[str, Any]] = []
    for row in rows:
        text = _message_text(row)
        role = row.get("role")
        if role == "user" and text:
            events.append({"event_type": "transcript", "payload": {"role": "caller", "text": text}})
        elif role == "assistant" and text:
            events.append(
                {"event_type": "transcript", "payload": {"role": "assistant", "text": text}}
            )
    return events
...
def _summary_has_transcript(summary: Any) -> bool:
    if not isinstance(summary, dict):
        return False
    return bool((summary.get("transcript") or {}).get("available"))
...
def _summary_narration_prompt(
    ...
    ("señor" vs. tuteo) NO se escribe aquí; sale de `formalidad` de la persona del tenant
...
# -- el resumen escrito, en el chat, sin pedirlo (§3C del plan REFERENCIA 2.0) --------------
# El resumen ya se redactaba bien y se guardaba en `phone_calls.summary`, pero MORÍA ahí:
# solo aparecía entrando a Actividad. El listón del dueño es el de REFERENCIA: al colgar le
...
# una llamada que nunca ocurrió (regla dura portada de REFERENCIA: jamás fingir que algo pasó).
...
async def _write_phone_summary_to_main_chat(
    ...
    REFERENCIA deja lo que uno no pidió.
...
async def _finalize_phone_call_summary(
    ...
    if _summary_has_transcript(call.get("summary")):
        return PhoneSummaryDelivery(created=False)
    events = await repo.list_phone_call_events(tenant_id=call["tenant_id"], call_id=call["id"])
    # Solo si la llamada SÍ conectó: un `failed`/`no_answer` no debe reciclar
    # el saludo sembrado en el hilo como si hubiera habido conversación.
    if (
        not _phone_call_turns(events)
        and call.get("conversation_id")
        and str(call.get("status") or "") == "completed"
    ):
        ...
        events = list(events) + _transcript_events_from_messages(rows)
...
async def _cerrar_llamada_media(
    owner: Request | WebSocket,
    *,
    call: dict[str, Any],
    live: LiveCall | None = None,
    provider_status: str = "completed",
) -> PhoneSummaryDelivery:
    """Cierra la llamada cuando el Media Stream termina: status + resumen + chat."""
    duration = None
    if live is not None:
        duration = max(0, int(datetime.now(UTC).timestamp() - live.iniciada))
    delivery = PhoneSummaryDelivery(created=False)
    async with _repo_transactions(owner)(call["tenant_id"]) as tenant_repo:
        current = await tenant_repo.get_phone_call(tenant_id=call["tenant_id"], call_id=call["id"])
        if current is None:
            return delivery
        effective = _monotonic_status(str(current.get("status") or ""), provider_status)
        fields: dict[str, Any] = {"status": effective}
        if effective in TERMINAL_STATUSES:
            if current.get("ended_at") is None:
                fields["ended_at"] = datetime.now(UTC)
            if duration is not None and current.get("duration_seconds") is None:
                fields["duration_seconds"] = duration
        updated = await tenant_repo.update_phone_call(
            tenant_id=call["tenant_id"], call_id=call["id"], fields=fields
        )
        if updated is None:
            return delivery
        await tenant_repo.add_phone_call_event(
            ...
            call_id=call["id"],
            event_type="status",
            payload={"status": effective, "provider_status": provider_status, "source": "media"},
        )
        delivery = await _finalize_phone_call_summary(
            tenant_repo, updated, llm=owner.app.state.llm_router
        )
    return delivery
...
def _twilio_cuentas_activas(cuentas: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        row
        for row in cuentas
        if row.get("connector_key") == TWILIO_CONNECTOR_KEY
...
async def _credentials_para_cuenta(
    vault: Any, tenant_id: uuid.UUID, account: dict[str, Any]
) -> TwilioCredentials:
...
async def _credentials(repo: Repo, vault: Any, tenant_id: uuid.UUID) -> TwilioCredentials:
    ...
    return await _credentials_para_cuenta(vault, tenant_id, account)
...
def _repo_transactions(owner: Request | WebSocket) -> RepoTransactionFactory:
    override = getattr(owner.app.state, "phone_repo_transaction_factory", None)
    if override is not None:
        return override

    @asynccontextmanager
    async def transaction(tenant_id: uuid.UUID) -> AsyncIterator[Repo]:
        async with owner.app.state.get_session(tenant_id) as session:
...
async def setup_incoming_calls(
    ...
    accounts = _twilio_cuentas_activas(
        list(await repo.list_connector_accounts(tenant_id=current_user.tenant_id))
    )
    if not accounts:
        raise HTTPException(
            status_code=409,
            detail="Conecta tu propio número de Twilio en Configuración antes de recibir llamadas.",
        )
    ...
    async with httpx.AsyncClient(timeout=12.0) as http_client:
        for account in accounts:
            credentials = await _credentials_para_cuenta(vault, current_user.tenant_id, account)
            ...
            await _configure_twilio_incoming_webhook(
                ...
                incoming_url,
                http_client=http_client,
                status_callback_url=f"{_base_para_twilio(request)}/v1/phone/twilio/status",
            )
            configured.append(credentials.phone_number)
    await repo.add_audit_log(
        tenant_id=current_user.tenant_id,
        actor_user_id=current_user.user_id,
        action="phone.incoming_configured",
        target=", ".join(configured),
        meta={"provider": "twilio", "phone_numbers": configured},
    )
    return {
        "status": "ready",
        "phone_number": configured[0],
        "phone_numbers": configured,
        "agent_name": inbound_agent["agent_name"],
        "agent_template_name": inbound_agent["name"],
        "webhook_url": incoming_url,
    }


@router.get("/calls/activa", dependencies=[Depends(rate_limit)])
async def get_active_call(
    current_user: CurrentUser = Depends(get_current_user), repo: Repo = Depends(get_repo)
) -> dict[str, Any]:
    """Llamada en curso + transcripción acumulada, para verla en vivo (polling 1-2s)."""
    _require_telephony(current_user)
    rows = await repo.list_phone_calls(
        tenant_id=current_user.tenant_id, user_id=current_user.user_id
    )
    activa = next(
        (row for row in rows if row.get("status") in {"queued", "ringing", "in_progress"}),
        None,
    )
    if activa is None:
        return {"activa": False}
    live = live_call(str(activa["id"]))
    if live is not None:
        snap = live.snapshot()
        snap["call"] = _out(activa)
        return snap
    events = await repo.list_phone_call_events(
        tenant_id=current_user.tenant_id, call_id=activa["id"]
    )
    transcript = []
    for event in events:
        if event.get("event_type") != "transcript":
        ...
        transcript.append(
            {
                "quien": "cliente" if payload.get("role") == "caller" else "agente",
                "texto": texto,
    ...
    return {
        "activa": True,
        "call_id": str(activa["id"]),
        "sid": activa.get("provider_call_sid") or "",
        "numero": activa.get("from_e164")
        if activa.get("direction") == "incoming"
        else activa.get("to_e164"),
        "direccion": "in" if activa.get("direction") == "incoming" else "out",
        "status": activa.get("status"),
...
async def queue_call_whisper(
    ...
    Paridad con REFERENCIA (frente 6a), adaptada a que Edecán conversa por turnos: este
    ...
    enqueue_whisper(str(call_id), body.text)
    return {
        "id": event["id"],
        "call_id": call_id,
        "text": body.text,
...
async def _consentimiento_vigente(repo: Any, *, tenant_id: Any, phone_e164: str) -> bool:
    ...
    if not _requiere_consentimiento_del_destinatario():
        return True
    return await repo.has_phone_consent(tenant_id=tenant_id, phone_e164=phone_e164, kind="voice")
...
def _phone_system_prompt(call: dict[str, Any], persona: Any) -> str:
    """Prompt de la llamada: la plantilla manda, no el cerebro de chat.

    `build_system_prompt` mete arquitectura cognitiva + speech tags + el
    ejemplo MAL «¿En qué puedo ayudarte?». El modelo rápido copia eso y
    tira el prompt de Asistente de Recepción (40k caracteres) como si no existiera.
    """
    name = str(call.get("agent_name") or getattr(persona, "nombre_asistente", None) or "Edecan")
    return "\n".join(
        [
            f"Eres {name}. Estás en una llamada telefónica en español.",
            "Habla en frases cortas y naturales. Sin Markdown, sin menús, sin herramientas.",
            "Ya te presentaste al contestar. No vuelvas a saludar de cero.",
            "PROHIBIDO preguntar «¿en qué puedo ayudarte?» o variantes. Contesta lo que "
            "la persona acaba de decir. Si solo saludó, responde el saludo y retoma tu "
            "pregunta pendiente (quién es, qué necesita de verdad).",
            "No escribas speech tags ni acotaciones entre corchetes: el audio las leería.",
            _phone_operating_context(call),
            "Recuerda: eres "
            + name
            + ". Sigue las instrucciones de la plantilla. No eres un chatbot genérico.",
        ]
    )
...
async def _phone_reply(
    ...
    *,
    ...
    # `owner` es Request en Gather o WebSocket en Media Streams. No construir
    # `Request(websocket.scope)`: Starlette exige scope HTTP y tira AssertionError,
    # Twilio ve 31921 y la llamada muere en un segundo.
    override = getattr(owner.app.state, "phone_turn_runner", None)
    if override is not None:
    ...
    get_session = owner.app.state.get_session
    tenant_id = call["tenant_id"]
    async with get_session(tenant_id) as session:
        ...
        llm = owner.app.state.llm_router
        ...
        # `persist` del Media Stream escribe en OTRA transacción. Si aún no
        # committeó, `list_messages` no trae lo que la persona acaba de decir.
        # Sin este turno el modelo saluda de cero y parece sorda.
        speech_text = " ".join(speech.split()).strip()
        last = messages[-1] if messages else None
        if speech_text and (last is None or last.role != "user" or last.content != speech_text):
            messages.append(ChatMessage(role="user", content=speech_text))
        ...
        response = await llm.complete(
            ...
            CompletionRequest(
                model="",
                system=_phone_system_prompt(call, persona),
                messages=messages,
                max_tokens=180,
                temperature=0.3,
...
async def outgoing_voice(
    ...
    if await _puede_usar_realtime(request, vault=vault, repo=repo, tenant_id=call["tenant_id"]):
        return Response(
            stream_twiml(
                stream_url=_wss_media_url(request),
                call_id=str(call["id"]),
                from_number=str(call.get("to_e164") or ""),
                inbound=False,
            ),
            media_type="application/xml",
        )
...
async def incoming_voice(
    ...
    if await _puede_usar_realtime(request, vault=vault, repo=repo, tenant_id=tenant_id):
        return Response(
            stream_twiml(
                stream_url=_wss_media_url(request),
                call_id=str(call["id"]),
...
async def twilio_media_stream(
    ...
    """Twilio Media Streams: audio en vivo, transcripción y respuesta continua."""
    ...
    while True:
        ...
        if payload.get("event") == "start":
            start = parse_stream_start(payload)
            break
        if payload.get("event") in {"stop", "closed"}:
            return
    if start is None:
        return
    call_uuid = as_call_uuid(start["call_id"])
    call = None
    if start.get("call_sid"):
        call = await repo.get_phone_call_by_provider_sid(provider_call_sid=start["call_sid"])
    if call is None and call_uuid is not None:
        call = await repo.get_phone_call_global(call_id=call_uuid)
    if call is None:
        await websocket.close(code=1008)
        return

    settings = websocket.app.state.settings
    stt = await _stt_para_tenant(vault, repo, call["tenant_id"], settings)
    tts = await _tts_para_tenant(vault, repo, call["tenant_id"], settings)
    if not isinstance(stt, DeepgramSTT) or not isinstance(tts, ElevenLabsTTS):
        await websocket.close(code=1013)
        return

    inbound = bool(start.get("inbound")) or call.get("direction") == "incoming"
    persona = _external_phone_persona(
        await repo.get_persona(tenant_id=call["tenant_id"], user_id=call["user_id"])
    )
    agent_name = call.get("agent_name") or persona.nombre_asistente
    ...
    if not opening:
        opening = (
            f"Hola, soy {agent_name}. Esta conversación quedará registrada. ¿En qué te ayudo?"
            if inbound
            else f"Hola. Soy {agent_name}, un asistente automatizado. El motivo es: {call['goal']}"
        )

    live = register_live_call(
        LiveCall(
            call_id=str(call["id"]),
            provider_sid=start.get("call_sid") or str(call.get("provider_call_sid") or ""),
            numero=str(call.get("from_e164") if inbound else call.get("to_e164") or ""),
            direccion="in" if inbound else "out",
        )
    )
    voice_id = str(call.get("voice_id") or getattr(tts, "_default_voice_id", "") or "")
    deepgram = DeepgramLive(stt._api_key)

    async def persist(quien: str, texto: str) -> None:
        # El `repo` del WebSocket vive en UNA transacción hasta colgar. Si
        # escribimos ahí, el cierre (otra sesión) no ve ni una palabra y
        # Actividad dice que no quedó registro. Cada turno se committea solo.
        role = "caller" if quien == "cliente" else "assistant"
        async with _repo_transactions(websocket)(call["tenant_id"]) as tenant_repo:
            await tenant_repo.add_phone_call_event(
                tenant_id=call["tenant_id"],
                call_id=call["id"],
                event_type="transcript",
                payload={"role": role, "text": texto},
            )
            await tenant_repo.add_message(
                tenant_id=call["tenant_id"],
                conversation_id=call["conversation_id"],
                role="user" if quien == "cliente" else "assistant",
                content={"text": texto},
            )

    async def reply(speech: str) -> str:
        return await _phone_reply(websocket, call=call, repo=repo, speech=speech)

    async def speak(text: str) -> bytes:
        return await synthesize_ulaw_8000(
            api_key=tts._api_key,
            voice_id=voice_id,
            text=text,
            model_id=str(getattr(tts, "_model_id", None) or "eleven_flash_v2_5"),
        )

    try:
        await deepgram.connect()
        await pump_twilio_audio(
            websocket,
        ...
        logger.warning(
            "phone_media_stream_failed call_id=%s tenant_id=%s",
            call.get("id"),
            call.get("tenant_id"),
            exc_info=True,
        )
        ...
        drop_live_call(str(call["id"]))
        try:
            delivery = await _cerrar_llamada_media(websocket, call=call, live=live)
            if delivery.created:
                await _enqueue_phone_summary_push(
            ...
            logger.warning(
                "phone_media_hangup_failed call_id=%s tenant_id=%s",
                call.get("id"),
                call.get("tenant_id"),
                exc_info=True,
            )
...
async def inbound_number_status(request: Request, repo: Repo = Depends(get_platform_repo)) -> None:
    """StatusCallback del número: Twilio no manda call_id, solo CallSid."""
    params = await _form(request)
    call = await repo.get_phone_call_by_provider_sid(provider_call_sid=params.get("CallSid", ""))
    if call is None:
        return
    await call_status(call["id"], request, repo)
...
async def call_status(
    ...
    if call["id"] != call_id:
        raise HTTPException(status_code=404, detail="Llamada no encontrada.")
    await _verify_webhook(request, params=params, repo=repo, tenant_id=call["tenant_id"])
    # El <Stream statusCallback> reusa esta ruta y manda StreamEvent sin
    # CallStatus. Eso se normalizaba a failed y mataba el resumen.
    if str(params.get("StreamEvent") or "").strip():
        return
    raw_status = str(params.get("CallStatus") or "").strip()
    if not raw_status:
        return
    provider_status = normalize_twilio_status(raw_status)
...
# -- transcripción en vivo (frente 6b, paridad REFERENCIA) -----------------------------------
# REFERENCIA transmite la transcripción por WebSocket apenas Pipecat la produce. Edecán
...
async def get_call_transcript(
    ...
    """Turnos de la transcripción en vivo desde un cursor (frente 6b, paridad REFERENCIA).
    ...
    `calls/{call_id}`: 404 si la llamada no es de este tenant+usuario (REFERENCIA: paridad de
