...
def _summary_narration_prompt(
    ...
    system = "\n".join(
        [
            ...
            "Escribe la narración en tercera persona sobre el interlocutor: cómo se llama y de "
            "qué organización habla (solo si lo dijo), el motivo real de la llamada, cualquier "
            "dato accionable que haya dejado (correos, teléfonos, direcciones, montos) y qué "
            "queda pendiente de decidir. Un párrafo breve y natural, como lo reportaría un "
            "asistente ejecutivo real — nunca una lista genérica de temas.",
            ...
            "afirmes compromisos, envíos o acciones que la transcripción no respalde.",
            contexto,
            "Responde ÚNICAMENTE un objeto JSON, sin texto antes ni después, con esta forma "
            'exacta: {"key_points": ["..."], "commitments": ["..."], "next_steps": ["..."]}',
            "- key_points: de 1 a 3 frases con la narración descrita arriba.",
...
async def call_status(
    ...
    async with _repo_transactions(request)(call["tenant_id"]) as tenant_repo:
        ...
        summary_created = await _finalize_phone_call_summary(
