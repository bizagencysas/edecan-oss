...
mime, origin_x, origin_y}` (`format` acepta png/jpeg/webp). Ese WP aterrizó
...
from edecan_db.session import get_session as db_get_session
...
realtime_router = APIRouter(prefix="/v1/remote", tags=["remote-realtime"])
...
_REMOTE_WS_AUTH_TIMEOUT_SECONDS = 10.0
_REMOTE_WS_FRAME_INTERVAL_SECONDS = 0.25


def _remote_user_from_token(token: str, settings: Settings) -> CurrentUser:
    try:
...
# Visor del teléfono: WebP 1600, no PNG Retina. JPEG queda de respaldo en el companion.
_FORMATO_FRAME_REMOTO = "webp"
_CALIDAD_FRAME_REMOTO = 68
_ANCHO_FRAME_REMOTO = 1600


def _params_captura_remota(
    *,
    session_id: uuid.UUID,
    quality: int = _CALIDAD_FRAME_REMOTO,
    max_width: int = _ANCHO_FRAME_REMOTO,
) -> dict[str, Any]:
    return {
        "format": _FORMATO_FRAME_REMOTO,
        "quality": quality,
        "max_width": max_width,
...
class PointerInputIn(BaseModel):
    ...
    `nx`/`ny` (y `start_nx`/`start_ny` para `drag`) son ADITIVOS y opcionales:
    la coordenada NORMALIZADA 0.0..1.0 del toque dentro del frame. Cuando
    llegan, el companion las resuelve contra el tamaño real del display en
    las unidades de su backend de input (puntos lógicos en macOS) y las usa en
    lugar de `x`/`y` — que es lo único que hace que un toque aterrice donde el
    dedo lo puso, porque el frame que ve el teléfono viaja reducido a
    `max_width` (1600 por defecto) y no coincide ni con los píxeles Retina de
    la captura ni con los puntos que consume `CGEvent`.

    `x`/`y` siguen siendo OBLIGATORIOS y no cambian de significado: son el
    único camino para los clientes que todavía no mandan la fracción (el panel
    web, versiones viejas de la app móvil)."""
    ...
    nx: float | None = Field(
        ...
        le=1.0,
        description=(
            "Fracción horizontal 0.0..1.0 del toque dentro del frame. Si llega "
            "(junto con 'ny'), el companion la resuelve contra el ancho real "
            "del display e IGNORA 'x'."
        ),
    )
    ny: float | None = Field(
        default=None,
        ge=0.0,
        le=1.0,
        description="Fracción vertical 0.0..1.0. Se manda siempre junto con 'nx'.",
    )
    start_nx: float | None = Field(
        default=None, ge=0.0, le=1.0, description="Equivalente normalizado de 'start_x' (drag)."
    )
    start_ny: float | None = Field(
        default=None, ge=0.0, le=1.0, description="Equivalente normalizado de 'start_y' (drag)."
    )
    ...
    def _validate_action_payload(self) -> PointerInputIn:
        ...
        # Medio par normalizado dejaría el gesto partido entre dos espacios de
        # coordenadas distintos — el mismo error que este campo viene a
        # arreglar. Mismo criterio EXACTO que
        # `edecan_companion.actions._read_normalized_pair`.
        if (self.nx is None) != (self.ny is None):
            raise ValueError("nx y ny se mandan juntos o no se manda ninguno")
        if (self.start_nx is None) != (self.start_ny is None):
            raise ValueError("start_nx y start_ny se mandan juntos o no se manda ninguno")
        return self
...
async def get_frame(
    ...
    return {
        ...
        "capture_mode": (
            result_data.get("capture_mode", "unknown")
...
async def send_input(
    ...
    if isinstance(body, PointerInputIn):
        ...
        # Aditivo: solo viajan si el cliente las mandó, así el payload de un
        # cliente viejo llega al companion EXACTAMENTE igual que antes.
        if body.nx is not None and body.ny is not None:
            params["nx"] = body.nx
            params["ny"] = body.ny
        if body.start_nx is not None and body.start_ny is not None:
            params["start_nx"] = body.start_nx
            params["start_ny"] = body.start_ny
...
@realtime_router.websocket("/sessions/{session_id}/stream")
async def stream_screen(
    websocket: WebSocket,
    session_id: uuid.UUID,
) -> None:
    """Stream persistente de frames del companion, con backpressure.

    El cliente autentica en el primer frame; después el servidor pide el último
    frame a ritmo fijo de 4 FPS. El companion/SCStream mantiene la captura viva
    y esta ruta solo distribuye el último PNG, sin acumular una cola de frames.
    El contador durable se actualiza en una transacción corta como máximo una
    vez por segundo.
    """
    ...
    try:
        try:
            first = await asyncio.wait_for(
                websocket.receive_json(), timeout=_REMOTE_WS_AUTH_TIMEOUT_SECONDS
            )
        ...
        if not isinstance(token, str) or not token.strip():
            ...
            return
        try:
            current_user = _remote_user_from_token(token, settings)
        except ValueError:
            await websocket.close(code=4401)
            return
        if not current_user.tenant.flags.get(FLAG_REMOTE_VIEW, False):
        ...
        async with db_get_session(current_user.tenant_id) as db_session:
            repo = SqlRepo(db_session)
            remote_session = await repo.get_remote_session(
                tenant_id=current_user.tenant_id, session_id=session_id
            )
        if remote_session is None:
            await websocket.close(code=4404)
            return
        if remote_session["status"] == "ended":
            await websocket.close(code=4409)
            return
        manager = _get_companion_manager(websocket)
        if manager is None or not manager.is_connected(current_user.tenant_id):
        ...
        await websocket.send_json(
            {
                "type": "ready",
                "protocol": "edecan.remote.stream.v1",
                "session_id": str(session_id),
                "interval_ms": int(_REMOTE_WS_FRAME_INTERVAL_SECONDS * 1000),
            }
        )
        sequence = int(remote_session.get("frames_count") or 0)
        last_persist = 0.0

        async def capture_once() -> None:
            nonlocal sequence, last_persist
            result = await manager.send_command(
                current_user.tenant_id,
                _SCREENSHOT_ACTION,
                _params_captura_remota(session_id=session_id),
            )
            ...
            if (
                ...
                raise RuntimeError("captura remota inválida")
            # Comprueba que el payload realmente sea base64 antes de publicarlo.
            try:
                base64.b64decode(image_b64, validate=True)
            except (ValueError, base64.binascii.Error) as exc:
                raise RuntimeError("captura remota no codificada correctamente") from exc
            sequence += 1
            now = time.monotonic()
            if now - last_persist >= 1.0:
                async with db_get_session(current_user.tenant_id) as frame_db:
                    await SqlRepo(frame_db).record_remote_session_frame(
                        tenant_id=current_user.tenant_id, session_id=session_id
                    )
                last_persist = now
            await websocket.send_json(
        ...
        while True:
            try:
                message = await asyncio.wait_for(
                    websocket.receive_json(), timeout=_REMOTE_WS_FRAME_INTERVAL_SECONDS
                )
            except TimeoutError:
                await capture_once()
                continue
        ...
        logger.warning("fallo en stream remoto de pantalla", exc_info=True)
