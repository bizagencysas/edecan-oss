...
# Access token TTL largo (30 días) para instalación local 24/7 estilo Referencia:
# el móvil no necesita refrescar el token cada 30 min (el refresh POST no llega
# de forma fiable por el túnel de Cloudflare). El token vive en el Keychain del
# iPhone (device-bound). Después de 30 días el usuario re-empareja.
