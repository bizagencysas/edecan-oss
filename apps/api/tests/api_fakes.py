...
class FakeRepo:
    ...
    async def delete_user_account(self, user_id: uuid.UUID) -> dict[str, Any]:
        ...
        blockers = set(preflight["blocked"])
        if blockers:
            return {"deleted": False, "blocked": sorted(blockers)}
        if user_id not in self.users:
            return {"deleted": False, "blocked": []}
        del self.users[user_id]
        self.memberships[:] = [row for row in self.memberships if row["user_id"] != user_id]
        self.personas = {key: row for key, row in self.personas.items() if key[1] != user_id}
        for collection in (
            self.memory_items,
            self.reminders,
            self.contacts,
            self.transactions,
            self.phone_agent_templates,
        ):
            for row_id in [
                row_id for row_id, row in collection.items() if row.get("user_id") == user_id
            ]:
                del collection[row_id]
        for conversation_id in [
            row_id for row_id, row in self.conversations.items() if row.get("user_id") == user_id
        ]:
    ...
    async def account_deletion_preflight(self, user_id: uuid.UUID) -> dict[str, Any]:
        blocked: set[str] = set()
        memberships = [row for row in self.memberships if row.get("user_id") == user_id]
        for membership in memberships:
            tenant_id = membership["tenant_id"]
            member_count = sum(row.get("tenant_id") == tenant_id for row in self.memberships)
            if any(
                row.get("tenant_id") == tenant_id and row.get("user_id") == user_id
                for row in self.files.values()
            ):
                blocked.add("files_s3")
            if membership.get("role") == "owner" and member_count > 1:
                blocked.add("tenant_ownership")
            elif member_count <= 1:
                if any(
                    row.get("tenant_id") == tenant_id for row in self.connector_accounts.values()
                ):
                    blocked.add("connector_credentials")
                if any(row.get("tenant_id") == tenant_id for row in self.subscriptions.values()):
                    blocked.add("billing_subscription")
        return {"ready": not blocked, "blocked": sorted(blocked)}
    ...
    async def list_user_missions(
        ...
        return [
            dict(row)
            for row in list(self.missions.values())
    ...
    async def list_user_automations(
        ...
        return [
            dict(row)
            for row in list(self.automations.values())
            if row.get("tenant_id") == tenant_id and row.get("user_id") == user_id
        ][:limit]
    ...
    async def list_user_devices(
        ...
        return [
            dict(row)
            for row in list(self.devices.values())
            if row.get("tenant_id") == tenant_id and row.get("user_id") == user_id
        ][:limit]
    ...
    async def create_conversation(
        ...
        row: Row = {
            ...
            "channel": channel,
            "is_main": False,
            # Selector de modelos: NULL = automático (migración 0027).
            "chat_model": None,
            "chat_effort": None,
            "created_at": _now(),
            "updated_at": _now(),
    ...
    async def get_main_conversation(
        ...
        for row in self.conversations.values():
            if row["tenant_id"] == tenant_id and row["user_id"] == user_id and row.get("is_main"):
                return dict(row)
        return None
    ...
    async def search_conversations(
        ...
        for conv in self.conversations.values():
            ...
            if q in title:
                ...
                continue
            for msg in self.messages.get(conv["id"], []):
    ...
    async def list_phone_call_events_since(
        ...
        """Doble en memoria de `SqlRepo.list_phone_call_events_since` (frente 6b): mismo
        contrato -- cursor desconocido o ausente cae al historial completo -- salvo que
        acá la posición se ancla al índice en la lista (ya en orden de inserción, que es
        el mismo orden cronológico que produce el `ORDER BY` real) en vez de al par
        `(occurred_at, created_at)`."""
        events = await self.list_phone_call_events(tenant_id=tenant_id, call_id=call_id)
        if after_id is None:
            return events
    ...
    async def update_conversation_model(
        ...
        *,
        ...
        if row is None or row["tenant_id"] != tenant_id or row["user_id"] != user_id:
            return None
        # Igual que el SQL real: las dos columnas se escriben siempre, porque
        # `None` es "volver a automático" y no "no cambiar".
        row["chat_model"] = model
        row["chat_effort"] = effort
        return dict(row)
    ...
    async def update_message_content(
        ...
        *,
        ...
        for row in self.messages.get(conversation_id, []):
            if row["tenant_id"] == tenant_id and row["id"] == message_id:
                row["content"] = content
                row["updated_at"] = _now()
                return dict(row)
        return None
    ...
    async def list_cross_chat_message_snippets(
        ...
        *,
        ...
        conversations = [
            ...
            if row["tenant_id"] == tenant_id
            and row["user_id"] == user_id
            and row["id"] != exclude_conversation_id
        ]
        conversations.sort(key=lambda row: row["updated_at"], reverse=True)
        rows: list[Row] = []
        for conversation in conversations[:conversations_limit]:
            messages = [
                message
                for message in self.messages.get(conversation["id"], [])
                if message["tenant_id"] == tenant_id and message["role"] in {"user", "assistant"}
            ]
            for message in messages[-messages_per_conversation:]:
                item = dict(message)
                item["conversation_id"] = conversation["id"]
                item["conversation_title"] = conversation.get("title") or "Conversación anterior"
    ...
    async def quality_snapshot_all_tenants_since(self, *, since: datetime) -> Row:
        """Misma forma segura que `SqlRepo`, usando los registros fake disponibles."""
        messages: dict[str, dict[str, int]] = {}
        for rows in self.messages.values():
            for row in rows:
                if row.get("created_at", _now()) < since:
                    continue
                role = str(row.get("role") or "unknown")
                item = messages.setdefault(role, {"total": 0, "tokens_in": 0, "tokens_out": 0})
                item["total"] += 1
                item["tokens_in"] += int(row.get("tokens_in") or 0)
                item["tokens_out"] += int(row.get("tokens_out") or 0)
        return {
            "missions": {"by_status": {}, "success_rate": None},
            "automations": {"by_status": {}, "success_rate": None},
            "messages": messages,
            "usage_by_tenant": await self.sum_usage_all_tenants_since(since=since),
            "cost": {
                "status": "unknown",
                "estimated_usd": None,
                "unknown_events": 0,
    ...
    async def delete_all_memory(self, *, tenant_id: uuid.UUID, user_id: uuid.UUID) -> int:
        ids = [
            memory_id
            for memory_id, row in self.memory_items.items()
            if row["tenant_id"] == tenant_id and row["user_id"] == user_id
        ]
        ...
        return [
            {
                ...
                "feedback_ref": row.get("meta", {}).get("feedback_ref"),
                "created_at": row.get("created_at"),
            }
            for row in self.audit_log
            if row.get("tenant_id") == tenant_id
            and row.get("actor_user_id") == user_id
            and row.get("action") == "quality.feedback_received"
        ]
