...
async def test_quality_dashboard_exposes_only_durable_aggregates(client, fake_repo) -> None:
    headers = await _register_superadmin(client, fake_repo, email="quality@example.com")
    response = await client.get("/v1/admin/quality?days=14", headers=headers)

    assert response.status_code == 200
    body = response.json()
    assert body["format"] == "edecan-quality-dashboard.v1"
    assert body["period_days"] == 14
    assert body["missions"]["success_rate"] is None
    assert body["cost"]["status"] == "unknown"
    assert body["feedback"] == {"by_kind": {}, "by_category": {}}
    assert "hallucination_rate" in body["unavailable_metrics"]
    assert "content" not in response.text


async def test_quality_dashboard_rechaza_periodo_fuera_de_limite(client, fake_repo) -> None:
    headers = await _register_superadmin(client, fake_repo, email="quality-limit@example.com")
    response = await client.get("/v1/admin/quality?days=91", headers=headers)
