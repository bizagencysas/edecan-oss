...
async def test_create_schedule_success(client, fake_session: _FakeSession) -> None:
    ...
    assert body["trigger"] == {
        ...
        # Sin `BYMINUTE` el minuto lo hereda el alta, así que la frase acota
        # la hora en vez de inventar ":00" (edecan_automations.describe).
        "descripcion": "Todos los días entre las 9:00 y las 9:59 UTC",
    }
...
async def test_schedule_trigger_trae_la_rrule_ya_traducida(
    client, fake_session: _FakeSession
) -> None:
    """La frase se arma UNA vez acá (la hora es del servidor, ver
    `edecan_automations.describe`) — web e iOS solo la pintan."""
    tenant_id = uuid.uuid4()
    headers = auth_headers(user_id=uuid.uuid4(), tenant_id=tenant_id, plan_key="free_selfhost")
    fake_session.respuestas = [
        [
            _row(
                tenant_id=tenant_id,
                trigger=json.dumps({"kind": "schedule", "rrule": "FREQ=MINUTELY;INTERVAL=15"}),
            )
        ]
    ]
    ...
    assert response.status_code == 200
    trigger = response.json()[0]["trigger"]
    assert trigger["rrule"] == "FREQ=MINUTELY;INTERVAL=15"  # la cruda sigue viajando
    assert trigger["descripcion"] == "Cada 15 minutos"


async def test_rrule_intraducible_manda_descripcion_nula_no_una_aproximacion(
    client, fake_session: _FakeSession
) -> None:
    tenant_id = uuid.uuid4()
    headers = auth_headers(user_id=uuid.uuid4(), tenant_id=tenant_id, plan_key="free_selfhost")
    fake_session.respuestas = [
        [
            _row(
                tenant_id=tenant_id,
                trigger=json.dumps(
                    {"kind": "schedule", "rrule": "FREQ=MONTHLY;BYDAY=FR;BYSETPOS=-1"}
                ),
            )
        ]
    ]
    ...
    assert response.status_code == 200
    trigger = response.json()[0]["trigger"]
    assert trigger["descripcion"] is None
    assert trigger["rrule"] == "FREQ=MONTHLY;BYDAY=FR;BYSETPOS=-1"


async def test_webhook_trigger_no_gana_campo_descripcion(
    client, fake_session: _FakeSession
) -> None:
    tenant_id = uuid.uuid4()
    headers = auth_headers(user_id=uuid.uuid4(), tenant_id=tenant_id, plan_key="free_selfhost")
    fake_session.respuestas = [
        [
            _row(
                tenant_id=tenant_id,
                trigger=json.dumps({"kind": "webhook", "hook_secret": "muy-secreto"}),
            )
        ]
    ]
    ...
    assert response.status_code == 200
    assert "descripcion" not in response.json()[0]["trigger"]
...
async def test_list_runs_redacta_args_de_tools(client, fake_session: _FakeSession) -> None:
    tenant_id = uuid.uuid4()
    automation_id = uuid.uuid4()
    headers = auth_headers(user_id=uuid.uuid4(), tenant_id=tenant_id, plan_key="free_selfhost")
    fake_session.respuestas = [
        ...
        [
            {
                ...
                "detalle": json.dumps(
                    {
                        "resultado": "Token sk-secret123 no debe salir",
                        "tool_log": [
                            {
                                "type": "tool_start",
                                "name": "enviar_correo",
    ...
    assert response.status_code == 200
    detail = response.json()[0]["detalle"]
    assert "sk-secret123" not in json.dumps(detail)
    assert "args" not in json.dumps(detail)
    assert detail["tool_log"][0]["name"] == "enviar_correo"


async def test_list_automation_suggestions_es_solo_lectura(
    client, fake_session: _FakeSession
) -> None:
    tenant_id = uuid.uuid4()
    automation_id = uuid.uuid4()
    ...
    body = response.json()
    assert len(body) == 1
    assert body[0]["automation_id"] == str(automation_id)
    assert body[0]["requires_user_confirmation"] is True
    assert all(
