...
async def test_callback_without_code_shows_error_page_not_422(client) -> None:
    """Guardarraíl: cuando el proveedor redirige SIN `code` (usuario canceló o
    rechazó scopes), el callback ya NO debe reventar con un 422 críptico
    "Field required", sino servir una página HTML legible que explique qué pasó.
    """
    response = await client.get(
        "/v1/connectors/google/callback",
        params={"state": "loquesea"},  # falta `code` a propósito
    )
    assert response.status_code == 400  # NO 422
    assert "text/html" in response.headers.get("content-type", "")
    assert "No se pudo conectar" in response.text


async def test_callback_provider_error_muestra_motivo_real(client) -> None:
    """Cuando LinkedIn rechaza los scopes redirige con
    `?error=unauthorized_scope_error` y sin `code`. El callback debe mostrar
    ese error y una pista accionable (qué producto habilitar), no un 422."""
    response = await client.get(
    ...
    assert response.status_code == 400
    assert "unauthorized_scope_error" in response.text
    assert "Community Management API" in response.text
