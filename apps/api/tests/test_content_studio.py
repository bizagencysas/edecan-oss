...
async def test_linkedin_publish_encola_un_event_id_uuid_derivado_del_urn(
    client, app, fake_repo, monkeypatch
):
    """El aviso de "Publicación terminada" no llegaba NUNCA.

    `provider_id` es un URN de LinkedIn ("urn:li:share:123"), y viajaba tal
    cual como `event_id`; el handler del worker exige UUID y reventaba con
    `ValueError: notify_important_event requiere user_id y event_id UUID`, así
    que el job se reintentaba seis veces y moría. Este test reproduce
    exactamente la validación del handler
    (`apps/worker/edecan_worker/handlers/notify_important_event.py`) sobre el
    payload que el router encola.
    """
    user_id, tenant_id = uuid.uuid4(), uuid.uuid4()
    account = await fake_repo.create_connector_account(
        tenant_id=tenant_id,
    ...
    async def fake_create_post(http, bundle, **kwargs):  # noqa: ANN001
        return {"id": "urn:li:share:7489008051144880128"}
    ...
    assert response.status_code == 200
    payload = queued[-1][1]
    assert payload["kind"] == "content_published"
    # La validación literal del handler del worker.
    assert uuid.UUID(str(payload["event_id"]))
    # Estable: dos publicaciones confirmadas del MISMO post derivan el mismo
    # `event_id`, que es lo que conserva la deduplicación de
    # `record_notification_event`. Un `uuid4()` de relleno la habría perdido.
    assert payload["event_id"] == str(
        uuid.uuid5(uuid.NAMESPACE_URL, "linkedin:urn:li:share:7489008051144880128")
    )
...
async def test_linkedin_publish_guarda_autor_y_verificacion_en_el_audit_log(
    ...
    await fake_repo.create_connector_account(
        tenant_id=tenant_id,
        connector_key="linkedin",
        external_account_id="urn:li:organization:123456789",
        display_name="Página",
    ...
    assert entry["meta"]["author"] == "urn:li:organization:123456789"
...
async def test_publicar_borrador_por_draft_id_publica_de_verdad_y_lo_cuenta_en_el_chat(
    ...
    assert publicados[0][1]["org_urn"] == "urn:li:organization:123456789"
...
async def test_publicar_borrador_sin_confirmar_no_pone_el_check_en_el_chat(
    ...
    # Desde el 02-ago-2026 el mensaje sin confirmar arranca con la INCERTIDUMBRE
    # ("envié... pero NO puedo confirmar"), nunca con "Publicado": el caso real fue
    # que la persona leyó "Publicado" y dio el resto por hecho, y el post no estaba.
    assert "no puedo confirmar" in mensajes[0].lower()
    assert not mensajes[0].lower().startswith("publicado")
...
async def test_organization_linkedin_fallback_org_urn_and_400_error(
    client, app, fake_repo, monkeypatch
):
    """El borrador Acme usa el despachador multi-red sin tocar APIs reales.

    El camino actual no llama `create_linkedin_post` directamente: construye
    el paquete multi-red y delega en `publish_organization_all_networks`. El test
    debe aislar ese boundary y comprobar el URN que recibe, no preparar el
    viejo camino LinkedIn-only.
    """
    user_id, tenant_id = uuid.uuid4(), uuid.uuid4()
    sesion, _publicados = await _preparar_publicacion(
        app, fake_repo, monkeypatch, tenant_id=tenant_id
    )
    # Cambiar external_account_id de la cuenta conectada a un person URN
    for acc in fake_repo.connector_accounts.values():
        acc["external_account_id"] = "urn:li:person:example123"

    monkeypatch.setattr(
        content_studio,
        "_get_organization_linkedin_token_and_urn",
        lambda: ("linkedin-token", "urn:li:organization:123456789"),
    )
    llamadas: list[dict[str, Any]] = []

    async def fake_publish_all_networks(**kwargs: Any) -> dict[str, Any]:
        llamadas.append(kwargs)
        return {
            "linkedin": {"ok": True, "post_id": "urn:li:share:900"},
            "instagram": {"ok": False, "error": "sin cuenta"},
            "facebook": {"ok": False, "error": "sin cuenta"},
            "threads": {"ok": False, "error": "sin cuenta"},
            "x": {"ok": False, "error": "sin cuenta"},
        }

    monkeypatch.setattr(
        content_studio, "publish_organization_all_networks", fake_publish_all_networks
    )
    ...
    _sembrar_borrador(
        ...
        user_id=user_id,
        target="organization",
        draft_id="organization-draft-1",
    )

    respuesta = await client.post(
        "/v1/content/social/drafts/organization-draft-1/publish",
        headers=auth_headers(user_id=user_id, tenant_id=tenant_id),
    )

    assert respuesta.status_code == 200
    assert llamadas[0]["linkedin_org_urn"] == "urn:li:organization:123456789"

    async def fake_publish_all_networks_error(**kwargs: Any) -> dict[str, Any]:
        return {
            network: {"ok": False, "error": "LinkedIn rechazó las credenciales."}
            for network in ("linkedin", "instagram", "facebook", "threads", "x")
        }

    monkeypatch.setattr(
        content_studio, "publish_organization_all_networks", fake_publish_all_networks_error
    )
    _sembrar_borrador(
        sesion,
        tenant_id=tenant_id,
        user_id=user_id,
        target="organization",
        draft_id="organization-draft-2",
    )

    error_resp = await client.post(
        "/v1/content/social/drafts/organization-draft-2/publish",
        headers=auth_headers(user_id=user_id, tenant_id=tenant_id),
    )
    assert error_resp.status_code == 502
    assert "No pude publicar en ninguna red" in error_resp.json()["detail"]
