...
@respx.mock
async def test_put_llm_workers_ai_valida_y_guarda_config_del_tenant(client, app) -> None:
    fake_vault = _install_vault(app)
    respx.get("https://api.cloudflare.com/client/v4/accounts/account-123/ai/models/search").mock(
    ...
    response = await client.put(
        ...
        json={
            "kind": "workers_ai",
            "api_key": "token-tenant",
            "model_principal": "@cf/meta/modelo",
    ...
    assert stored["kind"] == "workers_ai"
    assert stored["extra"]["account_id"] == "account-123"
    assert stored["api_key"] == "token-tenant"
...
async def test_put_llm_claude_cli_valida_subproceso_ok(client, app, monkeypatch) -> None:
    ...
    async def fake_exec(*args: Any, **kwargs: Any) -> _FakeProcess:
        assert str(args[0]).endswith("claude")
