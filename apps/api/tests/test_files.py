...
async def test_download_file_streams_private_object_with_safe_headers(client, monkeypatch) -> None:
    ...
    # `private` sigue siendo obligatorio (nunca una caché compartida), pero ya
    # no `no-store`: el contenido bajo un `file_id` es inmutable y `no-store`
    # obligaba al teléfono a rebajar los mismos megas en cada apertura.
    assert response.headers["cache-control"] == "private, max-age=604800, immutable"
    assert response.headers["x-content-type-options"] == "nosniff"


def _png_de_prueba(lado: int) -> bytes:
    """PNG real de `lado`x`lado` generado EN el test (nunca un archivo del dueño).

    Con ruido, no con un fondo plano: un PNG plano pesa unos pocos KB y haría
    pasar el test aunque la miniatura no ahorrara nada. Lo que se quiere
    reproducir es el caso real —un render de varios MB— así que la imagen
    tiene que ser igual de incompresible.
    """
    import io

    from PIL import Image

    imagen = Image.effect_noise((lado, lado), 48).convert("RGB")
    salida = io.BytesIO()
    imagen.save(salida, format="PNG")
    return salida.getvalue()


async def test_thumbnail_reduce_la_imagen_y_la_deja_cacheable(client, monkeypatch) -> None:
    """La card pedía el PNG de 2160x2160 entero solo para dibujarlo a 1280."""
    import io
    ...
    original = _png_de_prueba(1600)
    uploaded = await client.post(
        "/v1/files",
        files={"file": ("post.png", original, "image/png")},
        headers=headers,
    )
    file_id = uploaded.json()["id"]

    response = await client.get(f"/v1/files/{file_id}/thumbnail?max=640", headers=headers)

    assert response.status_code == 200
    with Image.open(io.BytesIO(response.content)) as reducida:
        assert max(reducida.size) == 640
    # Lo que de verdad importa: por el túnel viajan megas menos. Un factor de
    # 10 es conservador frente a lo medido (6,2 MB -> 130 KB).
    assert len(response.content) * 10 < len(original)
    assert response.headers["cache-control"] == "private, max-age=604800, immutable"
    assert response.headers["x-content-type-options"] == "nosniff"
    assert response.headers["content-disposition"] == "inline"


async def test_thumbnail_devuelve_el_original_si_no_puede_reducirlo(client, monkeypatch) -> None:
    """Reparar, no descartar: unos bytes que Pillow no entiende no pueden
    dejar a la card sin imagen — se devuelve el original tal cual."""
    import edecan_api.routers.files as files_module
    ...
    response = await client.get(f"/v1/files/{uploaded.json()['id']}/thumbnail", headers=headers)
...
async def test_thumbnail_rechaza_lo_que_no_es_imagen(client, monkeypatch) -> None:
    import edecan_api.routers.files as files_module
...
async def test_thumbnail_never_crosses_tenants(client, monkeypatch) -> None:
    import edecan_api.routers.files as files_module
    ...
    uploaded = await client.post(
        "/v1/files",
        files={"file": ("privada.png", _png_de_prueba(256), "image/png")},
        headers=auth_headers(user_id=uuid.uuid4(), tenant_id=uuid.uuid4()),
    )

    response = await client.get(
        f"/v1/files/{uploaded.json()['id']}/thumbnail",
        headers=auth_headers(user_id=uuid.uuid4(), tenant_id=uuid.uuid4()),
    )
...
async def test_thumbnail_requires_authentication(client) -> None:
    response = await client.get(f"/v1/files/{uuid.uuid4()}/thumbnail")
