    ...
    async def execute(self, clause, params=None) -> _FakeResult:
        ...
        if primer == "SELECT" and es_missions:
            rows = [
                ...
                and row["user_id"] == params["user_id"]
                and (params.get("include_archived") or row.get("archived_at") is None)
                and (
                    params.get("search") is None
                    or params["search"].strip("%").lower() in row["objetivo"].lower()
                )
                and (params.get("status") is None or row["status"] == params["status"])
            ]
        ...
        if primer == "UPDATE" and es_missions:
            ...
            if row is not None and row["tenant_id"] == params["tenant_id"]:
                ...
                if "presupuesto" in params:
                    raw = params["presupuesto"]
                    row["presupuesto"] = json.loads(raw) if isinstance(raw, str) else raw
            return _FakeResult()
...
async def test_list_missions_rechaza_status_desconocido(client) -> None:
    headers = auth_headers(
        user_id=uuid.uuid4(), tenant_id=uuid.uuid4(), plan_key=PLAN_WITH_MISSIONS
    )
    response = await client.get("/v1/missions?status=not-a-status", headers=headers)
    assert response.status_code == 422


async def test_list_missions_oculta_archivadas_y_permite_incluirlas(
    client, fake_session: FakeSession
) -> None:
    tenant_id, user_id = uuid.uuid4(), uuid.uuid4()
    archived = fake_session.seed_mission(
        mission_id=uuid.uuid4(), tenant_id=tenant_id, user_id=user_id, status="done"
    )
    archived["archived_at"] = datetime.now(UTC)
    visible = fake_session.seed_mission(
        mission_id=uuid.uuid4(), tenant_id=tenant_id, user_id=user_id, status="done"
    )
    headers = auth_headers(user_id=user_id, tenant_id=tenant_id, plan_key=PLAN_WITH_MISSIONS)
    ...
    response = await client.get("/v1/missions?include_archived=true", headers=headers)
    assert {item["id"] for item in response.json()} == {str(archived["id"]), str(visible["id"])}


async def test_missions_inbox_agrega_atencion_actividad_y_counts(
    client, fake_session: FakeSession
) -> None:
    tenant_id, user_id = uuid.uuid4(), uuid.uuid4()
    fake_session.seed_mission(
        mission_id=uuid.uuid4(), tenant_id=tenant_id, user_id=user_id, status="waiting_confirmation"
    )
    fake_session.seed_mission(
        mission_id=uuid.uuid4(), tenant_id=tenant_id, user_id=user_id, status="paused"
    )
    fake_session.seed_mission(
        mission_id=uuid.uuid4(), tenant_id=tenant_id, user_id=user_id, status="done"
    )
    ...
    response = await client.get("/v1/missions/inbox", headers=headers)

    assert response.status_code == 200
    body = response.json()
    assert len(body["attention"]) == 1
    assert len(body["active"]) == 1
    assert body["counts"] == {"waiting_confirmation": 1, "paused": 1, "done": 1}
...
async def test_reproduction_devuelve_manifiesto_sin_argumentos_de_tools(
    client, fake_session: FakeSession
) -> None:
    ...
    fake_session.seed_mission(
        ...
        user_id=user_id,
        status="done",
        objetivo="Investiga usando api_key=sk-test-supersecreto",
        plan=[
            {
                "seq": 1,
                "agente": "research",
                "instruccion": "Investiga api_key=sk-test-supersecreto",
            }
        ],
    )
    fake_session.seed_step(
        ...
        instruccion="Usa api_key=sk-test-supersecreto para investigar",
        usage={
            "pending_tool_call": {"name": "usar_computadora", "args": {"cmd": "rm -rf"}},
            "provenance": {"workflow_version": "phase3-w7", "model_alias": "principal"},
        },
    )
    headers = auth_headers(user_id=user_id, tenant_id=tenant_id, plan_key=PLAN_WITH_MISSIONS)

    response = await client.get(f"/v1/missions/{mission_id}/reproduction", headers=headers)

    assert response.status_code == 200
    body = response.json()
    assert body["workflow_version"] == "phase3-w7"
    assert body["steps"][0]["provenance"]["model_alias"] == "principal"
    assert "args" not in body["steps"][0]
    assert "pending_tool_call" not in body["steps"][0]
    assert "sk-test-supersecreto" not in response.text
...
async def test_get_mission_detalle_reconstruye_trace_sin_prompts_ni_args(
    client, fake_session: FakeSession
) -> None:
    tenant_id, user_id, mission_id = uuid.uuid4(), uuid.uuid4(), uuid.uuid4()
    fake_session.seed_mission(
        mission_id=mission_id, tenant_id=tenant_id, user_id=user_id, status="done"
    )
    fake_session.seed_step(
        mission_id=mission_id,
        tenant_id=tenant_id,
        ...
        usage={
            ...
            "finished_at": "2026-08-20T00:00:05+00:00",
            "input_tokens": 12,
            "output_tokens": 8,
            "provenance": {
                "model_alias": "principal",
                "workflow_version": "phase3-w7",
                "tools_requested": ["buscar_web"],
            },
        },
    )
    headers = auth_headers(user_id=user_id, tenant_id=tenant_id, plan_key=PLAN_WITH_MISSIONS)
    ...
    assert [node["kind"] for node in trace] == ["mission", "agent", "tool"]
    assert trace[1]["model_alias"] == "principal"
    assert trace[1]["duration_ms"] == 5000
    assert trace[1]["token_usage"] == {"input_tokens": 12, "output_tokens": 8}
    assert all("instruccion" not in node and "args" not in node for node in trace)
...
async def test_get_mission_detalle_no_expone_args_de_tool_ni_secretos(
    client, fake_session: FakeSession
) -> None:
    tenant_id, user_id, mission_id = uuid.uuid4(), uuid.uuid4(), uuid.uuid4()
    fake_session.seed_mission(mission_id=mission_id, tenant_id=tenant_id, user_id=user_id)
    fake_session.seed_step(
        ...
        agente="content",
        status="waiting_confirmation",
        instruccion="usa la credencial sk-secret123",
        usage={
            "pending_tool_call": {
                "id": "call-1",
                "name": "enviar_correo",
                "args": {"token": "sk-secret123"},
            }
        },
    )
    headers = auth_headers(user_id=user_id, tenant_id=tenant_id, plan_key=PLAN_WITH_MISSIONS)
    ...
    assert response.status_code == 200
    detail = response.json()["steps"][0]
    assert "sk-secret123" not in json.dumps(detail)
    assert "args" not in json.dumps(detail)
...
async def test_archive_exitoso_conserva_resultado_y_oculta_mision(
    client, fake_session: FakeSession
) -> None:
    tenant_id, user_id = uuid.uuid4(), uuid.uuid4()
    mission_id = uuid.uuid4()
    fake_session.seed_mission(
        ...
        user_id=user_id,
        status="done",
        resultado="resultado durable",
    )
    headers = auth_headers(user_id=user_id, tenant_id=tenant_id, plan_key=PLAN_WITH_MISSIONS)

    response = await client.post(f"/v1/missions/{mission_id}/archive", headers=headers)

    assert response.status_code == 200
    assert response.json()["resultado"] == "resultado durable"
    assert fake_session.missions[str(mission_id)]["archived_at"] is not None


async def test_archive_rechaza_mision_activa(client, fake_session: FakeSession) -> None:
    tenant_id, user_id = uuid.uuid4(), uuid.uuid4()
    mission_id = uuid.uuid4()
    fake_session.seed_mission(
        mission_id=mission_id, tenant_id=tenant_id, user_id=user_id, status="running"
...
async def test_pause_y_resume_conservan_la_mision_y_reencolan(
    ...
    paused = await client.post(f"/v1/missions/{mission_id}/pause", headers=headers)
...
async def test_steer_guarda_instruccion_sin_reiniciar(
    ...
    steered = await client.post(
        f"/v1/missions/{mission_id}/steer",
        json={"instruction": "Usa más datos de Medellín."},
        headers=headers,
    )
