...
def test_request_http_no_acepta_scope_websocket() -> None:
    """Regresión: `Request(websocket.scope)` revienta y Twilio cierra con 31921.

    Media Streams llega con scope `websocket`. Starlette.Request exige `http`.
    Si el handler construye Request ahí, la llamada suena dos veces y se cae.
    """
    from starlette.requests import Request

    with pytest.raises(AssertionError):
        Request({"type": "websocket", "asgi": {"version": "3.0", "spec_version": "2.3"}})


def test_media_handler_no_construye_request_desde_el_websocket() -> None:
    import inspect

    source = inspect.getsource(phone.twilio_media_stream)
    assert "Request(websocket.scope)" not in source
...
async def test_setup_incoming_calls_configures_current_twilio_number(
    ...
    async def fake_configure(
        account_sid, auth_token, phone_sid, webhook_url, *, http_client, status_callback_url=""
    ):
        seen["phone_sid"] = phone_sid
        seen["webhook_url"] = webhook_url
        seen["status_callback_url"] = status_callback_url
    ...
    assert response.json()["phone_numbers"] == ["+573001111111"]
    assert response.json()["agent_name"] == "Sofía"
    assert seen == {
        "phone_number": "+573001111111",
        "phone_sid": "PN" + "2" * 32,
        "webhook_url": "http://localhost:8000/v1/phone/twilio/incoming",
        "status_callback_url": "http://localhost:8000/v1/phone/twilio/status",
    }


async def test_setup_incoming_calls_configura_todos_los_numeros_twilio(
    app, client, fake_repo, monkeypatch
) -> None:
    tenant_id, user_id = uuid.uuid4(), uuid.uuid4()
    await fake_repo.create_connector_account(
        tenant_id=tenant_id,
        connector_key="twilio",
        external_account_id="+12025550101",
        display_name="+12025550101",
        scopes=["AC" + "1" * 32],
    )
    await fake_repo.create_connector_account(
        tenant_id=tenant_id,
        connector_key="twilio",
        external_account_id="+573001234567",
    ...
    async def fake_verify(_account_sid, _auth_token, phone_number, *, http_client):
        return "PN" + phone_number[-8:]

    async def fake_configure(
        _account_sid, _auth_token, _phone_sid, webhook_url, *, http_client, status_callback_url=""
    ):
        configured.append(webhook_url)
    ...
    assert response.status_code == 200
    assert set(response.json()["phone_numbers"]) == {"+12025550101", "+573001234567"}
    assert len(configured) == 2
    assert set(configured) == {"http://localhost:8000/v1/phone/twilio/incoming"}
...
def test_phone_system_prompt_uses_agent_not_chat_architecture() -> None:
    prompt = phone._phone_system_prompt(
        {
            "agent_template_name": "Asistente de Recepción",
            "agent_name": "Asistente de Recepción Dupont",
            "agent_prompt": "Eres Asistente de Recepción Dupont, asistente del señor Alex. Pregunta con quién hablas.",
            "goal": "Atender la llamada entrante y ayudar a la persona.",
            "recipient_name": None,
        },
        SimpleNamespace(nombre_asistente="Edecan"),
    )
    assert "Eres Asistente de Recepción Dupont" in prompt
    assert "asistente del señor Alex" in prompt
    assert "PROHIBIDO preguntar" in prompt
    assert "Tool Orchestrator" not in prompt
    assert "No escribas speech tags" in prompt
...
async def test_signed_status_webhook_updates_activity_state(
    ...
    assert summary["key_points"] == [
        "El interlocutor llamó para confirmar la entrega y avisó que enviará la dirección mañana."
    ]
...
async def test_call_status_ignores_stream_lifecycle_without_call_status(
    app, client, fake_repo
) -> None:
    tenant_id, user_id = uuid.uuid4(), uuid.uuid4()
    conversation = await fake_repo.create_conversation(
        tenant_id=tenant_id, user_id=user_id, title="En curso", channel="phone"
    )
    call_sid = "CA" + "4" * 32
    call = await fake_repo.create_phone_call(
        tenant_id=tenant_id,
    ...
    app.state.phone_webhook_token_loader = lambda _tenant_id: "hook-token"
    path = "/v1/phone/twilio/status"
    params = {"CallSid": call_sid, "StreamEvent": "stream-started"}
    response = await client.post(
        path,
        data=params,
        headers={
    ...
    assert response.status_code == 204
    assert fake_repo.phone_calls[call["id"]]["status"] == "in_progress"
    assert fake_repo.phone_calls[call["id"]].get("summary") is None


async def test_placeholder_summary_is_replaced_when_transcript_arrives(fake_repo) -> None:
    tenant_id, user_id = uuid.uuid4(), uuid.uuid4()
    conversation = await fake_repo.create_conversation(
        tenant_id=tenant_id, user_id=user_id, title="Llamada", channel="phone"
    )
    call = await fake_repo.create_phone_call(
        ...
        goal="Atender",
        status="failed",
        provider_call_sid="CA" + "5" * 32,
    )
    first = await phone._finalize_phone_call_summary(fake_repo, call, llm=None)
    assert first.created is True
    assert fake_repo.phone_calls[call["id"]]["summary"]["transcript"]["available"] is False

    await fake_repo.add_phone_call_event(
        tenant_id=tenant_id,
        call_id=call["id"],
        event_type="transcript",
        payload={"role": "caller", "text": "Buenas noches."},
    )
    updated = await fake_repo.get_phone_call(tenant_id=tenant_id, call_id=call["id"])
    second = await phone._finalize_phone_call_summary(fake_repo, updated, llm=None)
    assert second.created is True
    summary = fake_repo.phone_calls[call["id"]]["summary"]
    assert summary["transcript"]["available"] is True
    assert "Buenas noches" in " ".join(summary["key_points"])


async def test_finalize_uses_chat_messages_when_transcript_events_are_missing(fake_repo) -> None:
    """Media Stream puede commitear el chat y no los eventos, o al revés.

    Si el cierre no ve `transcript` events, igual tiene que narrar lo que
    quedó en el hilo de la llamada. Si no, Actividad miente: «no quedó
    ninguna intervención» con la conversación al lado.
    """
    tenant_id, user_id = uuid.uuid4(), uuid.uuid4()
    conversation = await fake_repo.create_conversation(
        tenant_id=tenant_id, user_id=user_id, title="Llamada", channel="phone"
    ...
    await fake_repo.add_message(
        ...
        conversation_id=conversation["id"],
        role="assistant",
        content={"text": "Hola, habla Asistente de Recepción Dupont."},
    )
    await fake_repo.add_message(
        tenant_id=tenant_id,
        conversation_id=conversation["id"],
        role="user",
        content={"text": "Hola, buenas noches. Soy Manuel González."},
    )
    call = await fake_repo.create_phone_call(
        tenant_id=tenant_id,
        user_id=user_id,
        conversation_id=conversation["id"],
        direction="incoming",
        from_e164="+573001234568",
        to_e164="+573001234567",
        goal="Atender",
        status="completed",
        provider_call_sid="CA" + "6" * 32,
    )
    delivery = await phone._finalize_phone_call_summary(fake_repo, call, llm=None)
    assert delivery.created is True
    summary = fake_repo.phone_calls[call["id"]]["summary"]
    assert summary["transcript"]["available"] is True
    assert "Manuel González" in " ".join(summary["key_points"])
...
class TestTextoDelResumenEnElChat:
    ...
    def test_sin_trato_cableado_para_ninguna_formalidad(self) -> None:
        ...
        niveles, así que no puede traer segunda persona ni un "señor" cableado."""
        for direction, call in (
            ...
            prohibidas = ("señor", "usted", "tuyo", "tu llamada", "puedes", "puede reintentar")
...
async def test_queue_call_whisper_requires_ownership_and_active_call(client, fake_repo) -> None:
    ...
    assert [event["event_type"] for event in fake_repo.phone_call_events[call["id"]]] == ["susurro"]
...
async def test_phone_reply_injects_pending_whisper_once_with_jarvis_prefix(
    ...
    con el mismo prefijo exacto que usa REFERENCIA, y se consume una sola vez."""
    tenant_id, user_id = uuid.uuid4(), uuid.uuid4()
    call = await _phone_call_in_progress(fake_repo, tenant_id=tenant_id, user_id=user_id)
    await fake_repo.add_phone_call_event(
        tenant_id=tenant_id,
    ...
    assert first_request.messages[0].content == "Hola"
    whisper_messages = [
        message
    ...
    assert whisper_messages[0].content == (
        "[NOTA URGENTE DEL SEÑOR — incorpórala con naturalidad en tu respuesta]: "
...
async def test_gather_trata_las_teclas_del_otro_lado_como_un_turno(app, client, fake_repo) -> None:
...
async def test_respuesta_de_solo_tonos_no_deja_la_llamada_en_silencio(
    ...
    response = await _turno(client, fake_repo, call, {"CallSid": sid, "SpeechResult": "Marque 3"})
...
class TestNumeroSalientePorPais:
    ...
    destinatario le aparece como llamada extranjera. REFERENCIA siempre marcó desde el número
...
async def test_get_call_transcript_polls_only_new_turns_since_cursor(client, fake_repo) -> None:
...
async def test_get_active_call_vacia_o_en_curso(client, fake_repo) -> None:
    tenant_id, user_id = uuid.uuid4(), uuid.uuid4()
    headers = auth_headers(user_id=user_id, tenant_id=tenant_id)
    vacia = await client.get("/v1/phone/calls/activa", headers=headers)
    assert vacia.status_code == 200
    assert vacia.json() == {"activa": False}
    ...
    await fake_repo.add_phone_call_event(
        ...
        call_id=call["id"],
        event_type="transcript",
        payload={"role": "assistant", "text": "Hola, habla Edecán."},
    )
    viva = await client.get("/v1/phone/calls/activa", headers=headers)
    assert viva.status_code == 200
    body = viva.json()
    assert body["activa"] is True
    assert body["call_id"] == str(call["id"])
    assert body["transcript"][0]["texto"] == "Hola, habla Edecán."
    assert body["transcript"][0]["quien"] == "agente"
