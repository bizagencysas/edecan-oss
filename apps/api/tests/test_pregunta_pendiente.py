...
Regresión de un fallo real: el usuario pidió un post de LinkedIn, contestó la tarjeta de
...
async def test_la_tarjeta_de_un_turno_llega_como_pregunta_pendiente_al_siguiente(
    ...
    primera = await _mandar(
        client, headers, conversation_id, "Créame un post de LinkedIn sobre tecnología."
    )
...
async def test_un_turno_sin_tarjeta_no_deja_pregunta_pendiente(client, monkeypatch) -> None:
    ...
    await _mandar(client, headers, conversation_id, "Créame un post de LinkedIn sobre tecnología.")
