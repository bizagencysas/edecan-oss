...
def test_remote_stream_rechaza_handshake_sin_auth(app) -> None:
    with TestClient(app) as test_client:
        path = "/v1/remote/sessions/00000000-0000-0000-0000-000000000001/stream"
...
# ---------------------------------------------------------------------------
# Coordenada NORMALIZADA (`nx`/`ny`) — ADITIVA: el router solo la reenvía.
# La conversión a coordenada real la hace el companion, que es el único que
# conoce la geometría del display (`actions._pointer_display_bounds`).
# ---------------------------------------------------------------------------


async def test_input_pointer_forwards_normalized_coordinates_when_provided(
    client, fake_manager: _FakeCompanionManager
) -> None:
    headers, session = await _create_active_control_session(client, fake_manager)
    fake_manager.responses.append({"ok": True, "result": {}})
    ...
    response = await client.post(
        f"/v1/remote/sessions/{session['id']}/input",
        json={"tipo": "pointer", "x": 800, "y": 517, "nx": 0.5, "ny": 0.5, "accion": "click"},
        headers=headers,
    )

    assert response.status_code == 200
    _, _, params = fake_manager.calls[-1]
    assert params["nx"] == 0.5
    assert params["ny"] == 0.5
    # `x`/`y` siguen viajando tal cual: el companion decide cuál usa.
    assert params["x"] == 800
    assert params["y"] == 517


async def test_input_pointer_forwards_normalized_drag_start(
    client, fake_manager: _FakeCompanionManager
) -> None:
    headers, session = await _create_active_control_session(client, fake_manager)
    fake_manager.responses.append({"ok": True, "result": {}})
    ...
    _, _, params = fake_manager.calls[-1]
    assert params["start_nx"] == 0.25
    assert params["start_ny"] == 0.1


async def test_input_pointer_without_normalized_coordinates_sends_the_exact_legacy_payload(
    client, fake_manager: _FakeCompanionManager
) -> None:
    """Cliente viejo (panel web): el payload al companion no gana ni una clave."""
    headers, session = await _create_active_control_session(client, fake_manager)
    fake_manager.responses.append({"ok": True, "result": {}})
...
@pytest.mark.parametrize(
    "extra",
    [
        {"nx": 0.5},  # media coordenada
        {"ny": 0.5},
        {"nx": 1.5, "ny": 0.5},  # fuera de 0.0..1.0
        {"nx": -0.1, "ny": 0.5},
        {"nx": 0.5, "ny": 2.0},
    ],
)
async def test_input_pointer_rejects_invalid_normalized_coordinates(
    client, fake_manager: _FakeCompanionManager, extra: dict
) -> None:
    headers, session = await _create_active_control_session(client, fake_manager)

    response = await client.post(
        f"/v1/remote/sessions/{session['id']}/input",
        json={"tipo": "pointer", "x": 1, "y": 2, "accion": "click", **extra},
        headers=headers,
    )
