...
@pytest.mark.parametrize(
    ...
    + ([(LanzarCampanaTool, "campaigns", "FLAG_CAMPAIGNS")] if _PREMIUM_TOOL_CLASSES else []),
    ids=[
...
_SUPERFICIES_DE_ENCOLADO: dict[str, str] = {
    ...
    "create_linkedin_post": (
        "DOS caminos, ninguno con flag propio (CrearPostLinkedInTool.requires_flags es el "
        "frozenset() por defecto de Tool -- capacidad base, igual que memory_consolidate): "
        "(1) conversations.py::_stream_crear_post_linkedin_directo encola cuando el atajo de "
        "chat 'crear_post_linkedin' resuelve destino/tema (gateado solo por "
        "_tool_requires_flags_satisfechos, que aquí siempre pasa) Y (2) "
        "run_automation.py::_delegate_create_linkedin_post cuando accion.kind== "
        "'create_linkedin_post' de una automatización (creada vía "
        "linkedin_automations_seed.py/cron) -- ese camino SÍ revalida "
        "'automations.rules' antes de despachar, igual que el resto de run_automation."
    ),
    "create_organization_linkedin_post": (
        "Variante product-led del post de LinkedIn para la página de Acme: "
        "fydesign elige la pantalla y entrega el brief, el worker escribe el cuerpo "
        "con un modelo directo (sin pasar por CrearPostLinkedInTool/seed) y pide el "
        "visual a fydesign. Se encola directo (como create_linkedin_post) -- sin flag "
        "propio, capacidad base del worker."
    ),
    "run_persistent_agent": (
        "POST /v1/agents/workers/{id}/tasks o aprobación de un handoff; el worker "
        "revalida worker enabled/status, tools seguras, flags del plan y timeout "
        "del presupuesto antes de ejecutar."
    ),
...
def test_solo_tres_modulos_de_routers_invocan_send_command() -> None:
    ...
    `preparacion.py` (pantalla de preparación de Windows) es el CUARTO
    camino que este test anticipaba: gatea con su propio
    `_require_companion_ide` (mismo flag `companion.ide`) y con
    `require_paired_ide_device` como dependencia del router completo -- la
    misma frontera que ya usa `ide.advanced_router` -- así que se agrega a
    la lista en vez de fallar."""
    llamadores = {
        archivo.stem
        for archivo in _ROUTERS_DIR.glob("*.py")
        if "send_command" in archivo.read_text(encoding="utf-8")
    }
    assert llamadores == {"ide", "remote", "conversations", "preparacion"}
