...
from edecan_companion.platform_paths import (
    AUDIT_LOG_FILENAME,
    DEFAULT_CONFIG_DIR,
    DEFAULT_CONFIG_PATH,
    DEFAULT_SANDBOX_DIR,
...
TEAMVIEWER_OPT_IN_ENV = "EDECAN_ENABLE_TEAMVIEWER"
ALLOW_ALL_APPS_ENV = "EDECAN_ALLOW_ALL_APPS"

# Los cuatro `DEFAULT_*` de arriba viven en `platform_paths` -- una sola
# fuente de verdad para las rutas por plataforma (docs/edecan-windows.md
# §2). Se re-exportan acá (en vez de solo importarlos donde se usan) porque
# el resto de este módulo y sus tests ya los referencian como
# `config.DEFAULT_SANDBOX_DIR`, etc.
...
# (macOS: `open -a <app>`; Linux: resuelve el lanzador .desktop por nombre de
# archivo o por su "Name=" visible, p. ej. "firefox" o "Visual Studio Code").
# Ejemplo: ["Safari", "Notas"]
allowed_apps: []

# Opt-in del dueño para que Edecán pueda abrir cualquier app gráfica que le
...
def load_config(path: Path | str | None = None) -> CompanionConfig:
    ...
    if allow_all_apps:
        if "open_app" not in auto_approve:
            auto_approve.append("open_app")
    elif os.environ.get(TEAMVIEWER_OPT_IN_ENV, "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    }:
        if "TeamViewer" not in allowed_apps:
            allowed_apps.append("TeamViewer")
        if "open_app" not in auto_approve:
            auto_approve.append("open_app")
