...
class SemanticSearchService:
    ...
    def _resolve_indexable_path(self, workspace_id: str, relative_path: str) -> Path | None:
        """Resuelve ``relative_path`` con el chequeo de frontera oficial del
        workspace (``WorkspaceStore.resolve`` -- la misma función que usa
        ``GET /file`` vía ``ide_files.FileService.read``), y de paso confirma
        que el archivo TODAVÍA existe en disco.

        Único punto de esta comprobación en todo el módulo: la usan tanto
        ``_build_index_now`` (para no indexar lo que está fuera del
        workspace) como ``search`` (para no devolver, aunque el índice en
        disco aún no se haya reconstruido, un fragmento cuyo archivo ya no
        resuelve dentro del workspace o ya fue borrado). Devuelve ``None``
        en ambos casos -- "fuera del workspace" y "ya no existe" se tratan
        igual desde acá porque para quien busca el resultado es el mismo:
        no hay nada válido que mostrar.
        """
        try:
            absolute = self.workspaces.resolve(workspace_id, relative_path)
    ...
    def _build_index_now(self, workspace_id: str) -> _BuildResult:
        ...
        for relative_path in relative_paths:
            if len(new_chunks) >= self.max_total_chunks:
                truncated = True
                break
            # Frontera del workspace: el MISMO chequeo que usa ``GET /file``
            # (``ide_files.FileService.read`` -> ``WorkspaceStore.resolve``),
            # no una copia paralela hecha con ``root / relative_path`` que
            # nunca sigue symlinks y por eso se desincroniza. ``git ls-files``
            # (la fuente de ``relative_paths``) SÍ lista symlinks como
            # archivos normales, así que sin esto un ``enlace.txt`` apuntando
            # fuera del workspace (p. ej. a ``/etc/passwd``) se indexaba tal
            # cual, aunque ``GET /file`` sobre esa misma ruta ya lo rechazaba
            # con un 422 -- la fuga medida en la auditoría.
            absolute = self._resolve_indexable_path(workspace_id, relative_path)
            if absolute is None:
                continue
            try:
                ...
                continue
            if file_stat.st_size > self.max_file_bytes_for_index:
                continue
    ...
    def search(
        ...
        *,
        ...
        # Un archivo puede haberse borrado (o un ancestro haberse vuelto un
        # symlink hacia afuera) entre el último reindexado y esta consulta:
        # el índice en disco no se toca acá -- eso lo hace el próximo
        # reindexado -- pero tampoco hay que devolver un fragmento que ya no
        # es válido. Se cachea por ruta dentro de esta única búsqueda porque
        # varios chunks comparten el mismo archivo.
        valid_path_cache: dict[str, bool] = {}
        ...
        for chunk in index["chunks"].values():
            ...
            if not embedding:
                continue
            path = chunk["path"]
            is_valid = valid_path_cache.get(path)
            if is_valid is None:
                is_valid = self._resolve_indexable_path(workspace_id, path) is not None
                valid_path_cache[path] = is_valid
            if not is_valid:
                continue
