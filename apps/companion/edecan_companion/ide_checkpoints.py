...
_reloj_checkpoints_lock = threading.Lock()
_ultimo_now_us = 0
"""Último valor que entregó ``_now_us()`` en este proceso -- ver su
docstring. Empieza en 0: cualquier ``time.time()`` real de hoy lo supera de
sobra, así que la primera llamada siempre usa el reloj de verdad."""


def _now_us() -> int:
    """Microsegundos desde epoch, pero garantizado ESTRICTAMENTE creciente
    dentro de este proceso -- nunca repite ni retrocede, aunque el reloj del
    sistema operativo no tenga resolución suficiente para distinguir dos
    llamadas seguidas.

    Hallazgo real (ronda de cierre de Windows, ver ``docs/opencode-windows.md``):
    ``test_workspace_cap_evicts_oldest_checkpoint`` falló de forma
    intermitente contra la VM de Windows real. Causa raíz: ``time.time()``
    en Windows históricamente tiene mucha menos resolución que en
    macOS/Linux (el tick del temporizador del sistema, típicamente unos
    ~15.6 ms salvo que algo suba la resolución del reloj a mano) -- tres
    checkpoints creados en sucesión rápida (el patrón exacto de ese test)
    pueden terminar con el MISMO ``created_at_us``. En ese empate,
    ``_enforce_workspace_cap`` (que ordena por ``created_at_us`` para
    decidir "el más viejo") deja de tener ningún criterio real: cae en lo
    que sea que ``Path.glob()`` devuelva para desempatar -- que la propia
    documentación de la stdlib no promete en ningún orden en particular, y
    que aquí además no tiene ninguna relación con el orden de creación
    (los archivos se nombran con un UUID aleatorio). El resultado medible
    era que a veces se borraba el checkpoint equivocado.

    En vez de cambiar qué significa ``created_at_us`` para quien ya lo lee
    (sigue siendo microsegundos desde epoch, el esquema en disco no
    cambia), esta función simplemente nunca entrega un valor menor o igual
    al último que ya dio DENTRO de este proceso -- si el reloj del sistema
    no avanzó lo suficiente, se avanza a mano en 1 microsegundo. No
    resuelve empates ENTRE procesos distintos (dos companions corriendo a
    la vez sobre el mismo workspace no tienen ninguna fuente de verdad
    compartida de todas formas, ni siquiera con un reloj perfecto), pero
    ese no es el caso que rompía el test: una sola ``CheckpointStore``, un
    solo proceso, con su propio candado."""

    global _ultimo_now_us
    with _reloj_checkpoints_lock:
        valor = max(int(time.time() * 1_000_000), _ultimo_now_us + 1)
        _ultimo_now_us = valor
        return valor
