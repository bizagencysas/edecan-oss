...
_COMANDOS: tuple[ComandoIDE, ...] = (
    ...
    ComandoIDE(
        ...
        # Vive en el servidor multi-tenant (apps/api), no en este companion
        # local: pantalla "Panel" (/app/panel) para el consumo, "Facturación"
        # (/app/facturacion) para el plan en sí. ``/usage`` es informativo
        # -- ver ``_MENSAJE_INFORMATIVO["usage"]`` en ``ide_runtime.py``.
        capacidad="apps/api (edecan_schemas.plans, limits.*) + web /app/panel, /app/facturacion",
    ),
    ComandoIDE(
        nombre="memory",
        descripcion="Muestra o borra la memoria que el agente guardó de este workspace.",
        # OJO: NO es la memoria de cuenta de /app/memoria (otro producto,
        # edecan_core.memory, fuera del alcance de este companion) -- es la
        # memoria POR WORKSPACE del agente de código, ya cableada de verdad
        # en ide_runtime.py (bloque ``if nombre == "memory"``, mismo
        # ``ide_memoria.MemoriaStore`` que ``ide_memory_list``/
        # ``ide_memory_forget`` y que el botón "Memoria y conocimiento" del
        # IDE en la web).
        capacidad="ide_memoria.MemoriaStore (self.sessions.memoria) -- memoria del workspace",
        argumentos="opcional",
    ),
    ComandoIDE(
        nombre="agents",
    ...
    ComandoIDE(
        ...
        # El registro de servidores vive en el servidor multi-tenant
        # (apps/api/edecan_api/routers/mcp.py, /v1/mcp/servers), administrado
        # en Ajustes > Conexiones > "Herramientas externas (MCP)"
        # (/app/ajustes#conexiones). Este companion no lo administra: solo
        # EJECUTA las herramientas que ya le llegan armadas en cada turno
        # (ver ``ide_agent_mcp_pending``/``ide_agent_mcp_resolve``, que
        # resuelven una llamada puntual en curso, no un listado). ``/mcp``
        # es informativo -- ver ``_MENSAJE_INFORMATIVO["mcp"]``.
        capacidad="apps/api routers/mcp.py (/v1/mcp/servers) + web /app/ajustes#conexiones",
        argumentos="opcional",
    ),
    ComandoIDE(
        nombre="doctor",
    ...
    ComandoIDE(
        ...
        # Se conecta/prueba en Ajustes > Conexiones > "Voz"
        # (/app/ajustes#conexiones); el companion no sintetiza audio.
        # ``/voice`` es informativo -- ver ``_MENSAJE_INFORMATIVO["voice"]``.
        capacidad="packages/voice (sintetizar_voz) + web /app/ajustes#conexiones",
        argumentos="opcional",
    ),
    ComandoIDE(
        ...
        # El control remoto SÍ existe de verdad en este companion
        # (actions.py: captura de pantalla + teclado/mouse, gateado por
        # config.remote_input_enabled) pero se activa como sesión en vivo
        # con su propio consentimiento desde su propia pantalla dedicada,
        # "Control remoto" (/app/remoto, grupo "Herramientas técnicas" --
        # NO bajo Ajustes), nunca desde un comando de una línea de chat.
        # ``/remote-control`` es informativo por eso -- ver
        # ``_MENSAJE_INFORMATIVO["remote-control"]``.
        capacidad="apps/api routers/remote.py + companion actions.py + web /app/remoto",
        argumentos="opcional",
    ),
    ComandoIDE(
        nombre="workflows",
        descripcion="Lista los flujos de trabajo guardados.",
        # "Flujos de trabajo guardados" = Automatizaciones de la cuenta
        # (trigger + acción + corridas, apps/api routers/automations.py,
        # tabla automations/automation_runs), administradas en
        # /app/automatizaciones -- servidor multi-tenant, fuera del alcance
        # de este companion. NO son las Agent Skills de /app/skills (esas
        # son instrucciones en lenguaje natural para el agente, otro
        # concepto -- ver docs/skills.md). ``/workflows`` es informativo --
        # ver ``_MENSAJE_INFORMATIVO["workflows"]``.
        capacidad="apps/api routers/automations.py + web /app/automatizaciones",
        argumentos="opcional",
    ),
)
