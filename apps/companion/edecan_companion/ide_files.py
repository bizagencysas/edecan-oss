...
class FileService:
    ...
    def write(self, workspace_id: str, path: str, content: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Reintentos SOLO en Windows: un antivirus/indexador puede tener
            # `target` abierto un instante (ver
            # `platform_paths.reemplazar_con_reintentos`) -- en POSIX se
            # comporta exactamente igual que el `os.replace` de siempre.
            reemplazar_con_reintentos(temp, target)
        finally:
            if temp.exists():
