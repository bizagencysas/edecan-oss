...
# Instrucción real cuando la máquina donde corre el companion es Linux, para
# los tres linters cuyo `como_instalar` del catálogo cita `brew` (pensado
# para macOS): confirmado en vivo (edecan-prod, 1-ago-2026) que ninguno de
# los tres tiene paquete `apt` propio Y que `brew` no está instalado ahí --
# `brew install X` es una instrucción FALSA en Linux, misma familia que el
# bug ya corregido de `open_app`/portapapeles con `xdg-open`/`xclip`. Se
# resuelve en `_como_instalar_para_plataforma`, no en el catálogo, porque
# `_CATALOGO` se construye UNA vez al importar el módulo -- si el texto
# quedara fijo ahí, no reflejaría la plataforma real del proceso que
# pregunta (importa para las pruebas, que corren las dos ramas en la misma
# máquina vía `monkeypatch.setattr(sys, "platform", ...)`).
_INSTALACION_LINUX_DE_LINTERS_BREW: dict[str, str] = {
    "swiftlint": (
        "En Linux no hay `brew` por defecto. Instálalo compilándolo con el toolchain de "
        "Swift para Linux: `git clone https://github.com/realm/SwiftLint && cd SwiftLint "
        "&& swift build -c release`. Alternativa: instala Homebrew para Linux "
        "(https://brew.sh) y corre `brew install swiftlint`."
    ),
    "ktlint": (
        "En Linux no hay `brew` por defecto. Instálalo con el script oficial (necesita "
        "Java -- si no la tienes, `sudo apt install default-jre-headless` en "
        "Debian/Ubuntu o `sudo dnf install java-latest-openjdk-headless` en Fedora): "
        "`curl -sSLO "
        "https://github.com/pinterest/ktlint/releases/latest/download/ktlint && chmod a+x "
        "ktlint && sudo mv ktlint /usr/local/bin/`."
    ),
    "golangci-lint": (
        "En Linux no hay `brew` por defecto. Instálalo con el toolchain de Go: `go install "
        "github.com/golangci/golangci-lint/cmd/golangci-lint@latest` (deja el binario en "
        "$(go env GOPATH)/bin -- agrégalo al PATH)."
    ),
}


def _como_instalar_para_plataforma(definicion: _DefinicionLinter) -> str:
    """``definicion.como_instalar``, pero reemplazado por la variante Linux
    si esta plataforma es Linux y ese linter es uno de los que citan `brew`
    en el catálogo (ver `_INSTALACION_LINUX_DE_LINTERS_BREW`)."""
    if sys.platform.startswith("linux"):
        instalacion_linux = _INSTALACION_LINUX_DE_LINTERS_BREW.get(definicion.nombre)
        if instalacion_linux is not None:
            return instalacion_linux
    return definicion.como_instalar
    ...
    for definicion in _CATALOGO:
        ...
        if ejecutable is None:
            motivo = (
                ...
                f"{_como_instalar_para_plataforma(definicion)} No se descarga nada solo."
            )
        detectados.append(
            LinterDetectado(
