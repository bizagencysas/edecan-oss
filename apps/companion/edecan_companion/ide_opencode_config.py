...
def _cuentas_windows_candidatas() -> list[str]:
    """Lista ordenada de cuentas a probar con ``icacls``, sin duplicados.

    El hallazgo real (confirmado en vivo contra la VM, ver MEMORY.md de la
    fase de validación): ``_cuenta_windows_actual()`` arma
    ``{USERDOMAIN}\\{USERNAME}``, que en una máquina unida a un dominio AD
    de verdad es exactamente lo que ``icacls`` necesita -- pero en una
    máquina standalone/workgroup (el caso real medido, y el más común para
    quien instala esta app en su propia PC o en un servidor recién creado)
    ``USERDOMAIN`` vale ``WORKGROUP``, que ``icacls`` no puede resolver a
    un SID (error 1332). El resultado medido fue que el token de Cloudflare
    seguía legible por ``BUILTIN\\Users`` pese a que el código "best-effort"
    no revienta nada -- silenciosamente no protege lo que dice proteger.

    La sintaxis ``.\\usuario`` (un punto, sin nombre de dominio) es la forma
    documentada por Microsoft para referirse a una cuenta LOCAL sin
    necesidad de saber si la máquina está en un dominio o en un workgroup
    -- funciona en ambos casos porque no depende de resolver ningún nombre
    de dominio, solo del SAM local. Por eso es el candidato de respaldo
    universal aquí.

    Orden de intento:
    1. ``DOMINIO\\usuario`` -- solo si ``USERDOMAIN`` no es el workgroup por
       defecto (indicio real de dominio AD; ahorra un intento fallido en el
       caso común de VM/PC sin dominio).
    2. ``.\\usuario`` -- cuenta local, funciona en workgroup y, si la cuenta
       actual también existe como cuenta local, en dominio también.
    3. ``DOMINIO\\usuario`` con el workgroup por defecto -- último recurso,
       por si alguna vez existe de verdad un dominio AD llamado
       ``WORKGROUP`` (raro, pero más barato probarlo que no cubrir el caso).
    4. Si no hay ni ``USERDOMAIN`` ni ``USERNAME`` (proceso sin perfil de
       usuario cargado), se cae al mismo respaldo que ``_cuenta_windows_actual``
       (``getpass.getuser()``).

    ``_restringir_windows`` prueba estos candidatos en orden y se queda con
    el primero que ``icacls`` acepta (código de salida 0)."""
