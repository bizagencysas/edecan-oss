...
class PuenteDePermisos:
    ...
    async def _get(self, ruta: str, *, params: dict[str, Any] | None = None) -> httpx.Response:
        try:
            ...
            raise ErrorOpencode(f"No se pudo conectar con opencode en GET {ruta}: {exc}") from exc
    ...
    async def _post(self, ruta: str, *, json_body: dict[str, Any] | None = None) -> httpx.Response:
        try:
            ...
            raise ErrorOpencode(f"opencode no respondió a tiempo en POST {ruta}: {exc}") from exc
        except httpx.TransportError as exc:
            raise ErrorOpencode(f"No se pudo conectar con opencode en POST {ruta}: {exc}") from exc
        if resp.status_code >= 400:
