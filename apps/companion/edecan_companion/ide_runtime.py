...
from edecan_companion.ide_opencode import BinarioOpencodeNoEncontrado, ErrorOpencode
...
class IDERuntime:
    ...
    async def _lsp_servidor_para(self, workspace_id: str) -> Any:
        ...
        ``self.sessions.motor_opencode`` es ``None`` tanto cuando esta
        instalación corre el motor VIEJO (``EDECAN_IDE_MOTOR`` distinto de
        opencode) COMO cuando el motor vigente es opencode pero aún nadie
        corrió un turno de agente en este proceso (``motor_opencode`` es
        perezoso a propósito, ver ``SessionManager._asegurar_motor_opencode``).
        Preguntar solo por ``is None`` no distingue esos dos casos: medido en
        vivo con un ``SessionManager`` recién construido, con motor "opencode"
        vigente y SIN turnos corridos todavía, esto respondía "esta
        instalación todavía corre el motor anterior" -- FALSO, el motor
        vigente era opencode (auditoría, hallazgo "(a)", sección "el motor
        anterior"). El diagnóstico correcto pregunta el motor CONFIGURADO
        (``motor_vigente()``), no el objeto perezoso; y si el motor
        configurado SÍ es opencode, se inicializa aquí mismo en vez de
        rendirse -- vía ``servidor_opencode_para`` (no un ``await`` directo:
        ver su docstring para por qué el puente de permisos necesita correr
        en el loop persistente, no en el ``asyncio.run()`` desechable de
        este dispatch).
        """
        if self.sessions.motor_vigente() != "opencode":
            raise IDESessionError(
                "El LSP solo está disponible con el motor opencode; esta instalación "
                "tiene EDECAN_IDE_MOTOR configurado al motor anterior."
            )
        workspace_root = self.workspaces.root(workspace_id)
        return self.sessions.servidor_opencode_para(str(workspace_root))
...
async def execute_ide_action(
    ...
    # ``BinarioOpencodeNoEncontrado`` (RuntimeError, no ValueError/OSError):
    # ``ide_lsp_symbols``/``ide_lsp_status`` llegan hasta
    # ``self.sessions.motor_opencode.servidor_para(...)`` -- que la lanza si
    # no hay ``opencode`` en PATH/``EDECAN_OPENCODE_BIN`` -- y, a diferencia
    # de ``ide_lsp_definition``/``ide_lsp_references`` (que pasan por
    # ``LspOpencodeNoDisponibleError`` arriba), nada la atrapaba: escapaba
    # cruda de este método entero y llegaba como un 500 sin mensaje a la capa
    # HTTP en vez del ``{"ok": false, ...}`` de siempre (medido en vivo,
    # edecan-prod, 1-ago-2026: ``journalctl`` mostraba el traceback completo
    # detrás de un simple "Internal Server Error"). Va en ESTE except, no en
    # el de ``LspOpencodeNoDisponibleError`` de arriba: es un problema de
    # ENTORNO (falta instalar el binario), no una capacidad ausente para
    # siempre en la superficie de opencode.
    #
    # ``CredencialesCloudflareFaltantesError``/``CredencialCloudflareInvalidaError``/
    # ``ErrorConfiguracionOpencode`` son la MISMA familia de bug medida en
    # vivo (edecan-prod, 1-ago-2026) y encima se disparan ANTES que
    # ``BinarioOpencodeNoEncontrado`` en el camino real: ``generar_opencode_json``
    # corre antes de resolver el binario, así que en una instalación Linux
    # nueva (sin binario Y sin credenciales, el caso típico) el que de verdad
    # se recorre es este, y hasta ahora escapaba crudo igual que el otro.
    except (
        IDEWorkspaceError,
        IDECloneError,
        IDEFileError,
