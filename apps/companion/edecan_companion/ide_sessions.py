...
from edecan_companion import ide_modos, ide_refutador, pty_compat
...
_PATRON_SOLICITUD_ARCHIVO = re.compile(
    r"\b(?:crea|crear|escribe|escribir|genera|generar)\s+(?:el\s+)?archivo\s+"
    r"[`\"']?(?P<ruta>[A-Za-z0-9._/-]+)[`\"']?",
    re.IGNORECASE,
)
...
def _archivo_solicitado(prompt: str, workspace_root: Path) -> Path | None:
    """Devuelve un archivo relativo solicitado de forma explícita y segura."""
    match = _PATRON_SOLICITUD_ARCHIVO.search(prompt)
    if match is None:
        return None
    candidato = (workspace_root / match.group("ruta").strip()).resolve()
    try:
        candidato.relative_to(workspace_root.resolve())
    except ValueError:
        return None
    return candidato
...
class SessionManager:
    def __init__(
        ...
        # Cableado de ``ide_ejecutor_plan.py`` (plan EXTERNO, pegado por el
        # dueño desde otra IA -- ver el docstring de ese módulo, sección
        # "Integración"). A diferencia de ``self.plans`` de arriba (en
        # memoria, un solo paso activo, propuesto por EL MODELO), este vive
        # en disco bajo ``state_dir`` -- sobrevive un reinicio del companion,
        # que es justo el caso de uso: un documento largo con dependencias no
        # lineales que puede tardar más que una sesión de la app abierta.
        self.ejecutor_planes = EjecutorPlanStore(state_dir)
    ...
    def motor_vigente(self) -> str:
        """Envoltorio público de ``_motor_vigente`` -- lo necesita
        ``IDERuntime._lsp_servidor_para`` (otro módulo) para diagnosticar
        CORRECTAMENTE qué motor corre. Antes de que existiera, ese método
        preguntaba ``self.sessions.motor_opencode is None``, que confundía
        "el motor vigente es el viejo" con "el motor vigente es opencode
        pero todavía no se inicializó" (``motor_opencode`` es perezoso a
        propósito, ver ``_asegurar_motor_opencode``) -- daba el mismo
        diagnóstico falso en todo proceso recién arrancado. Ver también
        ``servidor_opencode_para`` un poco más abajo.
        """
        return self._motor_vigente()

    def servidor_opencode_para(self, directorio_trabajo: str | Path) -> Any:
        """Arranca (o reutiliza) el ``ServidorOpencode`` de ``directorio_trabajo``
        para un llamador que NO es un turno de agente (hoy: ``IDERuntime``
        para LSP) -- inicializa el motor perezoso si hace falta y corre la
        corrutina real en el loop PERSISTENTE de ``_bucle_opencode``, nunca
        en un ``asyncio.run()`` de una sola llamada como el que usa
        ``IDERuntime.dispatch`` para ``ide_lsp_status``/``ide_lsp_symbols``.

        Esto no es cosmético: ``MotorOpencode.servidor_para`` arranca
        ``PuenteDePermisos`` con ``asyncio.create_task(...)`` en el loop
        donde se lo llama, y ese puente tiene que sobrevivir ENTRE llamadas
        (docs/opencode-motor.md §3, "el puente arranca antes del primer
        prompt, siempre, y se queda escuchando"). Si se llamara con
        ``await`` directo desde ``IDERuntime`` (dentro del ``asyncio.run()``
        desechable de un solo dispatch), la tarea del puente moriría
        cancelada apenas ese loop se cerrara -- y el ``ServidorOpencode``
        quedaría registrado en ``MotorOpencode._entradas`` con un puente ya
        muerto, roto en silencio hasta que el companion reinicie. Pasar por
        ``_ejecutar_opencode`` (el mismo punto de entrada que usa CADA turno
        de agente) es lo que evita eso.
        """
        motor = self._asegurar_motor_opencode()
        return self._ejecutar_opencode(motor.servidor_para(directorio_trabajo))
    ...
    @staticmethod
    def _shell_candidate_exists(path: str) -> bool:
        """Envoltorio de `Path(path).exists()` -- punto de inyección para
        pruebas: monkeypatchear esto en vez del método `Path.exists` de la
        clase entera, que afectaría a CUALQUIER otro uso de `Path` durante
        el test (incluidos los del propio pytest)."""
    ...
    def _default_terminal_argv(cls) -> list[str]:
        ...
        if not shell:
            # Sin $SHELL -- típico cuando el companion corre como servicio
            # systemd, sin sesión de login (ver docs/linux.md) -- el
            # mejor terminal disponible casi siempre es bash, no
            # `/bin/sh` (dash en Debian/Ubuntu, la última opción de esta
            # lista). Medido en vivo (edecan-prod, 1-ago-2026): con dash, el
            # cliente sin querer manda una secuencia de escape (p. ej. la
            # flecha arriba, que dash no interpreta como historial) y esos
            # bytes quedan pegados al INICIO del siguiente comando que
            # llegue -- sin historial, sin autocompletado, y además
            # ensuciando lo próximo que el dueño escriba. `/bin/zsh` sigue
            # primero porque, si está, es el shell por defecto de macOS.
            for candidato in ("/bin/zsh", "/bin/bash", "/usr/bin/bash", "/bin/sh"):
                if cls._shell_candidate_exists(candidato):
                    shell = candidato
                    break
            else:
                shell = "/bin/sh"
    ...
    def _run_opencode_agent(
        ...
        try:
            ...
            solicitado = _archivo_solicitado(prompt_final, workspace_root)
            if cerro_con_texto and solicitado is not None and not solicitado.is_file():
                relativo = solicitado.relative_to(workspace_root)
                session.append(
                    "status",
                    f"Verificación: {relativo} no existe todavía; le pido a opencode "
                    "un único reintento.",
                )
                reintento = (
                    "Verificación obligatoria: el archivo solicitado todavía no existe en el "
                    f"workspace. Crea ahora `{relativo}` con exactamente lo pedido y confirma "
                    "solo después de comprobar que existe en disco."
                )
                cerro_reintento = self._ejecutar_opencode(
                    self._turno_opencode(
                        session=session,
                        workspace_root=workspace_root,
                        prompt=reintento,
                ...
                cerro_con_texto = cerro_reintento and solicitado.is_file()
                if not cerro_con_texto:
                    error_turno = IDESessionError(
                        "opencode confirmó el turno, pero no creó el artefacto solicitado: "
                        f"{relativo}"
                    )
        ...
        *,
        ...
        try:
            ...
            if isinstance(opencode_session_id, str) and opencode_session_id:
                try:
                    await servidor.obtener_sesion(opencode_session_id)
                    sesion_valida = True
                except ErrorOpencode:
                    logger.warning(
                        "La sesión de opencode %s ya no existe en el servidor -- se creará "
                        "una nueva.",
                        opencode_session_id,
                    )
                    opencode_session_id = None

            if not sesion_valida or not opencode_session_id:
    ...
    def responder_pregunta_agente(
        ...
        except ErrorOpencode as exc:
            raise IDESessionError(
                "La pregunta del agente ya no está activa o el turno fue detenido."
            ) from exc
        session.append(
    ...
    def rechazar_pregunta_agente(self, session_id: str, request_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info("Pregunta ya no estaba activa en opencode al rechazar: %s", exc)
    ...
    def _run_plan_execution(
        ...
        try:
            ...
            texto_final = resumen_final.resumen()
            exito_del_plan = resumen_final.exito_total

            # Cable 5: el sub-agente REFUTADOR -- ver ``_ejecutar_refutador``
            # para el gate de costo y ``ide_refutador`` para el prompt
            # adversarial. Corre DESPUÉS de que el reparto entero terminó
            # (nunca en paralelo con él: necesita ver el estado final de los
            # archivos, no uno a medio escribir) y nunca puede tumbar el
            # cierre del plan si él mismo revienta -- un refutador roto es
            # una auditoría que no corrió, no un motivo para dejar el turno
            # sin respuesta.
            veredicto_refutador: ide_refutador.VeredictoRefutador | None = None
            try:
                veredicto_refutador = asyncio.run(
                    self._ejecutar_refutador(
                        session,
                        workspace_id,
                        checkpoint_id,
                        plan,
            ...
            except Exception as exc:  # noqa: BLE001 - ver el comentario de arriba
                session.append(
                    "status",
                    f"Refutador: se cortó por un error interno ({exc}); se ignora su veredicto.",
                )
            if veredicto_refutador is not None:
                texto_final += veredicto_refutador.bloque_para_persona()
                if veredicto_refutador.verdict == "refutado":
                    # El punto entero de este cable: un plan cuyos pasos
                    # DIJERON "completada" cierra igual como fallido si una
                    # auditoría independiente, MEDIDA (ver
                    # ``VeredictoRefutador.tuvo_evidencia``), lo tumbó.
                    exito_del_plan = False

            session.append("assistant_final", texto_final)
        except Exception as exc:  # noqa: BLE001 - el cierre de abajo decide qué fue
    ...
    async def _ejecutar_refutador(
        ...
        pasos_por_id: dict[str, PasoReparto],
        *,
        cancelado: bool,
    ) -> ide_refutador.VeredictoRefutador | None:
        """Cable 5: un segundo sub-agente, independiente, que REFUTA lo que
        el reparto acaba de dar por terminado -- ver el docstring de
        ``ide_refutador`` para el porqué y la regla dura.

        Reusa la MISMA maquinaria que cualquier otro paso
        (``WorkersIDEAgent.run``, la misma que usa
        ``_ejecutar_paso_de_plan``), no una paralela: la independencia no
        viene de un mecanismo nuevo, viene de que este turno arranca con
        ``messages`` propio (``WorkersIDEAgent.run`` nunca comparte historial
        entre llamadas) y de un modelo distinto al del reparador
        (``ide_refutador.modelo_refutador``) -- nunca ve el razonamiento
        del reparador, solo el encargo original y lo que reportó.

        Devuelve ``None`` cuando el gate de costo decide que no vale la pena
        correrlo (``ide_refutador.motivo_para_omitir``) -- y siempre deja un
        evento en el hilo explicando por qué, corrido o no: el encargo pide
        que sea claro cuándo corre y cuándo no, no que quede en silencio.
        """
        checkpoint = self.checkpoints.get(checkpoint_id) if checkpoint_id is not None else None
        archivos_tocados = list((checkpoint or {}).get("paths") or [])
        completados = [
            pasos_por_id[id_]
            for id_, estado in estados.items()
            if estado.estado == "completada" and id_ in pasos_por_id
        ]

        motivo = ide_refutador.motivo_para_omitir(
            cancelado=cancelado,
            hay_pasos_completados=bool(completados),
            archivos_tocados=len(archivos_tocados),
        )
        if motivo is not None:
            session.append("status", f"Refutador: no corrió ({motivo}).")
            return None

        modelo = ide_refutador.modelo_refutador()
        pasos_para_refutar = [
            ide_refutador.PasoParaRefutar(
                titulo=paso.titulo,
                instrucciones=paso.instrucciones,
                resultado_reportado=(estados[paso.id].salida or "(sin salida reportada)")[:2000],
            )
            for paso in completados
        ]
        prompt = ide_refutador.construir_prompt(
            meta=plan.goal, pasos=pasos_para_refutar, archivos_tocados=archivos_tocados
        )
        session.append(
            "status",
            f"Refutador: auditando {len(archivos_tocados)} archivo(s) con {modelo} "
            "(independiente del agente que hizo el cambio, no ve su razonamiento).",
        )
        ...
        def write_event(
            ...
            *,
            ...
            if event_type == "tool":
                encontrado = re.match(r"Usando (\S+)\.", text)
                if encontrado:
                    herramientas_usadas.append(encontrado.group(1))
            elif event_type == "confirmation_required":
                vio_confirmacion_pendiente = True
            elif event_type == "assistant_final":
                respuesta_final = text
            session.append(event_type, f"[Refutador] {text}"[:2000], presentation=presentation)
        ...
        try:
            await self.workers_agent.run(
                ...
                model=modelo,
                semantic=self.semantic,
                # ``memoria=None`` a propósito: el refutador no debe dejar
                # notas de proyecto -- esa es responsabilidad del reparador,
                # y mezclar las dos ensuciaría de dónde vino cada recuerdo.
                # Mismo criterio para ``session_id``/``effort_store``/
                # ``plan_store``/``modo_store`` en ``None``: esta es una
                # corrida AISLADA con parámetros fijos, no atada al nivel de
                # esfuerzo ni al modo vigente de la conversación que aprobó
                # el plan -- su costo no debe moverse por una configuración
                # ambiental que nadie fijó pensando en el refutador.
            )
        except Exception as exc:  # noqa: BLE001 - un refutador roto es "no demostrado", no un fallo del plan
            session.append(
                "status", f"Refutador: se cortó por un error interno ({exc})."
            )
            return ide_refutador.VeredictoRefutador(
                verdict_bruto="no_demostrado",
                texto=f"El refutador no pudo terminar su auditoría: {exc}",
                modelo=modelo,
                herramientas_usadas=tuple(herramientas_usadas),
                archivos_auditados=tuple(archivos_tocados),
            )

        if vio_confirmacion_pendiente or respuesta_final is None:
            return ide_refutador.VeredictoRefutador(
                verdict_bruto="no_demostrado",
                texto="El refutador no entregó un veredicto (se detuvo antes de terminar, "
                "p. ej. pidió una herramienta que necesita confirmación humana).",
                modelo=modelo,
                herramientas_usadas=tuple(herramientas_usadas),
                archivos_auditados=tuple(archivos_tocados),
            )

        return ide_refutador.VeredictoRefutador.desde_respuesta(
            respuesta_final,
            modelo=modelo,
            herramientas_usadas=herramientas_usadas,
            archivos_auditados=archivos_tocados,
        )
    ...
    # ------------------------------------------------------------------ #
    # Plan EXTERNO (``ide_ejecutor_plan.py``): un documento que la persona
    # trae de otra IA (Antigravity, Codex...) y pega para que el IDE lo
    # ejecute paso a paso, sobreviviendo a cerrar la app. Ver el docstring
    # de ese módulo, sección "Integración", para el ciclo completo.
    # ------------------------------------------------------------------ #

    def preparar_plan_externo(
        self, markdown: str, *, titulo: str | None = None
    ) -> dict[str, Any]:
        """Interpreta ``markdown`` SIN persistir nada -- mismo principio que
        ``proponer_plan``/``approve_plan`` con el plan interno: la persona
        tiene que VER cómo se entendió el plan (pasos, dependencias,
        advertencias) antes de que exista algo ejecutable. Nunca lanza (ni
        ``interpretar_markdown`` lo hace): un plan roto vuelve con
        ``es_ejecutable: False`` y sus ``errores`` explícitos, no una
        excepción -- la persona corrige el texto y lo intenta de nuevo.
        """
        return interpretar_markdown(markdown, titulo_por_defecto=titulo).resumen()

    def crear_plan_externo(
        self,
        workspace_id: str,
        markdown: str,
        *,
        ...
        """Persiste un plan externo YA confirmado por la persona (ver
        ``preparar_plan_externo``). Valida ``workspace_id`` primero (mismo
        error que cualquier otra acción sobre un workspace inexistente) para
        no dejar un plan persistido en disco apuntando a un lugar que no
        existe -- ``avanzar_plan_externo`` lo necesita en cada paso.
        """
        self.workspaces.root(workspace_id)
        return self.ejecutor_planes.crear(
            markdown, session_id=session_id, workspace_id=workspace_id, titulo=titulo
        )

    async def avanzar_plan_externo(self, session_id: str, plan_id: str) -> dict[str, Any]:
        """Reclama y corre UN paso listo del plan externo ``plan_id``.

        Quien integre esto (comando/endpoint, todavía sin construir -- ver
        el docstring de ``ide_ejecutor_plan.py``) llama este método EN UN
        CICLO hasta que devuelva ``terminado: True``. Cada llamada:

        1. Reclama el próximo paso listo (``EjecutorPlanStore.iniciar_
           siguiente`` -- ``None`` si el plan ya terminó o está bloqueado
           por un paso fallado que alguien debe resolver primero con
           ``reintentar_paso``/``omitir_paso``).
        2. Lo corre como un turno normal y acotado de ``WorkersIDEAgent``
           (``_ejecutar_paso_de_plan_externo``) -- MISMO portón de
           verificación que cualquier otro turno (Cable 1, ver
           ``ide_workers_agent.MAX_REINTENTOS_DE_CIERRE_SIN_VERIFICAR``):
           este paso tampoco puede declararse "hecho" sin que el cambio
           real se haya verificado.
        3. Marca el paso ``hecho``/``fallado`` según el resultado REAL --
           nunca según lo que el sub-agente diga sobre sí mismo sin que el
           portón lo haya forzado a comprobarlo.

        Se ata a una sesión de agente YA EXISTENTE (``session_id``): ahí
        quedan los eventos del paso, visibles en el mismo hilo que
        cualquier otro turno. Usa el MISMO estado que ``start_agent``
        (``session.metadata["status"]``, bajo ``session._turn_lock``) para
        que un mensaje normal que llegue mientras el paso corre se encole
        en vez de arrancar un segundo turno simultáneo sobre el mismo
        workspace -- dos escrituras concurrentes al mismo archivo es
        exactamente la carrera que este método no puede permitirse crear.
        """
        session = self._get(session_id, "agent")
        with session._turn_lock:
            estado_previo = session.metadata.get("status")
            if estado_previo in _AGENT_BUSY_STATUSES:
                raise IDESessionError(
                    "Esta sesión ya tiene un turno en curso; espera a que termine "
                    "antes de avanzar el plan externo."
                )
            session.metadata["status"] = "running"
            self._save()

        try:
            plan = self.ejecutor_planes.obtener(plan_id)
            workspace_id = plan.get("workspace_id") or session.metadata.get("workspace_id")
            if not isinstance(workspace_id, str) or not workspace_id:
                raise IDEEjecutorPlanError(
                    "Este plan no tiene un workspace asociado; créalo con "
                    "crear_plan_externo."
                )
            paso = self.ejecutor_planes.iniciar_siguiente(plan_id)
            if paso is None:
                return {
                    "plan": self.ejecutor_planes.obtener(plan_id),
                    "paso": None,
            ...
            rules_block = rules.as_prompt_block() if rules else None
            checkpoint_id = self._create_turn_checkpoint(
                workspace_id, f"Plan externo: {str(paso['descripcion'])[:80]}"
            )
            ...
            try:
                respuesta = await self._ejecutar_paso_de_plan_externo(
                    session, workspace_id, paso, rules_block, track_file
                )
            except Exception as exc:  # noqa: BLE001 - un paso roto se marca fallado, no tumba el plan
                plan_actualizado = self.ejecutor_planes.marcar_fallado(
                    plan_id, paso["id"], motivo=str(exc)[:500]
                )
                if checkpoint_id is not None:
                    with contextlib.suppress(IDECheckpointError, OSError):
                        self.checkpoints.seal(checkpoint_id)
            ...
            plan_actualizado = self.ejecutor_planes.marcar_hecho(
                plan_id, paso["id"], nota=respuesta[:500]
            )
            if checkpoint_id is not None:
                with contextlib.suppress(IDECheckpointError, OSError):
                    self.checkpoints.seal(checkpoint_id)
            ...
            # Restaura el estado previo (normalmente "completed"/"failed" del
            # último turno normal de esta sesión) -- este método no es dueño
            # del ciclo de vida completo de la sesión, solo de la ventana en
            # la que un paso puntual corrió. No toca ``ended_at``: eso marca
            # el cierre de un TURNO normal, y restaurar el estado no es eso.
            with session._turn_lock:
                session.metadata["status"] = estado_previo or "completed"
                self._save()

    async def _ejecutar_paso_de_plan_externo(
        self,
        session: Session,
        workspace_id: str,
        paso: dict[str, Any],
        ...
        """Corre UN paso de un plan EXTERNO como un turno normal y acotado de
        ``WorkersIDEAgent`` -- MISMO ``MAX_TOOL_ROUNDS``, MISMO gate de
        herramientas peligrosas y MISMO portón de verificación
        (``ide_workers_agent.MAX_REINTENTOS_DE_CIERRE_SIN_VERIFICAR``) que
        cualquier otro turno: un paso de un plan externo tampoco puede
        declararse "hecho" sin que el cambio real se haya verificado.

        Gemelo de ``_ejecutar_paso_de_plan`` (la misma idea para el plan
        INTERNO de ``ide_plan``/``ide_equipo``) -- se separa en vez de
        unificar porque las formas de origen no coinciden: un ``PasoPlan``
        de ``ide_ejecutor_plan`` no declara rutas exclusivas (ese módulo
        modela dependencias entre pasos, no reparto de archivos), así que
        no hay "alcance" que anunciarle al sub-agente como sí hace
        ``_ejecutar_paso_de_plan``.
        """
        detalle = str(paso.get("detalle") or "").strip()
        cuerpo_detalle = f"\n\nDetalle de este paso:\n{detalle}" if detalle else ""
        prompt = (
            "Estás ejecutando UN paso de un plan externo que la persona trajo de "
            "otra IA y ya confirmó explícitamente; no propongas un plan nuevo. Sé "
            "conservador: toca solo lo que este paso pide.\n\n"
            f"Paso a ejecutar: {paso['descripcion']}{cuerpo_detalle}"
        )
        vio_confirmacion_pendiente = False
        respuesta_final: str | None = None
        titulo_paso = str(paso.get("descripcion") or "paso")[:80]
        ...
        def write_event(
            ...
            *,
            ...
            if event_type == "confirmation_required":
                ...
                respuesta_final = text
            session.append(
                event_type, f"[{titulo_paso}] {text}"[:2000], presentation=presentation
            )
        ...
        if vio_confirmacion_pendiente:
            raise RuntimeError(
                "Este paso pidió una herramienta que necesita confirmación humana "
                "explícita; no se ejecutó nada. Resuélvelo por fuera del plan y "
                "vuelve a avanzar el plan externo."
            )
        if respuesta_final is None:
            raise RuntimeError("El paso terminó sin una respuesta final del sub-agente.")
        return respuesta_final
