...
``ide_sessions.py`` ni de ``ide_workers_agent.py``. Lo inverso YA NO es
cierto -- este docstring lo decía antes y quedó mintiendo tras cablearse:
``ide_workers_agent.py`` importa ``ejecutar_intento``/``detectar_comando_de_
verificacion`` (ver ``_run_verification``) y los expone como la tool
``verificar``, un intento por llamada, con el estado del turno
(``verification_state``) llevando la cuenta de si el último cambio real
(escribir/editar archivo) quedó cubierto por un ``verificar`` posterior --
ver ``MAX_REINTENTOS_DE_CIERRE_SIN_VERIFICAR`` y el portón de salida al
final de ``WorkersIDEAgent.run()``. Eso es lo que cierra el hueco que este
módulo describía: el turno YA NO puede declarar terminado un cambio que
nunca se verificó.

Lo que SIGUE sin cablear es el bucle completo, ``ejecutar_hasta_que_pase``
con una función ``arreglar`` automática: hoy quien "arregla" entre un
intento fallido de 'verificar' y el siguiente es el propio modelo, dentro de
su bucle normal de turnos (``run()``, hasta ``MAX_TOOL_ROUNDS``) -- cada
ronda en la que llama 'verificar', lee el resumen, corrige con
escribir_archivo/editar_archivo y vuelve a llamar 'verificar' YA es ese
mismo bucle, sin reimplementarlo (ver el docstring de ``_run_verification``
en ``ide_workers_agent.py``). ``ejecutar_hasta_que_pase`` en sí solo tiene un
llamador de producción indirecto por ahora: ``ide_lint.py`` lo usa para su
propio ``_puente`` interno, pero ``ide_lint.py`` mismo todavía no tiene
ningún importador de producción fuera de sus tests.
