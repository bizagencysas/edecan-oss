...
def test_macos_ventanas_visibles_pone_al_frente_la_primera(monkeypatch):
    monkeypatch.setattr(actions.sys, "platform", "darwin")
        ...
        def CGWindowListCopyWindowInfo(options, window_id):
            ...
            return [
                ...
                },
                {
                    ...
                    "kCGWindowOwnerName": "Control Center",
                    "kCGWindowName": "",
                },
            ]
...
def test_screenshot_para_el_modelo_recorta_y_adjunta_ocr(companion_config, monkeypatch):
    monkeypatch.setattr(actions.sys, "platform", "darwin")
    png = _png_escritorio(2400, 1600)
    monkeypatch.setattr(
        actions,
        "_screenshot_via_screencapture",
        lambda params: (png, 2400, 1600, 0, 0),
    )
    monkeypatch.setattr(
        actions,
        "_macos_ventanas_crudas",
        lambda: [
            {
                "app": "Cursor",
                "titulo": "Edecan-Nuevo",
    ...
    result = actions._screenshot(
        {"format": "webp", "quality": 90, "max_width": 2560, "crop_frontmost": True},
        companion_config,
    )

    assert result["ventanas"][0]["app"] == "Cursor"
    assert result["foco"]["valor"] == "hola"
    assert result["texto_visible"] == ["hola del chat"]
    assert result["crop_b64"]
    assert result["crop_mime"] in {"image/webp", "image/jpeg"}
    assert result["crop_width"] < 2400


def test_screenshot_remoto_no_hace_ocr(companion_config, monkeypatch):
    monkeypatch.setattr(actions.sys, "platform", "darwin")
    monkeypatch.setattr(
        actions,
        "_screenshot_via_screencapture",
        lambda params: (b"\x89PNG\r\n\x1a\nfake-image-bytes", 1512, 982, 0, 0),
    )

    def boom(_data):
        raise AssertionError("el visor remoto no debe OCR")

    monkeypatch.setattr(actions, "_ocr_vision_macos", boom)
    result = actions._screenshot({}, companion_config)
    assert "texto_visible" not in result
    assert "crop_b64" not in result
...
def test_screenshot_options_acepta_webp():
    assert actions._screenshot_options({"format": "webp", "quality": 80, "max_width": 1920}) == (
        "webp",
        80,
        1920,
    )
...
def test_screenshot_options_rechaza_formatos_desconocidos():
    with pytest.raises(actions.ActionError, match="png.*jpeg.*webp"):
        actions._screenshot_options({"format": "gif"})


def _png_escritorio(width: int, height: int) -> bytes:
    from PIL import Image, ImageDraw

    image = Image.new("RGB", (width, height), (28, 28, 30))
    draw = ImageDraw.Draw(image)
    draw.rectangle([0, 0, width, 52], fill=(40, 40, 44))
    draw.rectangle([0, 52, 320, height], fill=(36, 36, 38))
    for i in range(10):
        top = 80 + i * 110
        draw.rectangle([360, top, width - 48, top + 88], fill=(48, 48, 52))
    buffer = io.BytesIO()
    image.save(buffer, format="PNG")
    return buffer.getvalue()


def test_optimize_screenshot_webp_baja_el_peso_antes_de_varios_megas():
    png = _png_escritorio(2400, 1600)

    datos, width, height, mime = actions._optimize_screenshot(
        png,
        width=2400,
        height=1600,
        image_format="webp",
    ...
    assert width == 1920
    assert height == 1280
    assert len(datos) < 400_000
    if mime == "image/webp":
        assert datos[:4] == b"RIFF" and datos[8:12] == b"WEBP"


def test_optimize_screenshot_webp_cae_a_jpeg_si_no_hay_encoder(monkeypatch):
    png = _png_escritorio(800, 600)
    monkeypatch.setattr(actions, "_pillow_tiene_webp", lambda: False)
...
# ---------------------------------------------------------------------------
# _screenshot_via_mss en Linux -- `mss` no está instalado en esta suite (es
# un extra opcional), así que se inyecta un doble en `sys.modules` (mismo
# patrón que `_install_fake_quartz` en `test_actions_input.py`).
#
# El caso que importa: medido en vivo (edecan-prod, 1-ago-2026, sin
# $DISPLAY) el error real de `mss` fue `mss.linux.xcbhelpers.XError`, que
# hereda DIRECTO de `Exception` -- ni de OSError, RuntimeError ni ValueError.
# Con el `except` angosto de antes, ese fallo escapaba crudo. Estas pruebas
# fijan que CUALQUIER excepción de `mss` (no solo esas tres) se convierte en
# `ActionError`.
# ---------------------------------------------------------------------------


class _ErrorDeMssQueNoHeredaDeNada(Exception):
    """Doble de `mss.linux.xcbhelpers.XError`: hereda directo de `Exception`,
    exactamente la forma medida en vivo que el `except` viejo no atrapaba."""


def _install_fake_mss(monkeypatch, *, al_capturar=None, monitores=None):
    import sys
    import types

    monitores = monitores or [
    ...
    modulo_mss = types.ModuleType("mss")
    modulo_mss.mss = lambda: _CapturaFalsa()  # type: ignore[attr-defined]
    modulo_tools = types.ModuleType("mss.tools")
    modulo_tools.to_png = lambda rgb, size: b"PNGDATA"  # type: ignore[attr-defined]
    modulo_mss.tools = modulo_tools  # type: ignore[attr-defined]

    monkeypatch.setitem(sys.modules, "mss", modulo_mss)
    monkeypatch.setitem(sys.modules, "mss.tools", modulo_tools)
    return modulo_mss


def test_screenshot_via_mss_wraps_an_exception_that_inherits_only_from_exception(monkeypatch):
    """El caso medido en vivo: ni OSError, ni RuntimeError, ni ValueError."""
    monkeypatch.setattr(actions.sys, "platform", "linux")
    _install_fake_mss(monkeypatch, al_capturar=_ErrorDeMssQueNoHeredaDeNada("sin display"))

    with pytest.raises(actions.ActionError, match="X11/Wayland"):
        actions._screenshot_via_mss({})


def test_screenshot_via_mss_still_reraises_action_error_untouched(monkeypatch):
    """El `except ActionError: raise` sigue funcionando: el widening a
    `Exception` no debe envolver un `ActionError` propio (p. ej. 'display'
    fuera de rango) en un mensaje genérico."""
    monkeypatch.setattr(actions.sys, "platform", "linux")
    _install_fake_mss(monkeypatch)

    with pytest.raises(actions.ActionError, match="fuera de rango"):
        actions._screenshot_via_mss({"display": 99})


def test_screenshot_via_mss_captures_successfully_on_linux(monkeypatch):
    monkeypatch.setattr(actions.sys, "platform", "linux")
    _install_fake_mss(monkeypatch)

    image_bytes, width, height, origin_x, origin_y = actions._screenshot_via_mss({"display": 1})

    assert image_bytes == b"PNGDATA"
    assert (width, height) == (1920, 1080)


def test_screenshot_via_mss_fills_in_display_from_discovered_session_before_capturing(
    monkeypatch,
):
    """En Linux, antes de tocar `mss` (que habla X11/Wayland EN ESTE
    PROCESO, no por subprocess), se rellena `os.environ` con lo que
    `linux_session` descubra -- `setdefault`, nunca pisa lo explícito."""
    monkeypatch.setattr(actions.sys, "platform", "linux")
    monkeypatch.delenv("DISPLAY", raising=False)
    monkeypatch.setattr(
        actions.linux_session, "descubrir_variables_de_sesion", lambda: {"DISPLAY": ":10.0"}
    )
    _install_fake_mss(monkeypatch)

    actions._screenshot_via_mss({})

    assert actions.os.environ.get("DISPLAY") == ":10.0"
