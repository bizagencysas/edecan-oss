...
# ---------------------------------------------------------------------------
# _PynputInputBackend.__init__ en Linux -- distinguir "no está instalado" de
# "está instalado pero no hay sesión X11", medido en vivo (edecan-prod,
# 1-ago-2026): pynput SIEMPRE levanta `ImportError` en Linux cuando no puede
# conectarse a X11 (así lo diseñó la propia librería), aunque el paquete esté
# instalado -- el mensaje viejo ("instala el extra") era una instrucción
# falsa en ese caso, misma familia que el `brew install` en Linux.
# ---------------------------------------------------------------------------


def _forzar_import_pynput_falla(monkeypatch) -> None:
    """`from pynput import keyboard, mouse` levanta `ModuleNotFoundError`
    (subclase de `ImportError`) sin necesitar que pynput esté instalado de
    verdad: un paquete falso con `__path__` vacío hace que Python no
    encuentre los submódulos `pynput.keyboard`/`pynput.mouse`."""
    paquete_falso = types.ModuleType("pynput")
    paquete_falso.__path__ = []  # type: ignore[attr-defined]
    monkeypatch.setitem(sys.modules, "pynput", paquete_falso)
    monkeypatch.delitem(sys.modules, "pynput.keyboard", raising=False)
    monkeypatch.delitem(sys.modules, "pynput.mouse", raising=False)


def test_pynput_backend_reports_missing_extra_when_the_package_is_really_absent(monkeypatch):
    monkeypatch.setattr(actions.sys, "platform", "linux")
    _forzar_import_pynput_falla(monkeypatch)
    monkeypatch.setattr(actions.importlib.util, "find_spec", lambda nombre: None)

    with pytest.raises(actions.ActionError, match="remote-control"):
        actions._PynputInputBackend()


def test_pynput_backend_reports_missing_display_when_the_package_is_installed(monkeypatch):
    """El paquete SÍ está (find_spec no da None), pero la importación igual
    falla con ImportError -- Linux, sin $DISPLAY. El mensaje debe hablar de
    la sesión gráfica, no repetir la instrucción de instalar el extra."""
    monkeypatch.setattr(actions.sys, "platform", "linux")
    _forzar_import_pynput_falla(monkeypatch)

    def _find_spec_falso(nombre: str):
        return object() if nombre == "pynput" else None

    monkeypatch.setattr(actions.importlib.util, "find_spec", _find_spec_falso)

    with pytest.raises(actions.ActionError, match=r"\$DISPLAY") as exc_info:
        actions._PynputInputBackend()

    assert "remote-control" not in str(exc_info.value)


def test_pynput_backend_distinguishes_invalid_display_from_no_display(monkeypatch):
    """Hallazgo medido en vivo (edecan-prod, 1-ago-2026): con `$DISPLAY=:99`
    (puesta pero inválida) el mensaje viejo decía "no la tiene ni pudo
    descubrirla", que era falso -- SÍ la tenía. El mensaje debe citar el
    valor puesto, no afirmar que no hay ninguna pista."""
    monkeypatch.setattr(actions.sys, "platform", "linux")
    ...
    with pytest.raises(actions.ActionError, match=r":99") as exc_info:
        actions._PynputInputBackend()

    assert "no responde" in str(exc_info.value)
    assert "ni pudo descubrirla" not in str(exc_info.value)


def test_pynput_backend_fills_in_display_from_discovered_session(monkeypatch):
    """Antes de intentar importar, en Linux se rellenan DISPLAY/WAYLAND_DISPLAY
    descubiertos si el proceso no los tiene -- confirma que se llama a
    `linux_session.descubrir_variables_de_sesion` y que `os.environ` se
    actualiza (sin pisar nada explícito)."""
    monkeypatch.setattr(actions.sys, "platform", "linux")
