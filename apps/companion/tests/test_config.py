...
def test_allow_all_apps_opt_in_no_habilita_comandos_ni_remote_input(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config_path = tmp_path / "companion.yaml"
    config_path.write_text("allowed_commands: []\n", encoding="utf-8")
    monkeypatch.setenv("EDECAN_ALLOW_ALL_APPS", "1")

    config = load_config(config_path)

    assert config.allow_all_apps is True
    assert config.allowed_commands == []
    assert config.auto_approve == ["open_app"]
    assert config.remote_input_enabled is False
