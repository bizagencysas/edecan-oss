...
def test_las_rutas_absolutas_no_se_persisten(tmp_path: Path):
    ...
    autocritica = _store(tmp_path)
    firma_con_ruta = (
        "exit=1:FAILED /Users/persona-de-prueba/repos/api/tests/test_pagos.py::test_cobro"
        " - AssertionError\n"
        "/Users/persona-de-prueba/repos/api/src/pagos.py:41: TypeError"
    )

    autocritica.registrar_bucle(
        _bucle(_intento(1, firma=firma_con_ruta), _intento(2, aprobado=True)),
        arreglo="Convierte el monto a Decimal antes de sumarlo en "
        "/Users/persona-de-prueba/repos/api/src/pagos.py.",
    )
...
def test_el_comando_guardado_tampoco_lleva_la_ruta_personal(tmp_path: Path):
    ...
    intento_con_venv = ResultadoIntento(
        intento=1,
        argv=("/Users/persona-de-prueba/repos/api/.venv/bin/python", "-m", "pytest", "-q"),
        aprobado=False,
        exit_code=1,
...
def test_la_firma_se_recupera_venga_con_ruta_absoluta_o_ya_normalizada(tmp_path: Path):
    autocritica = _store(tmp_path)
    firma_con_ruta = "exit=1:/Users/persona-de-prueba/repos/api/tests/test_a.py:9: SyntaxError"
    autocritica.registrar_bucle(
        _bucle(_intento(1, firma=firma_con_ruta), _intento(2, aprobado=True)),
    ...
    firma_de_otra_maquina = "exit=1:/home/otra/checkout/api/tests/test_a.py:9: SyntaxError"
