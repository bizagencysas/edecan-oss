...
# --------------------------------------------------------------------- #
# Frontera del workspace: un symlink hacia afuera no debe poder indexarse
# ni consultarse (fuga medida en la auditoría: /etc/passwd vía enlace.txt).
# --------------------------------------------------------------------- #


def test_symlink_hacia_afuera_no_se_indexa(tmp_path: Path):
    embedder = _EmbedderFalso("root")
    service, project, workspace_id = _make_service(tmp_path, embedder=embedder)
    objetivo_externo = tmp_path / "fuera_del_workspace.txt"
    objetivo_externo.write_text("root:x:0:0:root:/root:/bin/bash\n", encoding="utf-8")
    (project / "enlace.txt").symlink_to(objetivo_externo)
    (project / "adentro.py").write_text("un archivo normal, sin nada de root\n", encoding="utf-8")

    estado = service.build_index(workspace_id)

    assert estado["last_run"]["ok"] is True  # no crashea con el symlink presente
    assert estado["total_files"] == 1  # solo adentro.py; enlace.txt quedó afuera

    encontrado = service.search(workspace_id, "root")
    rutas = {fila["path"] for fila in encontrado["matches"]}
    assert "enlace.txt" not in rutas
    for fila in encontrado["matches"]:
        assert "root:x:0:0" not in fila["excerpt"]


def test_symlink_hacia_afuera_no_se_devuelve_ni_con_indice_viejo(tmp_path: Path):
    """Si el índice en disco ya tenía (por una versión anterior del código,
    o por edición manual del JSON) un chunk apuntando fuera del workspace,
    la frontera de CONSULTA -- no solo la de indexado -- debe descartarlo.
    """
    embedder = _EmbedderFalso("root")
    service, project, workspace_id = _make_service(tmp_path, embedder=embedder)
    objetivo_externo = tmp_path / "fuera_del_workspace.txt"
    objetivo_externo.write_text("root:x:0:0:root:/root:/bin/bash\n", encoding="utf-8")
    (project / "enlace.txt").symlink_to(objetivo_externo)
    service.build_index(workspace_id)  # construye el índice legítimo (sin el symlink)

    # Simula un índice contaminado: inyecta a mano un chunk que apunta al
    # symlink, como si hubiera quedado de una versión con la fuga.
    index_path = service._index_path(workspace_id)
    import json as _json

    payload = _json.loads(index_path.read_text(encoding="utf-8"))
    payload["chunks"]["enlace.txt:1-1"] = {
        "path": "enlace.txt",
        "start_line": 1,
        "end_line": 1,
    ...
    payload["files"]["enlace.txt"] = {"size": 10, "mtime_ns": 0, "chunk_ids": ["enlace.txt:1-1"]}
    index_path.write_text(_json.dumps(payload), encoding="utf-8")
...
# --------------------------------------------------------------------- #
# Invalidación al borrar: un archivo borrado debe desaparecer de los
# resultados de búsqueda AUNQUE todavía no haya corrido un reindexado
# completo desde que se borró (el índice en disco puede estar viejo).
# --------------------------------------------------------------------- #


def test_archivo_borrado_desaparece_de_la_busqueda_antes_de_reindexar(tmp_path: Path):
    embedder = _EmbedderFalso("contraseña")
    service, project, workspace_id = _make_service(tmp_path, embedder=embedder)
    (project / "a.py").write_text("contraseña de a\n", encoding="utf-8")
    (project / "b.py").write_text("contraseña de b\n", encoding="utf-8")
    ...
    (project / "b.py").unlink()
    # Ojo: NO se llama a build_index/start_indexing acá -- el índice en
    # disco todavía tiene los chunks de "b.py". La búsqueda debe igual no
    # devolverlo.
    encontrado = service.search(workspace_id, "contraseña", auto_index=False)

    rutas = {fila["path"] for fila in encontrado["matches"]}
    assert "b.py" not in rutas
    assert "a.py" in rutas
