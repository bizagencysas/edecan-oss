...
import pytest
from edecan_companion import ide_checkpoints as ide_checkpoints_module
...
def test_now_us_nunca_repite_aunque_el_reloj_del_sistema_no_avance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Cierre de Windows: reproduce en vivo (sin mockear ``_now_us`` en sí,
    solo el reloj de abajo) el hallazgo real que rompía
    ``test_workspace_cap_evicts_oldest_checkpoint`` de forma intermitente en
    la VM -- ``time.time()`` en Windows puede devolver el MISMO valor en
    llamadas sucesivas (resolución de reloj mucho más gruesa que en
    macOS/Linux). Congelando ``time.time()`` a un único valor fijo aquí se
    fuerza ese empate a propósito, y se confirma que ``_now_us()`` sigue
    entregando valores estrictamente crecientes de todas formas."""

    ide_checkpoints_module._ultimo_now_us = 0  # aislar de otros tests del proceso
    monkeypatch.setattr(ide_checkpoints_module.time, "time", lambda: 1_000.0)

    valores = [ide_checkpoints_module._now_us() for _ in range(5)]

    assert valores == sorted(set(valores))  # estrictamente creciente, sin repetidos
    assert len(set(valores)) == 5
