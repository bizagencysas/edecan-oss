...
def _limpiar_entorno_de_resolucion(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    (``sys._MEIPASS``, ``PATH``, la variable de entorno, y carpetas fijas en
    home/opt), cada test arranca de un estado conocido: sin bundle, sin
    variable, con un ``PATH`` vacío, y con home apuntando a un directorio nulo
    -- así ningún resto del entorno de quien corre la suite (por ejemplo, que ESTA
    Mac sí tenga opencode real en ``~/.opencode/bin``) contamina los tests."""
    monkeypatch.delattr(sys, "_MEIPASS", raising=False)
    monkeypatch.delenv(VARIABLE_ENTORNO_BINARIO, raising=False)
    monkeypatch.setenv("PATH", "")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: Path("/nonexistent_test_home")))
