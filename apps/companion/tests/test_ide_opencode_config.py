...
_RUN_REAL_EXTERNAL_TESTS = os.environ.get("EDECAN_RUN_REAL_EXTERNAL_TESTS") == "1"

requiere_credenciales = pytest.mark.skipif(
    not (_RUN_REAL_EXTERNAL_TESTS and _CUENTA_CLOUDFLARE and _TOKEN_CLOUDFLARE),
    reason=(
        "Los tests externos requieren EDECAN_RUN_REAL_EXTERNAL_TESTS=1 y credenciales "
        "Cloudflare explícitas; por defecto se saltan para no tocar servicios reales."
    ),
)
...
def test_cuenta_windows_actual_usa_userdomain_y_username(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("USERNAME", "example")
    assert _cuenta_windows_actual() == "MI-PC\\example"
...
def test_cuenta_windows_actual_cae_a_getpass_sin_userdomain_username(
    ...
    monkeypatch.setattr(ide_opencode_config_module.getpass, "getuser", lambda: "example-posix")
    assert _cuenta_windows_actual() == "example-posix"
...
def test_restringir_windows_arma_el_comando_icacls_esperado(
    ...
    """El comando exacto, con el SID -- no con el nombre de cuenta.

    Este test afirmaba antes ``MI-PC\\example:F``. Se midió en vivo contra la VM
    real (Windows Server 2022, cuenta local, ``USERDOMAIN=WORKGROUP``) que
    NINGÚN nombre resuelve: tanto ``.\\administrator`` como
    ``WORKGROUP\\administrator`` fallan con **error 1332** y el archivo se queda
    con los permisos heredados, es decir, con el token de Cloudflare legible.
    Con el SID, la misma llamada devuelve 0 y la lista queda en SYSTEM + el
    usuario. Por eso el SID va primero y es lo que este test fija.
    """
    ...
    def _run_falso(comando: list[str], **kwargs: object) -> subprocess.CompletedProcess:
        ...
        # La consulta del SID va por PowerShell; icacls es la segunda llamada.
        salida = sid if comando and comando[0] == "powershell" else ""
        return subprocess.CompletedProcess(comando, returncode=0, stdout=salida, stderr="")

    monkeypatch.setattr(ide_opencode_config_module.subprocess, "run", _run_falso)

    _restringir_windows(ruta)

    icacls = [c for c, _ in llamadas if c and c[0] == "icacls"]
    assert len(icacls) == 1, f"se esperaba UNA llamada a icacls, hubo {len(icacls)}"
    comando = icacls[0]
    assert comando[1] == str(ruta)
    assert "/inheritance:r" in comando
    assert "/grant:r" in comando
    assert f"*{sid}:F" in comando, "debe usar el SID, no el nombre de cuenta"
    assert "*S-1-5-18:F" in comando or "SYSTEM:F" in comando
    # Nunca shell=True -- mismo criterio que edecan_mcp.transport._argv_ejecutable.
    for _, kwargs in llamadas:
        assert kwargs.get("shell") is not True


def test_restringir_windows_sin_powershell_cae_a_los_nombres_de_cuenta(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Si no se puede consultar el SID (PowerShell ausente), se intentan los
    nombres igual -- valen en un dominio real, y es mejor intentarlo que dejar
    el archivo sin restringir."""
    ...
    def _run_falso(comando: list[str], **kwargs: object) -> subprocess.CompletedProcess:
        if comando and comando[0] == "powershell":
            return subprocess.CompletedProcess(comando, returncode=1, stdout="", stderr="")
        if comando and comando[0] == "powershell":
            raise OSError("no hay powershell en esta maquina")
...
def test_candidatas_con_dominio_real_prueba_dominio_primero(
    ...
    monkeypatch.setenv("USERDOMAIN", "MI-PC")
    monkeypatch.setenv("USERNAME", "example")
    assert _cuentas_windows_candidatas() == ["MI-PC\\example", ".\\example"]
...
def test_candidatas_sin_username_ni_userdomain_cae_a_getpass(
    ...
    monkeypatch.delenv("USERNAME", raising=False)
    monkeypatch.setattr(ide_opencode_config_module.getpass, "getuser", lambda: "example-posix")
    assert _cuentas_windows_candidatas() == ["example-posix"]
...
def test_restringir_windows_si_el_primer_candidato_falla_prueba_el_siguiente(
    ...
    def _run_falso(comando: list[str], **kwargs: object) -> subprocess.CompletedProcess:
        if comando and comando[0] == "powershell":
            return subprocess.CompletedProcess(comando, returncode=1, stdout="", stderr="")
        llamadas.append(comando)
        if comando[4] == "MI-DOMINIO\\example:F":
    ...
    assert [c[4] for c in llamadas] == ["MI-DOMINIO\\example:F", ".\\example:F"]
...
def test_restringir_windows_sin_cuenta_resuelta_no_llama_a_icacls(
    ...
    llamadas: list[list[str]] = []
    # Ni SID ni nombre: se limpia el entorno además del respaldo, porque
    # `_cuentas_windows_candidatas` mira USERNAME/USERDOMAIN por su cuenta.
    monkeypatch.delenv("USERNAME", raising=False)
    monkeypatch.delenv("USERDOMAIN", raising=False)
    monkeypatch.setattr(
    ...
    assert llamadas == [], "sin SID ni nombre de cuenta no se debe intentar nada"
...
def test_asegurar_permisos_restrictivos_en_windows_llama_a_restringir_windows_no_a_chmod(
    ...
    monkeypatch.setattr(ide_opencode_config_module, "_restringir_windows", llamadas_windows.append)
    monkeypatch.setattr(
...
def test_no_migra_variantes_a_un_provider_del_dueno_aunque_le_falten(tmp_path: Path) -> None:
    ...
    assert (
        "variants"
        not in contenido["provider"][PROVEEDOR_WORKERS_AI_ID]["models"][
...
async def test_generar_y_comprobar_extremo_a_extremo(tmp_path: Path) -> None:
    ...
    # El servidor puede conocer modelos adicionales publicados después de que
    # este workspace generó su configuración; lo importante es que no pierda
    # ninguno de los modelos que Edecán declaró.
    assert set(generado.modelos) <= set(info.modelos)
