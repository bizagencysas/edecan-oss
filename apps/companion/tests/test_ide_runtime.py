...
# ---------------------------------------------------------------------------
# pick_workspace_folder en Linux -- ver el docstring del bloque en
# ide_workspaces.py sobre el hallazgo medido en vivo: sin DBUS, zenity se
# queda 5 minutos colgado en vez de fallar rápido; y `entorno_fusionado()`
# debe pasarse al subprocess para que zenity SÍ pueda mostrarse cuando la
# sesión gráfica solo la tiene otro proceso del mismo usuario.
# ---------------------------------------------------------------------------


def test_linux_picker_fails_fast_without_any_graphical_session(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(ide_workspaces_module.sys, "platform", "linux")
    monkeypatch.setattr(ide_workspaces_module.os, "name", "posix")
    monkeypatch.setattr(ide_workspaces_module.linux_session, "entorno_fusionado", lambda: {})

    def _run_no_deberia_llamarse(*_args, **_kwargs):
        raise AssertionError("no debía intentar abrir zenity/kdialog sin sesión gráfica")

    monkeypatch.setattr(ide_workspaces_module.subprocess, "run", _run_no_deberia_llamarse)

    with pytest.raises(IDEWorkspaceError, match=r"\$DISPLAY"):
        ide_workspaces_module.pick_workspace_folder()


def test_linux_picker_passes_the_discovered_session_env_to_zenity(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    project = _project(tmp_path, "seleccion-linux")
    monkeypatch.setattr(ide_workspaces_module.sys, "platform", "linux")
    monkeypatch.setattr(ide_workspaces_module.os, "name", "posix")
    monkeypatch.setattr(
        ide_workspaces_module.linux_session,
        "entorno_fusionado",
        lambda: {"DISPLAY": ":10.0", "DBUS_SESSION_BUS_ADDRESS": "unix:path=/tmp/dbus-x"},
    )
    monkeypatch.setattr(ide_workspaces_module.shutil, "which", lambda nombre: f"/usr/bin/{nombre}")
    ...
    assert selected == str(project.resolve())
    assert observado["argv"][0] == "/usr/bin/zenity"
    assert observado["kwargs"]["env"] == {
        "DISPLAY": ":10.0",
        "DBUS_SESSION_BUS_ADDRESS": "unix:path=/tmp/dbus-x",
    }


def test_linux_picker_without_zenity_or_kdialog_mentions_the_apt_command(
    monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.setattr(ide_workspaces_module.sys, "platform", "linux")
    monkeypatch.setattr(ide_workspaces_module.os, "name", "posix")
    monkeypatch.setattr(
        ide_workspaces_module.linux_session, "entorno_fusionado", lambda: {"DISPLAY": ":0"}
    )
    monkeypatch.setattr(ide_workspaces_module.shutil, "which", lambda nombre: None)

    with pytest.raises(IDEWorkspaceError, match="apt install zenity"):
        ide_workspaces_module.pick_workspace_folder()
...
# ---------------------------------------------------------------------------
# Sin $SHELL (servicio systemd sin sesión de login, el caso real en
# producción Linux) -- antes caía a dash (`/bin/sh`) si no había zsh: sin
# historial, sin autocompletado, y (medido en vivo, edecan-prod, 1-ago-2026)
# ensuciando el SIGUIENTE comando cuando el cliente manda una secuencia de
# escape que dash no entiende. Ahora prefiere bash, casi siempre presente.
# `_shell_candidate_exists` mockeado en vez de `Path.exists` de la clase
# entera -- ver su docstring en `ide_sessions.py`.
# ---------------------------------------------------------------------------


def _fake_shell_candidates(disponibles: set[str]):
    def _existe(path: str) -> bool:
        return path in disponibles

    return _existe


def test_default_terminal_without_shell_env_prefers_bash_over_dash(
    monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.delenv("SHELL", raising=False)
    monkeypatch.setattr(
        SessionManager,
        "_shell_candidate_exists",
        staticmethod(_fake_shell_candidates({"/bin/bash", "/bin/sh"})),
    )

    argv = SessionManager._default_terminal_argv()

    assert argv == ["/bin/bash", "--noprofile", "--norc"]


def test_default_terminal_without_shell_env_prefers_zsh_when_available(
    monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.delenv("SHELL", raising=False)
    monkeypatch.setattr(
        SessionManager,
        "_shell_candidate_exists",
        staticmethod(_fake_shell_candidates({"/bin/zsh", "/bin/bash", "/bin/sh"})),
    )
...
def test_default_terminal_without_shell_env_falls_back_to_sh_only_as_last_resort(
    monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.delenv("SHELL", raising=False)
    monkeypatch.setattr(
        SessionManager, "_shell_candidate_exists", staticmethod(_fake_shell_candidates(set()))
    )
...
async def test_lsp_no_dice_motor_anterior_cuando_el_motor_vigente_es_opencode(
    companion_config: CompanionConfig, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    """Reproduce el hallazgo medido en la auditoría (sección "el motor
    anterior"): un ``SessionManager`` recién construido, con el motor
    vigente configurado en "opencode" pero SIN que ningún turno de agente
    hubiera corrido todavía en este proceso -- ``motor_opencode`` sigue
    ``None`` porque es perezoso a propósito (``_asegurar_motor_opencode``).

    ANTES, ``_lsp_servidor_para`` preguntaba solo ``motor_opencode is None``
    y con eso ya alcanzaba para responder "esta instalación todavía corre el
    motor anterior" -- FALSO, el motor vigente SÍ era opencode. Este test
    reemplaza ``MotorOpencode`` por un doble que registra si lo llegaron a
    pedir, sin arrancar ningún proceso real, para comprobar en aislamiento
    que ahora SÍ se inicializa en vez de rendirse."""

    class _MotorFalso:
        def __init__(self, *, obtener_modo: Any) -> None:
            self.pedido_para: list[str] = []

        async def servidor_para(self, directorio_trabajo: Any) -> Any:
            self.pedido_para.append(str(directorio_trabajo))
            return "SERVIDOR-FALSO"

    monkeypatch.setattr(ide_sessions_module, "MotorOpencode", _MotorFalso)

    # ``_authorize`` autoriza vía el runtime CACHEADO de ``_runtime_for``
    # (otra instancia); el workspace se persiste a disco, así que un
    # ``IDERuntime`` construido DESPUÉS lo carga fresco -- construirlo antes
    # dejaría a ``runtime.workspaces`` sin conocerlo todavía (mismo patrón
    # que ``test_workspace_file_operations_are_relative_and_persisted``).
    project = _project(tmp_path)
    workspace = await _authorize(companion_config, project)
    runtime = IDERuntime(companion_config, motor="opencode")
    assert runtime.sessions.motor_vigente() == "opencode"
    assert runtime.sessions.motor_opencode is None  # perezoso: nadie corrió un turno todavía

    servidor = await runtime._lsp_servidor_para(workspace["id"])

    assert servidor == "SERVIDOR-FALSO"
    # Se inicializó (dejó de ser None) y de verdad se le pidió el servidor
    # para ESTE workspace -- no se rindió antes de intentarlo.
    assert isinstance(runtime.sessions.motor_opencode, _MotorFalso)
    assert runtime.sessions.motor_opencode.pedido_para == [str(project.resolve())]

    runtime.sessions.shutdown()


async def test_lsp_si_dice_motor_anterior_cuando_de_verdad_es_el_motor_viejo(
    companion_config: CompanionConfig, tmp_path: Path
):
    """Contraparte del test anterior: con el motor EXPLÍCITAMENTE "viejo" (no
    el default de ``motor_opencode is None`` sin más), el mensaje sigue
    siendo el correcto -- este SÍ es el caso real que el mensaje describe."""
    runtime = IDERuntime(companion_config, motor="viejo")
    project = _project(tmp_path)
    workspace = await _authorize(companion_config, project)
    assert runtime.sessions.motor_opencode is None

    with pytest.raises(IDESessionError, match="motor anterior"):
        await runtime._lsp_servidor_para(workspace["id"])


class _MotorOpencodeSinBinario:
    """Doble de ``MotorOpencode`` cuando no hay ``opencode`` resoluble (ni en
    PATH ni en ``EDECAN_OPENCODE_BIN``) -- lo que de verdad lanza
    ``servidor_para`` en ese caso, sin arrancar ningún proceso real."""

    async def servidor_para(self, directorio_trabajo: Any) -> Any:
        from edecan_companion.ide_opencode import BinarioOpencodeNoEncontrado

        raise BinarioOpencodeNoEncontrado("no encontré un binario de opencode utilizable")


async def test_lsp_symbols_y_status_no_escapan_crudo_sin_binario_de_opencode(
    companion_config: CompanionConfig, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    """Medido en vivo (edecan-prod, 1-ago-2026): sin ``opencode`` resoluble,
    ``ide_lsp_symbols``/``ide_lsp_status`` dejaban escapar
    ``BinarioOpencodeNoEncontrado`` sin atrapar -- un 500 crudo en la capa
    HTTP en vez del ``{"ok": false, ...}`` de siempre. Antes de este fix,
    este test fallaba con la excepción sin capturar en vez de con el
    ``assert``."""
    monkeypatch.setenv("EDECAN_IDE_MOTOR", "opencode")
    project = _project(tmp_path)
    workspace = await _authorize(companion_config, project)
    runtime = ide_runtime_module._runtime_for(companion_config)
    runtime.sessions.motor_opencode = _MotorOpencodeSinBinario()  # type: ignore[assignment]
    ...
    assert simbolos["ok"] is False
    assert "binario de opencode" in simbolos["error"]
    # Es un problema de entorno, no una capacidad ausente para siempre.
    assert "lsp_no_disponible" not in simbolos
    ...
    assert estado["ok"] is False
    assert "binario de opencode" in estado["error"]


class _MotorOpencodeSinCredencialesCloudflare:
    """Doble de ``MotorOpencode`` para el caso medido en vivo (edecan-prod,
    1-ago-2026) que de verdad se recorre en una instalación Linux nueva: sin
    ``CLOUDFLARE_ACCOUNT_ID``/``CLOUDFLARE_API_TOKEN``, ``servidor_para``
    lanza esto ANTES de siquiera llegar a resolver el binario de opencode."""

    async def servidor_para(self, directorio_trabajo: Any) -> Any:
        from edecan_companion.ide_opencode_config import CredencialesCloudflareFaltantesError
...
async def test_lsp_symbols_y_status_no_escapan_crudo_sin_credenciales_cloudflare(
    companion_config: CompanionConfig, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    """Medido en vivo (edecan-prod, 1-ago-2026): ``CredencialesCloudflareFaltantesError``
    es la MISMA familia de bug que ``BinarioOpencodeNoEncontrado`` (arriba) y
    se dispara ANTES en el camino real -- ``generar_opencode_json`` corre
    antes de resolver el binario. Sin este fix escapaba cruda como un 500."""
    monkeypatch.setenv("EDECAN_IDE_MOTOR", "opencode")
    project = _project(tmp_path)
    workspace = await _authorize(companion_config, project)
    runtime = ide_runtime_module._runtime_for(companion_config)
    runtime.sessions.motor_opencode = _MotorOpencodeSinCredencialesCloudflare()  # type: ignore[assignment]
    ...
    assert simbolos["ok"] is False
    assert "CLOUDFLARE_ACCOUNT_ID" in simbolos["error"]
    assert "lsp_no_disponible" not in simbolos
    ...
    assert estado["ok"] is False
    assert "CLOUDFLARE_ACCOUNT_ID" in estado["error"]
