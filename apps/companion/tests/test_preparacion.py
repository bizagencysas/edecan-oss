...
- `detectar()`: dobles por requisito, nunca lanza, lista vacía fuera de
  Windows Y de Linux (Linux tiene su propio manifiesto informativo, ver
  abajo).
...
- `REQUISITOS_LINUX`: detección informativa, real (nunca "todo bien" sin
  haber comprobado nada) pero SIN instalación de un clic -- cada entrada
  tiene `instalar=None` siempre, así que ningún comando de esa lista puede
  llegar jamás a ejecutarse (misma regla de seguridad que el resto del
  módulo).
"""
...
def test_detectar_returns_empty_list_outside_windows_and_linux(monkeypatch: pytest.MonkeyPatch):
    """Ni Windows ni Linux (p. ej. BSD): sigue sin haber ninguna pantalla de
    preparación, ni ejecutable ni informativa. Se fuerza explícito con
    monkeypatch, no solo confiando en la plataforma real de quien corre la
    suite -- la intención del test es "ninguna de las dos", no "lo que sea
    que tenga esta máquina"."""
    monkeypatch.setattr(preparacion, "_plataforma_soportada", lambda: False)
    monkeypatch.setattr(preparacion, "_plataforma_es_linux", lambda: False)

    assert preparacion.detectar() == []


def test_detectar_on_linux_reports_every_requisito_with_dobles(monkeypatch: pytest.MonkeyPatch):
    """Con cada comprobación mockeada a "todo falta", `detectar()` en Linux
    reporta las cuatro filas de `REQUISITOS_LINUX`, en orden, y ninguna es
    instalable con un clic (`instalar=None` siempre en ese manifiesto)."""
    monkeypatch.setattr(preparacion, "_plataforma_soportada", lambda: False)
    monkeypatch.setattr(preparacion, "_plataforma_es_linux", lambda: True)
    monkeypatch.setattr(preparacion.shutil, "which", _which_mock({}))
    monkeypatch.delenv("EDECAN_OPENCODE_BIN", raising=False)
    monkeypatch.setattr(preparacion.importlib.util, "find_spec", lambda nombre: None)

    filas = preparacion.detectar()

    assert [fila["id"] for fila in filas] == [
        "linux_selector_de_carpetas",
        "linux_portapapeles",
        "linux_remote_control",
        "linux_opencode_binario",
    ]
    assert all(fila["estado"] == "falta" for fila in filas)
    assert all(fila["instalable"] is False for fila in filas)  # nunca instalación de un clic
    assert all("por_que" in fila and fila["por_que"] for fila in filas)
    # Ninguna instrucción falsa: `brew` no se cita como LA vía en Linux.
    assert all("Instálalo con `brew install" not in fila["por_que"] for fila in filas)


def test_detectar_on_linux_reports_cumplido_when_everything_is_satisfied(
    ...
    monkeypatch.setattr(
        preparacion.shutil,
        "which",
        _which_mock({"zenity": "/usr/bin/zenity", "xclip": "/usr/bin/xclip"}),
    )
    monkeypatch.setattr(preparacion.importlib.util, "find_spec", lambda nombre: object())
    monkeypatch.setenv("EDECAN_OPENCODE_BIN", "/opt/edecan/bin/opencode")
...
def test_detectar_on_linux_portapapeles_checks_wayland_tools_under_wayland(
    monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.setattr(preparacion, "_plataforma_soportada", lambda: False)
    monkeypatch.setattr(preparacion, "_plataforma_es_linux", lambda: True)
    monkeypatch.setenv("WAYLAND_DISPLAY", "wayland-0")
    # xclip presente pero NO es lo que hace falta bajo Wayland.
    monkeypatch.setattr(preparacion.shutil, "which", _which_mock({"xclip": "/usr/bin/xclip"}))

    filas = preparacion.detectar()

    por_id = {fila["id"]: fila for fila in filas}
    assert por_id["linux_portapapeles"]["estado"] == "falta"
...
def test_manifiesto_linux_ids_are_unique_and_never_collide_with_windows():
    ids_linux = [requisito.id for requisito in preparacion.REQUISITOS_LINUX]
    assert len(ids_linux) == len(set(ids_linux))
    ids_windows = {requisito.id for requisito in preparacion.REQUISITOS}
    assert not (set(ids_linux) & ids_windows)


def test_manifiesto_linux_nunca_tiene_instalacion_automatica():
    """Regla de seguridad central del módulo, aplicada al manifiesto de
    Linux: NINGUNA entrada puede tener un `instalar` -- la instalación de un
    clic sigue siendo exclusiva de Windows, ver el docstring del módulo."""
    for requisito in preparacion.REQUISITOS_LINUX:
        assert requisito.instalar is None, f"{requisito.id}: no debería ser instalable"


def test_ejecutor_instalar_refuses_a_known_linux_id_even_when_pretending_to_be_linux(
    monkeypatch: pytest.MonkeyPatch,
):
    """Aunque alguien intente forzar `_plataforma_soportada` a True (no
    debería poder pasar de verdad en Linux, `os.name` nunca es "nt" ahí),
    un id de `REQUISITOS_LINUX` sigue rechazándose por "sin instalación
    automática" -- nunca llega a construir un argv."""
    monkeypatch.setattr(preparacion, "_plataforma_soportada", lambda: True)
    ejecutor = preparacion.EjecutorPreparacion()

    with pytest.raises(preparacion.PreparacionError, match="instalación automática"):
        ejecutor.instalar("linux_selector_de_carpetas")
