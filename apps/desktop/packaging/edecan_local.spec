...
EDECAN_CORE_PACKAGES = [
    ...
    "edecan_connectors",
    "edecan_voice",
    # Dominio del gimnasio inteligente: `edecan_api.routers.gym` y
    # `edecan_worker.handlers.run_gym_checkin` importan `edecan_gym` de forma
    # estática (igual que `edecan_llm`/`edecan_connectors`). Sin esta entrada,
    # el router queda en el binario pero su dependencia no — y el montaje
    # defensivo de `edecan_api.main` lo omite en silencio en producción.
    "edecan_gym",
]
...
#     código.
#   - `certifi`: su dato NO-Python (`cacert.pem`, el paquete de raíces de
#     confianza TLS) normalmente lo agrega el hook automático de
#     `pyinstaller-hooks-contrib` sin que este `.spec` tenga que pedirlo --
#     hasta que ese hook no corre (build sin `pyinstaller-hooks-contrib`, o
#     una versión de certifi que el hook ya no reconoce) y entonces el
#     binario arranca, monta todos los routers y sirve `/healthz` en 200
#     limpio -- porque nada construye un `httpx.AsyncClient` hasta el primer
#     turno de chat real. Ahí `ssl.create_default_context()` llama
#     `certifi.where()`, la ruta apunta dentro de `_MEIPASS` y el archivo no
#     está: `FileNotFoundError: [Errno 2] No such file or directory` sin
#     nombre de archivo en el mensaje (measured 11-ago-2026, LLMRouter lo
#     reporta como "No se pudo inicializar el proveedor de inferencia" --
#     ver `packages/llm/edecan_llm/router.py`). Ningún test de arranque lo
#     detecta porque el arranque en sí nunca toca la red; solo se ve
#     publicando o chateando de verdad. `_collect()` explícito acá + el
#     `SystemExit` de abajo lo convierten en un fallo de BUILD, no en un
#     fallo silencioso semanas después en la Mac de producción.
# ---------------------------------------------------------------------------
for _pkg in (
    "pgserver", "fakeredis", "alembic", "uvicorn",
    "boto3", "botocore", "aiobotocore", "aioboto3", "certifi",
):
    _collect(_pkg)

if not any(dest.replace("\\", "/").rstrip("/").endswith("certifi") for _, dest in datas):
    raise SystemExit(
        "[edecan_local.spec] 'certifi' no aportó datos al bundle (¿falta "
        "pyinstaller-hooks-contrib, o certifi cambió su layout?) -- sin "
        "cacert.pem, cualquier httpx.AsyncClient() revienta en runtime con "
        "'[Errno 2] No such file or directory' al primer uso real (chat, "
        "LinkedIn, cualquier llamada saliente). No sigas sin resolver esto."
    )
...
hiddenimports += [
    ...
    # Pillow se importa perezoso en la captura. Sin estos plugins el sidecar
    # congela JPEG/PNG pero Image.save(..., WEBP) revienta y las fotos vuelven
    # a ser PNG de varios megas.
    "PIL",
    "PIL.Image",
    "PIL.JpegImagePlugin",
    "PIL.PngImagePlugin",
...
# `config/modelos.yml` es la autoridad del catálogo de modelos del chat y del
# perfil `chat_rapido`. Sin empaquetarlo, `task_router.RUTA_CONFIG_MODELOS`
# resuelve a `_MEIPASS/config/modelos.yml` (inexistente) en el binario congelado
# y el sidecar cae al fallback duro — dejando el modelo viejo (Scout) en vez del
# 70B que el dueño eligió (bug real 21-ago-2026). Se empaqueta en `config/`
# para que el binario lea EXACTAMENTE lo mismo que el árbol de desarrollo.
CONFIG_MODELOS_SRC = os.path.join(REPO_ROOT, "config", "modelos.yml")
if os.path.isfile(CONFIG_MODELOS_SRC):
    datas.append((CONFIG_MODELOS_SRC, "config"))
else:
    print(
        "[edecan_local.spec] aviso: no existe config/modelos.yml — el binario "
        "arrancará con el fallback duro de task_router (modelo por defecto viejo)."
    )
