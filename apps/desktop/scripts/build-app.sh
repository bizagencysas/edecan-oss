...
# OJO: este override REEMPLAZA `bundle.externalBin` entero, no lo extiende.
# Cualquier sidecar que solo se declare en `tauri.conf.json` queda FUERA del
# paquete y la build pasa igual, sin avisar -- exactamente lo que pasó la
# primera vez que se empaquetó opencode: `tauri.conf.json` lo listaba, el
# binario estaba descargado, el build decía "ok", y `Contents/MacOS/` no lo
# traía. En esta Mac no se notó porque el runtime cae al `~/.opencode` del
# desarrollador; en cualquier otra máquina el IDE se habría quedado sin motor.
# Si se suma un sidecar nuevo, va AQUÍ además de en el archivo base.
#
# `binaries/opencode` es el motor del IDE (docs/opencode-motor.md), no una
# integración opcional: va siempre, sin variable que lo active, igual que
# `download-opencode.sh` corre siempre en build-backend.sh.
TAURI_EXTERNAL_BIN_JSON='["binaries/edecan-local","binaries/fydesign-node","binaries/opencode"]'
if [[ "${EDECAN_BUNDLE_OLLAMA:-0}" == "1" ]]; then
  echo "    (EDECAN_BUNDLE_OLLAMA=1: sumando binaries/ollama a externalBin para esta build)"
  TAURI_EXTERNAL_BIN_JSON='["binaries/edecan-local","binaries/fydesign-node","binaries/opencode","binaries/ollama"]'
fi
...
# Comprobación de que lo que se pidió empaquetar de verdad quedó dentro.
#
# Existe porque falló en silencio: `tauri.conf.json` listaba `binaries/opencode`,
# el binario estaba descargado y verificado, `cargo tauri build` terminó con
# éxito... y el `.app` salió sin él, porque el `--config` de arriba reemplaza la
# lista entera. La app se firmó, se notarizó y se instaló con el motor del IDE
# ausente. Un build "correcto" que entrega un paquete incompleto es peor que uno
# que falla: nadie va a revisar `Contents/MacOS/` a mano cada vez.
if [[ "$PLATFORM" == "Darwin" ]]; then
  APP_MACOS_DIR="$DESKTOP_DIR/src-tauri/target/release/bundle/macos/Edecán.app/Contents/MacOS"
  FALTANTES=()
  for ruta in $(printf '%s' "$TAURI_EXTERNAL_BIN_JSON" | tr -d '[]"' | tr ',' ' '); do
    nombre="${ruta##*/}"
    [[ -f "$APP_MACOS_DIR/$nombre" ]] || FALTANTES+=("$nombre")
  done
  if (( ${#FALTANTES[@]} > 0 )); then
    echo "error: el paquete se construyó SIN estos sidecars: ${FALTANTES[*]}" >&2
    echo "  esperados en: $APP_MACOS_DIR" >&2
    echo "  Revisa TAURI_EXTERNAL_BIN_JSON en este mismo script: el --config de" >&2
    echo "  'cargo tauri build' REEMPLAZA bundle.externalBin, no lo extiende." >&2
    exit 1
  fi
  echo "==> Sidecars verificados dentro del paquete: $(ls "$APP_MACOS_DIR" | tr '\n' ' ')"
fi
