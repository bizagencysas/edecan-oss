...
# opencode es el MOTOR del IDE (docs/opencode-motor.md), no una integracion
...
# de desarrollo. Por eso corre SIEMPRE y sin variable que lo active -- mismo
# criterio (y misma numeracion "[0/5]", igual que build-backend.sh en
# macOS/Linux) que Ollama arriba, aunque a diferencia de Ollama este paso no
# es condicional. El script sale de inmediato si el binario ya esta y su
# SHA-256 coincide, asi que no penaliza las recompilaciones.
Write-Host "==> [0/5] Descargando el motor opencode (scripts/download-opencode.ps1)..."
& (Join-Path $ScriptDir "download-opencode.ps1")
if ($LASTEXITCODE -ne 0) { throw "download-opencode.ps1 fallo (codigo $LASTEXITCODE)." }
