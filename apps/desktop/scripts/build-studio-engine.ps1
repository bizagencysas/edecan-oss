...
# --------------------------------------------------------------------------- #
# Invoke-NodeScript -- ejecutar JavaScript sin que PowerShell le coma las
# comillas.
#
# `node -e '<codigo>'` NO funciona de forma fiable desde PowerShell: al pasar el
# script por la linea de comandos, las comillas internas se pierden y node
# recibe `require(node:fs)` en vez de `require("node:fs")`. Medido en la VM de
# Windows Server 2022 el 01-ago-2026, con este error exacto:
#
#   SyntaxError: missing ) after argument list
#
# Bloqueaba el build del instalador de Windows en tres sitios distintos de este
# mismo archivo. La solucion robusta no es escapar mejor -- es no pasar codigo
# por la linea de comandos: se escribe a un archivo temporal y se ejecuta.
# --------------------------------------------------------------------------- #
function Invoke-NodeScript {
    param(
        ...
        [string[]] $ScriptArgs = @(),
        [switch] $Module,
        # Donde se escribe el temporal. IMPORTA: node resuelve los `import`/
        # `require` de un ARCHIVO relativos a la carpeta del propio archivo, no
        # al directorio de trabajo (a diferencia de `node -e`, que sí usa el
        # cwd). Un script que importe "playwright" tiene que escribirse DENTRO
        # del paquete que tiene ese `node_modules`, o falla con
        # ERR_MODULE_NOT_FOUND -- medido en la VM el 01-ago-2026.
        [string] $WorkDir = ([System.IO.Path]::GetTempPath())
    )
    $ext = if ($Module) { ".mjs" } else { ".cjs" }
    $tmp = Join-Path $WorkDir ("edecan-node-" + [guid]::NewGuid().ToString("N") + $ext)
    try {
        # UTF8 sin BOM: node rechaza un BOM al inicio de un modulo.
        [System.IO.File]::WriteAllText($tmp, $Code, (New-Object System.Text.UTF8Encoding($false)))
        & $NodeExe $tmp @ScriptArgs
    } finally {
...
$FfmpegVersion = "8.1-latest"
$FfmpegAsset = "ffmpeg-n8.1-latest-win64-gpl-8.1.zip"
$FfmpegRelease = "latest"
$FfmpegUrl = "https://github.com/BtbN/FFmpeg-Builds/releases/download/$FfmpegRelease/$FfmpegAsset"
$FfmpegSha256 = "b916c1d36c1eadf9c7091433168ee0512a57548f5bd17085f85ce20159b6e2bd"
...
try {
    ...
    # `ffmpeg -L` escribe su banner de version/copyright en **stderr** -- es su
    # comportamiento normal, no un error. Con el `$ErrorActionPreference = "Stop"`
    # de la cabecera de este script, ese stderr capturado por `2>&1` se convertia
    # en un error TERMINANTE y el build de Windows moria siempre aqui, en
    # cualquier maquina:
    #
    #   ffmpeg.exe : ffmpeg version n8.1.2-... Copyright (c) 2000-2026 ...
    #   FullyQualifiedErrorId : NativeCommandError
    #
    # Medido en la VM de Windows Server 2022 el 01-ago-2026: reproducible al
    # 100%, y bloqueaba el instalador entero antes de llegar a `cargo tauri
    # build`. El `2>&1` SI hace falta (la licencia que se valida abajo viaja por
    # stderr), asi que lo que se acota es la politica de errores, no la captura.
    ...
    # Mismo caso que `-L` unas lineas arriba: `-buildconf` tambien escribe por
    # stderr y con `ErrorActionPreference = "Stop"` reventaria el build. Se
    # encontro al arreglar el primero -- habria fallado aqui dos pasos despues,
    # costando otra vuelta entera de VM.
    $EapPrevio = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    try {
        & $FfmpegTarget -buildconf 2>&1 |
            Out-File -Encoding utf8 (Join-Path $LicensesDir "FFMPEG-BUILD-CONFIGURATION.txt")
    } finally {
        $ErrorActionPreference = $EapPrevio
    }
    @(
    ...
    try {
        ...
        Push-Location $StagedEngine
        try {
            Invoke-NodeScript -NodeExe $NodeRuntime -Module -WorkDir $StagedEngine -Code @'
...
'@
    ...
    Invoke-NodeScript -NodeExe $NodeRuntime -ScriptArgs @($StagedPackage) -Code @'
...
'@
    ...
    Invoke-NodeScript -NodeExe $NodeRuntime -ScriptArgs @($StagedEngine, $NpmCli) -Code @'
const cp = require("node:child_process"), fs = require("node:fs"), path = require("node:path");
const root = path.resolve(process.argv[2]), npm = process.argv[3];
for (let round = 0; round < 8; round++) {
  let out = "";
  try {
    out = cp.execFileSync(process.execPath, [npm, "ls", "--json", "--omit=dev", "--depth=0"],
      { cwd: root, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
  } catch (error) { out = String(error.stdout || ""); }
  const tree = JSON.parse(out);
  const extras = Object.entries(tree.dependencies || {})
    .filter(([, meta]) => meta && meta.extraneous).map(([name]) => name);
  if (!extras.length) process.exit(0);
  for (const name of extras) {
    const target = path.resolve(root, "node_modules", name);
    const modules = path.resolve(root, "node_modules") + path.sep;
    if (!target.startsWith(modules)) throw new Error("unsafe dependency path");
...
'@
    ...
    # Guardia de firma (equivalente Windows de la leccion de macOS con
    # Contents/Resources/): build-app.ps1 conecta la firma Authenticode
    # opcional via bundle.windows.signCommand para que Tauri firme todo
    # .exe/.dll bajo bundle.resources (studio-engine/ completo, incluye
    # ffmpeg.exe/ffprobe.exe/yt-dlp.exe/esbuild.exe/Chromium). Pero
    # should_sign() (tauri-bundler 2.11.4, sign.rs) solo reconoce esas dos
    # extensiones -- un addon nativo `.node` (una DLL de Windows con otro
    # nombre) pasaria SIN FIRMAR y sin ningun aviso, exactamente el modo de
    # fallo silencioso que rompio la notarizacion en macOS. Hoy
    # (packages/fydesign-engine/package-lock.json, 2026-07-30) no existe
    # ninguno -- este chequeo evita que una futura dependencia lo
    # reintroduzca sin que nadie lo note.
    $NativeAddons = @(Get-ChildItem $StagedEngine -Recurse -File -Filter "*.node" -ErrorAction SilentlyContinue)
    if ($NativeAddons.Count -gt 0) {
        $Listado = ($NativeAddons | ForEach-Object { $_.FullName }) -join ", "
        throw "Studio contiene addon(s) nativo(s) .node que el firmador de Tauri NO firma (solo reconoce .exe/.dll): $Listado. Firmarlos aparte o retirar la dependencia que los introdujo antes de continuar."
    }
