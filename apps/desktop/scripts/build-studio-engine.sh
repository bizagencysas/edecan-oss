...
# Ruta de rebuild offline explícita para una máquina que ya tiene el recurso
# Studio completo. Exige que el recurso existente tenga node_modules y las
# herramientas fijadas, y usa el Node 22 local únicamente como sidecar.
if [[ "${EDECAN_REUSE_STUDIO_ENGINE:-0}" == "1" ]]; then
  NODE_SOURCE="${EDECAN_STUDIO_NODE_PATH:-/opt/homebrew/opt/node@22/bin/node}"
  NODE_LIB_SOURCE="$(dirname "$NODE_SOURCE")/../lib/libnode.127.dylib"
  for required in "$NODE_SOURCE" "$ENGINE_RESOURCE/package.json" \
    "$ENGINE_RESOURCE/node_modules" "$ENGINE_RESOURCE/tools/ffmpeg" \
    "$ENGINE_RESOURCE/tools/ffprobe" "$ENGINE_RESOURCE/tools/yt-dlp" \
    "$NODE_LIB_SOURCE"; do
    if [[ ! -e "$required" ]]; then
      echo "error: recurso Studio offline incompleto; falta $required." >&2
      exit 1
    fi
  done
  NODE_VERSION_LOCAL="$("$NODE_SOURCE" --version)"
  if [[ "$NODE_VERSION_LOCAL" != v22.* ]]; then
    echo "error: EDECAN_STUDIO_NODE_PATH debe apuntar a Node 22 (detectado: $NODE_VERSION_LOCAL)." >&2
    exit 1
  fi
  mkdir -p "$BINARIES_DIR"
  find "$BINARIES_DIR" -maxdepth 1 -name 'fydesign-node-*' -exec rm -f {} +
  NODE_SIDECAR="$BINARIES_DIR/fydesign-node-$TARGET_TRIPLE"
  cp "$NODE_SOURCE" "$NODE_SIDECAR"
  chmod +x "$NODE_SIDECAR"
  cp "$NODE_LIB_SOURCE" "$ENGINE_RESOURCE/tools/libnode.127.dylib"
  install_name_tool -add_rpath \
    '@loader_path/../Resources/studio-engine/tools' "$NODE_SIDECAR"
  echo "==> Studio offline reutilizado: $ENGINE_RESOURCE + Node $NODE_VERSION_LOCAL"
  exit 0
fi
