...
stop_running_edecan() {
  ...
  # toca procesos de Referencia, Python u otras aplicaciones.
...
# `codesign --force` REEMPLAZA la firma entera, y lo que no se le vuelva a pasar
# se pierde: los entitlements y el hardened runtime que el empaquetador ya había
# puesto desaparecen en silencio. Se veía como "di los permisos y no funcionan",
# porque bajo hardened runtime el permiso de TCC y el entitlement son dos
# candados distintos -- y acá se estaba cerrando el segundo al reinstalar.
ENTITLEMENTS_EDECAN="$DESKTOP_DIR/src-tauri/Edecan.entitlements"
ENTITLEMENTS_OPENCODE="$DESKTOP_DIR/src-tauri/Opencode.entitlements"
FIRMA_OPCIONES=(--options runtime --timestamp=none)

if [[ -n "$SIGNING_IDENTITY" ]]; then
  # 1. Firmar opencode con sus entitlements de JIT para el runtime de Bun
  if [[ -f "$STAGED_APP/Contents/MacOS/opencode" && -f "$ENTITLEMENTS_OPENCODE" ]]; then
    codesign --force --sign "$SIGNING_IDENTITY" --identifier cc.edecan.desktop.opencode \
      "${FIRMA_OPCIONES[@]}" --entitlements "$ENTITLEMENTS_OPENCODE" \
      "$STAGED_APP/Contents/MacOS/opencode"
  fi

  # 2. Firmar edecan-local con los entitlements de Edecán
  if [[ -f "$STAGED_APP/Contents/MacOS/edecan-local" && -f "$ENTITLEMENTS_EDECAN" ]]; then
    codesign --force --sign "$SIGNING_IDENTITY" --identifier cc.edecan.desktop \
      "${FIRMA_OPCIONES[@]}" --entitlements "$ENTITLEMENTS_EDECAN" \
      "$STAGED_APP/Contents/MacOS/edecan-local"
  fi

  # 3. Firmar la app exterior SIN --deep para no sobreescribir los entitlements de opencode
  if [[ -f "$ENTITLEMENTS_EDECAN" ]]; then
    codesign --force --sign "$SIGNING_IDENTITY" --identifier cc.edecan.desktop \
      "${FIRMA_OPCIONES[@]}" --entitlements "$ENTITLEMENTS_EDECAN" \
      "$STAGED_APP"
  else
    codesign --force --sign "$SIGNING_IDENTITY" --identifier cc.edecan.desktop \
      "${FIRMA_OPCIONES[@]}" "$STAGED_APP"
  fi
fi
