...
stop_running_edecan() {
  ...
  # toca procesos de Aria, Python u otras aplicaciones.
...
ENTITLEMENTS="$DESKTOP_DIR/src-tauri/Edecan.entitlements"
FIRMA_OPCIONES=(--options runtime --timestamp=none)
if [[ -f "$ENTITLEMENTS" ]]; then
  FIRMA_OPCIONES+=(--entitlements "$ENTITLEMENTS")
else
  echo "aviso: no encontré $ENTITLEMENTS; la app quedará sin entitlements y" >&2
  echo "       Automatización, Micrófono y la carga del backend van a fallar." >&2
fi
if [[ -n "$SIGNING_IDENTITY" ]]; then
  codesign --force --deep --sign "$SIGNING_IDENTITY" "${FIRMA_OPCIONES[@]}" "$STAGED_APP"
  ...
  # Y necesita los entitlements MÁS que el envoltorio: es este proceso, no la
  # ventana, el que manda Apple Events y abre el micrófono.
  codesign --force --sign "$SIGNING_IDENTITY" --identifier cc.edecan.desktop \
    "${FIRMA_OPCIONES[@]}" "$STAGED_APP/Contents/MacOS/edecan-local"
  codesign --force --sign "$SIGNING_IDENTITY" --identifier cc.edecan.desktop \
