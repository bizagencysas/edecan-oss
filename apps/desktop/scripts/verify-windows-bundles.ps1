...
function Assert-AllBinariesSigned {
    param(
        [Parameter(Mandatory = $true)][string] $Root,
        [Parameter(Mandatory = $true)][string] $DesktopExePath
    )

    # Paridad con la leccion de macOS: alli `codesign --verify --deep
    # --strict` sobre el .app entero revienta si CUALQUIER Mach-O dentro
    # de Contents/Resources/ (Chromium, ffmpeg, yt-dlp, esbuild de
    # studio-engine) quedo sin firmar -- por eso build-backend.sh tuvo que
    # firmarlos aparte antes de que Tauri empaquetara. Windows no tiene un
    # gate equivalente: SmartScreen solo mira el instalador descargado, no
    # los binarios que trae adentro. Este chequeo lo suple: si el exe
    # principal viene con Authenticode valido (Fase 3, ver
    # docs/edecan-windows.md S5 y la firma opcional que build-app.ps1
    # conecta via bundle.windows.signCommand), exige que TODOS los .exe/
    # .dll instalados -- no solo los nombrados en el plan de firma --
    # tambien lo tengan. Sin firma configurada (el estado de hoy),
    # Get-AuthenticodeSignature del exe principal da "NotSigned" y esta
    # funcion no hace nada, igual que el resto del build sin firmar.
    $MainSignature = Get-AuthenticodeSignature -LiteralPath $DesktopExePath -ErrorAction SilentlyContinue
    if (-not $MainSignature -or $MainSignature.Status -ne "Valid") {
        return
    }
    Write-Host "==> exe principal firmado (Authenticode Valid): verificando el resto del paquete..."
    $Unsigned = @(
    ...
    if ($Unsigned.Count -gt 0) {
        throw "el paquete esta parcialmente firmado: $($Unsigned.Count) binario(s) sin Authenticode valido (equivalente Windows del bug de notarizacion de macOS con Contents/Resources/): $($Unsigned -join ', ')."
    }
}
...
try {
    ...
    $MsiDesktopExe = Assert-InstalledPayload $MsiExtract
    Assert-AllBinariesSigned $MsiExtract $MsiDesktopExe
    ...
    Assert-AllBinariesSigned $NsisInstall $InstalledExe
