...
pub(crate) const DESKTOP_USER_AGENT: &str = "EdecanDesktop/0.7";
const BACKEND_PID_FILE: &str = "backend-sidecar.pid";
...
/// Misma UI local que `main`, apuntando a `/app/` con `#edecan_mini=1` para
/// que la web pinte el compositor compacto en vez del chat completo.
pub fn current_mini_ui_url(app: &AppHandle) -> Option<String> {
    Some(mini_ui_url(&current_local_ui_url(app)?))
}

fn mini_ui_url(main_url: &str) -> String {
    let with_app = main_url.replacen("/?", "/app/?", 1);
    if with_app.contains('#') {
        format!("{with_app}&edecan_mini=1")
    } else {
        format!("{with_app}#edecan_mini=1")
    }
}
...
pub async fn start_backend(app: AppHandle) {
    ...
    *app.state::<BackendState>().0.lock().unwrap() = Some(child);
    if let Err(error) = persist_backend_pid(&app, child_pid) {
        eprintln!("[edecan-desktop] no se pudo guardar el PID del backend: {error}");
    }
...
fn build_command(
    ...
    // Una app INSTALADA no puede caer en modo desarrollo por una variable
    // suelta. Pasó dos veces el 31-jul-2026: `EDECAN_LOCAL_DEV_CMD` quedó
    // puesta en el entorno de launchd, la app lanzó `uv run ... edecan` desde
    // el repo en vez de su propio sidecar, y como el código del repo no lleva
    // la web dentro (esa vive en `<_MEIPASS>/web` del binario congelado), la
    // ventana mostró `{"detail":"Not Found"}`. Sin ninguna señal de por qué.
    //
    // En una build de release hace falta ADEMÁS un opt-in explícito
    // (`EDECAN_LOCAL_DEV=1`). Así la variable sola —residuo de una sesión de
    // desarrollo, un script viejo, lo que sea— ya no puede secuestrar la app
    // de nadie. Las builds de debug siguen igual que siempre: ahí el modo
    // fuente ES lo que se quiere.
    let dev_opt_in = cfg!(debug_assertions)
        || matches!(
            std::env::var("EDECAN_LOCAL_DEV").as_deref(),
            Ok("1") | Ok("true")
        );
    let source_command =
        dev_opt_in && std::env::var_os("EDECAN_LOCAL_DEV_CMD").is_some() || cfg!(debug_assertions);
    if !source_command && std::env::var_os("EDECAN_LOCAL_DEV_CMD").is_some() {
        // Se ignora, pero no en silencio: quien la puso tiene que poder
        // enterarse de por qué no pasó nada.
        eprintln!(
            "[edecan] EDECAN_LOCAL_DEV_CMD está definida pero se IGNORA: \
             esta es una build instalada y usa su propio sidecar. \
             Para forzar el modo fuente, define también EDECAN_LOCAL_DEV=1."
        );
    }
    ...
    // La app de escritorio ES el asistente personal: un solo dueño, en su propia
    // máquina, pidiendo las cosas él mismo. Ahí la tarjeta de "¿confirmas?" le
    // pregunta lo que acaba de decir hace un segundo -- deja de ser un freno y
    // pasa a ser un peaje. Peor: un turno detenido esperando una aprobación que la
    // interfaz no alcanzó a pintar se ve idéntico a un cuelgue, y eso ya pasó de
    // verdad (`publicar_social` esperando una tarjeta que nunca apareció).
    //
    // El gate NO desaparece del proyecto: `EDECAN_SIN_CONFIRMACIONES` sigue en
    // `false` por defecto, así que un Edecán servido a varias personas -- donde
    // quien aprueba y quien pide no son la misma -- lo conserva intacto. Es la
    // decisión de ESTE empaquetado, no del núcleo. Quien quiera el freno en su
    // escritorio puede ponerla en `0` en el entorno: `env` no pisa lo que el
    // usuario ya definió, se aplica solo si no venía.
    let cmd = match std::env::var_os("EDECAN_SIN_CONFIRMACIONES") {
        Some(_) => cmd,
        None => cmd.env("EDECAN_SIN_CONFIRMACIONES", "1"),
    };
    Ok(with_expanded_path(with_opencode_env(with_ollama_env(cmd)))
...
/// Igual que `with_ollama_env`, pero para **opencode**, que desde el
/// 30-jul-2026 es el MOTOR del IDE (ver `docs/opencode-motor.md`), no una
/// integración opcional como Ollama. La diferencia práctica: si Ollama falta,
/// la app pierde una función; si opencode falta, el IDE no puede ejecutar
/// ningún agente. Por eso `download-opencode.sh` corre siempre en
/// `build-backend.sh`, sin variable que lo active.
///
/// Fija `EDECAN_OPENCODE_BIN`, que es exactamente la variable que ya lee
/// `apps/companion/edecan_companion/ide_opencode_binario.py`
/// (`VARIABLE_ENTORNO_BINARIO`) como segundo candidato de su búsqueda
/// bundle → env → PATH.
fn with_opencode_env(
    cmd: tauri_plugin_shell::process::Command,
) -> tauri_plugin_shell::process::Command {
    match resolve_opencode_sidecar() {
...
/// caminos que `resolve_ollama_sidecar` (ver su docstring): junto al
/// ejecutable actual una vez construido, o con el sufijo de target-triple en
/// `binaries/` si todavía no se corrió `cargo tauri build` después de la
/// descarga.
fn resolve_opencode_sidecar() -> Option<PathBuf> {
    let exe_name = if cfg!(target_os = "windows") {
        "opencode.exe"
    } else {
        "opencode"
    ...
    if let Ok(entries) = std::fs::read_dir(&source_dir) {
        for entry in entries.flatten() {
            if entry.file_name().to_string_lossy().starts_with("opencode-") {
                return Some(entry.path());
            }
        }
    }
...
pub fn kill_backend(app: &AppHandle) {
    ...
    #[cfg(not(target_os = "windows"))]
    {
        if terminate_verified_backend_tree(pid, &data_dir(app)) {
            remove_backend_pid_file(app);
            return;
        }
    }
...
fn backend_pid_path(app: &AppHandle) -> PathBuf {
    app.path()
        .app_data_dir()
        .expect("no se pudo resolver el directorio de datos de la app")
        .join(BACKEND_PID_FILE)
}

fn persist_backend_pid(app: &AppHandle, pid: u32) -> Result<(), String> {
    let path = backend_pid_path(app);
...
fn remove_backend_pid_file(app: &AppHandle) {
    let path = backend_pid_path(app);
    if let Err(error) = std::fs::remove_file(path) {
        if error.kind() != std::io::ErrorKind::NotFound {
            eprintln!("[edecan-desktop] no se pudo borrar el PID del backend: {error}");
        }
    }
}

/// Recupera únicamente el sidecar que una ejecución anterior de ESTA app
/// dejó registrado. Esto cubre cierres forzados (SIGKILL, crash o reemplazo
/// de la app durante desarrollo), donde `RunEvent::Exit` no alcanza a llamar
/// `kill_backend` y el proceso viejo conserva tanto el puerto como una
/// capacidad de autenticación que la WebView nueva ya no conoce.
fn terminate_stale_backend(app: &AppHandle) {
    let path = backend_pid_path(app);
    let Ok(raw_pid) = std::fs::read_to_string(&path) else {
        return;
    };
    ...
    if !recorded_pid_is_edecan_backend(pid, &data_dir(app)) {
        // El PID pudo reciclarse. Nunca señalamos un proceso que no podamos
        // identificar como nuestro sidecar y nuestra carpeta de datos.
        remove_backend_pid_file(app);
        return;
    }
    ...
    #[cfg(not(target_os = "windows"))]
    {
        terminate_verified_backend_tree(pid, &data_dir(app));
    }
...
#[cfg(not(target_os = "windows"))]
fn recorded_pid_is_edecan_backend(pid: u32, expected_data_dir: &Path) -> bool {
    ...
    command.contains("edecan-local")
        && command.contains("--data-dir")
        && command.contains(&expected_data_dir.to_string_lossy().to_string())
}

/// Termina el sidecar y cualquier proceso hijo que el bootloader de
/// PyInstaller haya creado. El árbol se captura ANTES de señalar al padre:
/// después de un crash/reinicio el hijo puede quedar adoptado por launchd y
/// ya no sería posible reconstruir la relación padre-hijo.
///
/// Cada PID se vuelve a verificar contra `edecan-local` y la carpeta de datos
/// exacta de esta instalación antes de enviar una señal. Así un PID reciclado
/// o un backend de otra instalación nunca se toca.
#[cfg(not(target_os = "windows"))]
fn terminate_verified_backend_tree(root_pid: u32, expected_data_dir: &Path) -> bool {
    let descendants = descendant_pids(root_pid);

    if recorded_pid_is_edecan_backend(root_pid, expected_data_dir)
        && !send_sigterm_and_wait_for_exit(root_pid)
    {
        send_sigkill(root_pid);
    }

    // Los hijos se recorren de abajo hacia arriba. Normalmente el cierre
    // limpio del root ya los habrá retirado; este segundo paso solo recoge
    // bootloaders/workers que hayan quedado huérfanos.
    for pid in descendants.iter().rev().copied() {
        if recorded_pid_is_edecan_backend(pid, expected_data_dir)
            && !send_sigterm_and_wait_for_exit(pid)
        {
            send_sigkill(pid);
        }
    }
    ...
    !process_is_alive(root_pid)
        && descendants.into_iter().all(|pid| {
            !process_is_alive(pid) || !recorded_pid_is_edecan_backend(pid, expected_data_dir)
        })
}
...
fn descendant_pids(root_pid: u32) -> Vec<u32> {
    ...
    let relationships: Vec<(u32, u32)> = String::from_utf8_lossy(&output.stdout)
        .lines()
        .filter_map(|line| {
    ...
    while let Some(parent) = parents.pop() {
        for (pid, candidate_parent) in &relationships {
...
#[cfg(target_os = "windows")]
fn recorded_pid_is_edecan_backend(pid: u32, _expected_data_dir: &Path) -> bool {
...
mod tests {
    use super::{
        expanded_path_value, mini_ui_url, parse_ready_port, studio_node_version_supported,
    };
    ...
    #[test]
    fn mini_url_keeps_capability_and_marks_the_compact_composer() {
        assert_eq!(
            mini_ui_url("http://127.0.0.1:8765/?edecan_desktop=1#edecan_capability=abc"),
            "http://127.0.0.1:8765/app/?edecan_desktop=1#edecan_capability=abc&edecan_mini=1"
        );
        assert_eq!(
            mini_ui_url("http://127.0.0.1:8765/?edecan_desktop=1"),
            "http://127.0.0.1:8765/app/?edecan_desktop=1#edecan_mini=1"
        );
    }
