...
pub(crate) fn close_action(window_label: &str, keep_resident: bool) -> WindowCloseAction {
    if matches!(window_label, "main" | "mini") && keep_resident {
...
mod tests {
    ...
    #[test]
    fn closing_mini_hides_without_quitting() {
        assert_eq!(close_action("mini", true), WindowCloseAction::Hide);
        assert_eq!(close_action("mini", false), WindowCloseAction::Exit);
    }
