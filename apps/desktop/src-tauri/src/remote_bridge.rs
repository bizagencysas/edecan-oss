...
fn capture_screen(
    ...
    if capture_mode == "screen_capture_kit_stream_ready" {
        if let Some(frame) =
            crate::screen_capture::latest_frame_fresh(std::time::Duration::from_secs(2))
        {
            return Ok(serde_json::json!({
                "image_b64": base64::engine::general_purpose::STANDARD.encode(frame.png),
                "width": frame.width,
                "height": frame.height,
                "mime": "image/png",
...
fn execute_input(
    ...
    if !unsafe { AXIsProcessTrusted() } {
        let granted_now = !ACCESSIBILITY_PROMPT_SHOWN
