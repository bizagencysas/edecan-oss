...
#[cfg(target_os = "macos")]
pub(crate) fn show_in_dock(app: &AppHandle) {
    if let Err(err) = app.set_activation_policy(tauri::ActivationPolicy::Regular) {
        eprintln!("[edecan-desktop] no se pudo activar Edecán en el Dock: {err}");
    }
    if let Err(err) = app.set_dock_visibility(true) {
        eprintln!("[edecan-desktop] no se pudo mostrar Edecán en el Dock: {err}");
    }
}

#[cfg(not(target_os = "macos"))]
pub(crate) fn show_in_dock(_app: &AppHandle) {}

#[cfg(target_os = "macos")]
pub(crate) fn hide_from_dock(app: &AppHandle) {
    if let Err(err) = app.set_dock_visibility(false) {
        eprintln!("[edecan-desktop] no se pudo ocultar Edecán del Dock: {err}");
    }
    if let Err(err) = app.set_activation_policy(tauri::ActivationPolicy::Accessory) {
        eprintln!("[edecan-desktop] no se pudo dejar Edecán residente: {err}");
    }
}

#[cfg(not(target_os = "macos"))]
pub(crate) fn hide_from_dock(_app: &AppHandle) {}
...
pub fn setup_tray(app: &AppHandle) -> tauri::Result<()> {
    ...
    let ask_quick = MenuItem::with_id(app, "ask_quick", "Preguntar rápido", true, None::<&str>)?;
    let missions = MenuItem::with_id(app, "missions", "Misiones", true, None::<&str>)?;
    let capture_window =
        MenuItem::with_id(app, "capture_window", "Capturar ventana", true, None::<&str>)?;
    ...
    TrayIconBuilder::new()
        ...
        .on_menu_event(move |app, event| {
            ...
            match id {
                ...
                "ask_quick" => mini::show_mini(app),
                "missions" => {
                    show_and_focus_main(app);
                    let _ = app.emit(
                        "edecan://navigate",
                        serde_json::json!({ "path": "/app/misiones" }),
                    );
                }
                "capture_window" => {
                    mini::show_mini(app);
                    commands::emit_clipboard_context(app);
                }
                "open_browser" => {
