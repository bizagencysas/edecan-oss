...
## Sembrado de las automatizaciones de LinkedIn (paridad Aria)
...
`edecan_local.acme_linkedin_profile_seed.ensure_acme_linkedin_profile_seeded`
(idempotente vía `_seed_version`, ver su docstring): aplica la voz/pilares/banco de
contexto reales de la página de LinkedIn de Acme al perfil editorial de este tenant.
...
async def _propagate_terminal_failure_to_mission(
    ...
    transición inicial de `agent_missions` DESPUÉS de resolver la inferencia
    administrada (`deps.llm_router_for`) -- un fallo de inicialización ANTES
    de ese punto nunca tocaba la fila de la misión. El `job` sí quedaba
    `status='error'` con
