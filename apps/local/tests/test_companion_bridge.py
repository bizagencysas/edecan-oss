...
async def test_bridge_rejects_unknown_actions(tmp_path: Path) -> None:
    manager = _Manager()
    app = SimpleNamespace(state=SimpleNamespace(companion_manager=manager))
    bridge = LocalCompanionBridge(app=app, data_dir=tmp_path)

    result = await bridge.execute("format_disk", {})

    assert result["ok"] is False
    assert "no disponible" in result["error"]


async def test_bridge_opens_apps_for_the_desktop_owner(
    ...
    async def fake_execute(action, params, config, approver):
        ...
        assert params["app"] == "Safari"
        assert await approver(action, params, config) is True
        assert config.allow_all_apps is True
        return {"ok": True, "result": {"app": "Safari", "launched": True}}

    monkeypatch.setattr(companion_bridge_module.actions, "execute", fake_execute)
    result = await bridge.execute("open_app", {"app": "Safari"})

    assert result == {"ok": True, "result": {"app": "Safari", "launched": True}}
...
async def test_real_local_protocol_supports_tree_editor_files_search_and_terminal(
    ...
    # "pwd" no existe como ejecutable independiente en Windows (es un builtin
    # de shell en POSIX, y aquí se corre siempre con shell=False). Usamos el
    # propio intérprete de Python -- garantizado presente y portable en las
    # tres plataformas -- para imprimir el cwd en vez de depender de pwd(1).
    # as_posix() evita que las barras invertidas de una ruta de Windows
    # confundan a shlex.split (que usa reglas POSIX de escape) más abajo.
    ...
    terminal = await manager.send_command(
        tenant_id, "run_command", {"command": pwd_command}
    )
...
async def test_local_terminal_rejects_commands_outside_the_owner_defaults(tmp_path: Path) -> None:
    manager = ConnectionManager()
    app = SimpleNamespace(state=SimpleNamespace(companion_manager=manager))
    LocalCompanionBridge(app=app, data_dir=tmp_path)

    result = await manager.send_command(
        uuid.uuid4(), "run_command", {"command": "definitely-not-a-real-bin --wipe"}
    )
...
async def test_desktop_owner_can_capture_screen_without_phone_session(
    ...
    assert (await bridge.execute("screenshot", {}))["ok"] is True
