...
def _set_home(monkeypatch: pytest.MonkeyPatch, home: Path) -> None:
    """Fuerza `Path.home()`/`Path.expanduser()` a resolver dentro de `home`,
    sin importar la plataforma. POSIX consulta `HOME`, pero
    `ntpath.expanduser` de Windows ignora `HOME` por completo y busca
    primero `USERPROFILE` -- si solo se fija `HOME`, el test quedaría
    determinado por el `USERPROFILE` real de quien lo corre en Windows."""
