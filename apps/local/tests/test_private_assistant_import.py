...
def test_private_documents_are_outside_repo_style_and_owner_only(tmp_path: Path) -> None:
    ...
    assert count == 1
    assert root.joinpath("context.md").read_text(encoding="utf-8") == "privado"
    # `_chmod_best_effort` es de mejor esfuerzo: en NTFS `os.chmod` solo puede
    # tocar el bit de solo-lectura, nunca replica 0o700/0o600 (ver
    # private_assistant_import._chmod_best_effort). El bit exacto solo se
    # puede verificar en POSIX.
    if os.name == "posix":
