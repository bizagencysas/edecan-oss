...
# ---------------------------------------------------------------------------
# _load_local_platform_config / _apply_local_platform_config
# ---------------------------------------------------------------------------


def test_load_local_platform_config_solo_acepta_allowlist(tmp_path: Path) -> None:
    (tmp_path / "platform-config.json").write_text(
        json.dumps(
            {
                "CLOUDFLARE_ACCOUNT_ID": "cuenta",
                "CLOUDFLARE_API_TOKEN": "token-privado",
                "WORKERS_AI_CHAT_MODEL": "@cf/zai-org/glm-4.7-flash",
                "WORKERS_AI_TIMEOUT_SECONDS": 42,
                "DATABASE_URL": "postgresql://no-debe-entrar",
            }
        ),
        encoding="utf-8",
    )

    assert rt._load_local_platform_config(tmp_path) == {
        "CLOUDFLARE_ACCOUNT_ID": "cuenta",
        "CLOUDFLARE_API_TOKEN": "token-privado",
        "WORKERS_AI_CHAT_MODEL": "@cf/zai-org/glm-4.7-flash",
        "WORKERS_AI_TIMEOUT_SECONDS": "42",
...
def test_load_local_platform_config_corrupta_no_rompe_arranque(tmp_path: Path) -> None:
    (tmp_path / "platform-config.json").write_text("{no-json", encoding="utf-8")
    assert rt._load_local_platform_config(tmp_path) == {}


def test_apply_local_platform_config_respeta_entorno_existente(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    (tmp_path / "platform-config.json").write_text(
        json.dumps(
            {
                "CLOUDFLARE_ACCOUNT_ID": "desde-archivo",
                "CLOUDFLARE_API_TOKEN": "token-desde-archivo",
            }
        ),
        encoding="utf-8",
    )
    monkeypatch.setenv("CLOUDFLARE_ACCOUNT_ID", "desde-entorno")
    monkeypatch.delenv("CLOUDFLARE_API_TOKEN", raising=False)

    rt._apply_local_platform_config(tmp_path)

    assert os.environ["CLOUDFLARE_ACCOUNT_ID"] == "desde-entorno"
    assert os.environ["CLOUDFLARE_API_TOKEN"] == "token-desde-archivo"
...
def test_mobile_public_url_prefiere_ip_privada_que_resuelve_android(
    ...
    monkeypatch.delenv("EDECAN_MOBILE_PUBLIC_URL", raising=False)
    monkeypatch.setattr(rt.socket, "gethostname", lambda: "MacBook-Pruebas")
...
def test_mobile_public_url_convierte_hostname_local_en_mdns_si_no_hay_ip(
    ...
    assert rt._mobile_public_url(8765) == "http://MacBook-Pruebas.local:8765"
...
async def test_run_orquesta_todo_en_orden_y_apaga_limpio(
    ...
    # En NTFS `os.chmod` es de mejor esfuerzo (solo el bit de solo-lectura,
    # nunca replica 0o700): el bit exacto solo se puede verificar en POSIX.
    if os.name == "posix":
