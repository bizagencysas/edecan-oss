...
async def test_mark_failure_run_mission_agotado_cierra_la_mision_con_error() -> None:
    ...
    await worker_loop._mark_failure(
        ...
        job["payload"],
        attempts_previos,
        "RuntimeError: proveedor de inferencia no disponible",
    )

    assert pool.conn.mission_updates == [
        (mission_id, "RuntimeError: proveedor de inferencia no disponible")
    ]
...
def _fake_deps_para_scheduler_tick() -> SimpleNamespace:
    ...
    función: `ensure_linkedin_automations_seeded` nunca lanza (atrapa
    cualquier excepción y solo loguea, ver su docstring) -- un
    `session_factory` que revienta al primer uso basta para ejercitar esa
    rama sin romper estos tests, que solo verifican el loop de `enqueue`."""

    def _session_factory_que_revienta(tenant_id: Any) -> Any:
        raise RuntimeError("sin Postgres fake en este test -- ver docstring del helper")

    return SimpleNamespace(
        settings=SimpleNamespace(), session_factory=_session_factory_que_revienta
    )
...
def test_event_log_cleanup_es_diario_no_por_tick() -> None:
    """La limpieza de event_log es un barrido diario: nunca en los ticks cortos
    (30s/60s/300s/semanales)."""
    assert "event_log_cleanup" in worker_loop.JOBS_PERIODICOS_DIARIOS
    todos = (
    ...
    assert "event_log_cleanup" not in todos
