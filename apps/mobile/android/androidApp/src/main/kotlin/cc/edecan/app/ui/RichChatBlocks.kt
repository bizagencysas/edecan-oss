    ...
    bloques.forEachIndexed { index, bloque ->
        if (bloque.esBloqueViaje()) {
            ...
            when (bloque) {
                ...
                is ChatBlock.SocialDraft -> TarjetaSocialDraft(
                    bloque = bloque,
                    previews = previews,
...
@Composable
private fun TarjetaSocialDraft(
    bloque: ChatBlock.SocialDraft,
    previews: Map<String, VistaPreviaPrivada>,
    previewsCargando: Set<String>,
    onCargarPreview: (ChatBlock.Media) -> Unit,
    onAction: (ChatAction) -> Unit,
    ...
    val imagen = remember(bloque.artifacts) {
        bloque.artifacts.firstOrNull { it.mime?.lowercase()?.startsWith("image/") == true }
    }
    var cargaSolicitada by remember(imagen?.fileId) { mutableStateOf(false) }
    LaunchedEffect(imagen?.fileId, cargaSolicitada) {
        if (imagen != null && !cargaSolicitada && imagen.fileId !in previews && imagen.fileId !in previewsCargando) {
            cargaSolicitada = true
            onCargarPreview(
                ChatBlock.Media(
                    mediaKind = "image",
                    artifact = imagen,
                    alt = bloque.altText.orEmpty(),
                    caption = null,
                ),
            )
        }
    }
    // `target` es un id de destino ABIERTO (no un enum cerrado a "personal"/"organization"):
    // "personal" es el único id reservado -- cualquier otro valor es una página de
    // organización que el propio tenant configuró en el backend (ver
    // `edecan_creative.marcas.BrandDestination`).
    val esDestinoDeOrganizacion = !bloque.target.isNullOrBlank() &&
        !bloque.target.equals("personal", ignoreCase = true)
    val previewImagen = imagen?.let { previews[it.fileId] as? VistaPreviaPrivada.Imagen }
    TarjetaRica {
        if (imagen != null) {
            when (val preview = previews[imagen.fileId]) {
                is VistaPreviaPrivada.Imagen -> ImagenPrivada(preview.descarga)
                else -> if (imagen.fileId in previewsCargando) {
        ...
        Text(
            text = "💼 Post para LinkedIn · listo",
            color = MaterialTheme.colorScheme.primary,
            style = MaterialTheme.typography.labelLarge,
            fontWeight = FontWeight.SemiBold,
        )
        SelectionContainerCompat {
            Text(bloque.copy, style = MaterialTheme.typography.bodyMedium)
        }
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
            if (esDestinoDeOrganizacion) {
                Button(onClick = {
                    onAction(
                        ChatAction.PrefillMessage(
                            id = "approve-organization-linkedin",
                            label = "Aprobar",
                            message = "Aprueba y publica este borrador de LinkedIn.",
                        ),
                    )
                }) { Text("Aprobar") }
                OutlinedButton(onClick = {
                    onAction(
                        ChatAction.PrefillMessage(
                            id = "edit-organization-linkedin",
                            label = "Hacer cambios",
                            message = "Haz cambios a este borrador de LinkedIn. Mantén el mismo destino y cambia esto: ",
                        ),
                    )
                }) { Text("Hacer cambios") }
            } else {
                OutlinedButton(onClick = {
                    ...
                    clipboard.setPrimaryClip(ClipData.newPlainText("Post de LinkedIn", bloque.copy))
                }) { Text("Copiar texto") }
                Button(
                    onClick = {
                        if (previewImagen != null) {
                            guardarImagenEnGaleria(context, previewImagen.descarga)
                        } else if (imagen != null) {
                            onAbrirArtefacto(imagen)
                        }
                    },
                    enabled = imagen != null,
                ) { Text("Guardar imagen") }
...
private fun guardarImagenEnGaleria(context: Context, descarga: DownloadedArtifact) {
    val mime = descarga.artifact.mime?.takeIf { it.startsWith("image/") } ?: "image/png"
    val filename = descarga.artifact.filename.ifBlank { "edecan-linkedin.png" }
    val values = ContentValues().apply {
        put(MediaStore.Images.Media.DISPLAY_NAME, filename)
        put(MediaStore.Images.Media.MIME_TYPE, mime)
        put(MediaStore.Images.Media.RELATIVE_PATH, "Pictures/Edecan")
        put(MediaStore.Images.Media.IS_PENDING, 1)
    }
    val resolver = context.contentResolver
    val uri = resolver.insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values) ?: return
    runCatching {
        resolver.openOutputStream(uri)?.use { output -> output.write(descarga.bytes) }
        values.clear()
