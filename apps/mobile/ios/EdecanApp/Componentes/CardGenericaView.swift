...
struct CardGenericaView: View {
    ...
    /// Tocar una imagen de la card la abre a tamaño completo. La card dibuja una
    /// MINIATURA (1280 px) por memoria; el visor baja el original aparte, así que
    /// esto no reusa el bitmap de la tarjeta.
    let onAbrirImagen: (ArtifactRef) -> Void
    ...
    private func artefactosDeImagen(_ nodo: NodoCard) -> [String: ArtifactRef] {
        ...
        case .video(let video):
            return [video.artifact.fileId: video.artifact]
...
private struct NodoCardView: View {
    ...
    var body: some View {
        ...
        case .imagen(let imagen):
            ImagenNodoView(nodo: imagen, client: client, onAbrir: onAbrirImagen)
        case .video(let video):
            VideoNodoView(nodo: video, client: client)
...
private struct StackNodoView: View {
    ...
    private var hijos: some View {
        ForEach(Array(contenedor.hijos.enumerated()), id: \.offset) { _, hijo in
            NodoCardView(nodo: hijo, client: client, artefactos: artefactos, onAction: onAction, onResponder: onResponder, onAbrirImagen: onAbrirImagen)
        }
    }
...
private struct ImagenNodoView: View {
    let nodo: ImagenNode
    let client: APIClient?
    let onAbrir: (ArtifactRef) -> Void

    @State private var imagen: UIImage?
    @State private var cargaFinalizada = false
    /// Cambiarlo reinicia la `.task` — es el "Reintentar" de esta vista.
    ...
    var body: some View {
            ...
            if let imagen {
                ...
                ZStack {
                    ...
                    VStack(spacing: 8) {
                        ...
                        // Antes no había salida: una sola descarga fallida
                        // dejaba la card con el icono roto PARA SIEMPRE. Mismo
                        // botón que `VistaPreviaMultimedia`.
                        Button("Reintentar") {
        ...
        // Tocar la imagen la abre a tamaño completo. Solo cuando YA hay imagen:
        // mientras carga o si falló, el toque no hace nada (y el botón
        // "Reintentar" del estado de error sigue siendo suyo, sin que este
        // gesto se lo robe).
        .contentShape(Rectangle())
        .onTapGesture {
            guard imagen != nil else { return }
            onAbrir(nodo.artifact)
        }
        .accessibilityAddTraits(imagen != nil ? .isButton : [])
        .accessibilityHint(imagen != nil ? "Toca para ver la imagen completa" : "")
        .task(id: "\(nodo.artifact.fileId)#\(intento)") {
            guard imagen == nil, !cargaFinalizada else { return }
            guard let client else {
                cargaFinalizada = true
                return
            }
            let esperado = nodo.artifact.fileId
            do {
                // Miniatura del servidor en vez del original: la card dibuja a
                // 1280 px y el original es un PNG de 2160x2160. Bajo la misma
                // puerta que ``VistaPreviaImagenAdjunta`` (`limitadorImagenesChat`,
                // en `BurbujaMensaje.swift`): un hilo con varias cards de imagen
                // no debe decodificarlas todas a la vez.
                let miniatura = try await limitadorImagenesChat.ejecutar {
                    let descargado = try await client.descargarMiniatura(
                        nodo.artifact, maxPixeles: 1_280
                    )
                    return await Task.detached(priority: .userInitiated) {
                        cgImagePreviaAcotada(data: descargado.data, maxPixelSize: 1_280)
                    }.value
                }
                guard !Task.isCancelled, nodo.artifact.fileId == esperado else { return }
                imagen = miniatura.map(UIImage.init(cgImage:))
                cargaFinalizada = true
            } catch {
                // El `try?` que había aquí se tragaba también el
                // `CancellationError` que `APIClient.realizar` construye a
                // propósito para decir "esto NO es una caída de red": SwiftUI
                // cancela la tarea al reciclar la fila o al cambiar la
                // identidad de la vista, y eso quedaba latcheado como imagen
                // rota permanente. Mismo criterio que
                // `VistaPreviaMultimedia.cargar` y `SocialDraftCardView`.
                guard !Task.isCancelled, nodo.artifact.fileId == esperado else { return }
                cargaFinalizada = true
            }
        }
...
/// Video dentro de una card compuesta. Espejo de ``ImagenNodoView`` para el
/// layout (mismo `aspecto`/`esquinas`), pero reproduce con `AVPlayer` en vez de
/// pintar un bitmap. Reusa el MISMO par descarga+temporal de
/// ``VistaPreviaMultimedia`` (`client.descargarArtefacto` +
/// `guardarPreviewTemporal`): los bytes siempre se obtienen por el Bearer
/// privado, nunca una URL pública. A propósito NO carga solo al aparecer: un
/// MP4 puede pesar megas, y un hilo con varias cards no debe bajarlas en
/// ráfaga al abrir el chat (misma decisión que `VistaPreviaMultimedia` para
/// video/audio). Se espera un toque explícito que descarga, crea el player y
/// reproduce con ``VideoPlayer``.
private struct VideoNodoView: View {
    let nodo: VideoNode
    let client: APIClient?
    ...
    var body: some View {
        ...
        .aspectRatio(proporcion ?? 16.0 / 9.0, contentMode: .fit)
        .clipShape(RoundedRectangle(cornerRadius: esquinas, style: .continuous))
        .accessibilityLabel(nodo.alt.isEmpty ? "Video" : nodo.alt)
        .onDisappear { limpiarRecursos() }
    }
    ...
    private func cargar() async {
        guard estado != .cargando else { return }
        guard let client else {
            estado = .error
            return
        }
        ...
        let esperado = nodo.artifact.fileId
        do {
            let descarga = try await client.descargarArtefacto(nodo.artifact)
            guard !Task.isCancelled, nodo.artifact.fileId == esperado else { return }
            let url = try await Task.detached(priority: .userInitiated) {
                try guardarPreviewTemporal(descarga)
            }.value
            guard !Task.isCancelled, nodo.artifact.fileId == esperado else {
