...
struct VistaPreviaMultimedia: View {
    ...
    var onAbrir: ((ArtifactRef) -> Void)? = nil
    ...
    private var mediaLista: some View {
        ...
        case .image:
            if let imagen {
                ...
                .buttonStyle(.plain)
                .accessibilityLabel(bloque.alt.isEmpty ? "Pantalla de tu Mac" : bloque.alt)
                .accessibilityHint("Toca para ampliar y hacer zoom")
            }
        case .video:
            if let player {
    ...
    private func cargar(expectedFileId: String) async {
        ...
        do {
            switch bloque.mediaKind {
            case .image:
                // Esta imagen se carga SOLA al aparecer (ver el docstring del
                // tipo), así que un hilo con varios `MediaBlock` de imagen
                // dispara varias descargas a la vez apenas se abre -- misma
                // puerta que `VistaPreviaImagenAdjunta`/`ImagenNodoView`
                // (`limitadorImagenesChat`, en `BurbujaMensaje.swift`). Video
                // y audio quedan FUERA de la puerta a propósito: solo bajan
                // tras un toque explícito, no en ráfaga al abrir el hilo.
                let decoded = try await limitadorImagenesChat.ejecutar { () -> UIImage in
...
func guardarPreviewTemporal(_ descarga: DownloadedArtifact) throws -> URL {
...
func limpiarPreviewTemporal(_ url: URL) {
