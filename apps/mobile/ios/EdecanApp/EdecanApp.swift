...
private struct RaizDeLaApp: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .edecanNotificationRoute)) { notification in
            ...
            // Frente 3 (deeplink): `conversationId` viaja junto a la ruta
            // desde `publicarRuta` (PushNotifications.swift), tanto para el
            // tap en caliente como para el relanzamiento en frío. Se asigna
            // antes que `rutaPendiente` porque `RootTabView` lee ambos en el
            // mismo `onChange`, disparado por este último.
