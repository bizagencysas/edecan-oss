        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                if let proxima = automatizacion.nextRunAt {
                    // "(tu hora)" no sobra: la agenda de abajo va en hora del
                    // servidor (UTC) y esta línea la formatea en la del
                    // teléfono. Sin el rótulo, dos horas distintas para lo
                    // mismo parecen un error.
                    Text("Próxima: \(proxima.formatted(date: .abbreviated, time: .shortened)) (tu hora)")
                        .font(.caption2)
                        .foregroundStyle(.secondary)
                }
            }

            Label(automatizacion.trigger.resumen, systemImage: "clock")
                .font(.subheadline)
                .foregroundStyle(.primary)
            if case .schedule(let rrule, let descripcion) = automatizacion.trigger,
               descripcion == nil {
                // El servidor no supo traducir esta regla. Antes que
                // inventarle una hora se enseña tal cual: quien la escribió a
                // mano puede verificarla, y nadie lee una hora falsa.
                Text(rrule)
...
private struct NuevaAutomatizacionSheet: View {
    ...
    private enum Preset: String, CaseIterable, Identifiable {
        ...
        /// Palabra por palabra lo que `edecan_automations.describe` devolverá
        /// después para esa misma `rrule`: se elige "Todos los días a las 9:00
        /// UTC" y eso mismo se lee luego en la lista. El "UTC" no es un
        /// tecnicismo de más — las agendas corren en la hora del SERVIDOR, y
        /// omitirlo haría creer que son las 9:00 de quien está mirando.
        var etiqueta: String {
            switch self {
            case .diario: return "Todos los días a las 9:00 UTC"
            case .semanal: return "Todos los lunes a las 9:00 UTC"
            case .mensual: return "El día 1 de cada mes a las 9:00 UTC"
            ...
            Form {
                ...
                Section {
                    ...
                    if preset == .personalizada {
                        TextField("FREQ=DAILY;BYHOUR=9;BYMINUTE=0", text: $rruleManual)
                    ...
                    if preset == .personalizada {
                        Text(
                            "RFC 5545 en hora UTC. Pon siempre BYMINUTE junto a BYHOUR: sin él el "
                            + "minuto se hereda de la hora en que la creas."
                        )
                    } else {
                        Text("Las agendas corren en hora del servidor (UTC).")
                    }
                }
                Section {
