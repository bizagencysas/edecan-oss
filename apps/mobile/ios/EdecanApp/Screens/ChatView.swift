...
struct ChatView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .task {
                ...
                // Frente 4: en frío se aterriza en un chat nuevo y vacío, no
                // en la última conversación. El historial sigue completo y
                // accesible por el botón de conversaciones; el borrador que
                // se recupera aquí es el del propio "chat nuevo" (clave
                // "new"), no el de la conversación anterior. Un envío a
                // medias sí se recupera: eso lo maneja `iniciar` con el
                // intento pendiente, que no depende de este borrador.
                if textoActual.isEmpty {
                    textoActual = cargarBorrador(conversationId: nil)
                }
                await viewModel.iniciar(client: client, preferredConversationId: nil)
            }
    ...
    private func ejecutarAccion(_ action: ChatAction) {
        ...
        case .approveDraft(_, _, let draftId):
            // DESVIACIÓN del paso 4 (ver JSON de retorno): todavía no existe un
            // endpoint que publique un borrador SOLO por `draft_id` --
            // `POST /v1/content/social/publish` (`content_studio.py`) exige
            // `target`/`text`/`image_file_id` explícitos, no una referencia a
            // un borrador guardado server-side. Mostrar un error claro es más
            // seguro que simular una publicación o fallar en silencio; queda
            // para el paso 5 del plan de SDUI (el piloto backend-only).
            viewModel.errorMensaje = "Aprobar desde esta tarjeta todavía no está conectado a un endpoint de publicación (borrador \(draftId))."
        ...
        NavigationStack {
            Group {
                if viewModel.cargandoHistorial && viewModel.conversaciones.isEmpty {
                    ...
                    // La conversación "principal" (frente 5, paridad Aria —
