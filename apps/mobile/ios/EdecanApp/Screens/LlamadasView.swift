...
private struct DetalleLlamadaView: View {
    ...
    var body: some View {
        List {
            ...
            // Solo mientras la llamada no haya terminado: ahí es donde tiene
            // sentido ver turnos en vivo o susurrarle algo a Edecán (frente
            // 6c, paridad REFERENCIA) — ver `LlamadaEnVivoView`.
            if !llamadaTerminada {
                Section {
                    NavigationLink {
                        LlamadaEnVivoView(llamada: llamada)
                    } label: {
                        Label("Ver llamada en vivo", systemImage: "waveform")
