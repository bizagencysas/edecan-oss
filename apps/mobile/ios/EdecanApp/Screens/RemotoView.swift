...
///
/// Guardrail de producto (`DIRECCION_ACTUAL.md`, "Control remoto del Mac/PC
/// desde el móvil"): emparejamiento explícito + aprobación humana + indicador
/// visible + botón Terminar SIEMPRE alcanzable. Esta vista nunca muestra el
/// visor sin, a la vez, mostrar el banner "Sesión remota activa" Y el botón
/// Terminar/Salir — ver ``bannerDeSesion(_:)``: se renderiza siempre que
/// `viewModel.sesion` no sea `nil`, sin importar el `status` (pendiente,
/// activa, denegada o terminada).
///
/// **DÓNDE SE RELAJÓ, Y POR QUIÉN (2-ago-2026).** La "aprobación humana" ya NO
/// es un consentimiento por sesión: abrir esta pantalla inicia directamente una
/// sesión de CONTROL. Lo pidió el dueño, textualmente — *"me exige siempre pasar
/// dos switches para poder activar el remoto, elimina eso y solo deja que
/// 'remoto' abra automáticamente la pantalla de la Mac"* — y se le advirtió que
/// tocaba este guardrail antes de hacerlo. Su razón: el iPhone ya se vinculó por
/// QR una vez; reconfirmar en cada sesión es fricción sobre algo ya autorizado.
...
struct RemotoView: View {
    ...
    /// La misma posición que ``ultimoPuntoRemoto`` pero NORMALIZADA (`nx`/`ny`
    /// en 0…1). Es la que mandan los botones del dock (clic derecho, scroll):
    /// sin ella volverían a caer en el bug de mandar coordenadas del frame
    /// comprimido como si fueran puntos de la pantalla real.
    @State private var ultimaFraccionRemota: CGPoint?
    ...
    var body: some View {
        ...
        // Abrir "Remoto" YA abre la pantalla de la Mac CON control (mouse y
        // teclado), sin interruptores. Decisión explícita del dueño (2-ago-2026,
        // textual: "elimina eso y solo deja que 'remoto' abra automáticamente la
        // pantalla de la Mac"), tomada sabiendo que relaja el guardrail de
        // aprobación por sesión descrito arriba. Su razón: el iPhone ya se vinculó
        // por QR una vez, y volver a confirmar en cada sesión era fricción sobre
        // algo ya autorizado.
        //
        // Lo que NO se relajó, y no se debe relajar sin volver a preguntarle:
    ...
    private func visorInmersivo(sesion: RemoteSession) -> some View {
        ...
        .onChange(of: fotoSeleccionada) { _, item in
            ...
            Task { await enviarFotoSeleccionada(item) }
        }
    }

    /// Carga la foto elegida en la fototeca (bytes + un nombre razonable con la
    /// extensión real) y la manda a la Mac. `PhotosPickerItem` no expone el
    /// nombre original, así que se arma uno con la extensión del tipo real.
    private func enviarFotoSeleccionada(_ item: PhotosPickerItem) async {
        defer { fotoSeleccionada = nil }
        guard let datos = try? await item.loadTransferable(type: Data.self) else {
            viewModel.errorMensaje = "No se pudo leer la foto elegida."
            return
        }
        let extOriginal = item.supportedContentTypes.first?.preferredFilenameExtension ?? "jpg"
        let (bytes, ext) = Self.recomprimirAJPEG(datos, extensionOriginal: extOriginal)
        let sello = Self.selloDeNombre.string(from: Date())
        await viewModel.enviarArchivo(
            nombre: "Foto \(sello).\(ext)", datos: bytes, client: session.client
        )
    }

    /// Recomprime una foto a JPEG (calidad 0.8, lado mayor máx. 2560 px) para
    /// bajar el peso de HEIC/PNG/RAW y no topar el límite de 10 MiB. Si el JPEG
    /// no queda más chico que el original (p. ej. ya era un JPEG pequeño),
    /// conserva el original con su extensión real. El `UIImage` decodifica
    /// HEIC/RAW/PNG por igual, así que la salida siempre es un JPEG estándar.
    private static func recomprimirAJPEG(
        _ original: Data, extensionOriginal: String
    ) -> (Data, String) {
        guard let imagen = UIImage(data: original) else { return (original, extensionOriginal) }
        let maxLado: CGFloat = 2560
        let lado = max(imagen.size.width, imagen.size.height)
        let escalada: UIImage
        if lado > maxLado {
            let factor = maxLado / lado
            let nuevoTamano = CGSize(
                width: imagen.size.width * factor, height: imagen.size.height * factor
            )
            // scale = 1: sin este formato, el renderer multiplica por el factor
            // de pantalla del dispositivo y la imagen quedaría igual de grande.
            let formato = UIGraphicsImageRendererFormat.default()
            formato.scale = 1
            escalada = UIGraphicsImageRenderer(size: nuevoTamano, format: formato).image { _ in
                imagen.draw(in: CGRect(origin: .zero, size: nuevoTamano))
            }
        } else {
            escalada = imagen
        }
        guard let jpeg = escalada.jpegData(compressionQuality: 0.8), jpeg.count < original.count else {
            return (original, extensionOriginal)
        }
        return (jpeg, "jpg")
    }

    /// Sello legible y ordenable para nombrar fotos ("Foto 2026-07-24 18-03-11.jpg").
    private static let selloDeNombre: DateFormatter = {
    ...
    private func enviarAccionDock(_ accion: RemotePointerAccion, deltaY: Int = 0) {
        ...
        // Sin un toque previo, el centro exacto de la pantalla (0.5, 0.5).
        let fraccion = ultimaFraccionRemota ?? CGPoint(x: 0.5, y: 0.5)
        Task {
            await viewModel.enviarPointer(
                RemotePointerInput(
                    x: Int(punto.x), y: Int(punto.y), accion: accion, deltaY: deltaY,
                    nx: Double(fraccion.x), ny: Double(fraccion.y)
                ),
            ...
            Group {
                if let frame = viewModel.frame, let imagen = decodificarImagen(frame) {
                    Image(uiImage: imagen)
                        ...
                        // Doble tap = volver a 1× SOLO en sesiones de vista. En
                        // control se desactiva por completo (`including: .subviews`)
                        // en vez de con un `guard` adentro: un gesto de doble tap
                        // VIVO obliga a cualquier tap simple hermano a esperar la
                        // ventana del doble tap, que es justo el retraso que
                        // ``gestoDePuntero`` viene a eliminar.
                        .simultaneousGesture(
                            ...
                            including: sesion.esControl ? .subviews : .all
                        )
                } else {
                    reservaSinFrame(sesion: sesion)
    ...
    /// Toque simple → `input_pointer {accion: "click"}` **al instante**;
    /// mantener pulsado ≈0.45 s → `"double_click"`. Mapeados a coordenadas
    /// REALES vía ``RemoteCoordinateMapper`` (no las de la pantalla del
    /// teléfono, que casi nunca coinciden con la resolución del equipo
    ...
    ///
    /// **Por qué ya no hay un `SpatialTapGesture(count: 2)`**: componer
    /// `count: 2` con `count: 1` obliga a SwiftUI a esperar la ventana del
    /// doble tap (~0.3 s) antes de resolver el tap simple — o sea, CADA clic
    /// llegaba tarde. El esquema de acá abajo (tap = clic inmediato, mantener
    /// pulsado = doble clic) es el mismo que usa Referencia y no espera nada: el
    /// long press falla en cuanto el dedo se levanta antes de los 0.45 s, y
    /// `.exclusively` resuelve al tap en ese mismo instante.
    ///
    /// El `LongPressGesture` no reporta ubicación, así que va encadenado con
    /// un `DragGesture(minimumDistance: 0)` — el truco estándar para saber
    /// DÓNDE se mantuvo pulsado. Si ese arrastre se movió de verdad, se
    /// descarta: es un arrastre, no un doble clic (``gestoDeArrastre`` ya lo
    /// atiende).
    private func gestoDePuntero(sesion: RemoteSession, frame: RemoteFrame, tamano: CGSize) -> some Gesture {
        LongPressGesture(minimumDuration: Self.duracionDobleClic, maximumDistance: Self.distanciaArrastre)
            .sequenced(before: DragGesture(minimumDistance: 0, coordinateSpace: .local))
            .exclusively(before: SpatialTapGesture(count: 1))
            .onEnded { resultado in
                guard sesion.esControl, !sesion.esTerminal else { return }
                switch resultado {
                case .first(let secuencia):
                    guard case .second(_, let arrastre?) = secuencia,
                          abs(arrastre.translation.width) < Self.distanciaArrastre,
                          abs(arrastre.translation.height) < Self.distanciaArrastre
                    else { return }
                    enviarPointer(
                        local: puntoSinZoom(arrastre.location, tamano: tamano),
    ...
    /// Cuánto hay que mantener pulsado para que valga como doble clic.
    private static let duracionDobleClic: Double = 0.45

    /// Umbral en PUNTOS a partir del cual un dedo cuenta como arrastre y no
    /// como toque. Los 8 pt originales convertían en arrastre cualquier clic
    /// con el más mínimo temblor del dedo (el pulgar se mueve ~10-15 pt en un
    /// toque normal sobre una pantalla grande), así que muchos clics
    /// simplemente no llegaban. 22 pt deja pasar el toque y sigue muy por
    /// debajo de cualquier arrastre intencional.
    private static let distanciaArrastre: CGFloat = 22
    ...
    private func gestoDeArrastre(
        ...
        DragGesture(minimumDistance: Self.distanciaArrastre, coordinateSpace: .local)
            .onChanged { valor in
            ...
            .onEnded { valor in
                ...
                ultimaFraccionRemota = CGPoint(x: fin.nx, y: fin.ny)
                Task {
                    await viewModel.enviarPointer(
                        RemotePointerInput(
                            ...
                            nx: fin.nx, ny: fin.ny,
                            startNx: inicio.nx, startNy: inicio.ny
                        ),
                        client: session.client
                    )
                }
    ...
    private func enviarPointer(
        ...
        ultimaFraccionRemota = CGPoint(x: punto.nx, y: punto.ny)
        Task {
            await viewModel.enviarPointer(
                RemotePointerInput(
                    x: punto.x, y: punto.y, accion: accion, nx: punto.nx, ny: punto.ny
                ),
                client: session.client
            )
        }
...
private struct ControlesPunteroView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("Toca para hacer clic. Mantén pulsado para abrir (doble clic). Arrastra sobre la pantalla para mover o seleccionar. Pellizca para acercar; con zoom, arrastra para recorrer la pantalla (vuelve a 1× para arrastrar ventanas).")
                .font(.caption2)
                .foregroundStyle(.secondary)
        }
        .padding(14)
...
private struct CompartirRemotoView: View {
    ...
    @Binding var fotoSeleccionada: PhotosPickerItem?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            PhotosPicker(
                ...
                Label("Enviar una foto de la fototeca…", systemImage: "photo")
                    .frame(maxWidth: .infinity)
            }
            .buttonStyle(.borderedProminent)
            .disabled(deshabilitado)
            ...
            Button {
                ...
                Label("Enviar otro archivo…", systemImage: "doc")
                    .frame(maxWidth: .infinity)
            }
            .buttonStyle(.bordered)
            .disabled(deshabilitado)
