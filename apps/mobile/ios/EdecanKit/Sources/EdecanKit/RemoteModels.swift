...
/// `GET /v1/remote/sessions/{id}/frame` — un PNG/JPEG/WebP en base64 + el
...
public struct RemotePointerInput: Encodable, Sendable, Equatable {
    ...
    /// Fracción `0.0...1.0` del toque dentro del frame. Cuando viaja (junto
    /// con ``ny``), el companion la multiplica por el tamaño REAL de la
    /// pantalla de la Mac —en puntos lógicos, que es lo que consume
    /// `CGEvent`— e ignora ``x``/``y``. Es lo único que hace que el toque
    /// caiga exactamente donde el dedo lo puso: el frame viaja reducido a
    /// `max_width` y no coincide con ninguna resolución real. Ver
    /// ``RemoteCoordinateMapper/mapear(puntoLocalX:puntoLocalY:anchoElemento:altoElemento:frame:)``.
    public let nx: Double?
    public let ny: Double?
    /// Equivalente normalizado de ``startX``/``startY`` para `drag`.
    public let startNx: Double?
    public let startNy: Double?
    ...
    public init(
        ...
        nx: Double? = nil, ny: Double? = nil, startNx: Double? = nil, startNy: Double? = nil
    ) {
        self.x = x
        self.y = y
        self.accion = accion
        ...
        self.deltaX = deltaX
        self.deltaY = deltaY
        // Medio par normalizado partiría el gesto entre dos espacios de
        // coordenadas — el backend lo rechaza con 422, así que acá ni se
        // arma: o van los dos, o no va ninguno.
        if let nx, let ny {
        ...
        if let startNx, let startNy {
            self.startNx = min(max(startNx, 0), 1)
            self.startNy = min(max(startNy, 0), 1)
        } else {
            self.startNx = nil
            self.startNy = nil
        }
    ...
    enum CodingKeys: String, CodingKey {
        case tipo, x, y, accion, button, nx, ny
        case startX = "start_x"
        case startY = "start_y"
        case deltaX = "delta_x"
        case deltaY = "delta_y"
        case startNx = "start_nx"
        case startNy = "start_ny"
    }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        try container.encodeIfPresent(ny, forKey: .ny)
        try container.encodeIfPresent(startNx, forKey: .startNx)
        try container.encodeIfPresent(startNy, forKey: .startNy)
    }
}
...
public enum RemoteCoordinateMapper {
    ...
    /// [0, frame.height)` **más** la fracción normalizada `nx`/`ny` en
    /// `0.0...1.0`, o `nil` si cayó en la franja vacía del letterbox
    /// — el llamador (``RemotoView``) debe ignorarlo, nunca mandar un
    /// `input_pointer` con coordenadas inventadas (mismo contrato que la
    /// versión web).
    ///
    /// **Por qué existe `nx`/`ny` y por qué es lo que de verdad hay que
    /// mandar**: `frame.width`/`.height` NO son la resolución de la Mac. El
    /// companion captura en píxeles Retina (p. ej. 3456×2234) y después
    /// reduce la imagen a `max_width` (1600 por defecto) antes de mandarla,
    /// así que `frame.width` es 1600. Pero `CGEvent` —lo que de verdad mueve
    /// el puntero del otro lado— consume PUNTOS lógicos (p. ej. 2056×1329).
    /// Mandar `x`/`y` del frame como si fueran puntos dejaba cada toque al
    /// 77.8% del camino hacia la esquina superior izquierda, con el Dock y la
    /// franja derecha de la pantalla inalcanzables. La fracción no depende de
    /// ninguna de las tres resoluciones: el companion la multiplica por la
    /// suya y listo.
    ///
    /// `x`/`y` se siguen devolviendo tal cual (incluido el `+ originX/Y`) para
    /// no romper a nadie: el backend los ignora en cuanto llegan `nx`/`ny`.
    public static func mapear(
        puntoLocalX: Double, puntoLocalY: Double,
        anchoElemento: Double, altoElemento: Double,
        frame: RemoteFrame
    ) -> (x: Int, y: Int, nx: Double, ny: Double)? {
        ...
        let fraccionX = min(max(relX / contenido.width, 0), 1)
        let fraccionY = min(max(relY / contenido.height, 0), 1)
        let x = Int((fraccionX * Double(frame.width)).rounded())
        let y = Int((fraccionY * Double(frame.height)).rounded())
        return (
