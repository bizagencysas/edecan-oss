...
struct APISessionRaceTests {
    @Test func privacidadExportaYBorraMemoriaConBearer() async throws {
        let tokens = LockedAuthTokenStore(access: "access-privacy", refresh: "refresh-privacy")
        let calls = LockedCounter()
        let session = stubSession { request in
            #expect(request.value(forHTTPHeaderField: "Authorization") == "Bearer access-privacy")
            calls.increment()
            if request.url?.path == "/v1/privacy/export" {
                #expect(request.httpMethod == "GET")
                return (200, Data(#"{"format":"edecan-user-export.v1","memory":[]}"#.utf8))
            }
            #expect(request.url?.path == "/v1/memory")
        ...
        let export = try await api.exportarDatosPrivacidad()
        #expect(String(data: export, encoding: .utf8)?.contains("edecan-user-export.v1") == true)
        try await api.borrarMemoriaCompleta()
    ...
    @Test func sinJWTUsaPairingDurableYSiAmbosFallanExpiraPeroConservaElPairing() async throws {
        let tokens = LockedAuthTokenStore(access: nil, refresh: nil)
        ...
        // Una sesión inválida no debe convertir el siguiente arranque en un
        // escaneo QR obligatorio. El usuario puede reautenticarse con la URL
        // ya guardada; el borrado del pairing es una acción explícita.
        #expect(durable.snapshot() == .init(deviceId: "device-revocado", deviceToken: "durable-revocado"))
    ...
    @Test func ideIniciaTerminalYAgenteComoSesionesDirectas() async throws {
        ...
        let session = stubSession { request in
            ...
            case "/v1/ide/agents":
                #expect(json["workspace_id"] as? String == "workspace-1")
                #expect(json["provider"] as? String == "workers_ai")
