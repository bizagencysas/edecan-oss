    ...
    @Test func decodificaAutomatizacionDeAgendaActiva() throws {
        let json = """
        {
          ...
          "trigger": {"kind": "schedule", "rrule": "FREQ=DAILY;BYHOUR=9;BYMINUTE=0",
                      "descripcion": "Todos los días a las 9:00 UTC"},
        ...
        guard case .schedule(let rrule, let descripcion) = automatizacion.trigger else {
            Issue.record("se esperaba trigger .schedule")
            return
        }
        #expect(rrule == "FREQ=DAILY;BYHOUR=9;BYMINUTE=0")
        #expect(descripcion == "Todos los días a las 9:00 UTC")
        // Lo que ve la persona en la lista: la frase del servidor, no la rrule.
        #expect(automatizacion.trigger.resumen == "Todos los días a las 9:00 UTC")
        #expect(automatizacion.accion.instruccion == "Resume las ventas de hoy")
        #expect(automatizacion.accion.agente == nil)
    }

    /// El servidor manda `descripcion: null` cuando no puede traducir la regla
    /// con certeza ("el último viernes del mes", `UNTIL`, …). El teléfono NO
    /// improvisa una hora: cae a un rótulo honesto y deja la `rrule` cruda a
    /// mano para el detalle.
    @Test func agendaQueElServidorNoSupoTraducirNoSeInventaUnaHora() throws {
        let json = """
        {
          "id": "a4", "nombre": "Cierre de mes", "descripcion": "",
          "trigger": {"kind": "schedule", "rrule": "FREQ=MONTHLY;BYDAY=FR;BYSETPOS=-1",
                      "descripcion": null},
        ...
        #expect(descripcion == nil)
        #expect(rrule == "FREQ=MONTHLY;BYDAY=FR;BYSETPOS=-1")
        #expect(automatizacion.trigger.resumen == "Agenda personalizada")
    }

    /// Un backend anterior a `trigger.descripcion` no debe romper la app ni
    /// hacerle mostrar una hora inventada.
    @Test func agendaSinCampoDescripcionDecodificaIgual() throws {
        let trigger = try APIClient.crearDecoder().decode(
            AutomationTrigger.self,
            from: Data(#"{"kind": "schedule", "rrule": "FREQ=DAILY;BYHOUR=9"}"#.utf8)
        )
        guard case .schedule(_, let descripcion) = trigger else {
            Issue.record("se esperaba trigger .schedule")
            return
        }
        #expect(descripcion == nil)
        #expect(trigger.resumen == "Agenda personalizada")
    }

    @Test func resumenDeWebhookNoHablaDeAgenda() throws {
        let trigger = try APIClient.crearDecoder().decode(
            AutomationTrigger.self,
            from: Data(#"{"kind": "webhook", "has_secret": true, "hook_url": "https://x/y"}"#.utf8)
        )
        #expect(trigger.resumen == "Se dispara por webhook entrante")
    }
