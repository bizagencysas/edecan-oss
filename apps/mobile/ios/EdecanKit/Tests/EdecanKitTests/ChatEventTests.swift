...
struct ChatEventTests {
    ...
    @Test func followUpTurnReabreElTurnoTrasDone() throws {
        ...
        var terminal = SSETerminalState()
        let aceptoPrimero = terminal.aceptar(.textDelta(text: "primero"))
        let aceptoDonePrimero = terminal.aceptar(.done(usage: nil))
        #expect(aceptoPrimero)
        #expect(aceptoDonePrimero)
        #expect(terminal.finalizado)
        let aceptoFollowUp = terminal.aceptar(.followUpTurn(pending: 2))
        #expect(aceptoFollowUp)
        #expect(!terminal.finalizado)
        let aceptoSegundo = terminal.aceptar(.textDelta(text: "segundo"))
        let aceptoDoneSegundo = terminal.aceptar(.done(usage: nil))
        #expect(aceptoSegundo)
        #expect(aceptoDoneSegundo)
        #expect(terminal.finalizado)
        try terminal.validarCierre()
    }
    ...
    @Test func toolEndDecodificaBloquesV1YToolCallId() throws {
        ...
        #expect(hotel.address == "Calle Mayor 1")
        #expect(hotel.imageURL?.absoluteString == "https://img.example.com/hotel.jpg")
    ...
    @Test func toolEndDecodificaBorradorSocialConDestinoDeOrganizacionAbierto() throws {
        // "acme" es un id de destino de organización arbitrario configurado por el
        // tenant: `target` ya no es un enum cerrado a "personal"/"organization".
        let evento = try decodificar(
            #"{"type":"tool_end","name":"crear_contenido_social","result_preview":"Borrador listo","blocks":[{"type":"social_draft","status":"ready","platform":"linkedin","target":"acme","copy":"Un post real sobre crédito.","parts":["Un post real sobre crédito."],"alt_text":"Tarjeta con el logo D","offline_visual":false,"artifacts":[{"file_id":"018f7f4c-07f4-7ed0-93c8-cf0525d1092b","filename":"post.png","mime":"image/png"}],"requires_human_confirmation":true}]}"#
        )
        guard case .toolEnd(_, _, _, _, _, let blocks, _) = evento,
              case .socialDraft(let social) = blocks.first
        else {
            Issue.record("se esperaba .socialDraft")
            return
        }
        #expect(social.draft.target == SocialContentTarget(rawValue: "acme"))
        #expect(social.draft.target?.isPersonal == false)
        #expect(social.draft.platform == .linkedin)
        #expect(social.draft.copy == "Un post real sobre crédito.")
        #expect(social.draft.imageArtifact?.filename == "post.png")
        #expect(social.draft.requiresHumanConfirmation == true)
    }

    @Test func borradorSocialSinDestinoConocidoDecodificaTargetNulo() throws {
        let evento = try decodificar(
            #"{"type":"tool_end","name":"crear_contenido_social","result_preview":"Borrador listo","blocks":[{"type":"social_draft","status":"ready","platform":"x","copy":"Un post corto.","parts":["Un post corto."],"alt_text":"","offline_visual":true,"artifacts":[],"requires_human_confirmation":true}]}"#
        )
        guard case .toolEnd(_, _, _, _, _, let blocks, _) = evento,
              case .socialDraft(let social) = blocks.first
        else {
        ...
        #expect(social.draft.target == nil)
        #expect(social.draft.imageArtifact == nil)
    }

    @Test func borradorSocialConPlataformaDesconocidaCaeAUnsupported() throws {
        let evento = try decodificar(
            #"{"type":"tool_end","name":"crear_contenido_social","result_preview":"Borrador listo","blocks":[{"type":"social_draft","status":"ready","platform":"instagram","copy":"Texto","parts":[],"artifacts":[],"requires_human_confirmation":true}]}"#
        )
        guard case .toolEnd(_, _, _, _, _, let blocks, _) = evento,
              case .unsupported(let type, _) = blocks.first
        else {
            Issue.record("se esperaba bloque unsupported")
            return
        }
        #expect(type == "social_draft")
    }
    ...
    /// DESVIACIÓN del MVP de SDUI (paso 4): antes `send_message` era un alias
    /// legado de `prefill_message` -- nunca autoenviaba. `chat.py` (paso 1 del
    /// plan) ya define `SendMessageAction` como una acción PROPIA y distinta
    /// (mismo contrato que `QuestionOption.value`: manda el mensaje visible de
    /// una vez, turno normal). Este test reemplaza al viejo
    /// `prefillLegacyNuncaSeConvierteEnAutoenvio`, que ya no describe el
    /// contrato real: `prefill_message` sigue sin autoenviar nunca, pero
    /// `send_message` ahora sí, a propósito.
    @Test func prefillMessageNuncaAutoenviaYSendMessageSiEnviaAPropósito() throws {
        let prefill = try JSONDecoder().decode(
            ChatAction.self,
            from: Data(#"{"id":"one","label":"Usar","action":"prefill_message","message":"Reserva esto"}"#.utf8)
        )
        let envio = try JSONDecoder().decode(
            ChatAction.self,
            from: Data(#"{"id":"two","label":"Usar","action":"send_message","message":"Reserva esto"}"#.utf8)
        )
        #expect(prefill == .prefillMessage(id: "one", label: "Usar", message: "Reserva esto"))
        #expect(envio == .sendMessage(id: "two", label: "Usar", message: "Reserva esto"))
    }
