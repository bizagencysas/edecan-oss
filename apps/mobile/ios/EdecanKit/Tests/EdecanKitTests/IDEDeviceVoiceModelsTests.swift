...
struct IDEModelsTests {
    ...
    @Test func conversacionesIDEReconcilianSesionesSinDuplicarlas() throws {
        let suite = "IDEConversationsTests.\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defer { defaults.removePersistentDomain(forName: suite) }
        let store = IDEConversationLocalStore(defaults: defaults)
        ...
        let session = try APIClient.crearDecoder().decode(
            ...
            {
              "id":"agent-1","kind":"agent","workspace_id":"workspace-1",
              "workspace_name":"Edecán","status":"completed",
              "started_at":"2026-07-23T10:31:00Z",
              "ended_at":"2026-07-23T10:35:00Z","exit_code":0,
              "command":null,"provider":"codex",
              "title":"Corrige el inicio de sesión y valida sus pruebas"
            }
            """.utf8)
        )

        let first = store.reconcile(remoteSessions: [session])
        let second = store.reconcile(remoteSessions: [session])

        #expect(first.count == 1)
        #expect(second.count == 1)
        #expect(second.first?.sessionIds == ["agent-1"])
        #expect(second.first?.workspaceName == "Edecán")
    }

    @Test func conversaciónIDEAgrupaContinuacionesPersistentes() throws {
        let suite = "IDEConversationsAppendTests.\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defer { defaults.removePersistentDomain(forName: suite) }
        let store = IDEConversationLocalStore(defaults: defaults)

        func session(_ id: String, minute: Int) throws -> IDESession {
            try APIClient.crearDecoder().decode(
                IDESession.self,
                from: Data("""
                {
                  "id":"\(id)","kind":"agent","workspace_id":"workspace-1",
                  "workspace_name":"Mi app","status":"running",
                  "started_at":"2026-07-23T10:\(minute):00Z",
                  "ended_at":null,"exit_code":null,"command":null,
                  "provider":"codex","title":"Trabajo"
                }
                """.utf8)
            )
        }
        ...
        let first = store.append(
            session: try session("agent-1", minute: 31),
            prompt: "Audita el flujo de pagos y corrige errores"
        )
        let continued = store.append(
            session: try session("agent-2", minute: 36),
            prompt: "Ahora agrega las pruebas",
            to: first.id
        )

        #expect(continued.id == first.id)
        #expect(continued.sessionIds == ["agent-1", "agent-2"])
        #expect(store.load().count == 1)
    }

    @Test func títulosIDECompactanIntenciónYAPIKey() {
        #expect(
            IDEConversationReference.compactTitle(
                from: "Configura la API key de ElevenLabs para generar voz"
            ) == "Configurar API Key · ElevenLabs"
        )
        #expect(
            IDEConversationReference.compactTitle(
                from: "Revisa el repositorio completo y corrige el error del login sin romper las pruebas"
            ) == "Revisa el repositorio completo y corrige el error…"
        )
    }
}
