...
struct ModelsTests {
    ...
    @Test func conversationAceptaTituloNulo() throws {
        ...
        #expect(conversacion.isMain == false)
    }

    @Test func conversationDecodificaBanderaDeConversacionPrincipal() throws {
        let json = """
        {"id": "c-main", "title": "Actividad", "channel": "api", "is_main": true, "created_at": "2026-07-01T10:00:00Z"}
        """
        let conversacion = try APIClient.crearDecoder().decode(Conversation.self, from: Data(json.utf8))
        #expect(conversacion.isMain == true)
    }
