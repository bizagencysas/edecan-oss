...
### Share Extension y App Group

La extensión `EdecanShareExtension` comparte texto, URLs, imágenes y archivos
con la app mediante el grupo `group.cc.edecan.app`. Antes de firmar una build
física o de distribución, el Team elegido en Apple Developer debe tener creado
ese App Group y debe estar asociado a los App IDs principal y `.share`.

El build de Simulator no necesita ese registro; el instalador físico no debe
ejecutarse hasta que el provisioning profile incluya
`com.apple.security.application-groups = group.cc.edecan.app`.
