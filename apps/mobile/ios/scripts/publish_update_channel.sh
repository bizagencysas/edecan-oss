#!/usr/bin/env bash
# Publica únicamente ios-stable.json o ios-preview.json. Cada intento parte
# del HEAD remoto exacto; si otro productor gana la carrera, vuelve a leer el
# árbol y preserva todos sus manifiestos antes de reintentar.
set -u -o pipefail
...
CHANNEL_FILE="ios-$CHANNEL.json"
SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
GENERATOR="$SCRIPT_DIR/generate_ios_update_manifest.py"
...
spec = importlib.util.spec_from_file_location("ios_update_manifest", module_path)
if spec is None or spec.loader is None:
    raise SystemExit("cannot load iOS manifest validator")
...
publish_attempt() {
  ...
  if [[ -n "$base" ]]; then
    ...
    if git cat-file -e "$base:$CHANNEL_FILE" 2>/dev/null; then
      current_file=$(mktemp "${TMPDIR:-/tmp}/edecan-ios-current.XXXXXX") || return 1
  ...
  index=$(mktemp "${TMPDIR:-/tmp}/edecan-ios-update-index.XXXXXX") || return 1
  rm -f "$index"
  ...
  if [[ -n "$base" ]]; then
    commit=$(printf 'release: move iOS %s channel\n' "$CHANNEL" |
    ...
    commit=$(printf 'release: initialize iOS %s channel\n' "$CHANNEL" |
...
for ((attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1)); do
  ...
  if (( status == 2 )); then
    echo "Invalid or regressive iOS channel transition; channel left unchanged." >&2
    exit 1
  fi
