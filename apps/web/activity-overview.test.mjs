...
// --- AgentActivityCenter: aprobar el plan es real ---------------------------
//
// El dueño vio un plan de 6 pasos con "Aprobar (no conectado aún)" en gris
// mientras el backend lo estaba esperando. El endpoint
// `POST .../plan/{plan_id}/approve` ya existía; lo que faltaba era que este
// parser guardase el `id` del plan. La suite estaba verde: ningún test miraba
// si el botón hacía algo.

const activityCenter = readFileSync(
  new URL("./src/components/ide/AgentActivityCenter.tsx", import.meta.url),
  "utf8",
);

test("la tarjeta de plan conserva el id que manda el backend -- sin él no se puede aprobar", () => {
  assert.match(activityCenter, /interface PlanArtifact \{[\s\S]*?\bid: string;/);
  assert.match(activityCenter, /return \{ id: plan\.id, meta: plan\.goal, pasos \}/);
  // Un evento sin id se descarta en vez de pintar un botón que fallaría.
  assert.match(activityCenter, /typeof plan\.id !== "string" \|\| !plan\.id\) continue/);
});

test("Aprobar llama al endpoint real y ya no dice 'no conectado aún'", () => {
  assert.match(activityCenter, /approveIdeAgentPlan\(run\.sessionId, planId\)/);
  assert.doesNotMatch(activityCenter, /Aprobar \(no conectado aún\)/);
  assert.doesNotMatch(activityCenter, /no expone ningún endpoint para llamarlo/);
});

test("aprobar pide confirmación: reparte pasos entre subagentes que escriben archivos", () => {
  assert.match(activityCenter, /confirmandoAprobar/);
  assert.match(activityCenter, /¿Ejecutar el plan\?/);
});

test("si aprobar falla se muestra el error, no se traga en silencio", () => {
  assert.match(activityCenter, /\{errorAprobar && \(/);
  assert.match(activityCenter, /role="alert"/);
});
