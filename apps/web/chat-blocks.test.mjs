...
test("parsea gráfica de chat y descarta series incompletas", () => {
  const block = parseChatBlock({
    schema_version: 1,
    type: "chart",
    title: "Hoteles por noche",
    series: [
      { label: "Medellín", value: 120 },
      { label: "roto" },
  ...
  if (block?.type === "chart") {
    assert.equal(block.title, "Hoteles por noche");
    assert.equal(block.series.length, 2);
    assert.equal(block.series[0].label, "Medellín");
  }
  assert.equal(parseChatBlock({ schema_version: 1, type: "chart", title: "Vacío", series: [] }), null);
});

test("parsea bloque de fuentes con retrieved_at y descarta URLs internas", () => {
  const block = parseChatBlock({
    schema_version: 1,
    type: "sources",
    citations: [
      {
        id: "c1",
        title: "Hotel en Medellín",
        url: "https://example.com/hotel",
        source: "Booking",
        retrieved_at: "2026-08-22T18:00:00Z",
      },
      { id: "bad", title: "Local", url: "http://127.0.0.1/secret" },
    ],
  });
  assert.equal(block?.type, "sources");
  if (block?.type === "sources") {
    assert.equal(block.citations.length, 1);
    assert.equal(block.citations[0].title, "Hotel en Medellín");
    assert.equal(block.citations[0].retrieved_at, "2026-08-22T18:00:00Z");
  }
});
...
test("parsea card de hotel con foto y dirección", () => {
  const block = parseChatBlock({
    schema_version: 1,
    type: "hotel",
    offer_id: "H1",
    name: "Hotel Centro",
    city: "Medellín",
    address: "Carrera 70 # 24-21, Laureles",
    image_url: "https://img.trivago.com/hotel.jpg",
    price: "63",
    currency: "USD",
    source_mode: "live",
  });
  assert.equal(block?.type, "hotel");
  if (block?.type === "hotel") {
    assert.equal(block.address, "Carrera 70 # 24-21, Laureles");
    assert.equal(block.image_url, "https://img.trivago.com/hotel.jpg");
  }
  assert.equal(
    parseChatBlock({
      schema_version: 1,
