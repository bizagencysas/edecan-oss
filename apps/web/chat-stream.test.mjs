...
test("conserva la explicación pública del cierre sin inventarla", async () => {
  globalThis.window = {
    sessionStorage: new MemoryStorage(),
    localStorage: new MemoryStorage(),
    location: { pathname: "/app", assign() {} },
  ...
  globalThis.fetch = async () =>
    new Response(
      'data: {"type":"done","usage":{},"explanation":"Evidence: buscar_web\\nTools: buscar_web"}\n\n',
      { headers: { "Content-Type": "text/event-stream" } },
    );
  try {
    const events = [];
    await sendMessageStream("conversation-1", "¿Por qué?", (event) => events.push(event));
    assert.deepEqual(events, [
      {
        type: "done",
        usage: {},
        explanation: "Evidence: buscar_web\nTools: buscar_web",
      },
    ]);
  } finally {
    clearTokens();
