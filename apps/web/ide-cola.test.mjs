...
// El reparto del monolito (`page.tsx` -> cascarón + `estado-ide.ts` + los
// componentes de `estado/`, ver el encargo de partición) movió el compositor
// y el estado compartido fuera de `page.tsx`. Estas dos lecturas adicionales
// dejan que los checks de más abajo sigan verificando la MISMA propiedad de
// comportamiento, solo que en el archivo donde el código vive ahora.
const compositor = read("./src/components/ide/estado/Reposo.tsx");
...
test("el compositor ya no se bloquea mientras el agente trabaja", () => {
  ...
  assert.match(compositor, /disabled=\{!estado\.prompt\.trim\(\) \|\| estado\.commandBusy\}/);
  assert.doesNotMatch(pantalla, /disabled=\{!(estado\.)?prompt\.trim\(\) \|\| busy \|\| (estado\.)?commandBusy\}/);
  assert.doesNotMatch(pantalla, /if \(!text \|\| !workspace \|\| busy\) return;/);
});

test("detener y enviar conviven en el compositor", () => {
  assert.match(compositor, /aria-label="Detener trabajo"/);
  assert.match(compositor, /Mandar a la cola del agente/);
});
...
test("la pantalla sigue el ciclo completo del mensaje", () => {
  for (const token of [
    ...
    assert.match(pantalla, new RegExp(token));
  }
  ...
  assert.doesNotMatch(pantalla, /cancelIdeQueuedMessage/);
});
...
test("la torre de control mira todas las carpetas, no solo la abierta", () => {
  ...
  assert.match(estadoIde, /const rows = await getIdeAgents\(\);/);
...
test("desde la torre se puede saltar, detener y dirigir sin salir de ella", () => {
  ...
  assert.match(estadoIde, /conversation_id: conversationId,/);
});
