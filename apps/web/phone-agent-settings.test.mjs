...
test("Ajustes ofrece agentes de llamadas reutilizables con lenguaje humano", () => {
  ...
  assert.match(settings, /Exportar copia privada/);
  assert.match(settings, /Importar copia/);
  assert.match(settings, /No incluye Twilio/);
...
test("el cliente web usa el CRUD tenant-scoped de plantillas", () => {
  ...
  assert.match(api, /exportPhoneAgentTemplates/);
  assert.match(api, /importPhoneAgentTemplates/);
  assert.match(api, /preparePhoneCall/);
  assert.match(api, /"\/v1\/phone\/calls\/prepare"/);
  assert.match(api, /"\/v1\/phone\/agent-templates"/);
  assert.match(api, /"\/v1\/phone\/agent-templates\/export"/);
  assert.match(api, /"\/v1\/phone\/agent-templates\/import"/);
});
