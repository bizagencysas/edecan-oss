...
import { listDevices, type HostDevice } from "@/lib/api";
...
export default function RemotoPage() {
  ...
  useEffect(() => {
    if (hasRemoteViewFlag) {
      void loadHistory();
      void listDevices()
        .then((devices) => {
          const companions = devices.filter((device) => device.kind === "companion" || device.status === "active");
          setHosts(companions);
          setSelectedHostId((current) => current ?? companions[0]?.id ?? null);
        })
        .catch(() => undefined);
  ...
  useEffect(() => {
    ...
    if (!auto || !sessionId || session?.status === "ended" || session?.status === "denied") return;
    streamRef.current = openRemoteStream(
      sessionId,
      (next) => {
        setFrame(next);
        setRefreshing(false);
        setSession((prev) =>
          prev && prev.id === sessionId
            ? { ...prev, status: "active", frames_count: next.seq, started_at: prev.started_at ?? new Date().toISOString() }
            : prev,
        );
      },
      (streamError) => {
  ...
  }, [auto, session?.id, session?.status]);
          ...
          {hosts.length > 0 && (
            <Card>
              <CardBody className="space-y-2">
                <p className="text-sm font-medium">Hosts disponibles</p>
                <ul className="space-y-1">
                  {hosts.map((host) => (
                    <li key={host.id}>
                      ...
                      >
                        {host.nombre || host.plataforma} · {host.status}
                      </button>
                    </li>
                  ))}
                </ul>
