...
import { triggerResumen, type Automation, type AutomationRun } from "@/lib/api-automatizaciones";
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader
          ...
          description={triggerResumen(automation.trigger)}
          actions={
            <Button
              size="sm"
              variant="secondary"
        ...
        <CardBody className="space-y-4">
          {automation.trigger.kind === "schedule" && !automation.trigger.descripcion && (
            // El servidor no supo traducir esta regla (ver `triggerResumen`).
            // Antes que inventarle una hora, se enseña tal cual: quien la
            // escribió a mano puede verificarla, y nadie lee una hora falsa.
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Regla de recurrencia: <code>{automation.trigger.rrule}</code>
            </p>
          )}
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4 dark:border-slate-800">
            ...
            <div className="flex items-center gap-4 text-xs text-slate-500 dark:text-slate-400">
              {/* "(tu hora)" no sobra: la agenda de arriba va en hora del servidor (UTC)
                  y `formatDateTime` formatea en la del navegador. Sin el rótulo, dos horas
                  distintas para lo mismo parecen un error. Mismo texto que el detalle de
                  iOS (`AutomatizacionesView.swift`). */}
              {automation.next_run_at && (
                <span>Próxima: {formatDateTime(automation.next_run_at)} (tu hora)</span>
              )}
