...
/** Las etiquetas son, palabra por palabra, lo que `edecan_automations.describe`
 * devolverá después para esa misma `rrule`: se elige "Todos los días a las 9:00
 * UTC" y eso mismo es lo que se lee luego en la lista. El "UTC" no es un
 * tecnicismo de más — las agendas corren en la hora del SERVIDOR, y omitirlo
 * acá haría creer que son las 9:00 de quien está mirando. */
const PRESETS: Record<Exclude<PresetKey, "personalizada">, { label: string; rrule: string }> = {
  diario: { label: "Todos los días a las 9:00 UTC", rrule: "FREQ=DAILY;BYHOUR=9;BYMINUTE=0" },
  semanal: {
    label: "Todos los lunes a las 9:00 UTC",
    rrule: "FREQ=WEEKLY;BYDAY=MO;BYHOUR=9;BYMINUTE=0",
  },
  mensual: {
    label: "El día 1 de cada mes a las 9:00 UTC",
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {tipoDisparador === "schedule" && preset === "personalizada" && (
        <Field
          ...
          hint="RFC 5545 en hora UTC, ej. 'FREQ=WEEKLY;BYDAY=MO,WE,FR;BYHOUR=8;BYMINUTE=0'. Pon siempre BYMINUTE junto a BYHOUR: sin él el minuto se hereda de la hora en que la creas."
        >
          <Input
            id="automation-rrule"
            value={rruleManual}
