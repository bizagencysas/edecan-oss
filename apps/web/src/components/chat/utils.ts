...
/** Cualquier `[efecto]` de TTS. Conserva Markdown `[texto](url)` e `![alt](url)`. */
const SPEECH_TAG_RE =
  /[ \t]*(?<!!)\[(?![^\]]{0,120}\]\()[^\n\]]{1,120}\]/g;

/** Quita speech tags y efectos del texto visible / copiable. El audio las sigue usando. */
export function stripSpeechTags(text: string): string {
  return text.replace(SPEECH_TAG_RE, "").replace(/[ \t]{2,}/g, " ").replace(/^[ \t]+/, "");
}
