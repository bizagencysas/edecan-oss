...
const ENLACE_WORKERS_AI = "https://dash.cloudflare.com/?to=/:account/ai/workers-ai";

type Tab = "workers_ai" | "anthropic" | "openai_compat" | "vertex";
type Accion =
  | "workers_ai"
  | "claude_cli"
  | "codex_cli"
...
export function SelectorLLM({
  ...
  const [tab, setTab] = useState<Tab>("workers_ai");
        ...
        {tab === "workers_ai" && (
          <div className="space-y-3">
            <Field
              ...
              htmlFor="llm_workers_account_id"
              hint="Está en Workers AI → Use REST API."
            >
              <Input
                id="llm_workers_account_id"
                value={workersAccountId}
                onChange={(e) => setWorkersAccountId(e.target.value)}
            ...
            <CampoLlave
              id="llm_workers_token"
              label="Workers AI API token"
              value={workersToken}
              onChange={setWorkersToken}
              placeholder="Token con Workers AI Read/Edit"
              linkHref={ENLACE_WORKERS_AI}
              disabled={ocupado}
            />
            <Field label="Modelo" htmlFor="llm_workers_model" hint="ID completo del catálogo, comenzando por @cf/." >
              <Input
                id="llm_workers_model"
                value={workersModel}
                onChange={(e) => setWorkersModel(e.target.value)}
                placeholder="@cf/autor/modelo"
                autoComplete="off"
                disabled={ocupado}
              />
            </Field>
            <Feedback accion="workers_ai" />
            <Button
              size="sm"
              onClick={() =>
                void conectar("workers_ai", "workers_ai", {
                  api_key: workersToken,
                  model_principal: workersModel,
                  model_rapido: workersModel,
                  extra: { account_id: workersAccountId },
                })
              }
              loading={busyAccion === "workers_ai"}
              disabled={ocupado || !workersAccountId.trim() || !workersToken.trim() || !workersModel.trim()}
            >
              Conectar
            </Button>
          </div>
