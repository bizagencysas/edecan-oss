 ...
 *
 * `DiffPreview` (la propuesta de ⌘K) tokeniza con Shiki de solo lectura
 * (`lib/ide-syntax-highlight.ts`, §7 de la especificación) el "antes" y el
 * "después" completos del archivo -- la textarea de arriba, en cambio, sigue
 * sin ninguna librería nueva: el encargo de este paquete pide justo eso
 * ("en el editor no metas otra librería, CodeEditor ya resuelve la
 * edición"), y esa frase es sobre EDITAR, no sobre pintar un diff que ya no
 * se toca.
 */
...
import { HighlightedLineText, useIsDarkMode } from "@/components/ide/SyntaxHighlightedLine";
import { Button, Spinner } from "@/components/ui";
import { highlightLines, type ThemedToken } from "@/lib/ide-syntax-highlight";
...
type DiffPreviewRowKind = "add" | "del" | "hunk" | "ctx";

interface DiffPreviewRow {
  key: string;
  kind: DiffPreviewRowKind;
  /** Línea completa del diff, marcador (`+`/`-`/` `) incluido. */
  raw: string;
  /** 1-based en `before` (el archivo tal como está hoy); `null` si la fila no existe de ese lado. */
  beforeLine: number | null;
  /** 1-based en `after` (`proposal.newContent`); `null` si la fila no existe de ese lado. */
...
/**
 * Recorre el diff unificado llevando la cuenta de línea de cada lado, para
 * poder mirar el token de Shiki que le toca a cada fila (`tokensBefore`/
 * `tokensAfter` están indexados por número de línea real del archivo, no por
 * posición dentro del diff).
 */
function parseDiffRows(diff: string): DiffPreviewRow[] {
  const lines = diff
    .split("\n")
  ...
  lines.forEach((line, index) => {
    ...
    if (hunk) {
      ...
      rows.push({ key: `h${index}`, kind: "hunk", raw: line, beforeLine: null, afterLine: null });
      return;
    }
    if (line.startsWith("+")) {
      rows.push({ key: `a${index}`, kind: "add", raw: line, beforeLine: null, afterLine });
      afterLine += 1;
    } else if (line.startsWith("-")) {
      rows.push({ key: `d${index}`, kind: "del", raw: line, beforeLine, afterLine: null });
      beforeLine += 1;
    } else {
      rows.push({ key: `c${index}`, kind: "ctx", raw: line, beforeLine, afterLine });
      beforeLine += 1;
      afterLine += 1;
    }
  });
...
function tokensForRow(
  row: DiffPreviewRow,
  tokensBefore: ThemedToken[][] | null,
  tokensAfter: ThemedToken[][] | null,
): ThemedToken[] | null {
  if (row.kind === "del") return row.beforeLine ? (tokensBefore?.[row.beforeLine - 1] ?? null) : null;
  if (row.kind === "add") return row.afterLine ? (tokensAfter?.[row.afterLine - 1] ?? null) : null;
  if (row.kind === "ctx") {
    // Contexto: el mismo texto existe en los dos lados. Se prefiere el
    // "después" y se cae al "antes" solo si ese lado todavía no tokenizó.
    if (row.afterLine && tokensAfter?.[row.afterLine - 1]) return tokensAfter[row.afterLine - 1];
    if (row.beforeLine && tokensBefore?.[row.beforeLine - 1]) return tokensBefore[row.beforeLine - 1];
  }
  return null;
}

function DiffPreview({ diff, path, before, after }: { diff: string; path: string; before: string; after: string }) {
  const dark = useIsDarkMode();
  const rows = useMemo(() => parseDiffRows(diff), [diff]);
  const [tokensBefore, setTokensBefore] = useState<ThemedToken[][] | null>(null);
  const [tokensAfter, setTokensAfter] = useState<ThemedToken[][] | null>(null);

  useEffect(() => {
    // Se resetea al cambiar de propuesta: sin esto, mientras carga Shiki para
    // la nueva propuesta se verían -por un instante- los tokens de la
    // anterior, que ya no corresponden a estas líneas.
    setTokensBefore(null);
    setTokensAfter(null);
    let cancelled = false;
    void highlightLines(before, path, dark).then((tokens) => {
      if (!cancelled) setTokensBefore(tokens);
    });
    void highlightLines(after, path, dark).then((tokens) => {
      if (!cancelled) setTokensAfter(tokens);
    });
    return () => {
      cancelled = true;
    };
  }, [before, after, path, dark]);

  return (
    <div className="max-h-56 overflow-auto border-y border-slate-100 bg-slate-50/60 dark:border-slate-800 dark:bg-slate-950/40">
      {rows.map((row) => {
        const isCode = row.kind !== "hunk";
        return (
          <div
            key={row.key}
            className={[
              ...
              row.kind === "del" && "bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300",
              row.kind === "hunk" && "text-slate-400 dark:text-slate-500",
              row.kind === "ctx" && "text-slate-500 dark:text-slate-400",
            ]
              .filter(Boolean)
              .join(" ")}
          >
            {isCode ? (
              <>
                <span aria-hidden="true">{row.raw.slice(0, 1)}</span>
                <HighlightedLineText
                  tokens={tokensForRow(row, tokensBefore, tokensAfter)}
                  text={row.raw.slice(1) || " "}
                />
              </>
            ) : (
              row.raw.length ? row.raw : " "
            )}
          </div>
        );
      })}
            ...
            {phase === "proposal" && proposal && (
              <>
                {proposal.addedLines === 0 && proposal.removedLines === 0 ? (
                  ...
                  <DiffPreview diff={proposal.diff} path={path} before={panelContentRef.current} after={proposal.newContent} />
                )}
                <div className="flex items-center gap-2 px-3 py-2">
