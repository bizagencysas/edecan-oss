 ...
 * por carpeta (`GET /v1/ide/workspaces/{id}/tree?path=...&max_depth=1` por
 * cada expansión, ver `WP-V2-08` en `ROADMAP_V2.md` §7.8) — nunca trae el
 * sandbox completo de una sola vez.
 *
 * Actualizado al contrato de workspaces múltiples: la ruta original
 * (`GET /v1/ide/tree`, sin `workspaceId`) asumía un único sandbox global y ya
 * no es la que usa el resto del IDE (`estado-ide.ts` lee y escribe archivos
 * con `getIdeWorkspaceTree`/`getIdeWorkspaceFile`, ambas con `workspaceId`
 * como primer argumento) — este componente llevaba desconectado desde antes
 * de ese cambio, así que seguía llamando a la ruta vieja.
 */
...
import { getIdeWorkspaceTree, type IdeTreeNode } from "@/lib/api-ide";
...
export function FileTree({
  ...
  useEffect(() => {
    // Cambiar de workspace invalida todo lo cargado: sin esto, un árbol ya
    // abierto en el workspace anterior seguiría en pantalla (mismas rutas,
    // contenido de otra carpeta) hasta que alguien expandiera algo.
    setRoot(null);
    setChildrenByPath({});
    setExpanded({});
    void loadRoot();
  ...
  async function loadRoot() {
    setError(null);
    try {
      const tree = await getIdeWorkspaceTree(workspaceId, undefined, 1);
  ...
  async function toggleDir(path: string) {
    ...
    setLoadingPath(path);
    try {
      const tree = await getIdeWorkspaceTree(workspaceId, path, 1);
  ...
  if (root.length === 0) {
    return <p className="text-xs text-slate-400">La carpeta del proyecto está vacía.</p>;
  }
