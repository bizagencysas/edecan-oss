...
/**
 * Sin variantes `dark:` a propósito: estas fichas viven DENTRO del compositor,
 * y el compositor cuelga de una pantalla que se pinta clara a la fuerza
 * (`bg-white text-slate-950` en el contenedor de Forge Studio) aunque la app
 * esté en tema oscuro. Una variante oscura acá no se ve oscura: se ve como una
 * mancha marrón sobre blanco.
 */
const ESTILO_POR_ESTADO: Record<IdeEstadoDeMensaje, string> = {
  enviando: "border-slate-200 bg-slate-50 text-slate-500",
  encolado: "border-amber-200 bg-amber-50 text-amber-800",
...
export function MessageQueue({ mensajes, onRecuperarTexto, className }: MessageQueueProps) {
  ...
  return (
    <div
      aria-live="polite"
      className={cx("mb-2 space-y-1.5 border-t border-slate-200/70 px-1 pt-2", className)}
    >
        ...
        >
          ...
          {mensaje.estado === "fallido" && (
            <button
              ...
              className="shrink-0 rounded px-1.5 py-0.5 font-semibold underline underline-offset-2 hover:bg-rose-100"
            >
              Recuperar texto
            </button>
          )}
