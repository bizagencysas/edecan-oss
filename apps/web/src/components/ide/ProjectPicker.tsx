    ...
    <div className={`text-sm ${className}`}>
      ...
      {mode === "closed" && (
        <button
          ...
          disabled={disabled}
          onClick={openList}
          className="mt-2 flex w-full items-center justify-between gap-3 rounded-lg py-2 text-left transition hover:bg-[#f7f7f6] disabled:cursor-not-allowed disabled:opacity-50"
        >
          <span className="flex min-w-0 items-center gap-3">
            ...
            <span className="min-w-0">
              {value ? (
                <>
                  <span className="block truncate font-semibold text-slate-900">{value.name}</span>
                  <span className="block truncate text-xs text-slate-400">{value.path}</span>
                </>
              ) : (
                <span className="text-slate-400">Elige una carpeta del Mac…</span>
              )}
            </span>
          </span>
          <span className="shrink-0 text-xs font-semibold text-slate-400">Cambiar</span>
        </button>
      )}
        ...
        <div className="mt-2 space-y-0.5">
          ...
          {sorted.map((row) => (
            <button
              ...
              className="flex w-full items-center gap-3 rounded-lg px-1 py-2 text-left transition hover:bg-[#f7f7f6]"
            >
              <FileIcon className="h-4 w-4 shrink-0 text-slate-400" />
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate text-xs text-slate-400">{row.path}</span>
              </span>
              {row.id === value?.id && (
                <CheckIcon className="h-4 w-4 shrink-0 text-slate-900" />
              )}
            </button>
          ))}
          ...
          <button
            type="button"
            onClick={() => setMode("new")}
            className="flex w-full items-center gap-3 rounded-lg px-1 py-2 text-left text-slate-500 transition hover:bg-[#f7f7f6]"
          >
          ...
          <button
            ...
            className="mt-1 px-1 text-xs font-semibold text-slate-400 hover:text-slate-600"
          >
            Cancelar
          </button>
        </div>
        ...
        <div className="mt-2 space-y-3">
          <label className="block">
            ...
            <input
              ...
              className="mt-1 w-full rounded-lg border border-[#d9d9d7] bg-white px-3 py-2.5 text-sm outline-none focus:border-slate-500"
            />
          </label>
          ...
          <button
            ...
            className="w-full rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white transition disabled:opacity-40"
          >
            Abrir Finder
          </button>
          ...
          {showManualPath && (
            <div className="flex gap-2">
              <input
                ...
                className="min-w-0 flex-1 rounded-lg border border-[#d9d9d7] bg-white px-3 py-2.5 text-sm outline-none focus:border-slate-500"
              />
              <button
                type="button"
                disabled={!manualPath.trim() || busy}
                onClick={() => void authorize(manualPath.trim(), newName.trim() || undefined)}
                className="shrink-0 rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40"
              >
                Autorizar
              </button>
            </div>
