...
import type { IdeSessionStatus } from "@/lib/api-ide";
...
export function ProjectSidebar({
  ...
  function submitNewProject(event: FormEvent) {
    ...
    if (!name || !newProjectWorkspaceId) return;
    onCreateProject({ name, workspace_id: newProjectWorkspaceId });
  ...
  return (
    <aside
      className={cx(
        "flex h-full min-h-0 w-64 shrink-0 flex-col overflow-hidden border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900",
        className,
      )}
    >
          ...
          >
            {workspaces.length === 0 ? (
              <p className="text-xs text-slate-400">
                Primero autoriza una carpeta: usa el botón + junto al campo de mensaje y elige «Abrir carpeta del Mac».
              </p>
            ) : (
              <>
                <Input
                ...
                <Select
                  ...
                  onChange={(event) => setNewProjectWorkspaceId(event.target.value)}
                  className="!py-1.5 text-xs"
                >
                  <option value="">Elige una carpeta…</option>
                  {workspaces.map((workspace) => (
                    <option key={workspace.id} value={workspace.id} disabled={workspace.available === false}>
                      {workspace.name}
                      {workspace.available === false ? " (no disponible)" : ""}
                    </option>
                  ))}
                </Select>
              </>
            ...
            <div className="flex justify-end gap-1.5 pt-0.5">
              ...
              <Button
                ...
                size="sm"
                className="!px-2 !py-1 text-xs"
                disabled={workspaces.length === 0 || !newProjectName.trim() || !newProjectWorkspaceId}
              >
                Crear
              </Button>
            </div>
