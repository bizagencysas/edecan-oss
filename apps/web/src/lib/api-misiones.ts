 ...
 *
 * El refresh se coordina globalmente con `session-refresh`: el backend
 * consume cada refresh token una sola vez, así que los vertical slices no
 * pueden rotarlo en paralelo.
 */
...
export interface MissionReproduction {
  mission_id: string;
  objetivo: string;
  workflow_version: string | null;
...
export async function listMissions(options: { includeArchived?: boolean } = {}): Promise<Mission[]> {
  const query = options.includeArchived ? "?include_archived=true" : "";
  return apiJson<Mission[]>(`/v1/missions${query}`);
}

export async function getMissionsInbox(): Promise<MissionInbox> {
  return apiJson<MissionInbox>("/v1/missions/inbox");
}
...
export async function getMissionReproduction(id: string): Promise<MissionReproduction> {
  return apiJson<MissionReproduction>(`/v1/missions/${id}/reproduction`);
}
...
export async function pauseMission(id: string): Promise<Mission> {
  return apiJson<Mission>(`/v1/missions/${id}/pause`, { method: "POST" });
}

export async function resumeMission(id: string): Promise<Mission> {
  return apiJson<Mission>(`/v1/missions/${id}/resume`, { method: "POST" });
}

export async function steerMission(id: string, instruction: string): Promise<Mission> {
  return apiJson<Mission>(`/v1/missions/${id}/steer`, {
    method: "POST",
    body: { instruction },
  });
}
...
export async function archiveMission(id: string): Promise<Mission> {
  return apiJson<Mission>(`/v1/missions/${id}/archive`, { method: "POST" });
}
