...
const DESKTOP_MINI_KEY = "edecan_desktop_mini";
...
function captureDesktopLaunch(): void {
  ...
  if (hashParams.get("edecan_mini") === "1") {
    window.sessionStorage.setItem(DESKTOP_MINI_KEY, "1");
  }
  const capability = hashParams.get("edecan_capability");
  ...
  // Conserva `#edecan_mini=1` para que un refresh siga en el compositor compacto.
  const miniHash = window.sessionStorage.getItem(DESKTOP_MINI_KEY) === "1" ? "#edecan_mini=1" : "";
  window.history?.replaceState?.(
    null,
    "",
    `${window.location.pathname || "/"}${window.location.search || ""}${miniHash}`,
  );
}

/** Ventana nativa flotante (`#edecan_mini=1`): compositor compacto, sin chrome. */
export function isDesktopMiniWindow(): boolean {
  if (typeof window === "undefined") return false;
  const hash = window.location?.hash ?? "";
  if (hash.includes("edecan_mini=1")) {
    if (hasSessionStorage()) window.sessionStorage.setItem(DESKTOP_MINI_KEY, "1");
    return true;
  }
  if (!hasSessionStorage()) return false;
  captureDesktopLaunch();
  return window.sessionStorage.getItem(DESKTOP_MINI_KEY) === "1";
}
