...
1. Paquetes hermanos que reciben la configuración por duck-typing (p. ej.
   `edecan_llm.router.LLMRouter`, que espera la configuración de Workers AI)
   funcionan sin adaptar nombres sin importar qué app los construya.
...
class Settings(BaseSettings):
    ...
    # Historia operacional compartida con la API; solo se activa en prod y
    # requiere la migración `0046_provider_health_events`.
    ...
    # Modelo del alias "profundo": el ESCRITOR de posts (LinkedIn) pide un modelo
    # FUERTE, separado del chat. Sin declarar este campo, `extra="ignore"` botaba
    # el override y el escritor caía al chat model (llama-4-scout, 17B) -> texto
    # corto y titular sin gancho. GLM 5.2 es de los más capaces del catálogo; se
    # descartó para CHAT por latencia (~42s/vuelta) pero un post en background la
    # tolera de sobra. Editable en runtime vía `platform-config.json`.
    WORKERS_AI_MODEL_PROFUNDO: str | None = "@cf/zai-org/glm-5.2"
    # GLM 5.2 es lento: el default de 60s dejaba al escritor sin terminar (timeout
    ...
    # la metáfora conceptual con logos reales (como REFERENCIA `_generar_imagen_libre`).
    # Sin declarar estos campos, `extra="ignore"` botaba el override del
    # `platform-config.json` y el job del worker no tenía cómo llamar a OpenAI.
    IMAGES_API_KEY: str | None = None
    IMAGES_BASE_URL: str = "https://api.openai.com/v1"
    IMAGES_MODEL: str = "gpt-image-2"

    # --- fydesign: selector de pantallas + visuales para posts product-led -----
    # Preferencia: `FYDESIGN_DIR` (ruta al repo de fydesign) → el handler lanza
    # fydesign como subprocess de un solo uso (pick/visual/video/crítico), SIN
    # servidor. Cero calor en idle. Si solo `FYDESIGN_URL` está seteada, cae al
    # HTTP clásico (servidor corriendo en :3000).
    FYDESIGN_DIR: str | None = None
    FYDESIGN_URL: str | None = None
    EDECAN_API_KEY: str | None = None
