...
auditor de hechos + gates), genera la imagen con Flux.1 schnell y le monta el
titular con `imagen_editorial.componer_titular` / `componer_titular_acme`
(logo + gradiente Aurora para páginas), y entrega el post como CARD + PUSH en la
conversación PRINCIPAL (is_main), igual que el hilo de avisos de Aria.

Diseño de la imagen (por qué NO se usa `con_imagen=True` del tool): el tool, con
imagen, mete la foto por `generar_con_critica` (genera → juzga con visión →
regenera). Ese lazo de crítica falla de forma consistente en el worker y degrada
a la tarjeta editorial offline (sin foto). Aria tampoco usa crítico: hace Flux
DIRECTO + compositor. Así que este handler pide al tool solo el copy
(`con_imagen=False`) — que ahora SÍ expone `data["visual"]` (kicker/headline/
accent/support ya reparados) — y hace la foto él mismo: Flux directo + el
compositor tipográfico. Robusto y fiel a Aria.
...
Contrato del payload (todo opcional salvo lo marcado):
  ...
  destino:         str | None  -> "personal" | "<org>" (p.ej. "acme") | None
...
# gpt-image-2 (portado de Aria): quality "medium" -- ni low ni high (Ada:
...
# `contentMode: .fill` la recorta arriba y abajo, y ahí vive el titular. Ada,
...
async def handle(env, deps: Deps) -> None:  # noqa: ANN001 (JobEnvelope; ver handlers/__init__)
    ...
    try:
        ...
        async with deps.session_factory(None) as session:
            ...
            # Imagen: Flux directo + compositor (con el `visual` YA poblado). Se hace
            ...
            if con_imagen and copy_text and len(copy_text) >= piso_de_entrega:
                try:
                    img_png = await _generar_imagen(settings, copy_text, visual, destino)
                except Exception:
                    logger.warning(
                        "create_linkedin_post: falló la imagen gpt-image-2; entrego solo texto.",
                        exc_info=True,
                    )
                    img_png = None
                if img_png:
    ...
    try:
        async with deps.session_factory(None) as session:
            ...
            await repo.add_message(
                ...
                role="assistant",
                content={"text": _intro_del_post(destino, aviso), "presentation": [card_json]},
...
def _intro_del_post(destino, aviso: str) -> str:  # noqa: ANN001 (str | None; destino crudo)
...
async def _entregar_sin_card(
    ...
    *,
    ...
    """
    encabezado = (
        "Te escribí el post, pero se me trabó la tarjeta. Te lo dejo tal cual, para que lo "
        "copies:"
    )
    cuerpo = f"{encabezado}\n\n{copy_text}"
    if aviso:
...
async def _avisar_que_no_hubo_post(
    ...
    *,
    ...
    por algo que nunca pidió, y a la tercera semana el chat es un basurero de disculpas. Para
    esos turnos queda el push y el log, que es lo que había antes y lo correcto acá.
    ...
    if _lo_pidio_una_persona(payload):
        ...
        logger.info(
            ...
            "escribe en el chat: nadie lo pidió; queda el aviso.",
            destino,
            fallo or "sin código",
        )
...
async def _generar_imagen(settings, copy_text: str, visual: dict, destino) -> bytes | None:
    """gpt-image-2 como DIRECTOR DE ARTE (portado de Aria `_generar_imagen_libre`,
    ...
    `componer_titular_acme` (logo D + gradiente Aurora) si el destino es la
    ...
    también -- si no, vuelve el recorte que reportó Ada ("se ve encogida
    en la app")."""
    ...
    from edecan_creative.imagen_editorial import componer_titular, componer_titular_acme

    componer = componer_titular_acme if destino == "acme" else componer_titular
    return componer(
        base,
        ...
        # antes -- `componer_titular`/`componer_titular_acme` se ponen de
...
def _prompt_arte(texto: str, visual: dict) -> str:
    """Prompt de DIRECCIÓN DE ARTE portado de Aria (`features/linkedin_content.py
    ...
    idea = (
        (
            f"La idea concreta que la imagen debe comunicar: {claim}. PRUEBA: si esta misma "
...
def _armar_card(
    ...
    *,
    ...
    # (p. ej. "acme"): botón privilegiado "Aprobar y publicar".
    es_personal = destino in (None, "personal")
    ...
    return construir_card_generica(
        ...
        # encogida" que reportó Ada). Ver `_IMAGE_LADO`/`_COMPOSE_LADO` más
