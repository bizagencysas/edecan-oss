...
el proveedor genérico configurado por la plataforma. Si el
...
async def handle(env: JobEnvelope, deps: Deps) -> None:
    ...
    async with deps.session_factory(None) as session:
        ...
        if mime_imagen is not None:
            # Se resuelve perezosamente porque solo las imágenes necesitan
            # visión; PDF/texto/docs no deben inicializar inferencia.
