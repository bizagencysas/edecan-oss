...
Lo que SÍ cambia (§3C del plan REFERENCIA 2.0) es a dónde lleva el toque. El webhook
...
async def handle(env: JobEnvelope, deps: Deps) -> None:
    ...
    async with deps.session_factory(None) as session:
        ...
        if call is None or call.get("summary") is None:
            ...
            return
        if not await repo.claim_phone_call_summary_push(tenant_id=env.tenant_id, call_id=call_id):
            return
