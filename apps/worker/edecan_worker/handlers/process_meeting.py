...
6. Router LLM administrado (`deps.llm_router_for`, **PEREZOSO** — se
   resuelve recién AQUÍ, después de tener transcript): construye el prompt de
   minutas (`edecan_meetings.minutas.construir_prompt_minutas`), llama al
   proveedor administrado y parsea la respuesta (`parsear_minutas`, tolerante
   a fences). No consulta credenciales ni preferencias del tenant.
...
async def handle(env: JobEnvelope, deps: Deps) -> None:
    ...
    try:
        ...
        # Inferencia administrada, resuelta recién cuando ya hay transcript.
