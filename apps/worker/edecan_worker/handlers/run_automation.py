...
Paridad Aria (30-jul-2026, ver `edecan_schemas.automations`, "Segunda
...
async def handle(env: JobEnvelope, deps: Deps) -> None:
    ...
    if automation["accion"].get("kind") == "create_linkedin_post":
        await _delegate_create_linkedin_post(
            deps, tenant_id=tenant_id, automation_id=automation_id, user_id=user_id,
    ...
    async with deps.session_factory(None) as session:
        ...
        run_deps = RunnerDeps(
            ...
            save_run=_save_run_y_recordar(
                _make_save_run(deps.session_factory, tenant_id, automation_id, run_id),
                estado_terminal,
            ),
        )
...
async def _delegate_create_linkedin_post(
    ...
    *,
    ...
    job_id = await enqueue(deps.settings, "create_linkedin_post", payload, tenant_id)
    await save_run("done", {"delegado_a": "create_linkedin_post", "job_id": str(job_id)})
    logger.info(
        "run_automation: delegado a create_linkedin_post automation_id=%s job_id=%s",
        automation_id,
        job_id,
    )
