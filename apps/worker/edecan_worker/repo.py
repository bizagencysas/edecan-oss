...
class SqlRepo:
    ...
    async def create_file(
        ...
        *,
        ...
        # Espeja `apps/api/edecan_api/repo.py::create_file`. El job sube la
        # imagen al objectstore y registra su fila aquí para que la card la
        # pueda referenciar por `file_id` (GET /v1/files/{id}/content).
        now = utcnow()
        return await self._first(
            """
            INSERT INTO files (
    ...
    async def get_main_conversation(
        ...
        return await self._first(
            "SELECT * FROM conversations "
            "WHERE tenant_id = :tenant_id AND user_id = :user_id AND is_main LIMIT 1",
    ...
    async def resolve_main_conversation(
        ...
        # llamada), igual que el hilo de avisos de Aria. INSERT con ON CONFLICT
        ...
        row = await self._first(
            ...
            VALUES (:id, :tenant_id, :user_id, 'Actividad', 'auto', 'api', TRUE, :now, :now)
            ON CONFLICT (tenant_id, user_id) WHERE is_main
            DO NOTHING
            RETURNING *
            """,
            {"id": new_uuid(), "tenant_id": tenant_id, "user_id": user_id, "now": utcnow()},
        )
        if row is not None:
            return row
