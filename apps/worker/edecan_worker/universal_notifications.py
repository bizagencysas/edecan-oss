...
async def notify_important_event(
    ...
    try:
        # `event.push_data()` es lo único que le dice a la app a dónde ir al
        # tocar el push: cuando el evento trae `chat_id` (p. ej.
        # "content_created"), incluye también `deeplink` — frente 3, ver
        # `NotificationDestino.parse` en EdecanKit. Si el evento no trae
