...
async def test_run_automation_arma_runner_deps_con_ctx_correcto() -> None:
    ...
    assert run_deps.persona.nombre_asistente == "Referencia"
...
# directa, paridad REFERENCIA: ver docstring del módulo, "## Delegación directa a
...
async def test_run_automation_create_linkedin_post_encola_directo_sin_correr_agente(
    ...
    assert job_type == "create_organization_linkedin_post"
    ...
    [run_row] = session.automation_runs.values()
    assert run_row["status"] == "done"
    assert run_row["detalle"]["delegado_a"] == "create_organization_linkedin_post"
