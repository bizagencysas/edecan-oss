...
# Inferencia ADMINISTRADA (decisión de producto, ver
# `apps/worker/tests/test_llm_por_tenant.py` y el 409 de
# `apps/api/edecan_api/routers/credentials.py::put_llm_credentials`): Edecán
# resuelve la inferencia con su propio proveedor y NO existe selección de
# proveedor por tenant. `Deps.llm_router_for` devuelve SIEMPRE el router
# global y jamás abre la sesión ni el vault del tenant.
#
# Estos tests venían de un trabajo en curso que asumía lo contrario
# (bring-your-own, WP-V3-02) y afirmaban que `ctx.llm is not deps.llm_router`.
# Se corrigen para afirmar lo que el sistema hace de verdad: es el MISMO
# router. `_FakeLLMVault` se conserva porque el handler sigue construyendo un
# vault para OTRAS credenciales (conectores, MCP), no para elegir modelo.
...
def _sin_configurar_compute_next_run(
    rrule: str, after: datetime, anchor: datetime | None = None, timezone: str | None = None
) -> None:
...
async def test_run_automation_arma_runner_deps_con_ctx_correcto() -> None:
    ...
    session.seed_persona(user_id, nombre_asistente="Aria")
    deps = make_deps(session_factory=_session_factory(session), vault=lambda s: _FakeLLMVault())
    ...
    # Inferencia administrada: `ctx.llm` ES el router global. No hay
    # resolución por tenant que pueda devolver otra cosa.
    assert run_deps.ctx.llm is not None
    assert run_deps.ctx.llm is deps.llm_router
    assert run_deps.ctx.extras["approved_tool_calls"] == set()
    assert run_deps.ctx.extras["flags"]["automations.rules"] is True

    assert run_deps.llm_router is run_deps.ctx.llm  # el mismo router en ambos lugares
    assert run_deps.registry is REGISTRY_SENTINEL
    assert run_deps.flags["automations.rules"] is True
    assert run_deps.persona.nombre_asistente == "Aria"
...
# ---------------------------------------------------------------------------
# run_automation.handle — aviso universal al terminar (ver docstring del
# módulo, "## Aviso universal al terminar"; frente 2 de push-para-todo).
# Se monkeypatchea `notify_important_event` en vez de darle a `FakeSession`
# soporte para el SQL de `audit_log`: mismo patrón que
# `test_notify_incoming_phone_call.py::forbidden_notify` para verificar
# wiring, sin duplicar la suite exhaustiva propia de
# `test_universal_notifications.py`/`test_notify_important_event.py`.
# ---------------------------------------------------------------------------


def _install_fake_notify(monkeypatch: pytest.MonkeyPatch) -> list[dict[str, Any]]:
    llamadas: list[dict[str, Any]] = []

    async def _fake_notify(deps: Any, event: Any) -> None:
        llamadas.append(
            {
    ...
    monkeypatch.setattr(run_automation_module, "notify_important_event", _fake_notify)
    return llamadas


async def test_run_automation_done_avisa_automation_completed(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert llamadas[0]["kind"] == "automation_completed"
    # `event_id` es el `run_id` (único por corrida), NO `automation_id` (se
    # repite en cada corrida programada) — ver docstring del módulo.
    assert str(llamadas[0]["event_id"]) == run_row["id"]
    assert llamadas[0]["resource_id"] == automation_id


async def test_run_automation_error_avisa_automation_failed(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    llamadas = _install_fake_notify(monkeypatch)
    session = FakeSession()
    ...
    async def _side_effect(automation: dict[str, Any], deps: FakeRunnerDeps) -> None:
        await deps.save_run("error", {"error": "el modelo se equivocó"})
    ...
    assert len(llamadas) == 1
    assert llamadas[0]["kind"] == "automation_failed"
    assert llamadas[0]["resource_id"] == automation_id


async def test_run_automation_waiting_confirmation_no_avisa(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """`waiting_confirmation` no tiene tipo de evento propio (ver docstring
    del módulo): en la práctica un run headless nunca llega ahí de verdad
    (sin tools `dangerous` registradas), pero si algún día lo hiciera, este
    test deja constancia explícita de que NO debe avisar hasta que exista un
    tipo de evento pensado para ese caso."""
    llamadas = _install_fake_notify(monkeypatch)
    session = FakeSession()
    automation_id = uuid.uuid4()
    tenant_id = uuid.uuid4()
...
# ---------------------------------------------------------------------------
# run_automation.handle — accion.kind="create_linkedin_post" (delegación
# directa, paridad Aria: ver docstring del módulo, "## Delegación directa a
# create_linkedin_post"). A diferencia de `agent_instruction`, este camino
# NUNCA invoca `_FakeRunner`/el turno de agente -- solo encola el job.
# ---------------------------------------------------------------------------


async def test_run_automation_create_linkedin_post_encola_directo_sin_correr_agente(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session = FakeSession()
    automation_id = uuid.uuid4()
    ...
    # Nunca corre el turno de agente headless -- cero LLM en el disparo.
    assert _FakeRunner.calls == []

    assert len(encolados) == 1
    job_type, payload, tid = encolados[0]
    assert job_type == "create_linkedin_post"
    ...
    assert payload == {
        ...
        "tema": None,
        "con_imagen": True,
        # Marca de "esto no lo pidió nadie, lo disparó el reloj". `create_linkedin_post` la
        # usa para no escribirle disculpas en el chat cuando un slot se salta por falta de
        # fuente fresca -- que es comportamiento normal del motor, no un fallo que reportar
        # tres veces al día.
    ...
    [run_row] = session.automation_runs.values()
    assert run_row["status"] == "done"
    assert run_row["detalle"]["delegado_a"] == "create_linkedin_post"


async def test_run_automation_create_linkedin_post_no_avisa_automation_completed(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    llamadas = _install_fake_notify(monkeypatch)
    session = FakeSession()
    ...
    session.seed_automation(
        automation_id,
        tenant_id,
        accion=json.dumps({"kind": "create_linkedin_post", "destino": "personal"}),
    )
    ...
    # El aviso de ESTA automatización lo manda create_linkedin_post.handle()
    # cuando el post existe de verdad (content_created) -- no este handler,
    # que solo delegó el encolado (ver docstring del módulo).
    assert llamadas == []


async def test_run_automation_create_linkedin_post_desactivada_se_ignora() -> None:
    # Mismo guard que agent_instruction -- reafirma que la rama nueva pasa
    # por los mismos 3 guardas antes de mirar accion.kind.
    session = FakeSession()
    automation_id = uuid.uuid4()
    tenant_id = uuid.uuid4()
    session.seed_automation(
        ...
        tenant_id,
        enabled=False,
        accion=json.dumps({"kind": "create_linkedin_post"}),
    )
    deps = make_deps(session_factory=_session_factory(session), vault=lambda s: _FakeLLMVault())
...
async def test_scan_encola_vencida_y_reprograma_next_run_at(
    ...
    _install_fake_compute_next_run(
        monkeypatch, lambda rrule, after, anchor=None, timezone=None: proxima
    )
...
async def test_scan_pasa_next_run_at_persistido_como_anchor_no_el_now_volatil(
    ...
    def _fake_compute_next_run(
        ...
        llamadas.append({"rrule": rrule, "after": after, "anchor": anchor, "timezone": timezone})
...
async def test_scan_rrule_agotada_deja_next_run_at_en_null(
    ...
    _install_fake_compute_next_run(
        monkeypatch, lambda rrule, after, anchor=None, timezone=None: None
    )  # ya no hay más
...
async def test_scan_multiples_tenants_encola_cada_uno(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_fake_compute_next_run(
        monkeypatch,
        lambda rrule, after, anchor=None, timezone=None: datetime.now(UTC) + timedelta(days=1),
    )
