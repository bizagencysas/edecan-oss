...
async def test_un_copy_de_la_franja_no_deja_al_usuario_en_silencio(
    ...
    await handler.handle(_env(entorno, tema="Acme de Latinoamérica"), entorno.deps)
    ...
    assert "Acme de Latinoamérica" in mensajes[0]
...
async def test_un_turno_del_autopiloto_que_falla_no_le_escribe_al_chat(
    ...
    motor -- le llena el chat de disculpas por posts que nunca pidió. Queda el aviso en la
    actividad, que es donde sí corresponde.
    """
    from edecan_creative.redaccion import FALLO_SIN_FUENTE
