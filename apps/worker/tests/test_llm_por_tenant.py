...
async def test_tenant_id_none_devuelve_router_de_plataforma_sin_tocar_nada() -> None:
    ...
    vault = _FakeVault()
    deps = make_deps(
        _resolve_tenant_llm=True,
        session_factory=_session_factory_de(session),
        vault=lambda s: vault,
    )
...
async def test_session_execute_lanza_se_traduce_a_tenant_llm_not_connected() -> None:
    ...
    deps = make_deps(
        _resolve_tenant_llm=True,
        session_factory=_session_factory_de(_RaisingSession()),
        vault=lambda s: _FakeVault(),
    )
...
async def test_cache_de_exito_es_por_tenant_no_global() -> None:
    ...
    # Se relee el vault para aplicar cambios de proveedor sin reiniciar.
    assert len(session.execute_calls) == 3
    assert len(vault.get_calls) == 3
