...
def test_validar_item_extraido_confidence_user_stated_se_respeta() -> None:
    resultado = _validar_item_extraido(
        {"kind": "fact", "content": "Vive en Bogotá.", "confidence": 1.0},
        fuente_default="conversación hoy",
    )
    assert resultado is not None and resultado["confidence"] == 1.0


def test_validar_item_extraido_confidence_fuera_de_rango_se_recorta() -> None:
    alto = _validar_item_extraido(
        {"kind": "fact", "content": "x", "confidence": 5}, fuente_default="src"
    )
    bajo = _validar_item_extraido(
        {"kind": "fact", "content": "x", "confidence": -1}, fuente_default="src"
    )
    assert alto is not None and alto["confidence"] == 1.0
    assert bajo is not None and bajo["confidence"] == 0.0


def test_validar_item_extraido_negation_es_valido() -> None:
    resultado = _validar_item_extraido(
        {"kind": "negation", "content": "No desea ser contactada por teléfono."},
        fuente_default="conversación hoy",
    )
    assert resultado is not None and resultado["kind"] == "negation"
...
def test_validar_item_extraido_normaliza_reemplazos_sin_duplicados() -> None:
    ...
    resultado = _validar_item_extraido(
        {
            "kind": "fact",
            "content": "Acme está aprobada en ambas tiendas.",
...
async def test_handle_correccion_archiva_memoria_anterior_y_deja_solo_la_nueva_activa(
    ...
    fake_repo.memory_items[memoria_anterior_id] = {
        ...
        "user_id": user_id,
        "kind": "fact",
        "content": "Acme está aprobada en App Store y pendiente en Google Play.",
        "importance": 0.7,
        "source": "importado",
        "embedding": _unit_vector(90),
    ...
    _agregar_conversacion_con_mensaje(
        ...
        tenant_id=tenant_id,
        user_id=user_id,
        texto_usuario="Corrige esto: Acme ya está aprobada en ambas plataformas.",
    )

    deps = make_deps()
    await _use_platform_router_as_tenant_router(deps, monkeypatch)
    deps.llm_router.provider.reply = (
        '[{"kind":"fact","content":"Acme está aprobada en App Store y Google Play.",'
    ...
    nuevas = [
        row
        for row in fake_repo.memory_items.values()
        if row.get("content") == "Acme está aprobada en App Store y Google Play."
    ]
    assert len(nuevas) == 1
...
# ---------------------------------------------------------------------------
# Aristas causales (`porque`, PHASE2.md §93)
# ---------------------------------------------------------------------------


def test_validar_item_extraido_parsea_porque() -> None:
    resultado = _validar_item_extraido(
        {"kind": "event", "content": "Cambió a teletrabajo.", "porque": ["Nació su hijo."]},
        fuente_default="conversación hoy",
    )
    assert resultado is not None
    assert resultado["porque"] == ["Nació su hijo."]


def test_validar_item_extraido_porque_invalido_se_descarta() -> None:
    resultado = _validar_item_extraido(
        {"kind": "event", "content": "x", "porque": [123, "", None]},
        fuente_default="conversación hoy",
    )
    assert resultado is not None
    assert "porque" not in resultado


async def test_handle_enlaza_arista_causal_porque(monkeypatch) -> None:
    fake_repo = FakeRepo()
    monkeypatch.setattr(consolidate_module, "SqlRepo", lambda session: fake_repo)
    ...
    _agregar_conversacion_con_mensaje(
        ...
        tenant_id=tenant_id,
        user_id=user_id,
        texto_usuario="Nació mi hijo, así que cambié a teletrabajo.",
    )

    deps = make_deps()
    await _use_platform_router_as_tenant_router(deps, monkeypatch)
    deps.llm_router.provider.reply = (
        '[{"kind":"event","content":"Nació su hijo.","importance":0.8},'
        '{"kind":"event","content":"Cambió a teletrabajo.","importance":0.7,'
    ...
    causa = next(r for r in fake_repo.memory_items.values() if r["content"] == "Nació su hijo.")
    efecto = next(
        r for r in fake_repo.memory_items.values() if r["content"] == "Cambió a teletrabajo."
    )
    aristas_causales = [e for e in fake_repo.memory_edges if e["relation"] == "causo"]
    assert len(aristas_causales) == 1
    assert aristas_causales[0]["src_id"] == causa["id"]
    assert aristas_causales[0]["dst_id"] == efecto["id"]


# ---------------------------------------------------------------------------
# Promoción episódica → semántica (PHASE2.md §95)
# ---------------------------------------------------------------------------


async def test_handle_promueve_evento_reforzado_a_fact_con_arista_promoted_from(
    monkeypatch,
) -> None:
    fake_repo = FakeRepo()
    monkeypatch.setattr(consolidate_module, "SqlRepo", lambda session: fake_repo)
    ...
    tenant_id = uuid.uuid4()
    user_id = uuid.uuid4()
    evento_previo_id = uuid.uuid4()
    fake_repo.tenants[tenant_id] = {"id": tenant_id, "plan_key": "hosted_pro"}
    fake_repo.memory_items[evento_previo_id] = {
        "id": evento_previo_id,
        "tenant_id": tenant_id,
        "user_id": user_id,
        "kind": "event",
        "content": "Se mudó a Bogotá.",
        "importance": 0.6,
        "source": "importado",
        "embedding": _unit_vector(0),
    ...
    _agregar_conversacion_con_mensaje(
        ...
        tenant_id=tenant_id,
        user_id=user_id,
        texto_usuario="Me mudé a Bogotá.",
    )

    deps = make_deps()
    await _use_platform_router_as_tenant_router(deps, monkeypatch)
    deps.llm_router.provider.reply = (
        '[{"kind":"event","content":"Se mudó a Bogotá.","importance":0.7}]'
    )
    ...
    anterior = fake_repo.memory_items[evento_previo_id]
    assert anterior["superseded_at"] is not None

    facts = [
        r
        for r in fake_repo.memory_items.values()
        if r.get("superseded_at") is None and r["content"] == "Se mudó a Bogotá."
    ]
    assert len(facts) == 1
    assert facts[0]["kind"] == "fact"
    assert facts[0]["expires_at"] is None  # durable: sin caducidad de evento
    assert facts[0]["importance"] == 0.7  # max(0.7, 0.6)
    assert anterior["superseded_by"] == facts[0]["id"]

    promovidas = [e for e in fake_repo.memory_edges if e["relation"] == "promoted_from"]
    assert len(promovidas) == 1
    assert promovidas[0]["src_id"] == facts[0]["id"]
    assert promovidas[0]["dst_id"] == evento_previo_id
...
def test_importancias_con_decay_baja_solo_viejas_no_reforzadas() -> None:
    ahora = datetime.now(UTC)
    viejo_id = uuid.uuid4()
    fresco_id = uuid.uuid4()
    reforzado_id = uuid.uuid4()
    memorias = [
        {"id": viejo_id, "importance": 0.8, "created_at": ahora - timedelta(days=90)},
        {"id": fresco_id, "importance": 0.5, "created_at": ahora - timedelta(days=10)},
        {"id": reforzado_id, "importance": 0.9, "created_at": ahora - timedelta(days=90)},
    ]
    resultado = _importancias_con_decay(memorias, reforzados={reforzado_id}, dias=60, ahora=ahora)
    assert dict(resultado) == {viejo_id: 0.4}


def test_importancias_con_decay_respeta_piso() -> None:
    ahora = datetime.now(UTC)
    viejo_id = uuid.uuid4()
    memorias = [
        {"id": viejo_id, "importance": 0.05, "created_at": ahora - timedelta(days=90)},
    ]
    assert _importancias_con_decay(memorias, reforzados=set(), dias=60, ahora=ahora) == []


async def test_handle_degrada_memoria_vieja_no_reforzada(monkeypatch) -> None:
    fake_repo = FakeRepo()
    monkeypatch.setattr(consolidate_module, "SqlRepo", lambda session: fake_repo)
    ...
    viejo_id = uuid.uuid4()
    fake_repo.tenants[tenant_id] = {"id": tenant_id, "plan_key": "hosted_pro"}
    fake_repo.memory_items[viejo_id] = {
        "id": viejo_id,
        "tenant_id": tenant_id,
        "user_id": user_id,
        "kind": "fact",
        "content": "un recuerdo viejo",
        "importance": 0.8,
        "source": "importado",
        "embedding": _unit_vector(0),
    ...
    assert fake_repo.memory_items[viejo_id]["importance"] == 0.4
