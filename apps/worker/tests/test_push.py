...
@respx.mock
async def test_enviar_apns_habilita_http2(monkeypatch: pytest.MonkeyPatch) -> None:
    _, p8_pem = _par_ec_p8()
    ...
    monkeypatch.setattr(push.httpx, "AsyncClient", client_factory)
    respx.post(f"https://{push._APNS_HOST_PRODUCTION}/3/device/tok-h2").mock(
        return_value=httpx.Response(200)
    )

    response = await push.enviar_apns(cred, "tok-h2", "Edecán", "Listo")

    assert response.status_code == 200
    assert observed_options["http2"] is True
...
@respx.mock
async def test_enviar_apns_inyecta_category_dentro_de_aps() -> None:
    """`category` se inyecta DENTRO de `aps` (no fuera), tal como espera el
    `UNNotificationCategory` de iOS (`"GYM_CHECKIN"`)."""
    _, p8_pem = _par_ec_p8()
    cred = _cred_apns(p8_pem)
    respx.post(f"https://{push._APNS_HOST_PRODUCTION}/3/device/tok-cat").mock(
        return_value=httpx.Response(200)
    )

    await push.enviar_apns(
        cred, "tok-cat", "Edecán", "¿Vas a ir al gym hoy?", category="GYM_CHECKIN"
    )

    payload = json.loads(respx.calls.last.request.content)
    assert payload["aps"]["category"] == "GYM_CHECKIN"
    assert "category" not in payload  # solo dentro de `aps`, nunca suelto
    assert payload["aps"]["alert"]["title"] == "Edecán"


@respx.mock
async def test_enviar_apns_sin_category_no_inyecta_el_campo() -> None:
    """Aditivo: sin `category`, el `aps` no gana ninguna clave extra (back-compat)."""
    _, p8_pem = _par_ec_p8()
    cred = _cred_apns(p8_pem)
    respx.post(f"https://{push._APNS_HOST_PRODUCTION}/3/device/tok-nocat").mock(
        return_value=httpx.Response(200)
    )

    await push.enviar_apns(cred, "tok-nocat", "T", "C")
...
async def test_apns_500_no_limpia_el_token_solo_cuenta_fallo() -> None:
    ...
    assert resultado == push.ResultadoEnvioPush(enviados=0, fallidos=1)
    # Ningún UPDATE de limpieza. (Sí hay un INSERT de registro de entrega: ver
    # la sección "Rastro durable" más abajo.)
    assert not any("UPDATE devices" in sql for sql, _ in session.llamadas)


# ---------------------------------------------------------------------------
# Rastro durable de cada intento — `record_push_delivery`.
#
# POR QUÉ estos tests: en la app de escritorio el worker corre como sidecar de
# Tauri y su stdout va a `/dev/null`, así que un 403, un 429 o una excepción de
# red desaparecían sin dejar nada. "No me llegó la notificación" era
# indistinguible de "Apple respondió 200". Sin este rastro no se puede saber si
# ningún otro arreglo de push sirvió.
# ---------------------------------------------------------------------------


def _entregas_registradas(session: _FakeSession) -> list[dict[str, Any]]:
    """Los `meta` de las filas `notifications.push.delivery` que se insertaron."""
    entregas = []
    for sql, params in session.llamadas:
        if "INSERT INTO audit_log" in sql and params.get("action") == "notifications.push.delivery":
            entregas.append(json.loads(params["meta"]))
    return entregas


@respx.mock
async def test_apns_rechazado_guarda_status_y_reason_en_audit_log() -> None:
    """Un 403 `InvalidProviderToken` moría dentro de un `logger.warning`."""
    account_id = uuid.uuid4()
    device_id = uuid.uuid4()
    tenant_id, user_id = uuid.uuid4(), uuid.uuid4()
    _, p8_pem = _par_ec_p8()
    config = {"apns": _cred_apns(p8_pem)}
    device = _device_row(id=device_id, push_token="tok-403", push_platform="apns")
    session = _FakeSession(respuestas=[[{"id": account_id}], [device]])
    vault = _FakeVault(bundle=_FakeBundle(access_token=json.dumps(config)))
    deps = make_deps(session_factory=_session_factory_de(session), vault=lambda s: vault)

    respx.post(f"https://{push._APNS_HOST_PRODUCTION}/3/device/tok-403").mock(
        return_value=httpx.Response(
            403,
            json={"reason": "InvalidProviderToken"},
            headers={"apns-id": "8B1E4F60-9E4C-4C1F-9C9E-1B2A3C4D5E6F"},
        )
    )

    resultado = await push.enviar_push_a_usuario(
        deps, tenant_id=tenant_id, user_id=user_id, titulo="T", cuerpo="C"
    )

    assert resultado == push.ResultadoEnvioPush(enviados=0, fallidos=1)
    entregas = _entregas_registradas(session)
    assert len(entregas) == 1
    assert entregas[0]["outcome"] == "rechazado"
    assert entregas[0]["status_code"] == 403
    assert entregas[0]["reason"] == "InvalidProviderToken"
    assert entregas[0]["platform"] == "apns"
    assert entregas[0]["device_id"] == str(device_id)
    assert entregas[0]["provider_message_id"] == "8B1E4F60-9E4C-4C1F-9C9E-1B2A3C4D5E6F"
    # Ni el título, ni el cuerpo, ni el push token entran al registro.
    assert set(entregas[0]) == {
        "version",
        "platform",
        "outcome",
        "status_code",
    ...
    assert "tok-403" not in json.dumps(entregas[0])


@respx.mock
async def test_apns_aceptado_guarda_el_apns_id_para_poder_reclamar() -> None:
    account_id = uuid.uuid4()
    device_id = uuid.uuid4()
    _, p8_pem = _par_ec_p8()
    config = {"apns": _cred_apns(p8_pem)}
    device = _device_row(id=device_id, push_token="tok-ok", push_platform="apns")
    session = _FakeSession(respuestas=[[{"id": account_id}], [device]])
    vault = _FakeVault(bundle=_FakeBundle(access_token=json.dumps(config)))
    deps = make_deps(session_factory=_session_factory_de(session), vault=lambda s: vault)

    respx.post(f"https://{push._APNS_HOST_PRODUCTION}/3/device/tok-ok").mock(
        return_value=httpx.Response(200, headers={"apns-id": "AAAA-BBBB"})
    )
    ...
    assert resultado == push.ResultadoEnvioPush(enviados=1, fallidos=0)
    entregas = _entregas_registradas(session)
    assert entregas == [
        {
            "version": 1,
...
@respx.mock
async def test_excepcion_de_red_queda_registrada_con_la_clase_no_el_mensaje() -> None:
    account_id = uuid.uuid4()
    _, p8_pem = _par_ec_p8()
    config = {"apns": _cred_apns(p8_pem)}
    device = _device_row(push_token="tok-red", push_platform="apns")
    session = _FakeSession(respuestas=[[{"id": account_id}], [device]])
    vault = _FakeVault(bundle=_FakeBundle(access_token=json.dumps(config)))
    deps = make_deps(session_factory=_session_factory_de(session), vault=lambda s: vault)

    respx.post(f"https://{push._APNS_HOST_PRODUCTION}/3/device/tok-red").mock(
        side_effect=httpx.ConnectError("https://usuario:clave@proxy.interno/ no responde")
    )
    ...
    entregas = _entregas_registradas(session)
    assert len(entregas) == 1
    assert entregas[0]["outcome"] == "excepcion"
    assert entregas[0]["reason"] == "ConnectError"
    # El mensaje de la excepción podía traer una URL con credenciales.
    assert "proxy.interno" not in json.dumps(entregas[0])


@respx.mock
async def test_sin_credenciales_y_sin_dispositivos_tambien_dejan_rastro() -> None:
    """Los dos cortes tempranos daban EXACTAMENTE el mismo silencio que un
    fallo de Apple; ahora se distinguen sin mirar un log."""
    sin_cred = _FakeSession(respuestas=[[]])
    deps_sin_cred = make_deps(
        session_factory=_session_factory_de(sin_cred), vault=lambda s: _FakeVault()
    )
    await push.enviar_push_a_usuario(
        deps_sin_cred, tenant_id=uuid.uuid4(), user_id=uuid.uuid4(), titulo="T", cuerpo="C"
    )
    assert [e["outcome"] for e in _entregas_registradas(sin_cred)] == ["sin_credenciales"]

    _, p8_pem = _par_ec_p8()
    config = {"apns": _cred_apns(p8_pem)}
    sin_devices = _FakeSession(respuestas=[[{"id": uuid.uuid4()}], []])
    deps_sin_devices = make_deps(
        session_factory=_session_factory_de(sin_devices),
        vault=lambda s: _FakeVault(bundle=_FakeBundle(access_token=json.dumps(config))),
    )
    await push.enviar_push_a_usuario(
        deps_sin_devices, tenant_id=uuid.uuid4(), user_id=uuid.uuid4(), titulo="T", cuerpo="C"
    )
    assert [e["outcome"] for e in _entregas_registradas(sin_devices)] == ["sin_dispositivos"]


@respx.mock
async def test_si_el_registro_de_entrega_falla_el_push_igual_se_envia() -> None:
    """El rastro es observabilidad, no un requisito del envío."""
    account_id = uuid.uuid4()
    _, p8_pem = _par_ec_p8()
    config = {"apns": _cred_apns(p8_pem)}
    device = _device_row(push_token="tok-ok", push_platform="apns")

    class _SessionQueFallaAlInsertar(_FakeSession):
        async def execute(self, stmt: Any, params: dict[str, Any] | None = None) -> Any:
            if "INSERT INTO audit_log" in str(stmt):
                raise RuntimeError("audit_log no disponible")
            return await super().execute(stmt, params)

    session = _SessionQueFallaAlInsertar(respuestas=[[{"id": account_id}], [device]])
    vault = _FakeVault(bundle=_FakeBundle(access_token=json.dumps(config)))
    deps = make_deps(session_factory=_session_factory_de(session), vault=lambda s: vault)
