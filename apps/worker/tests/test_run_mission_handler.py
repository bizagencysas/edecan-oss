...
async def test_mision_pausada_no_se_ejecuta_sin_payload_de_reanudacion():
    session = FakeSession()
    mission_id = uuid.uuid4()
    tenant_id = uuid.uuid4()
...
async def test_mision_nueva_planifica_persiste_pasos_y_ejecuta():
    ...
    # Orchestrator(llm_router, registry) se construyó con el router GLOBAL
    # —inferencia administrada, `deps.llm_router_for` no resuelve por
    # tenant— y con el registry que arma `_build_registry` (aquí
    # monkeypatcheado a REGISTRY_SENTINEL, ver docstring de esa constante).
    assert len(FakeOrchestrator.instances) == 1
    assert FakeOrchestrator.instances[0].llm_router is not None
    assert FakeOrchestrator.instances[0].llm_router is deps.llm_router
