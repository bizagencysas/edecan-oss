...
## Mover tus agentes sin mover secretos

En **Llamadas → Agentes de llamadas** puedes descargar una **copia privada**
JSON y volver a importarla en otra instalación de Edecan. El contrato HTTP es:

- `GET /v1/phone/agent-templates/export`;
- `POST /v1/phone/agent-templates/import`.

El paquete conserva personalidad, voz, misión, alcance, contexto autorizado y
defaults. Deliberadamente no contiene ids internos, cuenta o token de Twilio,
teléfonos, destinatarios, transcripciones, llamadas ni timestamps. Al importar
puedes conservar perfiles con el mismo nombre (`conflict_policy: "skip"`) o
reemplazar solo su configuración futura (`"replace"`). La operación valida el
paquete completo, respeta el límite de 20 perfiles y nunca modifica los
snapshots de llamadas anteriores.
