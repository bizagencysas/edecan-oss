...
> **Inferencia vigente (2026-07-27):** chat, voz, llamadas y herramientas
> ligeras usan el `LLMProvider` administrado de Workers AI y el modelo se
> decide en el servidor. Los endpoints históricos que permitían mutar
> proveedor o modelo LLM permanecen documentados más abajo solo para explicar
> compatibilidad de clientes antiguos; el servidor actual los rechaza con
> `409` y no guarda credenciales LLM por tenant. Consulta
...
### `GET /v1/models/chat` — catálogo del selector de modelos

Auth: Bearer (access). Catálogo único del selector del chat: de aquí leen la hoja
"Seleccionar modelo" (los `principal: true`, en orden 1..N), la pantalla "Más modelos"
(`principal: false`), si se muestra la fila Esfuerzo (`soporta_esfuerzo`) y la etiqueta de
ceguera (`ve_imagenes`). Ningún cliente duplica la lista. La autoridad es
`config/modelos.yml` → `modelos_chat`, leída en runtime: agregar o retirar un modelo del
selector es editar ese YAML, sin migración.
...
```json
{
  ...
  "modelos": [{"id": "@cf/meta/llama-4-scout-17b-16e-instruct", "nombre": "Copla", "descripcion": "Ligero y al instante · ve imágenes", "orden": 1, "principal": true, "ve_imagenes": true, "soporta_esfuerzo": false, "contexto_ventana": 131072}]
}
```

### `PUT /v1/conversations/{id}/model` — fijar modelo y Esfuerzo

Auth: Bearer (access). Body: `{"model": "@cf/moonshotai/kimi-k2.6", "effort": "alto"}`. El
modelo es propiedad de la CONVERSACIÓN (columnas `chat_model`/`chat_effort`), no de la
credencial del tenant — por eso `PATCH /v1/credentials/llm/models` sigue devolviendo `409`.
`model: null` vuelve a automático (decide la cadena `WORKERS_AI_CHAT_MODEL` →
`MODELO_POR_DEFECTO`); `effort: null` equivale a `medio`. Respuesta `200`:
`{"model": ..., "effort": ...}` tal como quedó persistido. `404` si la conversación es de
otro usuario o no existe; `422` si el `model` no está en el catálogo de
`GET /v1/models/chat` o el `effort` está fuera de `bajo|medio|alto`.

El `effort` se guarda aunque el modelo activo no lo soporte (cambiar de Copla a Oda recuerda
el nivel previo): el gate es al APLICAR el turno, no al guardar. El Esfuerzo se traduce al
presupuesto de `max_tokens` por iteración del loop del agente — `bajo` 2048, `medio` 4096
(el valor de siempre), `alto` 8192 — y solo se aplica a los modelos con
`soporta_esfuerzo: true`.

`GET /v1/conversations` y `GET /v1/conversations/{id}` devuelven `model` y `effort`
(ambos `null` = automático) para restaurar la pastilla del composer al reabrir el chat.

### `POST /v1/conversations/{id}/messages` — turno del agente (SSE)

Auth: Bearer (access). Body: `{"text": "¿Qué tengo agendado mañana?"}` (`ChatMessageIn`).
Admite además `model` y `effort` opcionales como override del turno, que TAMBIÉN se
persisten en la conversación (así elegir y enviar en un mismo gesto no depende de una
carrera entre el PUT y este POST); ausentes = usar lo que la conversación ya tiene. `422`
si `model` no está en el catálogo. Si el turno lleva una imagen y el modelo efectivo no ve
imágenes, ESE turno corre con el default con visión del catálogo (degradación determinista,
sin evento SSE nuevo) y la selección persistida no cambia. Antes de abrir el stream, revisa la cuota diaria del plan (`limits.messages_per_day` sobre `usage_events` de hoy, `-1` ilimitado): si ya se agotó, responde una `429` normal (JSON, no SSE — el chequeo corre antes de construir el `StreamingResponse`), nunca un evento `error` dentro del stream.
