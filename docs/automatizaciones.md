...
- `trigger` (jsonb) — unión discriminada por `kind` (`edecan_schemas.automations.TriggerDef`):
  - `{"kind": "schedule", "rrule": "FREQ=DAILY;BYHOUR=9;BYMINUTE=0"}` — RFC 5545, interpretada con `python-dateutil` (`edecan_automations.engine.compute_next_run`). El backend recalcula `next_run_at` cada vez que se crea o se edita el `trigger`. Al leerla, el `trigger` sale además con `descripcion` (ver "La agenda en cristiano" abajo).
...
### Las agendas corren en UTC

`compute_next_run` le pasa a `dateutil` un `dtstart` **tz-aware UTC**, así que `BYHOUR=4` significa las 04:00 UTC — no la hora local del servidor ni la de quien mira la pantalla. No hay zona por tenant ni por usuario en el modelo de datos: si algún día la hubiera, cambian juntos `engine.compute_next_run` y `edecan_automations.describe.SCHEDULE_TIMEZONE`.

Cuidado con `BYHOUR` sin `BYMINUTE`: RFC 5545 hereda de `dtstart` las partes que la regla no fija, y aquí `dtstart` es el momento del alta. `FREQ=DAILY;BYHOUR=9` creada 9:39 corre **a las 9:39**, no a las 9:00, y ese minuto queda fijo (el barrido reancla en el `next_run_at` persistido). **Pon siempre `BYMINUTE` junto a `BYHOUR`.**

### La agenda en cristiano

Nadie lee `FREQ=DAILY;BYHOUR=4;BYMINUTE=0` y entiende "todos los días a las 4 de la mañana". `edecan_automations.describe.describe_rrule` traduce la regla a una frase (`"Todos los días a las 4:00 UTC"`, `"Cada 15 minutos"`) y `routers/automations.py::_public_automation` la publica ya armada en `trigger.descripcion`.

Se hace **en el servidor, una sola vez**, por dos razones:

1. La frase lleva una HORA, y sólo el backend sabe en qué zona corre de verdad la regla. Web e iOS reciben la frase hecha y sólo la pintan (`triggerResumen` en `apps/web/src/lib/api-automatizaciones.ts`, `AutomationTrigger.resumen` en `EdecanKit`), así que las dos superficies —y el chat, vía `gestionar_automatizacion`— dicen exactamente lo mismo.
2. La hora **no se convierte** a la de quien lee: va rotulada `UTC`. Convertir una recurrencia (no un instante) es traicionero — "los lunes a las 23:00 UTC" es domingo por la noche en América, o sea que hasta el día de la semana cambiaría. El instante concreto en hora local ya lo da `next_run_at` aparte, que sí es absoluto; por eso el detalle lo rotula "(tu hora)".

`descripcion` viene `null` cuando la regla no se puede traducir con certeza (`UNTIL`/`COUNT`, `BYSETPOS`, "el 2º viernes", `FREQ=YEARLY`, …). Ahí las superficies muestran "Agenda personalizada" y enseñan la `rrule` cruda en el detalle: **una hora equivocada es peor que una hora técnica**, así que ante la duda no se traduce.
