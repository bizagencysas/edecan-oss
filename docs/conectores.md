...
**Clave del conector**: `linkedin`. Edecán ya crea paquetes completos para LinkedIn (copy, manifiesto reutilizable e imagen original, entregados en el Mega Chat para web, iOS y Android) y **sí incluye un conector oficial de primera parte** para publicarlos — `packages/connectors/edecan_connectors/social/linkedin.py`, con el mismo patrón OAuth/callback que Google, Microsoft, Meta, X y YouTube.

### Crear la app OAuth

1. Entra a [LinkedIn Developer Portal](https://www.linkedin.com/developers/apps) → **Create app**. Necesitas una página de empresa de LinkedIn asociada a tu cuenta para poder crear la app (es un requisito de LinkedIn para verificar la app, no para lo que la app va a publicar después).
2. En la pestaña **Products** de la app, solicita **"Sign in with LinkedIn
   using OpenID Connect"** y **"Share on LinkedIn"**. La elegibilidad, el
   proceso de aprobación y el tiempo de activación dependen de la cuenta y de
   las reglas vigentes que muestre LinkedIn Developer Portal; Edecán no los
   puede prometer ni omitir.
3. En **Auth → OAuth 2.0 settings → Authorized redirect URLs**, agrega exactamente `{PUBLIC_BASE_URL}/v1/connectors/linkedin/callback`.
4. En **Auth**, copia el **Client ID** y el **Client Secret** al formulario guiado de LinkedIn en Ajustes.
...
### Alcance real: solo perfil personal, no Company Page

El conector publica **como la persona autorizada** (`author =
urn:li:person:{id}`, vía `w_member_social`). **No publica como una Company
Page**: eso necesita `w_organization_social` y acceso a la **LinkedIn Community
Management API**, que LinkedIn evalúa según la app y que este conector todavía
no implementa. Una sesión local de control de computadora puede ayudar a la
persona a completar manualmente una publicación ya revisada si esa capacidad
está habilitada y autorizada en su instalación, pero no sustituye el conector
oficial ni garantiza acceso a páginas de empresa.

LinkedIn entrega `refresh_token` solo a programas aprobados; sin uno, la cuenta debe volver a autorizarse cuando el token expire. El extractor web genérico no scrapea LinkedIn y Edecán no hace extracción masiva, spam, contacto masivo ni engagement artificial.

---

## Twilio (llamadas telefónicas)

**Clave del conector**: `twilio`. A diferencia de los conectores anteriores, Twilio **no usa OAuth** — no hay "autorizar la cuenta" con un botón y un callback; el operador pega sus credenciales directo en un formulario. Ver [`voz-telefonia.md`](./voz-telefonia.md) para cómo Edecán usa estas credenciales en una llamada real.

### Conseguir las credenciales

1. Crea una cuenta en [twilio.com](https://www.twilio.com/try-twilio) (tiene capa de prueba gratuita con saldo limitado).
2. En el **Twilio Console** ([console.twilio.com](https://console.twilio.com/)), la página principal ("Account Info") muestra tu **Account SID** (empieza con `AC` + 32 caracteres hexadecimales) y tu **Auth Token** (32 caracteres, hay que pulsar "Show" para verlo).
3. **Phone Numbers → Manage → Buy a number** — compra un número con capacidad de **Voice**. Anótalo en formato E.164 (ej. `+525512345678`).
4. En **Ajustes → Conexiones → Twilio**, pega Account SID, Auth Token y el número.

### Qué pasa al conectar

El formulario llama a `PUT /v1/connectors/twilio/credentials` con `{account_sid, auth_token, phone_number}`. El backend:

- valida el formato de los tres campos antes de nada (`AC` + 32 hex; token de 32 caracteres; número E.164);
- verifica contra la API real de Twilio que ese número pertenece de verdad a esa cuenta (`_verify_twilio_phone_ownership`) — no solo confía en el formato;
- **configura automáticamente el webhook de voz entrante** del número apuntando a `{PUBLIC_BASE_URL}/v1/phone/twilio/incoming` — no hace falta pegar ninguna URL a mano en la consola de Twilio como sí es necesario al montar un servicio de voz desde cero.

Gateado por el flag de plan `voice.telephony` y por la cuota `limits.phone_numbers` — ver `apps/api/edecan_api/routers/connectors.py`.
