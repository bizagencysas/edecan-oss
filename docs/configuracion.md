...
La inferencia que no pertenece al IDE es administrada por el host. Chat, voz,
llamadas y workers comparten la misma configuración de Workers AI. No existe
una credencial LLM por tenant ni un selector de modelo en los clientes.

| Variable | Obligatoria | Default | Para qué sirve |
|---|---|---|---|
| `CLOUDFLARE_ACCOUNT_ID` | Sí para inferencia real | *(vacío)* | ID de la cuenta que ejecuta Workers AI. |
| `CLOUDFLARE_API_TOKEN` | Sí para inferencia real | *(vacío)* | Token del host con permisos mínimos de Workers AI. Nunca se expone a tenants. |
| `WORKERS_AI_CHAT_MODEL` | Opcional | `@cf/zai-org/glm-4.7-flash` | Modelo automático para chat, llamadas, voz, herramientas ligeras y jobs fuera del IDE. |
| `WORKERS_AI_IDE_MODEL` | Opcional | `@cf/zai-org/glm-5.2` | Modelo de ingeniería para sesiones de texto del IDE. |
| `WORKERS_AI_IDE_VISION_MODEL` | Opcional | `@cf/moonshotai/kimi-k2.7-code` | Modelo multimodal que el IDE activa automáticamente cuando la sesión incluye imágenes. |
| `WORKERS_AI_TIMEOUT_SECONDS` | Opcional | `60` | Timeout HTTP por petición a Workers AI. |
| `WORKERS_AI_FALLBACK_MODEL` | Opcional | vacío | Modelo de respaldo para fallas del modelo primario antes del primer token; no reintenta respuestas parciales. |

El IDE conserva su runtime local separado, pero usa las dos variables
`WORKERS_AI_IDE_*` para decidir automáticamente la inteligencia adecuada. Ver
...
## v3 — modo local y compatibilidad histórica

Las rutas de CLI/Ollama descritas históricamente por v3 ya no participan en la
inferencia de primera parte. Se conservan algunos campos y endpoints de
diagnóstico para compatibilidad de datos y herramientas independientes; la
fuente de verdad actual es la sección **LLM** anterior.
...
| `LLM_CLI_TIMEOUT_SECONDS` | Opcional | `300` | Timeout de utilidades CLI heredadas que no forman parte del router activo. |
...
Nota: ninguna de estas variables elige el proveedor del chat. El router activo
usa exclusivamente la configuración Workers AI del host.
...
consulta para decidir qué mostrarte. La respuesta LLM existe solo como
compatibilidad para clientes antiguos; las aplicaciones actuales muestran la
inteligencia administrada como lista y no fuerzan un paso de conexión.
