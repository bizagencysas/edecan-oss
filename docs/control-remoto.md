...
#### Coordenadas: `nx`/`ny` (fracción normalizada) — el único camino correcto

Hay **tres** espacios de coordenadas en juego y no coinciden entre sí: (1) `screencapture` captura en **píxeles Retina** nativos (p. ej. 3456×2234); (2) `_optimize_screenshot` **reduce** esa imagen a `max_width` (1600 por defecto, y con `format: "jpeg"` el resize corre siempre), así que el `width`/`height` que viaja al cliente es el del JPEG reducido; (3) `CGEvent` no consume ninguno de los dos: consume el espacio global de pantalla en **puntos lógicos** (`CGDisplayBounds`, p. ej. 2056×1329). Mandar la coordenada del frame como si fueran puntos dejaba cada clic al 77.8% del camino hacia la esquina superior izquierda — el Dock y la franja derecha de la pantalla quedaban inalcanzables desde el móvil.

Por eso `PointerInputIn` acepta, además de `x`/`y`, los campos **opcionales** `nx`/`ny` (y `start_nx`/`start_ny` para `drag`): la posición del toque como **fracción `0.0..1.0` del frame**. Cuando llegan, el companion —el único que conoce la geometría real del escritorio— las multiplica por el tamaño del display en las unidades de su backend (`actions._pointer_display_bounds`: puntos lógicos en macOS, píxeles del escritorio virtual en Windows/Linux) y **las usa en lugar de `x`/`y`**; el origen del monitor (`origin_x`/`origin_y`, también en puntos) se suma ahí mismo, así que el multi-monitor sigue funcionando. La conversión ocurre SIEMPRE del lado de Python: el puente nativo (`apps/desktop/src-tauri/src/remote_bridge.rs`) recibe la coordenada ya resuelta y no cambió.

`x`/`y` siguen siendo obligatorios y con el mismo significado de siempre: son el único camino para los clientes que todavía no mandan la fracción (el panel web). Medio par (`nx` sin `ny`) es `422`. Clientes: iOS `RemoteCoordinateMapper.mapear` y Android `mapPointToRemoteCoords` devuelven la fracción junto a los píxeles del frame.
