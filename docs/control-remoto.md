...
| Captura de pantalla real del companion (acción `screenshot`) | **Real** — fase v2 aterrizó `_screenshot` en `apps/companion/edecan_companion/actions.py` mientras este WP estaba en curso; el contrato ya está verificado por lectura de código (`{image_b64, width, height}` coincide exactamente con lo que `routers/remote.py` espera). No se ejecutó un companion real en vivo como parte de este WP (exigiría macOS + el permiso de Grabación de Pantalla concedido a mano) — la verificación es de contrato, no un E2E manual | `docs/roadmap.md`; `routers/remote.py` sigue degradando con `501` de forma limpia para companions desactualizados o con `ide_enabled: false` |
...
| App móvil como visor | **P2** | §9 de este documento, `docs/roadmap.md` |
