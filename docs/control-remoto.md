...
Edecán puede, con aprobación explícita en ambos extremos, dejar que el dueño de un tenant vea la pantalla de su propio equipo desde el panel web. Este documento es el diseño completo de esa capacidad — a nivel de TeamViewer/AnyDesk y más allá — y también la referencia exacta de qué parte de ese diseño ya está construida hoy (WP-V2-09, `ROADMAP_V2.md` §5) frente a qué queda documentada para después (P2, `ROADMAP_V2.md` §6.2).
...
La regla de producto de `ARCHITECTURE.md` §0 y `ROADMAP_V2.md` §8.2 es innegociable: **control remoto = emparejamiento + aprobación humana + cifrado, nunca un backdoor silencioso**. Todo lo de abajo — construido o solo diseñado — respeta esa regla; la diferencia entre columnas es únicamente "ya hay código" vs. "todavía es solo este documento".
...
| Captura de pantalla real del companion (acción `screenshot`) | **Real** — WP-V2-08 aterrizó `_screenshot` en `apps/companion/edecan_companion/actions.py` mientras este WP estaba en curso; el contrato ya está verificado por lectura de código (`{image_b64, width, height}` coincide exactamente con lo que `routers/remote.py` espera). No se ejecutó un companion real en vivo como parte de este WP (exigiría macOS + el permiso de Grabación de Pantalla concedido a mano) — la verificación es de contrato, no un E2E manual | `ROADMAP_V2.md` §7.8; `routers/remote.py` sigue degradando con `501` de forma limpia para companions desactualizados o con `ide_enabled: false` |
| Tabla `remote_sessions` en Postgres | **Real** — las sesiones persisten ahí de verdad, vía `Repo.create_remote_session`/`list_remote_sessions`/`get_remote_session`/`record_remote_session_frame`/`mark_remote_session_denied`/`mark_remote_session_ended` (`edecan_api/repo.py`, consumidos desde `routers/remote.py`) | `ROADMAP_V2.md` §7.4; `packages/db/edecan_db/models.py::RemoteSession` |
| Tabla `devices` en Postgres (emparejamiento de apps móviles) | **Tabla creada, todavía sin ningún código que la use** — el emparejamiento QR + *fingerprint* de este documento (§4) y el visor móvil (§9) son P2; hasta que aterricen, `device_id` queda `NULL` en cada `remote_sessions` | `ROADMAP_V2.md` §7.4/§6.1; `packages/db/edecan_db/models.py::Device` |
...
| Niveles de permiso (ver / ver+puntero / control total) | **Parcial** — `kind` pasó de fijo en `"view"` a `"view" \| "control"` (WP-V4-10); `"control"` ya es input REAL de teclado/mouse (más que "ver+puntero", que sigue sin existir) — el nivel intermedio puramente informativo del diseño original de §6 no se construyó | §6 (diseño original) y §7bis (implementado) de este documento |
| Input injection (teclado/mouse remotos) | **Real (WP-V4-10, "fase 2")** — sobre el *polling* HTTP de hoy, NO sobre WebRTC (eso sigue siendo diseño, §5): `POST /v1/remote/sessions/{id}/input`, acciones `input_pointer`/`input_key` del companion, aprobación local por comando acotada a la sesión | §7bis de este documento; `apps/api/edecan_api/routers/remote.py`, `apps/companion/edecan_companion/actions.py`/`approval.py` |
| Indicador visible permanente **en el equipo controlado** (menú/bandeja del sistema) | **P2** — el companion de hoy es una CLI (`python -m edecan_companion`) sin presencia gráfica; solo hay líneas de log/consola por cada acción (incluidas las de input, WP-V4-10 no cambió esto) | §8 de este documento |
...
| App móvil como visor | **P2** | §9 de este documento, `ROADMAP_V2.md` §6.1 |
...
4. **Apoyarse en los gates del sistema operativo, nunca evadirlos.** macOS exige un clic humano explícito en Preferencias del Sistema para conceder permiso de Grabación de Pantalla (captura, real hoy) y de Accesibilidad (input injection, real hoy — WP-V4-10, §7bis). Cada vez que este diseño depende de uno de esos permisos, la arquitectura **espera** a que el usuario lo conceda por la vía normal del sistema operativo — nunca intenta automatizarlo, ni hay ninguna bandera de configuración que lo salte.
...
| Metadatos de sesión (`remote_sessions`: quién, cuándo, cuántos frames, duración) | Postgres, tabla `remote_sessions` (§7.4 `ROADMAP_V2.md`) | Es el rastro de auditoría — su pérdida o alteración impide demostrar después quién vio qué y cuándo. |
...
- **Captura**: en macOS, [ScreenCaptureKit](https://developer.apple.com/documentation/screencapturekit) (el mismo framework del sistema que ya usa la acción `screenshot` del companion, `ROADMAP_V2.md` §7.8, solo que en modo streaming continuo en vez de una captura suelta) produce frames que se codifican a **H.264** con el encoder de hardware del equipo (VideoToolbox) — bajo uso de CPU incluso en sesiones largas.
- **Video**: el stream H.264 va por un `RTCPeerConnection` estándar de WebRTC, como cualquier videollamada.
- **Eventos**: input remoto YA existe (WP-V4-10, §7bis) pero hoy viaja por `POST` HTTP, un comando a la vez — el diseño objetivo lo mueve a un **DataChannel** separado del PeerConnection, para mensajes de baja latencia tipo `{type: "mousemove", x, y}`, nunca mezclado con el canal de video. Migrar de HTTP a DataChannel no cambia ninguno de los 4 candados de §7bis, solo el transporte del evento en sí.
...
| **Ver + puntero** (`view_pointer`) | Además de ver, se muestra la posición del puntero del VISOR superpuesta en la pantalla del equipo controlado (como un puntero remoto de presentación) — sigue sin poder mover NADA en el equipo real, es puramente informativo/cosmético para señalar algo durante una sesión de soporte guiado. **No se construyó** (WP-V4-10 saltó directo a control total). | Un prompt distinto: "¿Permitir que además señalen con un puntero en tu pantalla?" |
| **Control total** (`full_control`, `kind="control"`) | Input injection real: mueve el mouse, hace clic, escribe. **Real desde WP-V4-10** (§7bis), sobre `POST` HTTP, no sobre WebRTC. | Un prompt POR COMANDO (más granular que "un tercer prompt" único), y solo tras el permiso de Accesibilidad ya concedido por el usuario en macOS (ver §7bis) — dos condiciones independientes, ambas obligatorias, más el flag de plan y `remote_input_enabled` del companion (4 candados en total, §7bis). |
...
3. **Companion** (`edecan_companion.actions._input_pointer`/`_input_key`) ejecuta el gesto vía `InputBackend` — en macOS, `_QuartzInputBackend` sobre `Quartz.CGEvent*` (dependencia opcional `pyobjc-framework-Quartz`, `pip install 'edecan-companion[remote-input]'`); en cualquier otro sistema operativo, error claro "no soportado en esta plataforma".
...
4. **Permiso de Accesibilidad de macOS**, que SOLO un clic humano en Ajustes del Sistema → Privacidad y Seguridad → Accesibilidad puede conceder — `_QuartzInputBackend` lo verifica (`Quartz.AXIsProcessTrusted()`) antes de sintetizar cualquier evento, nunca lo evade ni lo automatiza (mismo principio que el permiso de Grabación de pantalla que ya exige `screenshot`).

### Auditoría por comando

Cada `POST .../input` exitoso deja una fila nueva en `audit_log` (`remote.session.input`, `meta` con `tipo` y un resumen — nunca el contenido: `texto` se audita como `{"clave": "texto", "length": N}`, jamás el texto en claro, tanto en `audit_log` como en la bitácora del companion, `edecan_companion.audit._REDACTED_KEYS`, ahora también con `"texto"`). Una denegación del usuario en el companion marca la SESIÓN completa `denied` (no solo ese comando — mismo criterio conservador que ya usaba la vista para una captura denegada) y audita `remote.session.input_denied`, con el mismo patrón "commit de evidencia ANTES del raise" de `HOTFIXES_PENDIENTES.md` punto 8 que ya usaba `get_frame`.
...
| `action` | Cuándo | `meta` |
|---|---|---|
| `remote.session.requested` | `POST /v1/remote/sessions` exitoso (sesión creada `pending`) | `{"kind": "view"}` o `{"kind": "control"}` (WP-V4-10) |
...
| `remote.session.input` (WP-V4-10) | `POST .../input` exitoso — una fila POR COMANDO, nunca agregada | `{"tipo": "pointer"\|"key", ...}` — nunca el `texto` en claro, ver §7bis |
| `remote.session.input_denied` (WP-V4-10) | El companion respondió "acción rechazada" a un `input_pointer`/`input_key` — deniega la SESIÓN completa, no solo ese comando | `{..., "error": "..."}` |
...
La app iOS/Android (P2, `ROADMAP_V2.md` §6.1) no controla el Mac por su cuenta en ningún momento — es, literalmente, **el visor WebRTC** del diseño del §5 corriendo en un teléfono en vez de en el navegador:
...
|---|---|---|
| `POST /sessions` | Crea una sesión `pending`. Exige `consent: true` y companion conectado. `kind` (WP-V4-10, default `"view"`) — `"control"` exige además el flag `companion.remote_input`. | `422` sin consentimiento explícito o `kind` inválido · `403` `kind="control"` sin el flag · `503` sin companion conectado |
...
| Input remoto es `POST` por comando, no un canal continuo — cada clic/tecla es una request HTTP completa contra `apps/api`, no un DataChannel de baja latencia | Se construyó sobre el *polling* HTTP existente (WP-V4-10), no se esperó a WebRTC | Transporte WebRTC, §5/§7bis |
| Sin nivel "ver+puntero" (el escalón intermedio puramente informativo de §6) | Se saltó directo de "ver" a "control total" real al construir input remoto (WP-V4-10) | §6 (si algún día se prioriza) |
...
1. ~~Migración `0003_v2_expansion`~~ (WP-V2-01) — **hecho**: `remote_sessions` ya persiste de verdad (`Repo.create_remote_session/...`, `routers/remote.py`), deja de depender de estado en memoria de un solo proceso. La tabla `devices` existe pero sigue sin ningún código que la use — la puebla el punto 3 (emparejamiento) cuando aterrice.
2. ~~Acción `screenshot` del companion~~ (WP-V2-08) — **hecho**: frames reales en vez de `501` constante.
...
### 12.1bis Fase 2 (WP-V4-10) — sesión de control completa
...
- [`../ROADMAP_V2.md`](../ROADMAP_V2.md) §5 (WP-V2-09), §6.1 (apps móviles), §6.2 (control remoto completo — el resumen que este documento desarrolla en detalle), §7.2 (flag `companion.remote_view`), §7.4 (tablas `devices`/`remote_sessions`), §7.5 (`REMOTE_FRAME_MIN_INTERVAL_SECONDS`), §7.6 (montaje del router), §7.8 (acción `screenshot`), §8.2 (guardrail de producto).
- [`../ARCHITECTURE.md`](../ARCHITECTURE.md) §13 (dueño WP-V4-01, flag `companion.remote_input`) — WP-V4-10 (control remoto fase 2, §7bis de este documento) es quien lo consume.
