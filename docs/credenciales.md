# Credenciales de voz, imágenes, búsqueda y conectores por tenant

> **Inferencia vigente (2026-07-27):** LLM dejó de ser bring-your-own.
> Chat, voz, llamadas y tools ligeras usan Workers AI configurado por el host;
> el usuario no proporciona una API key LLM ni elige modelo. Los apartados
> históricos de LLM que aparecen más abajo describen el contrato v3 anterior
> y no autorizan al cliente actual a mutarlo: esos endpoints responden `409`.
> Consulta [`workers-ai.md`](./workers-ai.md). Voz, imágenes, búsqueda y
> conectores sí conservan sus credenciales propias por tenant.
