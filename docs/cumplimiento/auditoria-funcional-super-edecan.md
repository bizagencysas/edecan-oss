...
| Windows y Linux | Gates nativos implementados; ejecución CI pendiente | Windows tiene runner nativo que compila NSIS/MSI, extrae MSI, instala NSIS, comprueba FyDesign, espera el backend real y exige cierre sin huérfanos. Linux compila AppImage/`.deb`/`.rpm`, inspecciona FyDesign completo y ejercita el AppImage con Xvfb/Openbox. Los contratos, Rust, web y 215 pruebas del companion/local pasan en macOS, pero esta auditoría no presenta eso como un build Windows/Linux: falta subir el cambio y observar ambos jobs verdes. Wayland puede limitar captura/input según compositor; Linux ARM64 conserva el camino self-host sin PostgreSQL embebido. |
