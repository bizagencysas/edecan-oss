...
| Un solo punto de salida garantizado (`RunEvent::Exit`) mata el sidecar siempre | `lib.rs::run()` — Cmd+Q, **Salir completamente** y el panel de error convergen en `RunEvent::Exit`→`kill_backend`. En macOS, cerrar `main` ahora la oculta a propósito y no representa una salida. |
