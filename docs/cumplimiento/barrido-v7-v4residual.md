...
| `devices.py` (rutas de push: `POST/DELETE .../push-token`, `PUT/GET/DELETE .../push/credentials`) | `FLAG_NOTIFICATIONS_PUSH` (`"notifications.push"`) | `probar_notificaciones_push` (el registro del dispositivo sigue siendo responsabilidad de la app nativa) | `requires_flags={"notifications.push"}` | La prueba solo puede avisar al usuario actual; no recibe tokens, destinatarios ni texto libre |
