...
- Supresión puntual de memoria: `DELETE /v1/memory/{id}`.
- Supresión completa de la memoria del usuario autenticado: `DELETE /v1/memory` (solo afecta sus filas dentro de su tenant; devuelve `204`).
- Centro de privacidad: `GET /v1/privacy` muestra estos controles y declara explícitamente que el borrado completo de cuenta todavía requiere el flujo reforzado del operador.
...
- Exportación portable de datos de aplicación: `GET /v1/privacy/export` devuelve persona/configuración, conversaciones/mensajes, memoria, contactos, transacciones, recordatorios y metadatos de archivos del usuario autenticado en JSON versionado. Excluye credenciales, embeddings, argumentos de tools y claves S3. El export de categorías regulatorias adicionales puede ampliarse sin cambiar este contrato.
