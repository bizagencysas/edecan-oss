...
El IDE tampoco necesita un segundo proceso de companion. El runtime registra
un handler in-process en `ConnectionManager`: las rutas `/v1/ide/*` ya
autenticadas y gateadas llegan a las acciones reales de `edecan_companion`.
Archivos y editor quedan dentro del `sandbox_dir`; la terminal conserva la
allowlist de `allowed_commands`, usa `shell=False` y fija ese sandbox como
directorio de trabajo. Captura/teclado/mouse siguen siendo una superficie
separada y exigen el `session_id` de control remoto.
