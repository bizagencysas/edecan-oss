...
`python3.x`, evitando que sus permisos se mezclen con Referencia u otro proyecto.
...
persona haya habilitado. Edecán funciona como accesorio de la barra de menús:
no ocupa un espacio permanente en el Dock, incluso cuando su ventana está
abierta. Un clic izquierdo en el icono de Edecán en la barra
...
- Mantiene una sola instancia: volver a abrir Edecán recupera la ventana
  ...
  necesita adivinar entre Python, Terminal, Referencia u otro ejecutable.
...
- **macOS**: el repositorio público produce artefactos sin firmar. La firma y
  notarización pertenecen a la infraestructura privada de cada distribuidor;
  ninguna credencial ni automatización de cuenta de Apple se incluye aquí.
