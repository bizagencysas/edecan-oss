...
La forma más simple es **Ajustes → Avisos en tus teléfonos**. Allí eliges el
archivo `.p8` o el service account JSON desde tu equipo, completas los
identificadores y guardas. El navegador no persiste esos archivos en
`localStorage`: los envía una vez al backend autenticado, que valida la forma,
los cifra en el `TokenVault` y limpia el formulario. La misma pantalla muestra
si APNs/FCM están conectados, cuántos teléfonos registraron token y permite
activar o apagar cada categoría.

Para automatización o clientes propios, el contrato HTTP equivalente es:
...
APNs Provider API exige HTTP/2. El worker declara `httpx[http2]` y crea el
cliente de Apple con `http2=True`; no usa un fallback HTTP/1.1. Las pruebas
verifican esa configuración sin enviar una notificación real. La entrega real
solo puede comprobarse con la `.p8`, Bundle ID, entorno y token APNs del
dispositivo de cada instalación.
