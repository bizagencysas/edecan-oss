...
4. **El `opencode.json` que genera `ide_opencode_config.py` lleva el token de Cloudflare en
   ...
   del token.
5. **`chmod 0600` es no-op en Windows** (sin bits POSIX) -- restringirlo ahí necesitaría
   ACLs (`icacls`/`win32security`), no implementado ni probado (no hay Windows real desde
   esta Mac).
6. **La rama Windows de `ide_opencode_binario._argv_para_ejecutar`** (delegar en `cmd.exe`
   para binarios `.cmd`/`.bat`) solo se probó como lógica pura.
...
$ cd /Users/alguien/edecan && uv run ruff check apps/companion/
All checks passed!
```
