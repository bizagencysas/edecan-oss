...
```json
{
  "nombre_asistente": "Edecán Finanzas",
  "idioma": "es",
...
- **Borrar toda la memoria de un usuario**: `DELETE /v1/memory` vacía en una sola operación las filas del usuario autenticado dentro de su tenant. El borrado puntual sigue disponible en `DELETE /v1/memory/{id}`. El endpoint de cuenta completa y la cascada de todas las categorías de datos siguen siendo trabajos separados.
