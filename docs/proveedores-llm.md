...
class LLMProviderConfig:
    kind: str  # "workers_ai"|"anthropic"|"openai_compat"|"vertex"|"claude_cli"|"codex_cli"|"ollama"
...
`LLMRouter(settings, provider_config=mi_config)` construye el proveedor correspondiente y resuelve los alias `"principal"`/`"rapido"`/`"profundo"` según lo que traiga esa config (con un default sano por proveedor si falta algo — ver `edecan_llm.router.LLMRouter._config_models`). Sin `provider_config` (el valor por defecto, `None`), el router se comporta exactamente igual que antes de fase v3: resuelve desde `ANTHROPIC_API_KEY`/`OPENAI_COMPAT_*` de la plataforma (y `"profundo"` cae al mismo modelo que `"principal"`).
...
`WorkersAIProvider` (`kind="workers_ai"`) usa la cuenta del propio usuario.
La app pide `account_id`, un token con permisos Workers AI Read/Edit y el ID
completo del modelo (`@cf/autor/modelo`). La configuración se valida contra el
catálogo REST de Cloudflare y se cifra en el mismo `TokenVault` que los demás
proveedores; nunca usa silenciosamente la cuenta de otro tenant.
...
## Resolución de modelo (`"principal"`/`"rapido"`/`"profundo"`)
...
`LLMRouter.resolve("principal"|"rapido"|"profundo", tenant_flags)` prioriza esos
`provider_config.model_principal`/`model_rapido`/`model_profundo` fijados sobre cualquier
variable de entorno de plataforma. Si `model_profundo` no está fijado, cae al mismo modelo
que `"principal"` (no hay un tercer default hardcodeado por proveedor). Si un catálogo no
está disponible, `"principal"`/`"rapido"` caen a un default sano según el proveedor:
...
El downgrade a `"rapido"` cuando el plan del tenant no tiene el flag `models.premium` (`ARCHITECTURE.md` §10.13) aplica a `"principal"` **y también a `"profundo"`**, sin importar el proveedor elegido — un plan sin ese flag nunca paga el costo/latencia de un modelo profundo.

`reasoning_effort_profundo` (opcional, junto a `model_profundo`) fija el nivel de esfuerzo de razonamiento del modelo profundo cuando el proveedor lo soporta (`minimal`/`low`/`medium`/`high`/`xhigh`/`max`, según el proveedor — ver [`configuracion-minima.md`](./configuracion-minima.md)); si se omite, el proveedor usa su propio default.

## Privacidad, por proveedor

| Proveedor | Tus mensajes salen a... |
|---|---|
| Workers AI | A Cloudflare, bajo tu propia cuenta y API token. |
