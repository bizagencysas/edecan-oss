...
Este documento resume el modelo de amenazas de Edecán: qué se protege, de quién, qué puede salir mal (STRIDE) y qué mitigaciones existen hoy en el diseño. Es la contraparte técnica de [`../SECURITY.md`](../SECURITY.md), que cubre el proceso privado de reporte de vulnerabilidades.
...
## Frontera de autenticación y sesiones

- Las contraseñas usan Argon2id. El camino de email inexistente verifica un hash
  señuelo para reducir la señal temporal de enumeración, y los endpoints públicos
  de autenticación tienen límite configurable por ruta, IP e identidad hasheada en Redis.
- Cada par JWT incluye `jti` único y `sid` de sesión. El refresh se registra solo
  como hash de su token en Redis y se consume con `GETDEL`; dos intentos de uso del
  mismo refresh no pueden ganar a la vez. Logout elimina esa credencial server-side.
- Antes de rotar, la API relee usuario, tenant, membresía, estado y plan desde
  PostgreSQL. El access token conserva el plan firmado hasta 30 minutos; los flags
  siempre se derivan del catálogo server-side y nunca de flags enviados por el cliente.
- Web limita ambos JWT a `sessionStorage` (se borran al cerrar la pestaña) y aplica
  CSP/headers defensivos. iOS usa Keychain y Android almacenamiento cifrado.

**Riesgo residual conocido**: mientras la web use Bearer desde JavaScript, un XSS
activo en esa misma pestaña podría leer sus tokens. La CSP estricta y la ausencia de
persistencia entre sesiones reducen la probabilidad y duración, pero una futura capa
BFF/cookie `HttpOnly` sería una defensa más fuerte. No se presenta este límite como
...
- **Confirmación humana obligatoria en herramientas peligrosas**: cualquier `Tool` con `dangerous=True` (`enviar_correo`, `publicar_social` y, en la extensión comercial externa `edecan_premium`, `llamar_contacto`, `enviar_sms`, `lanzar_campana`) no se ejecuta automáticamente — el agente emite `confirmation_required` y detiene el turno hasta que el usuario apruebe explícitamente vía `POST /v1/conversations/{id}/confirm` (`ARCHITECTURE.md` §10.7). Esto es la barrera principal: aunque el LLM sea manipulado para *decidir* ejecutar una acción dañina, no puede *ejecutarla* sin que un humano la confirme en ese momento.
...
| `crear_podcast` (`edecan_creative`) | `tools.podcast` | `voz_avanzada.py::_require_tools_podcast` (`POST /v1/voz/podcasts`, fase v6 — aterrizó *durante* esta misma sesión de trabajo; verificado tras el landing) | ídem `[podcast:*]` |
...
| `delegar_mision` (`edecan_agents`) — solo lectura, responsable de la fase v6 | `agents.missions` | `missions.py::_require_agents_missions` | ídem `[agents:*]` |
| `vehiculo_estado`/`vehiculo_controlar` (`edecan_vehicles`) — solo lectura, fuera de alcance permanente (`docs/roadmap.md`) | `tools.vehicles` | `vehiculos.py::require_vehicles_flag` | ídem `[vehicles:*]` |
| `llamar_contacto`/`enviar_sms` (`edecan_premium`) — solo lectura, responsable de la fase v6 | `voice.telephony` | **Dos** routers: `consents.py::_require_voice_telephony` y `connectors.py::_require_voice_telephony` | ídem `[premium:*]` |
...
**Dónde vivía**: `packages/agents/edecan_agents/tools.py::DelegarMisionTool.run()`. Revisaba el flag base `agents.missions` (vía `requires_flags`, ya realmente exigido al ejecutar desde el fix del Hallazgo 1) pero nunca `LIMIT_MISSIONS_PER_DAY` — a diferencia de `POST /v1/missions` (`missions.py::_check_missions_quota`, fase v6), que sí lo revisa antes de encolar el mismo job `run_mission`. Un tenant con `agents.missions=True` (p. ej. `hosted_pro`, límite 20/día, o `hosted_business`, límite 100/día) podía seguir creando misiones sin límite por chat aunque ya había agotado su cupo diario del plan (cada una dispara un turno completo de agente headless en el worker — no es gratis para la plataforma). Ninguna sesión de trabajo previa lo había cerrado: quedó pendiente de triage tras fase v6 (`packages/agents/` no estaba en las rutas que ese WP podía escribir) y pinneado con un test que hasta ahora PASABA confirmando el hueco, no cerrándolo.
...
Los riesgos explotables se reportan mediante el proceso privado de [`../SECURITY.md`](../SECURITY.md). Las limitaciones no sensibles y el trabajo pendiente se mantienen en el roadmap y en issues públicos, sin publicar detalles que faciliten una explotación antes de corregirla.
