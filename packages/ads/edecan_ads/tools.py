...
class AdsResumenTool(Tool):
    ...
    async def run(self, ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(content=f"No pude consultar Meta Ads: {exc}", is_error=True)
        ...
        # Antes `data` no llevaba ninguna marca de demo/real (a diferencia del texto, que
        # SÍ menciona "modo offline" pero solo dentro del nombre de la campaña de ejemplo,
        # `StubAdsProvider.list_campaigns`): "gasto 0, 0 impresiones, 0 clics" es
        # indistinguible de una cuenta real en pausa si algo pinta `data.metricas` sin leer
        # el nombre de cada campaña. `source_mode` lo hace explícito en el propio dato.
        # Fail-closed (mismo criterio que `edecan_core.veracidad`): solo el proveedor real
        # declarado (`MetaAdsProvider.name == "meta"`) cuenta como "live"; cualquier otra
        # cosa —incluido `StubAdsProvider` y cualquier proveedor inyectado que no declare
        # `name`— se trata como "demo", nunca al revés.
        source_mode = "live" if str(getattr(provider, "name", "")) == "meta" else "demo"
        if source_mode == "demo":
            resumen = (
                "AVISO: no hay cuenta de Meta Ads conectada — lo que sigue son datos de "
                "EJEMPLO, no tus campañas ni tu gasto real.\n\n" + resumen
            )
        return ToolResult(
            content=resumen,
            data={"campanas": campanas, "metricas": metricas, "source_mode": source_mode},
        )
