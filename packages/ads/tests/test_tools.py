...
async def test_ads_resumen_sin_nombre_de_proveedor_se_marca_demo_fail_closed(make_ctx):
    """`_FakeProvider` (como cualquier proveedor inyectado que no declare `.name`) no debe
    poder pasar por `"live"` — fail-closed, mismo criterio que `edecan_core.veracidad`: solo
    el proveedor real declarado (`MetaAdsProvider.name == "meta"`) cuenta como real."""
    provider = _FakeProvider(campanas=[{"name": "Campaña A", "status": "PAUSED"}])
    tool = AdsResumenTool(provider_resolver=_resolver_fijo(provider))
    ...
    assert "no hay cuenta de Meta Ads conectada" in resultado.content


async def test_ads_resumen_con_provider_name_meta_se_marca_live(make_ctx):
    provider = _FakeProvider(campanas=[{"name": "Campaña real", "status": "ACTIVE"}])
    provider.name = "meta"
    tool = AdsResumenTool(provider_resolver=_resolver_fijo(provider))
    ...
    assert "AVISO" not in resultado.content
