...
def test_step_provenance_es_estable_y_no_guarda_prompt_ni_argumentos():
    perfil = PROFILES["research"]
    primero = _step_provenance(
        perfil,
        instruccion="Busca el informe de mercado",
        tools=["buscar_web", "buscar_web", "hora_actual"],
    )
    segundo = _step_provenance(
        perfil,
        instruccion="Busca el informe de mercado",
        tools=["hora_actual", "buscar_web"],
    )

    assert primero == segundo
    assert primero["agent_profile"] == "research"
    assert primero["model_alias"] == perfil.model_alias
    assert primero["tools_requested"] == ["buscar_web", "hora_actual"]
    assert primero["prompt_hash"] != "Busca el informe de mercado"
    assert "args" not in primero


@pytest.mark.parametrize(
    ("message", "expected"),
    [
        ("La herramienta no está disponible", "tool"),
        ("El proveedor devolvió 429", "provider"),
        ("No hay permiso para continuar", "permission"),
        ("Fallo inesperado", "unknown"),
    ],
)
def test_classify_failure_es_conservador(message: str, expected: str) -> None:
...
async def test_paso_exitoso_persiste_started_at_y_finished_at_en_usage(
    ...
    assert usage["provenance"]["agent_profile"] == "research"
    assert usage["provenance"]["model_alias"] == PROFILES["research"].model_alias
    assert usage["provenance"]["tools_requested"] == []
