...
- `describe.py` — `describe_rrule(rrule)`: la regla en cristiano ("Todos los
  días a las 4:00 UTC", "Cada 15 minutos"), o `None` si no se puede traducir
  con certeza. Vive acá y no en cada cliente porque la frase lleva HORA y sólo
  el servidor sabe en qué zona corre la regla (`compute_next_run` ancla en UTC)
  — `routers/automations.py` la publica en `trigger.descripcion` y web/iOS/chat
  sólo la pintan. Ver `docs/automatizaciones.md`, "La agenda en cristiano".
