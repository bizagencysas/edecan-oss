...
zona corre de verdad la regla es este backend —
`engine.compute_next_run` le pasa a `dateutil` un `dtstart` tz-aware **UTC**,
así que `BYHOUR=4` significa las 04:00 UTC y nada más—; ni la web ni el
teléfono pueden deducirlo del JSON. Por eso `routers/automations.py`
`_public_automation` publica el texto ya armado en `trigger.descripcion` y
las dos superficies solo lo pintan: una sola implementación, una sola
tabla de verdad sobre la zona.

**Por qué decimos "UTC" en vez de convertir a la hora del teléfono.**
...
"""Zona en la que `engine.compute_next_run` evalúa toda `rrule` (ancla
`dtstart` tz-aware UTC). Si algún día las automatizaciones pasan a correr en
la zona del tenant, este módulo y `engine` cambian juntos."""
...
def _frase_horaria(partes: dict[str, str]) -> str | None:
    """Sufijo de hora, ya con su zona: `" a las 9:00 UTC"`. `""` cuando la
    regla no fija ninguna hora (no hay nada que afirmar), `None` cuando la
    fija de una forma que no sabemos decir sin inventar."""
    ...
    if minutos_crudos is None:
        ...
        if len(horas) != 1:
            return None
        hora = horas[0]
        return f" entre las {hora}:00 y las {hora}:59 {SCHEDULE_TIMEZONE}"

    minutos = _lista_de_enteros(minutos_crudos, minimo=0, maximo=59)
    if minutos is None or len(minutos) != 1:
        return None
    minuto = minutos[0]
    return f" a las {_enumerar([f'{h}:{minuto:02d}' for h in horas])} {SCHEDULE_TIMEZONE}"


def describe_rrule(rrule: str) -> str | None:
    ...
    La hora que devuelve es la del SERVIDOR (`SCHEDULE_TIMEZONE`) y viene
    rotulada como tal; nunca se convierte a la zona de quien lee.
    """
    partes = _parse(rrule)
    if partes is None:
        return None
