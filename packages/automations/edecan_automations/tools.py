...
from .describe import describe_rrule
...
def _en_cristiano(rrule: str) -> str:
    """La agenda en minúscula, lista para encajar detrás de "corre ..." — en el
    chat vale el mismo criterio que en las pantallas: la `rrule` cruda no le
    dice nada a nadie. Si `describe_rrule` no puede traducirla con certeza cae
    a la regla tal cual, nunca a una hora aproximada."""
    frase = describe_rrule(rrule)
    if frase is None:
        return f"según la regla '{rrule}'"
...
class GestionarAutomatizacionTool(Tool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "rrule": {
                "type": "string",
                "description": (
                    "Regla de recurrencia RFC 5545, ej. 'FREQ=DAILY;BYHOUR=9;BYMINUTE=0' "
                    "(para 'crear'). Las horas son UTC. Incluye SIEMPRE BYMINUTE junto con "
                    "BYHOUR: sin él, RFC 5545 hereda el minuto del momento del alta, así que "
                    "'FREQ=DAILY;BYHOUR=9' creada 9:39 corre a las 9:39, no a las 9:00. "
    ...
    async def _crear(self, ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
        ...
        return ToolResult(
            content=f"Creé la automatización «{nombre}»: corre {_en_cristiano(rrule)}.",
            data={
    ...
    async def _listar(self, ctx: ToolContext) -> ToolResult:
        ...
        for i, fila in enumerate(filas, start=1):
            ...
            detalle_trigger = (
                f"corre {_en_cristiano(str(trigger.get('rrule') or ''))}"
                if es_agenda
                else "se dispara por webhook"
            )
