...
async def test_listar_con_automatizaciones(make_ctx, make_session) -> None:
    ...
    assert "Reporte diario" in resultado.content
    assert "corre todos los días" in resultado.content  # no la rrule cruda
    assert resultado.data["automatizaciones"][0]["trigger"]["rrule"] == "FREQ=DAILY"


async def test_listar_cae_a_la_regla_cruda_si_no_sabe_traducirla(
    make_ctx, make_session
) -> None:
    """Antes que una hora inventada, la regla tal cual — mismo criterio que
    las pantallas (`edecan_automations.describe`)."""
    session = make_session(
        [
            [
                {
    ...
    assert "FREQ=MONTHLY;BYDAY=FR;BYSETPOS=-1" in resultado.content
