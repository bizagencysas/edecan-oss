...
def _info_fidelidad_cotizacion(proveedor: QuoteProvider) -> InfoFidelidad:
    """`ToolResult.fidelidad` para `cotizar_activo`/`preparar_orden` — mismo contrato de
    veracidad que `edecan_voice.tools._info_fidelidad_tts` (`edecan_core.veracidad`).

    `StubQuotes`/`CoinGeckoQuotes` (los dos proveedores de este paquete) heredan
    `ProveedorDeclarado` y siempre tienen `info_fidelidad()`. Fail-closed para cualquier
    `QuoteProvider` inyectado por duck typing que no declare el contrato (mismo criterio que
    voz): se trata como SIMULADO en vez de asumir que es real sin haberlo comprobado.
    """
    if isinstance(proveedor, ProveedorDeclarado):
        return proveedor.info_fidelidad()
    return InfoFidelidad(
        familia="cotizaciones",
        fidelidad=Fidelidad.SIMULADO,
        fuente=type(proveedor).__name__,
        motivo_simulado=(
            "el proveedor de cotizaciones usado no declara si es real o simulado (ver "
...
class CotizarActivoTool(Tool):
    name = "cotizar_activo"
    description = (
        "Consulta la cotización actual de un criptoactivo (BTC, ETH, SOL y otros — ver "
        "'símbolos soportados' si falla). NO cubre acciones ni otros mercados: si se pide un "
        "símbolo que no es cripto, la tool falla explicando que no hay fuente conectada para "
        "eso, nunca inventa un precio. Es solo información de mercado — nunca un consejo de "
        "inversión ni una operación."
    )
    input_schema = {
        "type": "object",
        "properties": {
            "simbolo": {
                "type": "string",
                "description": (
                    "Símbolo del criptoactivo a cotizar, p. ej. 'BTC', 'ETH', 'SOL'. No "
                    "acciones ni otros mercados."
                ),
            },
        },
        "required": ["simbolo"],
    ...
    async def run(self, ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
        ...
        if not simbolo:
            return ToolResult(
                content="Error: necesito el símbolo del activo a cotizar (p. ej. 'BTC').",
                is_error=True,
            )
        ...
        try:
            ...
            # Nunca un precio inventado cuando la fuente real no cubre el símbolo (p. ej.
            # una acción contra CoinGecko): el mensaje de la excepción de cada
            # `QuoteProvider` ya dice qué falta (ver `CoinGeckoQuotes.quote`), y `is_error`
            # deja explícito para el modelo que esto NO es una cotización, es un fallo.
            return ToolResult(content=f"No pude cotizar '{simbolo}': {exc}", is_error=True)

        fidelidad = _info_fidelidad_cotizacion(proveedor)
        return ToolResult(
            content=(
...
class PrepararOrdenTool(Tool):
    ...
    input_schema = {
        ...
        "properties": {
            "simbolo": {
                "type": "string",
                "description": "Símbolo del criptoactivo, p. ej. 'BTC'. No acciones.",
            },
            "lado": {
                "type": "string",
                "enum": ["buy", "sell"],
    ...
    async def run(self, ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            # Mismo criterio que `CotizarActivoTool`: nunca crear un borrador con un precio
            # inventado cuando la fuente real no cubre el símbolo.
            return ToolResult(content=f"No pude cotizar '{simbolo}': {exc}", is_error=True)
        fidelidad = _info_fidelidad_cotizacion(proveedor)
