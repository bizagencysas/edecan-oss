...
no activó todavía, y publicar se rompe en silencio. Portado de Aria
(``features/acme_social.py:1917``, que la fija en ``"202606"`` por la misma razón).
...
``_linkedin_publish``/``_conectar_linkedin_acme`` de Aria
(``features/acme_social.py``): ``create_post`` acepta un ``org_urn`` opcional que,
...
``organizationAcls`` que Aria para descubrir esas páginas. A diferencia de Aria
...
# Mismo valor que Aria (`features/acme_social.py:1917`) porque es la versión que
...
async def get_organization_urns(http: httpx.AsyncClient, bundle: TokenBundle) -> list[str]:
    ...
    Portado de Aria (``features/acme_social.py::_conectar_linkedin_acme``): no
...
async def upload_image(
    ...
    *,
    ...
    await _esperar_imagen_disponible(http, bundle, image_urn)
    return image_urn


# Estados de una imagen en `GET /rest/images/{urn}`. `AVAILABLE` es el único con el que
# se puede adjuntar a un post.
_IMAGEN_LISTA = "AVAILABLE"
_IMAGEN_ESPERA_MAX_SEGUNDOS = 30.0
_IMAGEN_ESPERA_INTERVALO = 1.0


async def _esperar_imagen_disponible(
    http: httpx.AsyncClient, bundle: TokenBundle, image_urn: str
) -> None:
    """Espera a que LinkedIn TERMINE de procesar la imagen antes de adjuntarla.

    El bug que esto arregla, medido en vivo (31-jul-2026): el `PUT` de los bytes
    devuelve 201 al instante, pero la imagen queda en `WAITING_UPLOAD` y solo pasa a
    `AVAILABLE` uno o dos segundos después. Adjuntar el URN en ese hueco hace que
    `POST /rest/posts` responda **201 con un `x-restli-id` válido y el post NUNCA
    aparezca en la página** -- LinkedIn lo acepta y lo descarta en silencio.

    Es el mismo síntoma que causó el token sin permiso de organización, y por eso costó
    tanto encontrarlo: dos causas distintas, un único síntoma indistinguible de un éxito.
    La medición que lo separó:

        t=0s (justo tras el PUT) -> WAITING_UPLOAD
        t=2s                     -> AVAILABLE

    Se sale sin ruido si la consulta de estado no está disponible (403 sin permiso de
    lectura, o un endpoint que cambie): en ese caso se sigue adelante como antes, porque
    bloquear la publicación por no poder MIRAR el estado sería peor que el bug. La
    verificación posterior del post (`_verify_post`) es la red que atrapa el resto.
    """
    espera = 0.0
    url = f"{IMAGES_URL}/{quote(image_urn, safe='')}"
    while espera < _IMAGEN_ESPERA_MAX_SEGUNDOS:
        try:
            respuesta = await http.get(url, headers=_headers(bundle, json=False))
        except httpx.HTTPError:
            return
        if respuesta.status_code >= 400:
            # Sin permiso para consultar el estado: se sigue como antes. No es motivo
            # para no publicar.
            return
        try:
        ...
        if not estado:
            return
        await asyncio.sleep(_IMAGEN_ESPERA_INTERVALO)
        espera += _IMAGEN_ESPERA_INTERVALO
    logger.warning(
        "linkedin: la imagen %s no llegó a %s en %.0fs; se adjunta igual.",
        image_urn,
        _IMAGEN_LISTA,
        _IMAGEN_ESPERA_MAX_SEGUNDOS,
    )
...
async def verify_post(
    ...
    return "unknown", (
        f"LinkedIn respondió {response.status_code} al intentar confirmar el post."
    )
...
async def create_post(
    ...
    *,
    ...
    persona para publicar como organización, igual que ``_linkedin_publish`` de Aria
    (``features/acme_social.py``).
    ...
    verified, verification_note = await verify_post(http, bundle, post_id)
    if verified == "not_found":
        raise ConnectorError(
            f"LinkedIn aceptó la publicación pero al releer el post ({post_id}) responde "
            "que no existe -- no se publicó nada de verdad. Esto pasa cuando el token no "
            f"tiene permiso real para publicar como {author}; revisa los scopes de la "
            "cuenta conectada."
        )
