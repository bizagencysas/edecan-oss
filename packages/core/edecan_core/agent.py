...
class Agent:
    ...
    def _resolver_proveedor(
        ...
        if metadata is None:
            return self._llm_router.resolve(self._model_alias, flags)
        return self._llm_router.resolve(self._model_alias, flags, metadata=metadata)
    ...
    async def _run_turn(
        ...
        *,
        ...
        selected_specs = select_tool_specs(
            ...
            # `recent_user_texts` es una heurística de palabras y se queda corta
            # justo cuando el usuario contesta seguido: el mensaje que traía las
            # palabras clave se sale de la ventana. Esto no es heurística — es
            # quién preguntó, y contestar una pregunta siempre es continuar.
        ...
        provider, model = self._resolver_proveedor(flags, seleccion)
    ...
    async def _resume_turn(
        ...
        *,
        ...
        unavailable = [
            (call.name, reason) for call, tool, reason in resolved_calls if tool is None
        ]
        if unavailable:
    ...
    async def _continue_turn(
        ...
        *,
        ...
        # Presupuesto por iteración: sale del Esfuerzo del selector, y sin
        # selección es el 4096 de siempre (ver `_max_tokens_por_esfuerzo`).
            ...
            async for chunk in provider.stream(request):
                if chunk.type == "text" and chunk.text:
                    ...
                    iteration_input_tokens = max(
                        iteration_input_tokens, chunk.usage.input_tokens
                    )
...
def _cierre_de_emergencia(tool_log: list[dict[str, Any]]) -> str:
    ...
    fallos = [
        ...
        if str(evento.get("result_preview") or "").lstrip().lower().startswith(
            ("error:", "falló", "no se pudo", "no encontré")
        )
    ]
    if fallos:
