...
def select_tool_specs(
    ...
    *,
    ...
    # Hotel/vuelo: API nativa (cards), no Booking en la Mac ni un párrafo
    # inventado con `buscar_web`.
    if tokens.intersection({"hotel", "hoteles", "vuelo", "vuelos", "booking", "trivago"}):
        selected_names.discard("usar_computadora")
        selected_names.discard("buscar_web")
        selected_names.discard("navegar_web")
        selected_names.discard("extraer_datos_web")
...
def build_slash_command_guidance(user_text: str, *, language: str) -> str:
    ...
    if language == "en":
        return {
            ...
            # `/clear` is normally intercepted by the client BEFORE it ever
            # reaches here (it calls `POST /{conversation_id}/clear` directly,
            # no model turn involved -- see that endpoint's docstring). Seeing
            # it as plain text means an older or third-party client sent it as
            # a normal message instead. There is no tool for this: do not
            # claim the context was reset, do not invent that anything
            # happened. Say plainly that resetting needs an app update, and
            # answer nothing else from this message.
            "clear": (
                "The user's message is literally the text /clear. No tool exists to act on it "
                "from here, and no context was reset. Do not claim otherwise. Tell the person "
                "plainly that clearing needs a newer app version, and do not answer anything "
                "else as if it were a real request."
            ),
        }[command]
    return {
        "fix": (
        ...
        # `/clear` normalmente lo intercepta el cliente ANTES de que llegue
        # hasta acá (llama directo a `POST /{conversation_id}/clear`, sin
        # pasar por un turno del modelo -- ver el docstring de ese endpoint).
        # Verlo como texto plano significa que un cliente viejo o de terceros
        # lo mandó como mensaje normal. No hay ninguna tool para esto: no
        # afirmes que el contexto se reinició ni inventes que pasó algo. Dile
        # a la persona, con claridad, que limpiar necesita actualizar la app.
        "clear": (
            "El mensaje de la persona es literalmente el texto /clear. Desde acá no hay ninguna "
            "tool para actuar sobre eso, y no se reinició ningún contexto. No afirmes lo "
            "contrario. Dile con claridad que limpiar necesita una versión más nueva de la app, "
            "y no respondas nada más como si fuera un pedido real."
        ),
    }[command]
...
def _slash_command(text: str) -> str | None:
    match = re.match(r"^\s*/(fix|oss|changes|clear)(?:\s|$)", text, flags=re.IGNORECASE)
...
Hoteles y vuelos se buscan SOLO con `buscar_hoteles` / `buscar_vuelos`.
Esas tools ya pintan cards (foto, precio, dirección). No uses `buscar_web`,
no listes hoteles en un párrafo y no inventes teléfonos ni precios.
Si solo dan una fecha, check-out es el día siguiente.
No abras Booking ni Safari en la computadora: la persona está en el teléfono.
...
Hotels and flights go ONLY through `buscar_hoteles` / `buscar_vuelos`.
Those tools already render cards (photo, price, address). Do not use `buscar_web`,
do not list hotels in a paragraph, and do not invent phones or prices.
If they give only one date, check-out is the next day.
Do not open Booking or Safari on the computer: the person is on their phone.
