...
def construir_card_generica(
    *,
    ...
    video_alt: str = "",
    video_aspecto: AspectoImagen = "panoramica",
    ...
    (Alex, corrección del 31-jul: "haz que la imagen se vea en 500x500 para poder
    verla bien, así se ve encogida en la app").

    `video` es un ``ArtifactRef`` de un MP4: si viene, se emite un ``VideoNode``
    (reproducible en el chat con AVPlayer) EN LUGAR de la imagen. El video se
    prefiere para el chat; la imagen queda como fallback cuando no hay video. La
    publicación a LinkedIn sigue usando `imagen` (la subida de video a LinkedIn es
    un paso posterior fuera del alcance actual), así que este helper NO quita la
    imagen del contrato de publicación -- solo decide cuál nodo pinta la card.
    ...
    `badge` -> (video si lo hay, si no `imagen`) -> líneas de `cuerpo` -> (si
    hay `botones`) un `divisor` y una fila horizontal de botones.
    ...
    # Video se prefiere para el chat; imagen queda como fallback de publicación
    # y de visualización cuando no hay video.
    if video is not None:
        hijos.append(VideoNode(artifact=video, alt=video_alt, aspecto=video_aspecto, esquinas="m"))
