...
def assess_freshness(text: str) -> FreshnessDecision:
    ...
    # ORDEN EXPLÍCITA de buscar: "búscame eso en internet", "busca en la web/google".
    # No es una heurística de frescura -- es la persona pidiendo la búsqueda con todas
    # sus letras, y sin este disparador el modelo la ACTUABA en vez de ejecutarla
    # (caso real, 02-ago-2026: "## Resultado de búsqueda [Buscaré en Internet]" escrito
    # como texto, con una respuesta inventada de memoria debajo). También dispara la
    # intención de compra con dato vivo ("¿dónde lo venden?", "si lo venden en X",
    # "cuánto cuesta"), que solo se responde honestamente con la web de hoy.
    verbo_buscar = tokens & {"busca", "buscame", "buscar", "buscalo", "buscala", "googlea"}
    medio_web = tokens & {"internet", "web", "google", "linea", "online"}
    if verbo_buscar and medio_web:
        return FreshnessDecision(True, "el usuario pidió buscar en internet")
    if re.search(
        r"\b(?:donde|si|quien(?:es)?)\s+(?:lo|la|los|las)\s+vend\w+|\bdonde\s+compr\w+"
        r"|\bcuanto\s+(?:cuesta|vale)\b",
        normalized,
    ):
        return FreshnessDecision(True, "la petición pide disponibilidad o precio vivo")
