    ...
    MappingProxyType(
        {
            ...
            "automation_completed": (
                ...
                "Una automatización se ejecutó. Abre Edecán para ver el resultado.",
                "activity",
            ),
            "automation_failed": (
                "work",
                "Automatización con error",
                "Una automatización necesita tu atención. Abre Edecán para revisarla.",
                "activity",
            ),
            "reminder_triggered": (
                "work",
...
_DELIVERY_ACTION = "notifications.push.delivery"

# `reason` de APNs ("BadDeviceToken", "TooManyProviderTokenUpdates", ...) y el
# nombre de clase de una excepción de red son vocabulario del PROVEEDOR, no
# texto de la persona. Aun así se saneta a `[A-Za-z0-9_.-]` y se corta: nadie
# puede colar un cuerpo de error con datos dentro de `audit_log` por accidente.
_DELIVERY_REASON_MAX_CHARS = 64
...
def _sanear_motivo_entrega(raw: Any) -> str | None:
    if raw is None:
        return None
    texto = "".join(char for char in str(raw) if char.isalnum() or char in {"_", ".", "-"}).strip(
        ".-_"
    )
    if not texto:
        return None
    return texto[:_DELIVERY_REASON_MAX_CHARS]
...
async def record_push_delivery(
    ...
    *,
    ...
    """Deja en la BASE lo que respondió el proveedor de push.

    POR QUÉ en `audit_log` y no en un log de texto: en la instalación de
    escritorio el worker corre como sidecar de Tauri
    (`apps/desktop/src-tauri/src/backend.rs`), que redirige su stdout/stderr a
    los descriptores del proceso padre — en una app empaquetada eso es
    `/dev/null`. Todo lo que `edecan_worker.push` contaba con `logger.warning`
    (un 403, un 429, una excepción de red) se perdía sin dejar rastro, así que
    "no me llegó la notificación" era indistinguible de "Apple contestó 200".
    Una fila por intento es lo único que permite responder esa pregunta
    después, y es lo que tiene que existir ANTES de creerle a cualquier otro
    arreglo de push.

    Mantiene la misma promesa de privacidad que `record_notification_event`:
    no entra ni el título, ni el cuerpo, ni el push token — solo enums,
    identificadores opacos y el `reason` del proveedor ya saneado.
    """
    meta: dict[str, Any] = {
        "version": 1,
        "platform": platform,
    ...
    if status_code is not None:
        meta["status_code"] = int(status_code)
    motivo = _sanear_motivo_entrega(reason)
    if motivo is not None:
        meta["reason"] = motivo
    if provider_message_id is not None:
        identificador = _sanear_motivo_entrega(provider_message_id)
        if identificador is not None:
            meta["provider_message_id"] = identificador
    ...
    await session.execute(
        ...
        ),
        {
            ...
            "target": f"push:{device_id}" if device_id is not None else f"push:{user_id}",
            "meta": _json(meta),
        },
    )
...
_BRIEF_DELIVERY_ACTION = "notifications.brief.delivery"


async def record_daily_brief_delivery(
    session: Any, *, tenant_id: UUID, user_id: UUID, brief_key: str
) -> bool:
    """Registra de forma idempotente la entrega de un brief proactivo.

    ``brief_key`` identifica la OCURRENCIA concreta del brief (p. ej. la fecha
    ISO del día, ``"2026-08-20"``) — un token corto controlado por el llamador,
    no texto libre de usuario. Devuelve ``True`` la primera vez (la entrega
    debe realizarse) y ``False`` si ya existe una fila para esa clave: un
    redelivery de SQS o un doble disparo del scheduler no debe enviar el mismo
    brief dos veces (PHASE2 §55, "presupuesto de atención").

    Mismo patrón transaccional que :func:`record_notification_event`: advisory
    lock por clave + ``target`` único ``brief:<user_id>:<brief_key>``, de modo
    que la terna ``tenant_id + user_id + brief_key`` es idempotente incluso con
    dos workers concurrentes. No persiste el contenido del brief — solo la
    marca de que se entregó.
    """
    lock_key = f"{tenant_id}:{user_id}:{brief_key}"
    await session.execute(
        sql("SELECT pg_advisory_xact_lock(hashtextextended(:lock_key, 0))"),
        {"lock_key": lock_key},
    )
    target = f"brief:{user_id}:{brief_key}"
    existing = await session.execute(
        sql(
            """
            SELECT id
        ...
        {"tenant_id": tenant_id, "action": _BRIEF_DELIVERY_ACTION, "target": target},
    )
    row = existing.mappings().first()
    if row is not None:
        return False
    await session.execute(
        ...
        ),
        {
            ...
            "tenant_id": tenant_id,
            "user_id": user_id,
            "action": _BRIEF_DELIVERY_ACTION,
            "target": target,
            "meta": _json({"version": 1}),
        },
    )
    return True
