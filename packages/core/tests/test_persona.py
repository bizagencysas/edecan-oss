...
def test_linkedin_es_capacidad_creativa_y_no_una_prohibicion_del_prompt():
    ...
    assert "esta es la mac del dueño" in prompt_lower
    ...
    assert "this is the owner's computer" in prompt_en_lower
...
def test_core_identity_es_el_texto_canonico_entregado_sin_reescrituras():
    ...
    assert len(CORE_IDENTITY_ES.splitlines()) == 82
    assert hashlib.sha256(CORE_IDENTITY_ES.encode()).hexdigest() == (
        "6b6acd5c87d32d822cc9a7b4dd235339d954c1f31ebcdf99ee8ce8a430a731c6"
    )
...
def test_core_identity_ensena_el_tono_con_ejemplos_y_no_con_adjetivos():
    ...
    assert '"señor"' in core


def test_core_identity_prohibe_ofrecerse_en_vacio_y_fija_es_ve_sin_voseo():
    """Petición directa del dueño (núcleo 2.2).

    Textual: "que elimine eso de '¿Con qué quieres que te ayude?'... que
    simplemente hable conmigo como si fuera una persona sin necesidad de decir
    que está para ayudarme a cada rato". La familia completa de coletillas de
    asistente queda nombrada y prohibida en el núcleo —no en una nota al final—
    y el registro es es-VE con tuteo y cero voseo (también corregido por el
    dueño varias veces y con énfasis).
    """
    core = CORE_IDENTITY_ES.lower()

    # La prohibición nombra las frases exactas: un "sé natural" genérico no
    # cambia nada; la lista sí.
    assert "nunca te ofrezcas en vacío" in core
    assert "¿con qué quieres que te ayude?" in core
    assert "¿necesitas algo más?" in core
    assert "estoy para ayudarte" in core
    assert "cuando la respuesta termina, el mensaje termina" in core

    # Un saludo se contesta conversando, no con menú de servicios, y el
    # ejemplo MAL usa la coletilla real para que el modelo la reconozca.
    assert "menú de servicios" in core
    assert "¿en qué puedo ayudarte hoy?" in core

    # Cercanía sin perder profesionalidad: lo malo se dice de frente.
    assert "cercano no es blando" in core

    # es-VE, tuteo, cero voseo — con las formas prohibidas nombradas.
    assert "es-ve" in core
    assert "nunca voseo" in core
    for voseo in ('"vos"', '"querés"', '"tenés"', '"podés"', '"decime"'):
        assert voseo in core
...
def test_arquitectura_cognitiva_separa_nucleo_de_modulos_versionables():
    ...
    assert DEFAULT_COGNITIVE_ARCHITECTURE.version == "2.5"
...
def test_prompt_no_exige_speech_tags():
    prompt = build_system_prompt(PersonaConfig(), []).lower()
    assert "no uses etiquetas de prosodia ni speech tags" in prompt

    prompt_en = build_system_prompt(PersonaConfig(idioma="en"), []).lower()
    assert "do not use prosody tags or speech tags" in prompt_en
