...
def _slug_pilar(texto: str, indice: int) -> str:
    """Id determinista y legible para un pilar de contenido de un tenant: minúsculas,
    sin acentos, `_` como separador. Nunca vacío -- cae a un índice si el texto no deja
    ningún carácter alfanumérico (p. ej. un pilar escrito solo con emoji o símbolos)."""
    normalizado = texto.strip().lower()
    for con_acento, sin_acento in (
        ("á", "a"), ("é", "e"), ("í", "i"), ("ó", "o"), ("ú", "u"), ("ñ", "n")
    ):
        normalizado = normalizado.replace(con_acento, sin_acento)
    slug = re.sub(r"[^a-z0-9]+", "_", normalizado).strip("_")
    return slug or f"pilar_{indice}"


def territorios_desde_pilares(pilares: Sequence[str]) -> list[Territorio]:
    """Convierte los `content_pillars` de un tenant (texto libre, sin estructura) en una
    lista de `Territorio` que `siguiente_paso`/`territorio_actual` pueden rotar.

    Existe para cerrar un hueco real: sin esto, la rotación "sin tema pedido" de
    `redaccion.CrearPostLinkedInTool` SIEMPRE caía en `TERRITORIOS_POR_DEFECTO` -- un
    catálogo genérico (tecnología, finanzas, salud...) sin ninguna relación con la voz
    de la cuenta que el tenant configuró -- así que "escríbeme el post de hoy" nunca
    tocaba en verdad los pilares propios de esa cuenta. El único post que sonaba a la
    cuenta era el que llegaba con un `tema` explícito puesto a mano por quien orquesta
    el chat (una ocurrencia suya, no la rotación del motor), y de ahí salía la queja
    real: "esto es lo mismo que ya te mandé, no está rotando".

    Cada pilar se vuelve su propio territorio con `etiqueta`/`pilar` IDÉNTICOS al texto
    que escribió el tenant -- nunca se le agrega aquí un ejemplo de post ni una
    instrucción de estructura de más: eso ya lo aportan el resto del prompt del
    escritor (voz, formatos preferidos, banco de contexto), y mezclar un texto MODELO
    en el prompt de rotación es exactamente lo que produce clones. `query` reusa ese
    mismo texto como búsqueda: no hay forma genérica de saber si el pilar de un tenant
    cualquiera es noticiable o no, así que se deja que la red de seguridad de
    `redaccion._reunir_contexto` decida -- si la búsqueda no encuentra nada (el caso
    normal para una escena de vida cotidiana en vez de una noticia), cae sola al banco
    de contexto real que el tenant configuró.

    `pilares` vacío o sin ningún texto usable devuelve `[]`; el llamador decide el
    fallback (pasar `territorios=None`/omitirlo a `siguiente_paso` cae a
    `TERRITORIOS_POR_DEFECTO`, ver su firma)."""
    resultado: list[Territorio] = []
    vistos: set[str] = set()
    for indice, crudo in enumerate(pilares):
        etiqueta = str(crudo or "").strip()
        if not etiqueta:
            continue
        base = _slug_pilar(etiqueta, indice)
        id_unico = base
        sufijo = 2
        while id_unico in vistos:
            id_unico = f"{base}_{sufijo}"
            sufijo += 1
        vistos.add(id_unico)
        resultado.append(
            {"id": id_unico, "etiqueta": etiqueta, "pilar": etiqueta, "query": etiqueta}
        )
    return resultado
