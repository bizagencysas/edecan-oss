...
# Mínimo para que un texto sea EVALUABLE. No es el mínimo para entregarlo: eso lo decide quien
# orquesta, y `redaccion._MIN_TEXTO_ENTREGABLE_CHARS` usa los 80 caracteres que Aria mantiene
# duros incluso en permisivo (`linkedin_content.py:1610`).
_MIN_TEXTO_CHARS = 50
...
def _extraer_json(raw: str) -> Any:
    ...
    for indice in range(inicio, len(raw)):
        ...
        if caracter == "{":
            ...
            if profundidad == 0:
                try:
                    return json.loads(raw[inicio : indice + 1])
...
_SISTEMA_EDITOR_JEFE = (
    ...
    "autobiografía: eso está prohibido sin excepciones.\n\n" + PROMPT_EDITOR_HUMANIZADOR
)
    ...
    *,
    ...
    if hallazgos:
        reparado = await reparar_delatores(
            pulido, hallazgos, contexto, llamar_modelo, plataforma=plataforma
        )
