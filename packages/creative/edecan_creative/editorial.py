...
LinkedIn de REFERENCIA (``features/linkedin_content.py``), que es el que lleva meses publicando
...
15. LA FUENTE NO ES PROTAGONISTA. Prohibido nombrar el medio dentro del post ("según
    DiarioBitcoin", "de acuerdo con Reforma"): nombrarlo es hacerle publicidad gratis en
    un post ajeno y delata redacción de segunda mano. Los hechos se afirman directo; si
    hace falta marcar que el dato es de un reporte ajeno, "la nota" o "el reporte"
    bastan, sin nombre. El enlace de la fuente viaja aparte, donde sí corresponde.
...
_TEXT_TELLS: tuple[tuple[str, str, str], ...] = (
    ...
    # --- Relleno de NOTA DE PRENSA (medidos en posts reales del 01-ago-2026 que el
    # editor débil dejó pasar: "llega como una actualización clave... prometiendo mayor
    # calidad... continúa redefiniendo... es el último ejemplo de esa evolución"). Son la
    # voz del comunicado corporativo, no la de una persona con criterio: frases que
    # podrían abrir CUALQUIER lanzamiento de CUALQUIER empresa. ---
    (
        "llega_como",
        r"\b(?:llega|se presenta|se posiciona) como (?:una?|la|el)\s*"
        r"(?:actualizaci[oó]n|soluci[oó]n|herramienta|alternativa|respuesta|evoluci[oó]n|"
        r"revoluci[oó]n|apuesta|opci[oó]n)\b",
        "Quita el arranque de nota de prensa ('llega como una actualización/solución...'): "
        "abre con el hecho o la tensión concreta, no con el género del producto.",
    ),
    (
        "promesa_corporativa",
        r"\bpromet(?:e|iendo) (?:mayor|m[aá]s|una? (?:mayor|mejor))\b",
        "Quita la promesa corporativa ('prometiendo mayor calidad/eficiencia'): di QUÉ "
        "cambia en concreto y para quién, con el dato de la fuente.",
    ),
    (
        "sigue_redefiniendo",
        r"\b(?:contin[úu]a|sigue) (?:redefiniendo|transformando|revolucionando|"
        r"reinventando|cambiando la forma)\b",
        "Quita 'continúa redefiniendo/transformando...': es relleno que aplica a "
        "cualquier tecnología. Nombra el cambio específico que ESTA noticia trae.",
    ),
    (
        "ultimo_ejemplo",
        r"\b(?:el|un) (?:[úu]ltimo|nuevo|claro) ejemplo de (?:esa|esta|la)\b",
        "Quita 'es el último ejemplo de esa evolución/tendencia': no dice nada que el "
        "lector no supiera. Reemplázalo por la consecuencia concreta.",
    ),
    (
        "era_de",
        r"\ben la era de(?:l| la)?\b",
        "Quita 'en la era de...': marco gastado que no aporta información. Ve directo al hecho.",
    ),
    (
        "antes_y_despues",
        r"\bmarca (?:un antes y un despu[eé]s|un hito|un punto de inflexi[oó]n)\b",
        "Quita 'marca un antes y un después / un hito': afirma el cambio concreto y "
        "medible que la fuente sostiene, o nada.",
    ),
)
...
#
# Dos correcciones respecto de la versión anterior, ambas contra el motor de REFERENCIA
...
# 2. Antes, cualquier texto cuya última línea terminara en "?" era violación DURA. En REFERENCIA
...
def _nombres_propios_del_banco(banco: str) -> frozenset[str]:
    ...
    corrida donde el post decía "La máquina de Alex incluye..." nombrando al
    ...
    veces (Alex, Acme, cualquier otra marca personal del tenant) queda
...
def _violaciones_clasificadas(texto: str, plataforma: str) -> list[tuple[str, str]]:
    ...
    Tres clases, copiando el reparto que hace ``_finalizar_post`` en el motor de REFERENCIA
    ...
    - ``"estilo"``: juicio de gusto reparable con una pasada de editor. Es la clase que
      convertía cada tic en un turno perdido: en REFERENCIA estos hallazgos disparan una
    ...
    if plataforma_norm in _PLATAFORMAS_SIN_HASHTAGS and _tiene_hashtags(texto):
        encontradas.append(
            (
                "mecanica",
                f"Quita los hashtags: en {plataforma_norm} el texto no lleva hashtags por defecto.",
            )
        )
    ...
    frase_repetida = _frase_larga_repetida(texto)
    if frase_repetida:
        encontradas.append(
            (
                "estilo",
                f"La frase «{frase_repetida}» aparece DOS veces casi textual en el post. "
                "Repetirla no refuerza la idea: delata expansión perezosa. Di la idea una "
                "sola vez y usa el espacio de la segunda para un dato, una consecuencia o "
                "un matiz nuevo.",
            )
        )
...
def revisar(
    ...
    *,
    ...
    que hace ``_finalizar_post`` en el motor de REFERENCIA. Sin esto, el bypass permisivo de
    ...
    * **Página de organización** (``False``): el banco es información PÚBLICA de la marca
    ...
    return list(
        dict.fromkeys(mensaje for clase, mensaje in encontradas if clase in clases_visibles)
    )
...
def delatores_de_estilo(texto: str, plataforma: str = "linkedin") -> list[str]:
    ...
    (``auditoria.reparar_delatores``, port de ``linkedin_content.py:1511-1547``): en REFERENCIA
    ...
    REFERENCIA, porque cada uno ya trae la instrucción concreta de qué hacer.
    """
    return list(
        dict.fromkeys(
            mensaje
...
def _apoyo_repite_titular(headline: str, support: str) -> bool:
    ...
    borrador -- que es lo que hace ``_normalizar_visual`` en REFERENCIA.
    """
...
def normalizar(texto: str, plataforma: str = "") -> str:
    ...
    quemaban un intento completo de escritura + editor jefe + auditor. REFERENCIA no tiene
...
# Topes mecánicos del copy visual, calcados de `_normalizar_visual` (REFERENCIA,
...
def normalizar_visual(
    ...
    Port de ``_normalizar_visual`` (REFERENCIA, ``linkedin_content.py:274-290``). Lo que resuelve,
    ...
    - **Kicker y headline nunca faltan.** Degradan a ``titular`` y a ``tema``. Sin esto, un
      ...
      REFERENCIA no puede quedar así jamás: rellena aquí y vuelve a defenderse al componer.
...
def firma_estructura(texto: str, visual: Mapping[str, Any] | None = None) -> set[str]:
    ...
    Port de ``_firma_estructura`` (REFERENCIA, ``linkedin_content.py:313-330``). El cooldown de
    ...
    # La APERTURA LITERAL como firma (02-ago-2026): en el feed real de una página, casi
    # todos los posts abrían con las mismas dos palabras ("En Venezuela, ...") porque la
    # voz exige nombrar el país en la primera línea y el modelo cumple siempre por el
    # camino más perezoso. Las firmas de arriba no lo veían (no es porcentaje ni
    # pregunta), así que el feed se sentía calcado aunque tema y forma variaran. Alex:
    # "24 cosas que son siempre casi lo mismo... pero con otras palabras".
    arranque = _apertura_normalizada(texto)
    if arranque:
        firmas.add(f"apertura:{arranque}")
    return firmas


# Cuántas palabras identifican una apertura. Tres: con dos, "El bodeguero" y "El
# mayorista" serían la misma firma ("el ..."); con cuatro, "En Venezuela, alquilar un"
# y "En Venezuela, el fiao" contarían como aperturas distintas -- y son el mismo molde.
_PALABRAS_DE_APERTURA = 3


# Tras `_sin_acentos` + `casefold` ya no quedan tildes ni eñes: alfanumérico plano basta.
_RE_PALABRA_APERTURA = re.compile(r"[a-z0-9]+")

# Cuántas palabras seguidas repetidas delatan expansión perezosa. Seis: con menos, giros
# legítimos ("el que paga bien") dispararían falsos positivos; con más, se escapan
# repeticiones reales. Medido en un post real (02-ago-2026, cuenta personal): "paga todo
# puntual por canales que ningun score registra" apareció DOS veces textual en un post de
# ~110 palabras y el editor lo aprobó igual -- el candado determinista existe porque un
# juez con prisa no cuenta frases.
_PALABRAS_FRASE_REPETIDA = 6


def _frase_larga_repetida(texto: str) -> str:
    """La primera secuencia de `_PALABRAS_FRASE_REPETIDA` palabras que aparece dos veces
    (normalizada: sin acentos ni puntuación), o ``""`` si no hay ninguna."""
    palabras = _RE_PALABRA_APERTURA.findall(_sin_acentos((texto or "").casefold()))
    if len(palabras) < _PALABRAS_FRASE_REPETIDA * 2:
        return ""
    vistas: dict[str, int] = {}
    for i in range(len(palabras) - _PALABRAS_FRASE_REPETIDA + 1):
        ventana = " ".join(palabras[i : i + _PALABRAS_FRASE_REPETIDA])
        primera = vistas.setdefault(ventana, i)
        # Solapamiento excluido: "a b a b a b" no es una frase repetida, es una ventana
        # deslizándose sobre sí misma.
        if primera != i and i - primera >= _PALABRAS_FRASE_REPETIDA:
            return ventana
    return ""


def _apertura_normalizada(texto: str) -> str:
    """Las primeras `_PALABRAS_DE_APERTURA` palabras, sin acentos ni puntuación, o ``""``."""
    palabras = _RE_PALABRA_APERTURA.findall(_sin_acentos((texto or "").casefold()))
    if len(palabras) < _PALABRAS_DE_APERTURA:
        return ""
    return " ".join(palabras[:_PALABRAS_DE_APERTURA])
...
def firmas_prohibidas_recientes(historial: Sequence[Mapping[str, Any]] | None) -> set[str]:
    ...
    Port de ``_firmas_prohibidas_recientes`` (REFERENCIA, ``linkedin_content.py:337-355``). Lee
    ...
    # Las APERTURAS literales de los últimos 3 posts quedan vetadas: es el candado
    # contra el feed que abre siempre "En Venezuela, ..." (ver `firma_estructura`).
    for f in firmas:
        prohibidas.update(x for x in f if x.startswith("apertura:"))
    return prohibidas
...
def instruccion_cooldown_estructura(prohibidas: Iterable[str]) -> str:
    ...
    Port de ``_instruccion_cooldown_estructura`` (REFERENCIA, ``linkedin_content.py:358-370``).
    """
    ...
    aperturas = sorted(p.split(":", 1)[1] for p in prohibidas if p.startswith("apertura:"))
    if aperturas:
        lista = "; ".join(f"«{a}...»" for a in aperturas)
        reglas.append(
            f"PROHIBIDO abrir el post con {lista} — los posts recientes ya abrieron así. "
            "Busca una primera línea con OTRA entrada: un objeto, una hora del día, una "
            "acción concreta, un lugar; nunca el mismo molde."
        )
    if not reglas:
        return ""
