...
# `.headline em` en `foto_acme.html`: `linear-gradient(110deg, #6E62FF 0%, #56D2E6 100%)`.
...
# — ver `componer_titular_acme`. Equivalente al `{{LOGO}}` de `foto_acme.html`
# (Aria lo resuelve por `logo_path` en `features/acme_linkedin_content.py`).
...
_LOGO_ACME_PATH = Path(__file__).resolve().parent / "assets" / "d-symbol-white.png"


@lru_cache(maxsize=1)
def _logo_acme_bundled() -> bytes | None:
    """Carga el PNG del logo D empaquetado, una sola vez. `None` si no está presente —
    `componer_titular_acme` nunca debe reventar por un asset faltante, solo omite el
    logo (mismo criterio defensivo que `_fuente`: degradar, no bloquear)."""
    try:
        return _LOGO_ACME_PATH.read_bytes()
    except OSError:
        return None
...
def _ajustar_headline(
    ...
    'serif_italica' (plantilla Acme: titular COMPLETO en itálica, `foto_acme.html`
...
def _texto_con_gradiente(
    ...
    `background-clip: text` en `foto_acme.html` (`.headline em`, gradiente Aurora de la
...
def _dibujar_headline(
    ...
    *,
    ...
    El acento va en ITÁLICA y en color, como `.headline em` en `foto.html`/`foto_acme.html`
    de Aria, y se busca SIN distinguir mayúsculas: antes era `if accent in linea`, sensible a
    ...
    `_COLOR_HEADLINE_ACENTO` — la plantilla Acme (`foto_acme.html`) usa el gradiente
...
def _componer_titular_interno(
    ...
    *,
    ...
    `componer_titular_acme` (plantilla de la página de LinkedIn de Acme) — mismo
    ...
    if logo is not None:
        # Layout de `foto_acme.html` `.brand`: logo D (canal alfa real, `Image.paste`
...
def componer_titular(
    ...
    *,
    ...
    Aurora de la marca) usar `componer_titular_acme`.
...
def componer_titular_acme(
    ...
    *,
    ...
    (migración de `features/li_templates/foto_acme.html` de Aria — ver también
    `scripts/li_compose.py::_build_html`, que selecciona esta plantilla con `tipo ==
    "foto_acme"`). Mismo layout base que `componer_titular`, con tres diferencias:
    ...
    1. Logo D en la esquina superior izquierda (en vez de la barra de acento), pegado con
       ...
       (`assets/d-symbol-white.png`, ver `_logo_acme_bundled`); si ninguno de los dos
    ...
    2. Titular COMPLETO en serif itálica (`.headline { font-style: italic }` en
       `foto_acme.html` — ahí la itálica es del headline entero, no solo del acento).
    ...
    Devuelve PNG en bytes."""
    logo_bytes = logo_bytes or _logo_acme_bundled()
