...
3. **Los controles corren siempre**, no cuando el modelo se acuerde, y en el orden del motor
   que sí funciona a diario (`referencia/features/linkedin_content.py::_finalizar_post`):
...
from .fable import FABLE_ESCRITURA
from .investigacion import _acotar, leer_articulo, titulares_frescos
from .marcas import (
...
from .social import (
    ...
    perfil_autoriza_stance_polemica,
    save_agenda_state,
)
...
# lista de modelos de REFERENCIA (`_LC_MODELOS`: Opus primero, Sonnet de red de seguridad,
...
# Dos reintentos cortos, no las 6 rondas x 5 minutos de REFERENCIA: allá el motor es un cron y
...
#
# Por qué 150 y no los 80 de REFERENCIA (`linkedin_content.py:1610`): allá el piso solo separa
...
def _correccion_escena_ilustrativa(marcas: list[str]) -> str:
    """Lo que se le dice al escritor cuando su borrador perdió el camino limpio del auditor por
    traer un HECHO DURO que esta cuenta no necesitaba.

    Cierra el lazo que faltaba. El perfil de esta cuenta autoriza contar escenas cotidianas, y
    `auditoria.marcas_de_hecho_duro` sabe exactamente qué le sobra a un borrador para calificar
    como una: una cifra ("miles de personas"), un nombre propio que el banco no menciona, una
    fecha. Sin este mensaje, el escritor reintentaba a ciegas y la corrección que sí recibía
    ("ancla cada afirmación en la fuente") empujaba justo para el otro lado -- hacia MÁS
    material verificable, que es lo que no tiene. Con él, el reintento converge al único tipo
    de texto sobre el que se puede AFIRMAR que no hay invención posible.

    Sólo se arma para el destino que declaró el modo en su perfil; los demás nunca lo ven.
    """
    return (
        "Esta cuenta escribe ESCENAS: situaciones cotidianas con gente anónima y genérica ('un "
        "bodeguero que fía', 'quien paga la renta el primero'), contadas en presente. Ese es el "
        "único material que no necesita una fuente citada detrás, y este borrador lo perdió por "
        f"traer datos duros que no hacían falta: {'; '.join(marcas)}. Reescríbelo contando la "
        "escena completa SIN ninguna cantidad (ni en números ni en palabras: nada de 'miles', "
        "'la mayoría', 'ocho de cada diez'), SIN fechas, SIN comillas y SIN ningún nombre "
        "propio que no esté en el banco de contexto de esta cuenta. La fuerza del post tiene "
        "que salir de la escena concreta, nunca de una magnitud."
    )


def _pista_escena_ilustrativa(activo: bool, texto: str, fuentes: list[dict[str, str]]) -> list[str]:
    """`[_correccion_escena_ilustrativa(...)]` si vale la pena decirlo, `[]` si no.

    Vacía en dos casos, los dos importantes: cuando el destino no declaró el modo (no le
    incumbe), y cuando el borrador YA era una escena pura sin marcas -- ahí el auditor falló
    por otra cosa y mandarle al escritor una lista vacía de datos duros sería ruido que
    empeora el reintento.
    """
    if not activo:
        return []
    marcas = marcas_de_hecho_duro(texto, _formatear_fuentes(fuentes, _MAX_CONTEXTO_AUDITOR_CHARS))
    return [_correccion_escena_ilustrativa(marcas)] if marcas else []
...
# de `_borradores_recientes_contexto` (REFERENCIA, `linkedin_content.py:293-307`): con la lista de
...
_CAMPOS_PERFIL_EN_PROMPT: tuple[tuple[str, str], ...] = (
    ...
    # Modo de postura editorial de ESTA cuenta (ver `social.perfil_autoriza_stance_polemica`).
    # Se inyecta en el prompt del escritor para que sepa que esta cuenta TOMA LADO, no
    # describe; el editor jefe lo hace cumplir como gate (rechaza lo neutral). Campo
    # dedicado, no prosa en `purpose`, para que el cambio de voz sea un interruptor
    # verificable y no una sugerencia que el modelo ignora.
    ("editorial_stance", "Postura editorial (polemica = tomar lado; vacio = neutral)"),
)
...
class ContextoDeEscritura:
    ...
    `cuerpos` (url -> texto extraído) es el ARTÍCULO LEÍDO de cada titular que se dejó leer
    (`investigacion.leer_articulo`). Va aparte de `citables` a propósito: el cuerpo alimenta
    los prompts del escritor, el editor y el auditor (`_con_cuerpo`), pero JAMÁS se publica
    -- `sources`/el manifiesto siguen mostrando titular+URL, que es lo que el lector puede
    verificar por su cuenta. Meterlo en los dicts de `citables` lo habría filtrado a la card.
    """
    ...
    bloque: str
    origen: str
    cuerpos: dict[str, str] = field(default_factory=dict)
...
def _bloque_fuentes(fuentes: list[dict[str, str]], cuerpos: dict[str, str] | None = None) -> str:
    ...
    Con `cuerpos` (el artículo leído, ver `investigacion.leer_articulo`), cada fuente que
    se dejó leer lleva su EXTRACTO debajo. Es la diferencia entre escribir con material y
    escribir con humo: sin el cuerpo, el escritor solo tenía el titular y "fuente · hace
    N h", y de ahí no salen ni la fecha, ni el detalle concreto, ni el dato que cambia la
    lectura -- salen deducciones, que el auditor luego recorta enteras (defecto medido,
    01-ago-2026, capturas del dueño)."""
    cuerpos = cuerpos or {}
    lineas: list[str] = []
    for indice, fuente in enumerate(fuentes, start=1):
        lineas.append(f"[F{indice}] {fuente['title']} :: {fuente['snippet']} :: {fuente['url']}")
        cuerpo = cuerpos.get(fuente.get("url") or "")
        if cuerpo:
            lineas.append(f"EXTRACTO REAL DEL ARTÍCULO [F{indice}]:\n{cuerpo}")
    return (
...
async def _reunir_contexto(
    ...
    *,
    ...
    (escenas cotidianas del sector del tenant), no noticias. El día que la marca que nombra
    la escena tuvo un titular real y verificado ese mismo día (una ronda de inversión), la
    búsqueda lo
    ...
    REFERENCIA evita esto marcando cada uno de sus pilares como `"producto"` (banco únicamente,
    jamás busca) o `"mixto"` (busca con una consulta propia, acotada al tema del pilar, nunca
    a su etiqueta libre) -- ver `organization_linkedin_content.py:69-169`. Edecán no tiene ese
    ...
    "noticia_credito_ve" de REFERENCIA), se busca; el resto son escenas y van directo al banco.
    """
    ...
    if buscar_noticia:
        ...
        for indice, dias in enumerate(ventanas):
            ...
            if titulares:
                ...
                cuerpos = await _leer_articulos(citables)
                return ContextoDeEscritura(
                    citables=citables,
                    respaldo=respaldo,
                    bloque=_bloque_fuentes(citables, cuerpos),
...
# Cuántos artículos se intentan leer completos por corrida. Tres: el escritor rota de
# fuente entre intentos (`_fuente_sugerida`), así que leer solo el primero dejaba los
# reintentos otra vez con puro titular. Más de tres es latencia sin retorno -- el post
# usa UNA fuente principal (regla 13).
_MAX_ARTICULOS_LEIDOS = 3


async def _leer_articulos(citables: list[dict[str, str]]) -> dict[str, str]:
    """url -> cuerpo extraído, para los primeros `_MAX_ARTICULOS_LEIDOS` citables.

    Mejor esfuerzo en paralelo: el que no se deje leer simplemente no aparece en el
    dict y esa fuente queda como antes (solo titular). Ver
    `investigacion.leer_articulo` para el porqué completo -- el resumen: el material
    manda más que el modelo, y sin cuerpo el escritor solo puede especular."""
    objetivo = [f for f in citables[:_MAX_ARTICULOS_LEIDOS] if str(f.get("url") or "").strip()]
    if not objetivo:
        return {}
    try:
        async with httpx.AsyncClient() as http:
            textos = await asyncio.gather(*(leer_articulo(http, str(f["url"])) for f in objetivo))
    except Exception:  # noqa: BLE001 - la lectura es opcional de punta a punta
        logger.info("No se pudieron leer los artículos; el escritor sigue con titulares.")
        return {}
    return {str(f["url"]): t for f, t in zip(objetivo, textos, strict=True) if t}


def _con_cuerpo(fuentes: list[dict[str, str]], cuerpos: dict[str, str]) -> list[dict[str, str]]:
    """Las mismas fuentes, con el artículo leído pegado al `snippet` de cada una.

    Es la mitad que faltaría si solo se enriqueciera el prompt del ESCRITOR: el editor
    jefe y el auditor comprueban contra `snippet` (`auditoria._formatear_fuentes`), así
    que un hecho sacado del cuerpo del artículo les llegaría sin respaldo visible y lo
    vetarían como invención -- el mismo defecto, ya vivido con el banco de contexto, que
    documenta `_material_para_auditar`. Quien escribe y quien verifica tienen que ver el
    MISMO material. Devuelve COPIAS: los dicts originales viajan a `sources`/manifiesto
    y el cuerpo jamás se publica."""
    if not cuerpos:
        return fuentes
    resultado: list[dict[str, str]] = []
    for fuente in fuentes:
        cuerpo = cuerpos.get(str(fuente.get("url") or ""))
        if cuerpo:
            fuente = {
                **fuente,
                "snippet": f"{fuente.get('snippet', '')}\nEXTRACTO REAL DEL ARTÍCULO:\n{cuerpo}",
            }
...
def _mejor_candidato(candidatos: list[_Candidato], piso_palabras: int = 0) -> _Candidato | None:
    ...
    if piso_palabras:
        con_largo = [
            ...
            if len(normalizar(str(candidato.borrador.get("texto") or ""), _PLATAFORMA).split())
            >= piso_palabras
        ]
        if con_largo:
            utiles = con_largo
...
def _medio_mencionado(texto: str, citables: list[dict[str, str]]) -> str:
    """El nombre del MEDIO que el post menciona, o `""` si no menciona ninguno.

    Gate determinista de la regla 15 (`REGLAS_LINKEDIN`): "según DiarioBitcoin" dentro
    del post es publicidad gratis a un medio en el post del dueño -- reclamo textual de
    Alex (01-ago-2026: "¿DiarioBitcoin? ¿por qué le estamos haciendo marketing en mis
    posts?"). La regla ya viaja en el prompt, pero un modelo puede ignorar un prompt;
    este chequeo no. El nombre del medio se saca del `snippet` LIMPIO del citable
    ("Reforma · hace 3 h" -> "Reforma"), nunca del enriquecido con cuerpo.
    """
    plano = _sin_acentos(texto).casefold()
    for fuente in citables:
        nombre = str(fuente.get("snippet") or "").split("·")[0].strip()
        if not (3 <= len(nombre) <= 40) or "extracto" in nombre.casefold():
            continue
        nombre_plano = _sin_acentos(nombre).casefold()
        if nombre_plano and re.search(rf"(?<!\w){re.escape(nombre_plano)}(?!\w)", plano):
            return nombre
    return ""
...
def _fuente_sugerida(citables: list[dict[str, str]], intento: int) -> str:
    ...
    Adaptación de la rotación de fuente/agenda de REFERENCIA (`news_idx++`,
...
def _bloque_borradores_recientes(historial: list[dict[str, Any]]) -> str:
    ...
    Port de `_borradores_recientes_contexto` (REFERENCIA, `linkedin_content.py:293-307`). El
...
# Foco geográfico NEUTRO: sin filtro regional en la búsqueda. Es lo correcto por defecto
# cuando el tenant configuró sus PROPIOS pilares: el catálogo de focos genérico
# (`agenda.FOCOS_GEOGRAFICOS_POR_DEFECTO`) rota por México/Colombia/Chile, Wall Street y
# "mercados emergentes" -- pensado para el feed genérico sin perfil, no para una marca con
# territorio propio. Fallo medido (02-ago-2026): la página de una marca 100% venezolana
# llevaba el foco clavado en "Estados Unidos y Europa (Wall Street...)", y sus noticias
# salían del BID y de la región en vez de su país.
_FOCO_NEUTRO: agenda.FocoGeografico = {"etiqueta": "sin foco geográfico impuesto", "busqueda": ""}


def _busqueda_del_foco(base: str, foco: agenda.FocoGeografico | None) -> str:
    """El término de búsqueda del foco, o `""` si la consulta base YA lo contiene.

    Sin esto, los pilares de noticia cuya etiqueta ya nombra el país ("noticia de crédito
    y banca en Venezuela") producían consultas con el país duplicado ("...en Venezuela
    Venezuela") -- hallazgo del panel, confirmado ejecutando las seis consultas reales.
    """
    if not foco or not foco.get("busqueda"):
        return ""
    if str(foco["busqueda"]).casefold() in (base or "").casefold():
        return ""
    return str(foco["busqueda"])


def _foco_del_perfil(perfil: dict[str, Any]) -> agenda.FocoGeografico | None:
    """El foco geográfico que ESTA cuenta declaró (`search_focus`), o `None`.

    Genérico y por-tenant, jamás una región cableada en este repo: una marca de un solo
    país pone su país ("Venezuela"), una global puede no poner nada. El valor entra tanto
    a la CONSULTA de noticias (todas las búsquedas anclan ahí) como a la etiqueta que ve
    el escritor en la instrucción de cooldown.
    """
    foco = " ".join(str(perfil.get("search_focus") or "").split())
    if not foco:
        return None
    return {"etiqueta": foco, "busqueda": foco}
...
def _pilar_es_noticia(territorio: agenda.Territorio) -> bool:
    ...
    el patrón de REFERENCIA ("noticia de la semana", `pilar_id` `noticia_credito_ve`) -- es el
...
def _perfil_prohibe_numeros_extensos(perfil: dict[str, Any]) -> bool:
    ...
    """
    texto = " ".join(str(perfil.get(campo) or "") for campo in ("avoid", "voice", "notes"))
...
def _marca_para_una_sola_mencion(perfil: dict[str, Any]) -> str:
    ...
    dos ("Acme ofrece productos como Boost" ... "ofrece Acme Pass") sin que nada lo
...
def _con_cierre_garantizado(copy: str, perfil: dict[str, Any]) -> str:
    ...
    Puerto del cierre garantizado de REFERENCIA para su motor de LinkedIn de Acme
    (`organization_linkedin_content.py`): "la URL se agrega DESPUÉS de todos los filtros
    ...
    ## Deduplicación por DOMINIO, no por igualdad exacta (02-ago-2026)

    El chequeo viejo (`cierre.lower() in copy.lower()`) era frágil por diseño: si el
    escritor escribía el cierre POR SU CUENTA con una diferencia mínima (sin las comillas
    alrededor del dominio), la igualdad exacta fallaba y el post salía con el cierre DOS
    veces -- pasó en un post real de página, con la URL duplicada (y un post con la misma
    URL dos veces es, además, una señal de spam clásica para el filtro de LinkedIn). La
    regla ahora: cualquier línea del copy que ya contenga el DOMINIO del cierre se
    ELIMINA, y la línea canónica -- exacta, como la configuró el dueño -- se agrega una
    sola vez al final. El escritor puede escribir lo que quiera: el cierre publicado
    siempre es uno y siempre es el oficial.
    """
    cierre = str(perfil.get("closing_url") or "").strip()
    if not cierre:
        return copy
    dominio = re.search(r"(?:[a-z0-9-]+\.)+[a-z]{2,}", cierre.lower())
    if dominio is None:
        # Cierre sin dominio detectable (una frase pelada): cae al chequeo exacto de
        # siempre, que para texto sin URL es suficiente.
        if cierre.lower() in copy.lower():
            return copy
        return f"{copy.rstrip()}\n\n{cierre}"
    marca = dominio.group(0)
    lineas = [linea for linea in copy.splitlines() if marca not in linea.lower()]
    limpio = "\n".join(lineas).rstrip()
    return f"{limpio}\n\n{cierre}"
...
_SISTEMA_ESCRITOR = (
    ...
    # La skill Fable va ANTES de las reglas duras, en el MISMO orden que REFERENCIA arma su
    # `_SYS_VOZ` (`linkedin_content.py:536`: fable_calidad → voz → reglas, "las reglas al
    # final para que GANEN"). Es el método -- las cinco preguntas antes de escribir, la
    # autorrefutación después --; lo prohibido y lo verificable lo siguen mandando las
    # REGLAS y los gates. Pedido explícito del dueño (01-ago-2026): un skill vasto para
    # que el escritor piense como un modelo de frontera, sin importar lo que tarde.
    + FABLE_ESCRITURA
    + "\n\n"
    + REGLAS_LINKEDIN
    ...
    # Campos de DISCIPLINA, portados de `_SALIDA_JSON` de REFERENCIA
    ...
    # REFERENCIA (`features/linkedin_content.py::_normalizar_visual`), pensado para que un
    ...
    'IA · PRIVACIDAD, VENEZUELA · REGULACIÓN>", '
    ...
    # Instrucción anti-titular-poético, con el fallo real de REFERENCIA metido dentro del prompt
...
async def _escribir(
    ...
    *,
    ...
    # `_max_con_reintento` (REFERENCIA, `linkedin_content.py:635-687`): allá son 6 rondas x 2
...
def _post_desde_prosa(crudo: str) -> tuple[dict[str, Any] | None, str]:
    ...
    Port de `_post_desde_raw` (REFERENCIA, `linkedin_content.py:586-608`), que lo documenta como
...
# Cuánto del banco alcanza a ver el auditor. Antes eran 2.400 caracteres, contra el tope
# compartido de 3.500 de `auditoria._formatear_fuentes`, y ESO ERA UN DEFECTO MEDIDO, no una
# elección: con el banco real de una cuenta (5.789 caracteres, 24 escenas enumeradas) el corte
# caía a mitad de la escena 5 -- y a mitad de palabra --, así que las escenas 6 a 24 no le
# llegaban NUNCA al auditor. Una afirmación salida de ahí se vetaba con el motivo "no se
# menciona en las fuentes" aunque el perfil sí la respaldara: el escritor veía el banco entero
# (`social._CONTEXT_BANK_PROMPT_LIMIT`, 12.000) y el auditor un tercio. Ahora el auditor tiene su
# propio presupuesto (`auditoria._MAX_CONTEXTO_AUDITOR_CHARS`) y este tope entra completo debajo.
# Sigue habiendo un tope, y lo que se pierda sigue siendo la COLA del banco: fail-closed, igual
# que todo el auditor -- se pierde un turno, nunca se publica un invento.
# 9500 desde el 02-ago-2026 (era 6000): el banco editorial v5 de una cuenta real mide
# 7.9k -- con 6000, los ejes finales de la matriz no le llegaban NUNCA al auditor y sus
# escenas se vetaban como invención (el mismo defecto que motivó el número anterior).
# Cabe junto al artículo leído (2.8k) dentro del presupuesto del auditor (14000).
_MAX_BANCO_AUDITABLE_CHARS = 9500


def _recorte_limpio(texto: str, limite: int) -> str:
    """Recorta a `limite` caracteres SIN partir una línea por la mitad.

    Un corte a mitad de frase ("...y ese negocio es") no sólo pierde material: le entrega al
    auditor una fuente que parece truncada al azar, que es exactamente lo que se midió.
    """
    if len(texto) <= limite:
        return texto
    recortado = texto[:limite]
    corte = recortado.rfind("\n")
    return (recortado[:corte] if corte > limite // 2 else recortado).rstrip()
        ...
        *citables,
        {
            ...
            "url": "",
            "snippet": (
                f"{_BANCO_AUDITABLE_REGLA}\n{_recorte_limpio(respaldo, _MAX_BANCO_AUDITABLE_CHARS)}"
            ),
        },
    ]
...
def _lo_pidio_una_persona(args: dict[str, Any], ctx: ToolContext | None = None) -> bool:
    ...
    Por eso quien afirma es el que sabe, y hay DOS que saben, cada uno por su canal:

    - El handler del job (`create_linkedin_post._lo_pidio_una_persona`), que tiene el
      `origen` y el `conversation_id` del pedido, lo pone en `args`.
    - El endpoint del chat, donde por definición escribe una persona, lo pone en
      `ctx.extras["lo_pidio_una_persona"]` -- porque cuando el AGENTE invoca esta tool
      inline los `args` los arma un modelo, y un modelo no declara banderas de protocolo.

    Un llamador que no diga nada por ninguno de los dos canales conserva el comportamiento
    de siempre: post recurrente, con derecho a saltarse el turno.
    """
    if args.get("lo_pidio_una_persona"):
        return True
    extras = getattr(ctx, "extras", None) or {}
    return bool(extras.get("lo_pidio_una_persona"))
...
class CrearPostLinkedInTool(Tool):
    ...
    async def run(self, ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
        ...
        # organización (p. ej. `organization`) guardaban su territorio/formato/foco bajo la
        ...
        excluir: tuple[str, ...] = ()
        # El foco del PERFIL se calcula ANTES de las dos ramas: la búsqueda de un tema
        # pedido también tiene que anclar en él (hallazgo del panel: "post sobre X" para
        # una cuenta 100% venezolana buscaba X a secas, con país por defecto US, y traía
        # titulares de cualquier parte del mundo).
        foco_perfil = _foco_del_perfil(perfil)
        if tema:
            consulta = agenda.query_con_diversidad(tema, _busqueda_del_foco(tema, foco_perfil), ())
            encargo = (
            ...
            # El foco del PERFIL manda; sin él, la rotación genérica de focos solo aplica
            # al catálogo genérico -- con pilares propios del tenant el foco es NEUTRO
            # (ver `_FOCO_NEUTRO`: el fallo del feed venezolano buscando en Wall Street).
            foco = foco_perfil or (
                agenda.foco_actual(estado) if territorios_del_perfil is None else _FOCO_NEUTRO
            )
            excluir = agenda.terminos_a_excluir(bloqueados)
            consulta = agenda.query_con_diversidad(
                paso.territorio["query"],
                _busqueda_del_foco(paso.territorio["query"], foco),
                excluir,
            )
            encargo = (
            ...
            # Con foco FIJO del perfil, la nota de diversidad genérica ("no conviertas una
            # sola región en protagonista, alterna coberturas") se CONTRADICE con el foco en
            # la misma frase (hallazgo del panel): la diversidad de una cuenta de un solo
            # país es de ángulo y pilar, jamás de país.
            nota_diversidad = (
                "La variedad de esta cuenta es de ángulo, escena y pilar DENTRO de ese foco: "
                "nunca cambies de país ni de región."
                if foco_perfil
                else agenda._NOTA_DIVERSIDAD_POR_DEFECTO
            )
            instruccion_cooldown = agenda.instruccion_cooldown(
                bloqueados, foco["etiqueta"], nota_diversidad=nota_diversidad
            )
            ...
            # una escena.
            buscar_noticia = territorios_del_perfil is None or _pilar_es_noticia(paso.territorio)
        ...
        # revisar" y sin botón de publicar. La regla "no forzar un post" de REFERENCIA existe
        ...
        #
        # `lo_pidio_una_persona` llega del handler (payload del job) o del chat
        # (`ctx.extras`): el autopiloto no lo declara por ninguno de los dos canales y
        # conserva el derecho a saltarse el turno; todo lo demás lo pidió alguien.
        persona = _lo_pidio_una_persona(args, ctx)
        permisivo = bool(tema) or persona
        ...
        # Los JUECES al nivel del ESCRITOR (ver `_llamar_modelo_editorial`): editor jefe,
        # auditor y reparador corren en "profundo" (GLM 5.2), no en el modelo del chat.
        # Un juez de 17B aprobando lo que escribe uno de 355B es cómo se cuela el relleno
        # de nota de prensa que el dueño reportó como "le falta poder" (01-ago-2026).
        # Cuesta latencia (GLM ~40s/llamada) y el dueño la aceptó explícitamente: "no me
        # importa lo que tarde el cron, lo importante es la perfección".
        llamar_modelo = _llamar_modelo_editorial(ctx, alias=_ALIAS_ESCRITURA)
        ...
        # con pregunta -- condicionada al historial, como en REFERENCIA, y no absoluta.
        ...
        # Señal EXPLÍCITA del perfil (nunca una adivinanza sobre `purpose`) para el auditor de
        # hechos del PASO 2: ver `auditar_hechos`, parámetro `escenas_ilustrativas_autorizadas`,
        # y el docstring de `perfil_autoriza_escenas_ilustrativas` para el falso positivo real
        # que esto arregla. Un destino que no declaró `fact_check_mode` (la enorme mayoría)
        # sigue exactamente igual de estricto que antes.
        escenas_ilustrativas = perfil_autoriza_escenas_ilustrativas(perfil)
        # Señal EXPLÍCITA del perfil (nunca una adivinanza sobre `purpose`/`notes`) para el
        # editor jefe del PASO 1: cuando esta cuenta declaró postura polémica, el editor jefe
        # RECHAZA el borrador neutral (descripción sin juicio) y exige un `angulo` que tome
        # lado. Ver `auditoria.pulir_borrador`, parámetro `postura_polemica`, y el docstring
        # de `perfil_autoriza_stance_polemica`. Un destino que no declaró `editorial_stance`
        # (la enorme mayoría, incluida la página de Acme) sigue exactamente igual que hoy.
        postura_polemica = perfil_autoriza_stance_polemica(perfil)
        ...
        # publicable" (ver `_PRIORIDAD_*`). Diferencia deliberada con REFERENCIA: allá el motor es
        ...
        # porque el usuario pidió el tema. REFERENCIA degrada en silencio (es un cron, nadie lee el
        ...
        for intento in range(_MAX_INTENTOS):
            ...
            # Las fuentes ENRIQUECIDAS con el artículo leído (`_con_cuerpo`): son las que
            # ven el editor jefe y el auditor, para que verifiquen contra el MISMO material
            # que recibió el escritor. `citables_del_post` (limpio) sigue siendo lo que se
            # publica en `sources`. Y `solo_titulares` deja de ser un literal: si la fuente
            # elegida trae su cuerpo, el auditor ya no está mirando "solo titulares" y no
            # debe vetar como deducción lo que el artículo sí dice.
            fuentes_enriquecidas = _con_cuerpo(citables_del_post, contexto.cuerpos)
            hay_cuerpo = any(
                contexto.cuerpos.get(str(f.get("url") or "")) for f in citables_del_post
            )
            ...
            # una sola pasada contra doce gates a la vez. En REFERENCIA los mismos gates son la
            ...
            pulido = await pulir_borrador(
                ...
                postura_polemica=postura_polemica,
            )
            if pulido is None:
                correcciones = [
            ...
            fuentes_auditables = _material_para_auditar(fuentes_enriquecidas, contexto.respaldo)
            auditado, problemas = await auditar_hechos(
                texto,
                fuentes_auditables,
                llamar_modelo,
                # `solo_titulares` ya no es un literal: cuando la fuente elegida trae el
                # ARTÍCULO leído (`hay_cuerpo`), el auditor comprueba contra ese cuerpo; el
                # modo estricto "no des por buena ninguna deducción" queda para cuando de
                # verdad solo hay titular + snippet de buscador.
                solo_titulares=bool(citables_del_post) and not hay_cuerpo,
                escenas_ilustrativas_autorizadas=escenas_ilustrativas,
            )
            ...
            recorte_desproporcionado = len(texto) >= _MIN_LARGO_PARA_MEDIR_RECORTE and len(
                auditado
            ) < _FRACCION_MIN_TRAS_AUDITOR * len(texto)
            ...
            if auditado and (recorte_desproporcionado or munon):
                ...
                correcciones.extend(
                    _pista_escena_ilustrativa(escenas_ilustrativas, texto, fuentes_auditables)
                )
                candidatos.append(_Candidato(_PRIORIDAD_PULIDO, dict(borrador), correcciones))
                continue
            if auditado:
                texto = auditado
            elif permisivo and persona:
                # El auditor vetó, pero una PERSONA pidió el tema: se conserva el texto que
                # el editor jefe ya pulió y ancló en la fuente, y se SIGUE ADELANTE -- lo
                # mismo que hace `_finalizar_post` (`linkedin_content.py:1598-1600`). Antes
                # se quemaba el intento completo (escritor + editor + auditor, tres
                # llamadas) para volver a empezar de cero, y eso es la mitad de los dos
                # minutos de espera que la persona vive como "no funciona".
                #
                # `and persona`, no `permisivo` a secas: una automatización puede traer
                # `tema` fijo (permisivo=True) y sigue siendo el cron -- entregar ahí un
                # texto que el auditor vetó es publicar sin verificar en nombre de una
                # empresa con nadie mirando. El cron reintenta el ciclo; si nada pasa, se
                # salta el turno (ver el gate del rescate).
                logger.info(
            ...
            if _tiene_primera_persona(texto):
                reparado = await reescribir_sin_primera_persona(
                    texto, fuentes_enriquecidas, llamar_modelo
                )
                if reparado:
                    re_auditado, _ = await auditar_hechos(
                        reparado,
                        _material_para_auditar(fuentes_enriquecidas, contexto.respaldo),
                        llamar_modelo,
                        solo_titulares=bool(citables_del_post) and not hay_cuerpo,
                        escenas_ilustrativas_autorizadas=escenas_ilustrativas,
                    )
            ...
            # GATE DETERMINISTA DE LA REGLA 15: el post no le hace publicidad al medio.
            # Corre DESPUÉS del auditor (que reescribe y puede reintroducir el "según X")
            # y ANTES de los gates finales, con reintento explícito -- mismo trato que
            # cualquier violación reparable.
            #
            # Contra TODAS las fuentes de la corrida (`contexto.citables`), no solo la
            # elegida: el primer agujero real fue exactamente ése (01-ago-2026, 10:48) --
            # el post citaba "Según el sitio Robotics and Automation News", el medio de
            # OTRA fuente de la lista, y el gate que solo miraba la elegida lo dejó pasar.
            medio = _medio_mencionado(texto, contexto.citables)
            if medio:
                correcciones = [
                    f"El post nombra al medio '{medio}' ('según {medio}...'). Eso es hacerle "
                    "publicidad gratis en un post ajeno y delata redacción de segunda mano. "
                    "Reescribe afirmando los hechos directo; si necesitas marcar que el dato "
                    "es de un reporte ajeno, di 'la nota' o 'el reporte', sin nombre."
                ]
                candidatos.append(_Candidato(_PRIORIDAD_PULIDO, dict(borrador), correcciones))
                continue

            # PASO 4 -- LOS GATES DETERMINISTAS, AL FINAL Y BLANDOS EN PERMISIVO. Con
            # `permisivo` sólo sobrevive la primera persona, igual que en REFERENCIA. Al reparador
            ...
            if correcciones:
                ...
                prioridad = _PRIORIDAD_CRUDO if _tiene_primera_persona(texto) else _PRIORIDAD_CASI
        ...
        if not copy:
            # Todos los intentos rechazados. Para el AUTOPILOTO, aquí se acaba: la regla
            # "no forzar un post" de REFERENCIA aplica completa al cron -- nadie está esperando,
            # y entregar 3 veces al día un crudo con el sello "Sin revisar" y sin botón de
            # publicar es exactamente lo que el dueño reportó como fallo (01-ago-2026: la
            # card "personal" repetida en la página de Acme). El rescate existe para la
            # persona que pidió y está mirando; el turno del cron se salta con su código de
            # fallo y el próximo slot lo vuelve a intentar con material fresco.
            if not persona:
                return ToolResult(
                    content=_no_publicable(correcciones), data={"fallo": FALLO_NO_PUBLICABLE}
                )
            # Para quien SÍ lo pidió, se entrega el MEJOR que haya quedado, con aviso, en
            # vez de un mensaje vacío: un borrador con "revísalo antes de publicar" es
        ...
        # El copy PRE-cierre se conserva para la firma estructural (ver `copy_para_firma`
        # en `empaquetar_borrador_social`): el cierre pegado al final tapa el `?` real.
        copy_sin_cierre = copy
        ...
        resultado = await empaquetar_borrador_social(
            ...
            # El estado YA AVANZADO, en memoria: el empaquetado le anexa el historial a
            # ESTE en vez de re-leer la base (la re-lectura pisaba el avance del reloj:
            # `territorio_idx` clavado y el feed repitiendo escena, 02-ago-2026).
            agenda_estado=estado_siguiente,
            # Y el copy SIN el cierre garantizado, solo para la firma estructural: el
            # cierre pegado tapa el `?` final y cegaba la firma "cierre_pregunta".
            copy_para_firma=copy_sin_cierre,
        )
