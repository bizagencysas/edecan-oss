...
from .imagen_editorial import componer_titular, generar_con_critica
...
def _visual_publicable(
    ...
    cuestión de tiempo. Aria no puede quedar así jamás: `_normalizar_visual`
...
_FIELD_CHAR_LIMITS: dict[str, int] = {"context_bank": 16_000}
...
_CONTEXT_BANK_HEADER = (
    "BANCO DE CONTEXTO REAL DEL TENANT (nunca se convierte en autobiografía):\n\n"
    "REGLA SAGRADA: cualquier hecho de vida u operación de esta persona que aparezca en un "
    "post tiene que salir de este banco, tal cual está escrito. Prohibido añadir fechas, "
    'nombres, diálogos, lugares o cifras que el banco no liste, ni "mejorar" la historia.\n\n'
    ...
    "NUNCA nombres al dueño ni a sus proyectos personales por su nombre real (ni en primera, "
    "ni en tercera persona, ni con posesivos: nada de 'La máquina de X', 'la empresa de X', "
    "'según X'). El texto se sostiene por su argumento, no por atribuir identidad. Si sientes "
    "que necesitas nombrar a alguien, reescribe la idea como afirmación directa sobre el mundo."
)
...
def build_context_bank_prompt_block(profile: dict[str, Any]) -> str:
...
PLATFORMS: dict[str, PlatformSpec] = {
    ...
    # que usaba Aria (`aria/features/linkedin_content.py`) y el que
...
# `_MAX_TOKENS_ESCRITURA` de 1600 a 3000 en `redaccion.py`. Aria no tiene tope: usa el CLI.
_MAX_TOKENS_EDITORIAL = 2400


def _llamar_modelo_editorial(ctx: ToolContext) -> LlamarModelo:
    ...
    para el crítico de imagen. Usa el alias "principal" (no "rapido", que aquí reservamos para
    juicios perceptuales baratos como la crítica de imagen): auditar hechos y juzgar calidad
    editorial exige más razonamiento que aprobar un encuadre."""
...
def _contexto_calidad(
    *,
    ...
    if perfil.get("configured"):
        for campo in ("purpose", "audience", "voice", "avoid"):
...
def _claim_visual(visual: Mapping[str, str] | None, respaldo: str) -> str:
    ...
    `headline + ". " + support`, igual que Aria (`linkedin_content.py:966`). Sin `visual`
...
def _card_resumen_piloto(
    *,
    ...
    botones = [
        BotonCard(accion=CopyTextAction(id="copiar-texto", label="Copiar texto", text=copy))
    ]
    imagen = None
...
async def empaquetar_borrador_social(
    ...
    *,
    ...
    # imagen de `crear_post_linkedin` se parezca a la que Aria entrega (foto
    ...
    # es determinista y corre siempre, haya o no LLM disponible para la auditoría.
    #
    # `firma` y `copy` son nuevos y sirven al cooldown ESTRUCTURAL portado de Aria
    ...
    estado_agenda = agenda.registrar_historial(
        await get_agenda_state(ctx, platform_key),
        {
            "temas": agenda.detectar_temas(copy),
            "tema": topic,
            "plataforma": platform_key,
            "copy": copy[:_MAX_COPY_EN_HISTORIAL],
            "firma": sorted(firma_estructura(copy, visual_para_montar)),
        },
    )
    await save_agenda_state(ctx, platform_key, estado_agenda)
    ...
    if con_imagen:
        ...
        if offline_visual:
            ...
            try:
                image, motivo_rechazo = await _generar_imagen_con_critica(
                    ...
                    # EL MISMO texto que se va a montar encima, como en Aria
        ...
        # `accent` resaltado + `support` en apoyo). Es el mismo layout que Aria
...
class CrearContenidoSocialTool(Tool):
    ...
    async def run(self, ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
        ...
        # arreglárselo. En Aria es imposible que eso sea motivo de descarte:
        ...
        violaciones = revisar(copy, platform_key, banco_privado=context_bank_text(profile))
        if violaciones:
            mensaje = (
        ...
        # Segunda pasada editorial (`edecan_creative.auditoria`, portada del pipeline de Aria):
        ...
        if ctx.llm is not None:
            ...
            texto_auditado, problemas_hechos = await auditar_hechos(copy, sources, llamar_modelo)
            ...
            # esperado, y es mecánico de arreglar. Es el mismo pase que hace Aria en ese
            ...
            if revisar(texto_auditado, platform_key, banco_privado=banco_privado):
            ...
            violaciones_finales = revisar(
                texto_auditado, platform_key, banco_privado=banco_privado
            )
