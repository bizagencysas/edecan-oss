...
def _sin_lectura_de_articulos_por_red(monkeypatch: pytest.MonkeyPatch):
    """Ningún test de este paquete lee artículos por la red REAL, jamás.

    `redaccion._leer_articulos` intenta descargar el cuerpo de cada titular
    (`investigacion.leer_articulo`). Los fakes de estos tests usan URLs tipo
    `https://example.com/...` que SÍ existen en internet: sin este parche, cada
    test con titulares haría peticiones HTTP reales -- lento, no determinista y
    dependiente del entorno, la misma clase de fuga que ya costó 21 tests rojos
    con el `.env` (ver app.md, 30-jul-2026). Es la MISMA regla de la casa:
    ningún test depende de la máquina donde corre.

    Los tests que sí ejercitan la lectura re-parchean `redaccion.leer_articulo`
    con su propio fake por encima de éste (monkeypatch por test gana).
    """
    from edecan_creative import redaccion

    async def _sin_lectura(http, url, **kwargs):  # noqa: ANN001, ANN003
        return ""

    monkeypatch.setattr(redaccion, "leer_articulo", _sin_lectura)
