...
_TEXTO_OK = "Un banco mexicano ajustó su tasa de interés para tarjetas de crédito este trimestre."
_TEXTO_OK_2 = "El ajuste de tasas encarece el crédito revolvente para quienes ya arrastran saldo."
...
async def test_re_gate_si_el_auditor_reintroduce_primera_persona_la_repara_y_re_audita(
    ...
    monkeypatch.setattr(redaccion, "titulares_frescos", _fake)
    con_primera_persona = "Vimos que el ajuste de tasas encarece el crédito para muchos clientes."
    llm = FakeLLM(
        [
...
async def test_cerrar_con_pregunta_ya_no_tumba_el_borrador(
    ...
    monkeypatch.setattr(redaccion, "titulares_frescos", _fake)
    con_pregunta = (
        "El ajuste de tasas encarece el crédito revolvente. ¿Compensa mantener el saldo un mes "
        "más o conviene liquidarlo aunque duela el flujo?"
    )
