...
`test_port_motor_linkedin.py` (las piezas portadas de REFERENCIA): aquí solo se mide el largo de
...
def _args(**extra: Any) -> dict[str, Any]:
    args: dict[str, Any] = {
        "tema": "tasas de tarjetas en México",
        "destino": "personal",
        "con_imagen": False,
    }
    args.update(extra)
...
async def test_si_el_auditor_amputa_siempre_se_rescata_el_cuerpo_no_el_recorte(
    ...
    # `lo_pidio_una_persona=True`: el rescate es un derecho de la PERSONA que pidió (el
    # docstring de este test lo dice: "para que la persona decida"). El cron, sin esta
    # marca, ahora se salta el turno en vez de entregar un crudo "Sin revisar".
...
async def test_un_post_que_pasa_el_limite_de_la_plataforma_trae_su_propio_codigo(
    ...
    # Persona declarada: este camino llega al empaquetado vía el rescate (los gates
    # deterministas rechazan el texto repetitivo en los tres intentos), y el rescate es
    # de personas -- el cron sin la marca se salta el turno con `no_publicable`.
    result = await CrearPostLinkedInTool(uploader=UniqueUploader()).run(
        ctx, _args(lo_pidio_una_persona=True)
    )
...
async def test_el_aviso_de_rescate_viaja_en_data_y_dice_si_el_auditor_no_valido(
    ...
    # Persona declarada (el handler del job la pone cuando hay conversación): sin ella,
    # el cron ya no rescata y este camino ni existe.
    result = await CrearPostLinkedInTool(uploader=UniqueUploader()).run(
        ctx, _args(lo_pidio_una_persona=True)
    )
