"""Las piezas del motor de LinkedIn de REFERENCIA que Edecán no tenía, cableadas y probadas.
...
Todo el pipeline por intento queda así, en el orden de REFERENCIA (`_finalizar_post`,
...
async def test_el_fallo_medido_ahora_entrega_el_borrador(make_ctx, make_session):
    ...
    `social_draft`. Eso es lo que Alex vive como "no funciona".
    ...
    Con el orden de REFERENCIA el mismo escritor terco produce un post: el editor jefe reescribe
    ...
    assert (
        sum(1 for _a, _f, req in ctx.llm.calls if req.system == redaccion._SISTEMA_ESCRITOR) == 1
    )  # un solo pase del escritor
...
async def test_un_post_en_prosa_no_se_tira_por_el_sobre(make_ctx, make_session):
    ...
    )  # un solo pase del escritor (los JUECES tambien corren en 'profundo' desde 01-ago)
...
async def test_un_fallo_de_red_no_consume_un_intento_editorial(make_ctx, make_session):
    """Adaptación de `_max_con_reintento` (REFERENCIA, `linkedin_content.py:635-687`). Antes había
...
async def test_el_editor_jefe_reescribe_y_su_texto_es_el_que_sale(make_ctx, make_session):
    ...
    del escritor. El editor de REFERENCIA (`_revisar_calidad_editorial`) devuelve un borrador NUEVO
...
async def test_el_editor_jefe_recibe_los_borradores_recientes_con_su_texto(make_ctx, make_session):
...
async def test_reparar_delatores_reescribe_en_vez_de_matar_el_borrador():
    ...
    (`linkedin_content.py:1511-1544`). Los mismos diez patrones que en REFERENCIA disparan una
...
async def test_el_visual_llega_normalizado_al_compositor_y_al_critico(
    ...
    """Dos defectos en uno, los que Alex VE en vez de leer:
    ...
    - El `claim` contra el que el crítico juzga la imagen era `titular_visual`, un campo
      DISTINTO del que se pinta, y `support` no entraba nunca. En REFERENCIA
    ...
    await CrearPostLinkedInTool(uploader=UniqueUploader(), image_provider=FakeImageProvider()).run(
        ctx, _args(con_imagen=True)
    )
...
async def test_sin_visual_del_escritor_la_imagen_no_se_queda_sin_titular(
    ...
    explicara. Ahora degrada al titular y al tema, como `_normalizar_visual` de REFERENCIA."""
    compuesto: list[dict[str, str]] = []
