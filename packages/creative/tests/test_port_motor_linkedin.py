...
class EscritorTerco:
    ...
    _MALO = (
        ...
        "miden el modelo por el benchmark. La pregunta que queda es si deberías migrar."
    )
    _BUENO = (
        "El lanzamiento de Opus 4.7 mueve el costo por tarea, no el puntaje del benchmark. "
        "Quien ya tiene un pipeline montado paga la migración en horas de ingeniería."
    )
    ...
    _SIN_YO = (
        ...
        "modelo por el benchmark. La pregunta que queda es si conviene migrar."
    )
...
async def test_un_post_en_prosa_no_se_tira_por_el_sobre(make_ctx, make_session):
    ...
    prosa = (
        ...
        "diferencia completa el mes siguiente, sin aviso previo del banco."
    )
