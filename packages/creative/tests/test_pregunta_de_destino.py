...
async def test_con_dos_destinos_pregunta_antes_de_escribir(make_ctx, make_session):
    ...
    session.respuestas = [[_fila("linkedin_personal"), _fila("linkedin_organization")]]
    ...
    assert {o["label"] for o in bloque["options"]} == {"personal", "organization"}
...
async def test_si_el_usuario_ya_dijo_el_destino_no_se_lo_vuelve_a_preguntar(make_ctx, make_session):
    session = make_session()
    session.respuestas = [[_fila("linkedin_personal"), _fila("linkedin_organization")]]
    ctx = make_ctx(session=session)

    result = await CrearContenidoSocialTool(uploader=_Uploader()).run(
        ctx, _post(destino="organization")
    )
