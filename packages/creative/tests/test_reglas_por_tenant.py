...
hacían cumplir: la queja real fue el post de una app de cuotas de Acme (Ada, 31-jul-2026),
"no tiene sentido... una app de cuotas levantó $100MM y el producto convierte los pagos en historial no tiene
...
2. "Acme ofrece productos como el producto" y, en otro borrador, "ofrece Acme Pass": la
...
_PERFIL_ACME = {
    "purpose": (
        ...
        "alternativo para Latinoamérica. Cada post cuenta una escena latinoamericana concreta (el "
        "fiado, el día de pago, el alquiler, la transferencias, una app de cuotas) donde Acme entra una sola "
    ...
    "notes": "",
    "closing_url": "Descarga el nuevo buró de crédito alternativo para Latinoamérica en: www.example.com",
    "target_length": (
...
def test_perfil_prohibe_numeros_extensos_detecta_la_regla_del_avoid():
    assert _perfil_prohibe_numeros_extensos(_PERFIL_ACME) is True
...
def test_revisar_atrapa_el_100mm_cuando_el_gate_esta_activo():
    """La cifra exacta del post real que se publicó: "una app de cuotas levantó $100M..."."""
    texto = "una app de cuotas levantó $100M, la mayor ronda fintech de Latinoamérica este año."
...
def test_el_gate_de_numeros_sobrevive_al_modo_permisivo():
    ...
    texto = "una app de cuotas levantó $100M este año."

    violaciones = revisar(
        texto, "linkedin", permisivo=True, prohibir_numeros_extensos=True
    )
...
def test_un_texto_todo_en_palabras_pasa_el_gate():
    texto = "una app de cuotas levantó una cifra enorme de inversión este año, la mayor de una fintech."
...
def test_marca_para_una_sola_mencion_la_saca_del_dominio_de_cierre():
    assert _marca_para_una_sola_mencion(_PERFIL_ACME) == "acme"
...
def test_sin_la_frase_una_sola_vez_en_el_perfil_no_se_activa():
    ...
    perfil = {"closing_url": "www.example.com", "purpose": "Una página cualquiera."}
...
def test_piso_de_palabras_toma_el_menor_numero_del_target_length():
    assert _piso_palabras_del_perfil(_PERFIL_ACME) == 100
