...
async def test_x_long_copy_becomes_numbered_thread(make_ctx):
    ...
    result = await tool.run(
        make_ctx(),
        {
            "plataforma": "x",
            "tema": "Hilo",
            # Palabras DISTINTAS: un "palabra "*180 literal dispara (bien) el candado
            # nuevo de frases repetidas (`editorial._frase_larga_repetida`) y este test
            # mide el troceo del hilo, no ese gate.
            "texto": " ".join(f"palabra{i}" for i in range(180)),
            "con_imagen": False,
        },
    )
...
async def test_social_package_acepta_cualquier_id_de_destino_bien_formado(make_ctx):
    # `destino` ya no es un enum cerrado a "personal"/"organization": cualquier id con la
...
async def test_env_var_agrega_una_card_generica_junto_al_social_draft(make_ctx, monkeypatch):
    ...
    botones_planos = [
        boton for nodo in hijos if getattr(nodo, "nodo", None) == "stack" for boton in nodo.hijos
    ]
