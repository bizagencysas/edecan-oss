...
class Conversation(IDMixin, TenantScopedMixin, TimestampMixin, Base):
    ...
    `chat_model` / `chat_effort` (`0027_conversation_chat_model`, selector de
    modelos del chat): la selección es propiedad de la CONVERSACIÓN, no de la
    credencial del tenant (por eso `PATCH /v1/credentials/llm/models` sigue
    devolviendo 409). NULL en las dos = automático: decide la cadena de
    siempre (`WORKERS_AI_CHAT_MODEL` -> `MODELO_POR_DEFECTO`), así que
    estrenar las columnas no cambia ninguna conversación existente.
    `chat_model` NO lleva CHECK a propósito: el catálogo es DATO
    (`config/modelos.yml` -> `modelos_chat`) y rota sin migración; su
    validación vive en la API contra `modelo_chat_permitido`. `chat_effort`
    sí, porque su enum es del dominio y no del catálogo.
    """
    ...
    __table_args__ = (
        ...
        _enum_check("chat_effort", ("bajo", "medio", "alto")),
        Index(
    ...
    # NULL = automático (ver el docstring). El CHECK de `chat_effort` acepta
    # NULL sin caso especial: en SQL `NULL IN (...)` es NULL, no falso.
    chat_model: Mapped[str | None] = mapped_column(String, nullable=True)
    chat_effort: Mapped[str | None] = mapped_column(String, nullable=True)
    # `context_cleared_at` (revisión `0031_conv_context_cleared`, comando local
    # `/clear`): límite desde el que el turno siguiente arma el contexto que
    # ve el modelo. `NULL` = nunca se limpió (toda fila existente). Mover esta
    # columna a `now()` NO borra ningún `Message`: solo hace que
    # `Repo.list_messages(..., after=...)` deje de devolver lo anterior tanto
    # en `GET /{id}` como en el historial que arma el turno siguiente.
    context_cleared_at: Mapped[datetime | None] = mapped_column(
        TIMESTAMP(timezone=True), nullable=True
    )
...
class MemoryItem(IDMixin, TenantScopedMixin, TimestampMixin, Base):
    """`memory_items(tenant_id, user_id, kind: fact|preference|event|entity|negation,
    content text, embedding vector(1536) nullable, importance float, confidence float,
    source text, superseded_at, superseded_by, expires_at nullable)`. Las filas
    reemplazadas se conservan como historial, pero no participan en el contexto
    activo del asistente.

    `importance` = "qué tan útil es recordar esto en turnos futuros" (0.0-1.0).
    `confidence` = "qué tan seguros estamos de que esto es cierto" (0.0-1.0),
    distinto de `importance`: un hecho puede ser muy útil de recordar pero
    inferido (importance alta, confidence baja). `expires_at` permite caducar
    hechos temporales (p. ej. eventos): las búsquedas ignoran las filas con
    `expires_at <= now()`. `kind="negation"` representa conocimiento negativo
    ("el usuario NO quiere X") de primera clase para que el agente no
...
class PersistentAgent(IDMixin, TenantScopedMixin, TimestampMixin, Base):
    """Identidad durable de un worker always-on; no implica ejecución."""

    __tablename__ = "persistent_agents"
    __table_args__ = (
        _enum_check("status", ("idle", "running", "paused", "disabled")),
        UniqueConstraint("tenant_id", "user_id", "name", name="uq_persistent_agents_owner_name"),
        Index("ix_persistent_agents_tenant_status_updated", "tenant_id", "status", "updated_at"),
    )
...
class PersistentAgentHandoff(IDMixin, TenantScopedMixin, TimestampMixin, Base):
    """Envelope durable de coordinación entre workers; pendiente por defecto."""

    __tablename__ = "persistent_agent_handoffs"
    __table_args__ = (
        _enum_check("status", ("pending", "approved", "running", "done", "rejected", "error")),
    )

    source_worker_id: Mapped[uuid.UUID] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("persistent_agents.id", ondelete="CASCADE"),
        nullable=False,
    )
    destination_worker_id: Mapped[uuid.UUID] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("persistent_agents.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class UnifiedSession(IDMixin, TenantScopedMixin, TimestampMixin, Base):
    """Snapshot durable del estado operativo compartido por una conversación.

    El estado ejecutable nunca se guarda: solo el contrato serializable de
    `edecan_core.UnifiedSessionState`. La clave compuesta permite reanudar una
    conversación después de reiniciar API/desktop sin mezclar usuarios.
    """
    ...
    __table_args__ = (
        ...
        Index("ix_unified_sessions_tenant_user", "tenant_id", "user_id"),
    )
...
class Automation(IDMixin, TenantScopedMixin, TimestampMixin, Base):
    ...
    next_run_at nullable, last_run_at nullable, timezone default 'UTC')` —
    regla disparador→acción (WP-V2-07). `trigger`/`accion` son la forma pinned
    en `edecan_schemas.automations.TriggerDef`/`AccionDef`.

    `timezone` (migración `0029_social_drafts_tz`) es el huso en el que se
    interpreta el RRULE: sin ella, `compute_next_run` resuelve todo en UTC y
    una automatización con `BYHOUR=9` dispara a las 4:00 a.m. en Bogotá. En un
    producto multi-tenant esto NO puede ser una variable de entorno del
    proceso (un solo huso para todos los tenants), por eso va como columna de
    la fila. Nace en `'UTC'` (`server_default`) para que estrenar la columna no
    mueva ni un horario ya sembrado."""
    ...
    # `condition` (migración `0039_automation_condition`, PHASE2 §60, §62):
    # filtro opcional que decide si una corrida debida se ejecuta o se salta.
    # Forma pinned en `edecan_schemas.automations.Condition` (una cláusula
    # `{field, op, value}` o una lista combinada con AND). `nullable=True` SIN
    # default a propósito: `NULL` = sin condición = corre siempre, así que
    # estrenar la columna no cambia el comportamiento de ninguna fila existente.
    condition: Mapped[Any | None] = mapped_column(JSONB, nullable=True)
    enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text("true"))
    ...
    # Nombre IANA ('America/Bogota'). Sin CHECK: esa base de husos cambia sola
    # con el sistema operativo, y congelar la lista en el esquema obligaría a
    # una migración cada vez que el mundo mueve un huso — la validación va en
    # la app, con `ZoneInfo`.
    timezone: Mapped[str] = mapped_column(Text, nullable=False, server_default="UTC")
    # `consecutive_failures`/`disabled_at` (migración `0036_automation_failure_
    # tracking`, PHASE2 §61-62): contador de corridas fallidas seguidas y marca
    # temporal de cuándo el auto-disable la apagó. Cuando
    # `consecutive_failures >= 3`, el worker la desactiva (`enabled=false`,
    # `disabled_at=now()`) y emite UN aviso `automation_failed` keyeado en
    # `automation_id` (no en `run_id`, para que dedupe). Una corrida exitosa
    # (`save_run("done")`) reinicia el contador a 0.
...
class SocialDraft(IDMixin, TenantScopedMixin, TimestampMixin, Base):
    """`social_drafts(user_id, draft_id, platform, target, text,
    image_file_id nullable, status default 'borrador', published_provider_id
    nullable)` + `UNIQUE(tenant_id, draft_id)` — el borrador que hay DETRÁS de
    la tarjeta "Aprobar y publicar" (migración `0029_social_drafts_tz`).

    Sin esta fila el botón de la card no tiene a qué apuntar: el teléfono
    manda un `draft_id` y el endpoint de publicar necesita traducirlo a
    texto + imagen + destino, además de revalidar el `tenant_id` como promete
    el docstring de `ApproveDraftAction` (`edecan_schemas.chat`). Es la
    contraparte persistida de esa acción privilegiada.

    Molde: `AdDraft` (v4). Mismo espíritu que `Order`/`Invoice`: nace SIEMPRE
    en el estado inerte (`status="borrador"` por `server_default`, nunca lo
    decide el caller) y ninguna fila publica nada por sí sola —
    `published_provider_id` queda `NULL` hasta que un paso explícito y
    confirmado por el humano la empuje de verdad a la red social.

    `draft_id` es texto porque es el `card_id` que el handler ya calcula para
    la tarjeta (p. ej. `linkedin-3f9a1c22`); guardarlo tal cual evita un
    segundo identificador que mantener sincronizado. `platform`/`target` van
    sin CHECK a propósito: el catálogo de redes y de destinos es DATO del
    producto (`edecan_creative.marcas`) y crece sin migración — mismo criterio
    que `conversations.chat_model`. `status` SÍ lleva `_enum_check`: es una
    máquina de estados del dominio (aprobar/rechazar), no un catálogo, mismo
    criterio que `time_off.status`/`ad_drafts.status`. `image_file_id` SÍ
    lleva FK con `ondelete="SET NULL"` (a diferencia de
    `meetings.source_file_id`, que la omite): el borrador sigue siendo
    publicable como texto si el PNG se borra, pero un puntero colgante haría
    fallar la publicación con un error de S3 ilegible para el dueño — mismo
    criterio que `invoices.pdf_file_id`.
    """

    __tablename__ = "social_drafts"
    __table_args__ = (
        _enum_check("status", ("borrador", "publicado", "rechazado")),
        _enum_check("verification", ("confirmed", "unknown")),
        # Por tenant, no global: dos tenants pueden acuñar el mismo `card_id`
        # corto sin pisarse. Y es lo que le da idempotencia al escritor — si
        # el handler reintenta, choca contra este UNIQUE en vez de dejar dos
        # borradores que el botón no sabría distinguir.
        UniqueConstraint("tenant_id", "draft_id", name="uq_social_drafts_tenant_id_draft_id"),
    )
    ...
    # NOT NULL y sin `server_default` a propósito: publicar en el destino
    # equivocado (la página de una empresa en vez del perfil personal) es un
    # error caro y silencioso, así que el escritor está obligado a decir a
    # dónde va — un INSERT incompleto revienta aquí, no en el feed.
    target: Mapped[str] = mapped_column(Text, nullable=False)
    image_file_id: Mapped[uuid.UUID | None] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("files.id", ondelete="SET NULL"), nullable=True
    )
    # MP4 para reproducir en el chat (VideoNode de la card). NO se publica en
    # LinkedIn en este alcance: la publicación sigue mandando texto+imagen con
    # `image_file_id`, así que `video_file_id` queda `NULL` sin afectarla. Mismo
    # `ondelete="SET NULL"` que `image_file_id`: el borrador sigue publicable
    # como texto si el MP4 se borra (migración `0033_social_drafts_video_file_id`).
    video_file_id: Mapped[uuid.UUID | None] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("files.id", ondelete="SET NULL"), nullable=True
    )
    status: Mapped[str] = mapped_column(Text, nullable=False, server_default="borrador")
    published_provider_id: Mapped[str | None] = mapped_column(Text, nullable=True)
    # Si el post se pudo RELEER en la red después de crearlo. Un `2xx` no prueba
    # que exista: LinkedIn devolvió `201` con un id válido y no creó nada cuando
    # el token no tenía permiso de organización (31-jul-2026, ver `app.md`).
    # `unknown` por defecto a propósito — una fila publicada antes de que esto
    ...
    # `server_default=text(...)` fallaría con "str is not callable". Si hace
    # falta una, va ANTES de esta línea (o usando `sa.text`).
    text: Mapped[str] = mapped_column(Text, nullable=False)
...
# Mismo criterio de mixins/defaults que v2/v4/v5: JSONB nuevo no-nullable con
# `server_default=text("'[]'::jsonb")`; `estado`/`respuesta` con `_enum_check`
# por ser máquinas de estado/vocabulario pinned del dominio.
# ---------------------------------------------------------------------------


class WorkoutPlan(IDMixin, TenantScopedMixin, TimestampMixin, Base):
    """`workout_plans(user_id, fecha date, titulo, objetivo, duracion_min,
    ejercicios jsonb, imagen_url nullable)` — un plan de entrenamiento
    generado por `edecan_gym.generar_plan` para un día concreto.

    `ejercicios` es la lista JSON de `edecan_gym.Ejercicio.to_dict()`
    (`{nombre, musculo, series, repeticiones, descanso_seg, notas}`).
    `imagen_url` es la URL pública del collage (best-effort, `NULL` si no se
    pudo generar/subir)."""
    ...
    imagen_file_id: Mapped[str | None] = mapped_column(String, nullable=True)


class WorkoutSession(IDMixin, TenantScopedMixin, TimestampMixin, Base):
    """`workout_sessions(user_id, plan_id FK, estado, started_at nullable,
    ended_at nullable, series jsonb)` — máquina de estados de una sesión de
    entrenamiento (`edecan_gym.WorkoutSession`, `ESTADOS`).

    `series` es la lista JSON de `edecan_gym.SerieRegistrada.to_dict()`
    (`{ejercicio_idx, repeticiones, peso_kg, en}`); el `progreso` de la
    respuesta HTTP se deriva de `series` vía `WorkoutSession.resumen()`, no se
    guarda aparte. `estado` nace `'planned'` (nunca lo decide el caller, misma
    regla que `orders.status`)."""

    __tablename__ = "workout_sessions"
    __table_args__ = (
        _enum_check("estado", ("planned", "active", "paused", "completed", "cancelled")),
    )
    ...
    plan_id: Mapped[uuid.UUID] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("workout_plans.id", ondelete="CASCADE"), nullable=False
    )
    estado: Mapped[str] = mapped_column(String, nullable=False, server_default="planned")
    started_at: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=True), nullable=True)
    ended_at: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=True), nullable=True)
...
class GymCheckin(IDMixin, TenantScopedMixin, TimestampMixin, Base):
    """`gym_checkins(user_id, fecha date, respuesta: si|no, session_id
    nullable FK)` — el check-in diario del check-in proactivo "¿Vas a ir al
    gym hoy?".

    `respuesta="no"` no crea sesión (`session_id` queda `NULL`); `"si"` apunta
    a la `workout_sessions` que creó el check-in. `session_id` lleva
    `ondelete="SET NULL"` para conservar el registro del check-in aunque la
    sesión se borre."""
...
# ---------------------------------------------------------------------------
# Action Ledger (PHASE2.md §63-71) — registro de acciones reversibles del
# agente para el comando "deshacer" (§64, §70).
# ---------------------------------------------------------------------------


class ActionEffect(IDMixin, TenantScopedMixin, TimestampMixin, Base):
    """`action_effects(tenant_id, user_id, tool_name, target nullable,
    inverse_op jsonb, reversible bool)` — ledger de acciones del agente.

    Un turno puede ejecutar herramientas con efectos sobre el mundo (editar un
    archivo, crear una memoria, publicar un borrador). Para poder responder
    "¿qué cambiaste?" (§69) y ofrecer "deshacer" (§64), cada ejecución
    exitosa de una tool que declara cómo revertirse (`Tool.inverse`,
    `edecan_core.tools.base`) deja aquí una fila con su operación inversa.

    `inverse_op` es un JSONB libre: hoy guarda la descripción humana de cómo
    revertir (`Tool.inverse`), el `tool_name` y los `args` originales — la
    reversión REAL la ejecutan las tools que declaran el metadata, no esta
    tabla. `reversible` marca si la acción es candidata a deshacer (solo se
    registra `true` cuando la tool declara `inverse`); nace `false` por
    `server_default` para que ninguna fila quede marcada como reversible por
    accidente. `target` es opcional: sin un contrato de "qué operó la tool",
    se deja `NULL` y el resumen legible se deriva de `tool_name`.

    Índice compuesto `(tenant_id, user_id)` a propósito: `last_reversible_action`/
    `describe_last_actions` (`edecan_core.action_ledger`) siempre consultan por
    tenant+usuario, y el índice único de `tenant_id` del `TenantScopedMixin`
    no alcanza para ese filtro. Tenant-scoped (RLS `tenant_isolation`), misma
    red que el resto de tablas — ver migración `0038_action_effects`.
    """

    __tablename__ = "action_effects"
    __table_args__ = (Index("ix_action_effects_tenant_id_user_id", "tenant_id", "user_id"),)
    ...
    inverse_op: Mapped[dict[str, Any]] = mapped_column(
        JSONB, nullable=False, default=dict, server_default=text("'{}'::jsonb")
    )
    reversible: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text("false"))
...
(`ARCHITECTURE.md` §15) + 3 de telefonía v0.4 + 1 perfil editorial + 1
borrador social (`SocialDraft`, migración `0029_social_drafts_tz`) + 1 action
ledger (`ActionEffect`, migración `0038_action_effects`) = 57 tablas.
