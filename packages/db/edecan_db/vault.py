...
def _serialize_bundle(bundle: TokenBundle) -> bytes:
    ...
    payload = {
        ...
        # `extra` (p. ej. `organization_urns` capturado en el callback OAuth para
        # publicar como PÁGINA de empresa) antes no se guardaba y se perdía en
        # cada round-trip del vault -> publicar como organización nunca recibía
        # el URN. Duck-typed: un fake sin `extra` sigue sirviendo (default {}).
