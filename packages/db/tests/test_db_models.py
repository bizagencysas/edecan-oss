...
# El borrador detrás de la tarjeta "Aprobar y publicar"
# (migración `0029_social_drafts_tz`). La cobertura estructural DEDICADA vive
# en `test_social_drafts_migration.py` — mismo criterio que los bloques de
# arriba: aquí solo se extiende el inventario total.
EXPECTED_TABLES_SOCIAL_DRAFTS = {"social_drafts"}

# Las 3 tablas del feature gimnasio (`/v1/gym`, migración `0034_gym_tables`).
# La cobertura estructural DEDICADA (columnas/FKs/CHECKs) puede vivir en un
# test aparte; aquí solo se extiende el inventario total — mismo criterio que
# los bloques de arriba.
EXPECTED_TABLES_GYM = {"workout_plans", "workout_sessions", "gym_checkins"}

# La 1 tabla del action ledger (migración `0038_action_effects`, PHASE2.md
# §63-71). La cobertura estructural DEDICADA puede vivir en un test aparte;
# aquí solo se extiende el inventario total — mismo criterio que los bloques
# de arriba.
