...
def test_serialize_deserialize_bundle_preserva_extra():
    # `extra` (p. ej. `organization_urns` para publicar como PÁGINA de empresa)
    # antes se perdía en el round-trip del vault -> publicar como organización
    # nunca recibía el URN y el post caía silenciosamente al perfil personal.
    bundle = FakeTokenBundle(
        access_token="access-org",
        extra={"organization_urns": ["urn:li:organization:123456789"]},
    )

    result = _deserialize_bundle(_serialize_bundle(bundle))
    assert result.extra == {"organization_urns": ["urn:li:organization:123456789"]}
