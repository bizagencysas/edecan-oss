...
def _kill_tree_windows(pid: int, *, force: bool) -> None:
    """Mata el árbol de un PID en Windows con ``taskkill /T``.

    ``process.terminate()``/``process.kill()`` de Python solo tocan la raíz:
    Chrome deja procesos auxiliares (GPU, renderer) vivos si sólo se detiene
    el proceso que abrimos. ``os.killpg`` (usado en POSIX porque el proceso
    arrancó con ``start_new_session=True``) no existe en Windows; ``/T``
    recorre el árbol y ``/F`` fuerza cuando la primera pasada no alcanza. Ver
    docs/edecan-windows.md §3 ("Matar procesos"). No lanza si el PID ya no
    existe: ``taskkill`` simplemente devuelve código de error, que ignoramos
    porque el resultado deseado (nada vivo con ese PID) ya se cumple.
    """
...
def _stop_browser(process: subprocess.Popen[bytes]) -> None:
    ...
    try:
        if os.name == "posix":
            ...
            _kill_tree_windows(process.pid, force=False)
        ...
        try:
            if os.name == "posix":
                ...
                _kill_tree_windows(process.pid, force=True)
