...
def test_adapted_after_port_no_puede_mentir() -> None:
    """El registro de archivos adaptados tiene que seguir describiendo la realidad.

    `files` certifica que el árbol es el que el manifiesto dice. Eso solo se puede
    cumplir listando el hash ACTUAL, que para un archivo adaptado ya no es el que
    llegó del upstream. `adapted_after_port` existe para que esa diferencia quede
    escrita: sin él, actualizar `files` equivaldría a declarar que nuestro archivo
    modificado es el portado -- y este manifiesto no es un checksum cualquiera,
    es el registro de procedencia y relicenciamiento a Apache-2.0.

    Lo que se fija acá es la coherencia entre las dos mitades, que es justo lo que
    se rompe cuando alguien regenera hashes sin pensar: que cada adaptado siga en
    el inventario, que su hash actual sea el mismo que certifica `files`, y que de
    verdad difiera del upstream (si son iguales, no hubo adaptación y sobra la
    entrada).
    """
    manifest = _manifest()
    registro = manifest.get("adapted_after_port")
    if registro is None:
        return  # Sin adaptaciones todavía: nada que verificar.

    files = manifest["files"]
    for modulo in registro["modules"]:
        ruta = modulo["path"]
        assert ruta in files, f"{ruta} está como adaptado pero no está en el inventario"
        assert files[ruta] == modulo["current_sha256"], (
            f"{ruta}: el inventario y el registro de adaptación dicen hashes distintos"
        )
        assert modulo["upstream_sha256"] != modulo["current_sha256"], (
            f"{ruta}: figura como adaptado pero su hash es el del upstream"
        )
        assert modulo["reason"].strip(), f"{ruta}: una adaptación sin motivo no es un registro"
