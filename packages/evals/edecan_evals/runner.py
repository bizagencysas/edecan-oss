...
resuelto por `edecan_llm.LLMRouter` desde el entorno
(`CLOUDFLARE_ACCOUNT_ID` + `CLOUDFLARE_API_TOKEN`) — consume tokens reales,
así que documenta el costo en stdout y **nunca se debe invocar desde
`packages/evals/tests/`**. En AMBOS
...
def quality_metrics_from_suites(results: Sequence[ResultadoSuite]) -> dict[str, Any]:
    """Calcula métricas observadas con denominador y fuente explícitos.

    No mezcla feedback humano con resultados golden ni llama "precisión" a una
    métrica que no tiene casos esperados. Las tasas acá son del conjunto de
    evals ejecutado, no una garantía de producción.
    """
    by_name = {result.suite: result for result in results}

    def combine(names: tuple[str, ...]) -> dict[str, Any]:
        total = sum(by_name[name].total for name in names if name in by_name)
        passed = sum(by_name[name].aprobados for name in names if name in by_name)
        return {
            "passed": passed,
            "total": total,
            "rate": passed / total if total else None,
            "suites": [name for name in names if name in by_name],
        }

    return {
        "format": "edecan-quality-metrics.v1",
        "source": "golden_agent_eval",
        "mode": "offline_fake_provider_and_tools",
        "metrics": {
            "tool_accuracy": combine(("tool_choice",)),
            "memory_eval_pass_rate": combine(("memoria",)),
            "hallucination_resistance": combine(("seguridad_prompt_injection", "sin_linkedin")),
            "persona_consistency": combine(("persona_consistencia",)),
        },
        "unavailable": [
            "runtime_hallucination_rate",
            "runtime_tool_accuracy",
            "memory_precision_against_user_feedback",
            "voice_latency_on_physical_device",
            "production_cost_accuracy",
        ],
    }
...
def _construir_router(*, live: bool, guion: dict[str, GuionEntry]) -> LLMRouter:
    if live:
        # Deja que `LLMRouter` construya Workers AI desde el entorno,
        # verificado por `main` antes de llegar aquí. Nunca se debe alcanzar
        # esta rama desde `packages/evals/tests/`.
        settings = SimpleNamespace(
            CLOUDFLARE_ACCOUNT_ID=os.environ.get("CLOUDFLARE_ACCOUNT_ID"),
            CLOUDFLARE_API_TOKEN=os.environ.get("CLOUDFLARE_API_TOKEN"),
            WORKERS_AI_CHAT_MODEL=os.environ.get(
                "WORKERS_AI_CHAT_MODEL", "@cf/zai-org/glm-4.7-flash"
            ),
            WORKERS_AI_TIMEOUT_SECONDS=float(os.environ.get("WORKERS_AI_TIMEOUT_SECONDS", "60")),
        )
...
def escribir_metricas_artifact(metricas: dict[str, Any], *, directorio: Path | None = None) -> Path:
    """Escribe el resumen de métricas junto a los artifacts de las suites."""
    directorio = directorio or ARTIFACTS_DIR
    directorio.mkdir(parents=True, exist_ok=True)
    marca = datetime.now(UTC).strftime("%Y%m%dT%H%M%S%fZ")
    ruta = directorio / f"quality_metrics_{marca}.json"
    ruta.write_text(json.dumps(metricas, ensure_ascii=False, indent=2), encoding="utf-8")
    return ruta
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "CLOUDFLARE_ACCOUNT_ID y CLOUDFLARE_API_TOKEN en el "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.live and os.environ.get("EDECAN_RUN_REAL_EXTERNAL_TESTS") != "1":
        print(
            "--live requiere EDECAN_RUN_REAL_EXTERNAL_TESTS=1; aborta sin "
    ...
    if args.live and not (
        ...
        print(
            "--live requiere CLOUDFLARE_ACCOUNT_ID y CLOUDFLARE_API_TOKEN "
    ...
    todo_aprobado = True
    resultados: list[ResultadoSuite] = []
    for suite in suites:
        try:
    ...
    metricas = quality_metrics_from_suites(resultados)
    print(json.dumps(metricas, ensure_ascii=False, indent=2))
    metricas_path = escribir_metricas_artifact(metricas, directorio=args.artifacts_dir)
    print(f"Quality metrics artifact escrito en {metricas_path}\n")
