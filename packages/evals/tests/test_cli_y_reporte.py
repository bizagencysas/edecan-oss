...
from edecan_evals.runner import (
    ...
    imprimir_resumen,
    main,
    quality_metrics_from_suites,
)


@pytest.fixture(autouse=True)
def _sin_credenciales_live(monkeypatch: pytest.MonkeyPatch) -> None:
    """Evita que un `.env`/entorno real del desarrollador con Workers AI
...
def test_quality_metrics_declara_fuente_denominador_y_no_finge_produccion() -> None:
    resultados = [
        ResultadoSuite(
            suite="tool_choice",
            total=2,
        ...
        ResultadoSuite(
            suite="seguridad_prompt_injection",
            total=3,
            aprobados=3,
            casos=[],
        ),
    ...
    metrics = quality_metrics_from_suites(resultados)

    assert metrics["source"] == "golden_agent_eval"
    assert metrics["mode"] == "offline_fake_provider_and_tools"
    assert metrics["metrics"]["tool_accuracy"] == {
        "passed": 1,
        "total": 2,
        "rate": 0.5,
        "suites": ["tool_choice"],
    }
    assert "runtime_hallucination_rate" in metrics["unavailable"]
...
def test_main_live_requiere_gate_explicito_aunque_haya_credenciales(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    monkeypatch.setenv("CLOUDFLARE_ACCOUNT_ID", "account")
    monkeypatch.setenv("CLOUDFLARE_API_TOKEN", "token")
    monkeypatch.delenv("EDECAN_RUN_REAL_EXTERNAL_TESTS", raising=False)

    codigo = main(["--suite", "sin_linkedin", "--live"])

    assert codigo == 2
    assert "EDECAN_RUN_REAL_EXTERNAL_TESTS=1" in capsys.readouterr().err
