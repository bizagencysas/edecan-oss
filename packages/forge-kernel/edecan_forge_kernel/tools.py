...
class ToolRegistry:
    ...
    def _handle_run_command(self, raw_args: dict[str, Any]) -> ToolResult:
        ...
        with tempfile.TemporaryDirectory() as tmp_exec_dir:
            ...
            try:
                # `shlex.split` en modo POSIX (el default) trata `\` como carácter
                # de escape: una ruta Windows como `C:\Users\foo\script.py` en
                # `args.command` perdería sus barras invertidas y quedaría
                # corrupta. `posix=False` conserva `\` literal en `nt`, igual que
                # hace `shlex` para tokenizar líneas de comando de Windows (no es
                # idéntico a las reglas de `CommandLineToArgvW`, pero evita la
                # corrupción activa que `posix=True` garantiza en cualquier ruta
                # con backslashes).
                proc = subprocess.run(
                    shlex.split(args.command, posix=(os.name != "nt")),
                    cwd=target_dir,
                    capture_output=True,
                    text=True,
