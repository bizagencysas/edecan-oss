...
class Vfs:
    ...
    def populate_from_real_dir(
        ...
        for root, dirs, files in os.walk(real_dir):
            # `os.path.relpath` en Windows devuelve separadores `\` (p. ej.
            # "sub\\dir"). El VFS normaliza TODO a `/`, así que concatenar ese
            # resultado a mano con `f"{rel_root}/{d}"` produciría rutas mixtas
            # ("sub\\dir/archivo") que `normalize_vfs_path`/`is_ignored` (que
            # solo parten por `/`) tratarían como un único segmento corrupto.
            # `Path.as_posix()` fuerza `/` en cualquier plataforma.
            rel_root = Path(root).relative_to(real_dir).as_posix()
            if rel_root == ".":
                rel_root = ""
...
class ExecWindow:
    ...
    def reconcile_from_disk(self) -> None:
        ...
        for root, dirs, files in os.walk(self.target_dir):
            # Mismo motivo que en `populate_from_real_dir`: forzar `/` con
            # `as_posix()` para no mezclar separadores en la ruta del VFS.
            rel_root = Path(root).relative_to(self.target_dir).as_posix()
            if rel_root == ".":
                rel_root = ""
