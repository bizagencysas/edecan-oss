...
2. un **banco de 30 tareas reales** (15 sobre `edecan`, 15 sobre `acme`) con criterio
...
bench/
  ...
  acme.py    15 tareas reales sobre Acme (TS/React, Swift, Kotlin, SQL).
...
- `banco --verificar` sobre `acme` no se ha corrido: sus criterios levantan gradle,
  Xcode y un PostgreSQL efímero, y dos de ellos no son ejecutables en esta máquina porque
  falta Xcode.
