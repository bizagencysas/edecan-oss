...
from bench._compat import resolver_binario

_CHECKS = "packages/forge-probe/bench/checks"

_UV = (resolver_binario("uv"), "run", "--all-packages")
...
def _guion_node(nombre: str, descripcion: str, *, timeout_s: int = 300) -> Criterio:
    ...
    return Criterio(
        kind="command",
        descripcion=descripcion,
        comando=[resolver_binario("node"), f"{_CHECKS}/{nombre}"],
        timeout_s=timeout_s,
        debe_fallar_antes=True,
    )
...
def _web_unit() -> Criterio:
    return Criterio(
        ...
        comando=[resolver_binario("npm"), "--prefix", "apps/web", "run", "test:unit"],
        timeout_s=600,
        debe_fallar_antes=False,
    )
