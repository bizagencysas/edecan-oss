  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        platform: { type: 'string', enum: ['instagram-feed', 'instagram-story', 'tiktok', 'facebook', 'linkedin', 'landscape'], description: 'Formato/tamaño (default instagram-feed).' },
        engine: { type: 'string', enum: ['auto', 'gpt-image-2', 'vertex'], description: 'Motor visual para imagen/post. gpt-image-2 intenta el motor premium y cae a Imagen 4/Vertex si no está disponible.' },
        provider: { type: 'string', enum: ['vertex', 'muapi', 'openai', 'fal'], description: 'Proveedor explícito si no quieres usar el ruteo automático del engine.' },
        model: { type: 'string', description: 'Modelo/endpoint explícito del proveedor, si aplica.' },
      },
      required: ['prompt'],
    },
  },
