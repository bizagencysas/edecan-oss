...
interface Input {
  ...
  engine?: 'auto' | 'seedance' | 'omni' | 'kling' | 'direct' | 'avatar' | 'keyframe' | 'gpt-image-2' | 'vertex'; // video-ad: DIRECT (default) = one prompt + optional ref (logo) → ONE model (seedance/omni/kling), no pipeline; 'avatar' = real-person lip-sync; 'keyframe' = classic Opus multi-shot director. image/post: gpt-image-2 or vertex.
...
// Machine-readable progress line (parsed live by the Aria plugin → push per piece).
...
// Pure-Opus, on-brand HTML LANDING PAGE (no image model) — a real .html file the usuario can open.
...
async function generateForSize(
  ...
  const imageEngine: 'vertex' | 'gpt-image-2' =
    input.engine === 'gpt-image-2'
      ? 'gpt-image-2'
      : (input.engine === 'vertex' ? 'vertex' : (process.env.FY_IMAGE_ENGINE === 'gpt-image-2' ? 'gpt-image-2' : 'vertex'));
  ...
  } else if (input.engine === 'gpt-image-2') {
    const count = Math.max(1, Math.min(4, input.count || 1));
    const references: Array<{ data: string; mimeType: string }> = [];
    if (!input.noRefs) {
      for (const u of pickRefs(ctx)) { const inl = await urlToInline(u); if (inl) references.push(inl); }
    }
    log(`Imagen on-brand vía gpt-image-2 @ ${size.w}×${size.h}${count > 1 ? ` × ${count}` : ''}…`);
    for (let n = 0; n < count; n++) {
      if (n > 0) await sleep(1500);
      const img = await genCleanVertexImage(mediaPrompt, input.quality, aspect, references, 2, imageEngine);
      outputs.push({ dataUrl: img.dataUrl });
      usedModel = img.model;
    }
