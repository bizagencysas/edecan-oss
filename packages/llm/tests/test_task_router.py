...
def test_catalogo_de_chat_sale_del_yaml_y_respeta_el_orden_declarado() -> None:
    ...
    assert not any(row["ve_imagenes"] for row in catalogo if not row["principal"])
...
def test_modelo_chat_permitido_acepta_los_siete_y_rechaza_lo_de_afuera() -> None:
...
def test_esfuerzo_solo_en_los_que_razonan() -> None:
    ...
    assert modelo_chat_por_defecto() not in con_esfuerzo  # Copla no razona
