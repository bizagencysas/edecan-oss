...
# `asyncio.StreamReader` por defecto corta cada línea a 64 KiB
# (`asyncio.streams._DEFAULT_LIMIT`) — el framing de MCP por stdio es JSON
# delimitado por líneas SIN límite de tamaño en la spec, así que cualquier
# respuesta moderadamente grande (el contenido de un archivo, una fila de
# resultados) supera eso con facilidad. Medido contra `server-filesystem`
# oficial leyendo un archivo de 40 KB: `ValueError: Separator is found, but
# chunk is longer than limit`, y la llamada revienta en vez de devolver el
# contenido. 10 MiB es generoso para una respuesta MCP legítima (el propio
# cliente trunca a `MAX_RESULT_CHARS`=8000 más arriba en `client.py`, así que
# esto no cambia lo que el modelo termina viendo) y sigue acotado: un
# servidor hostil que mande una "línea" de gigabytes da un error legible
# (`MCPTransportError`, ver `_ensure_started`/`send`) en vez de un
# `ValueError` crudo de asyncio o un OOM sin fondo.
DEFAULT_STDIO_LINE_LIMIT_BYTES = 10 * 1024 * 1024
...
def _argv_ejecutable(command: Sequence[str]) -> list[str]:
    """Resuelve `command` a un argv que `CreateProcess` sepa lanzar en Windows.

    En POSIX devuelve la lista tal cual. En Windows hace falta porque los
    lanzadores más habituales de los servidores MCP —`npx`, `npm`, `yarn`,
    `pnpm`— **no son ejecutables**: son guiones por lotes (`npx.CMD`), y
    `CreateProcess` no los sabe ejecutar. `asyncio.create_subprocess_exec`
    falla con `FileNotFoundError: [WinError 2]` aunque el programa esté
    perfectamente instalado y en el PATH.

    Medido en Windows Server 2022 el 30-07-2026:

        create_subprocess_exec node  -> OK
        create_subprocess_exec npx   -> FileNotFoundError [WinError 2]
        create_subprocess_exec npm   -> FileNotFoundError [WinError 2]
        shutil.which("npx")          -> C:\Program Files\nodejs\npx.CMD

    Sin esto, **ningún servidor MCP basado en Node arranca en Windows**, que
    son la mayoría.

    La solución NO es `shell=True`: eso reintroduciría la interpretación por
    shell que este módulo evita a propósito (ver el docstring de arriba), y
    convertiría cualquier argumento con comillas o `&` en una inyección. Se
    resuelve la ruta real con `shutil.which` —que respeta `PATHEXT`— y, solo
    si resulta ser un guion por lotes, se delega en `cmd.exe /c` con el argv
    todavía separado, sin construir una cadena de shell.
    """
    if not command:
        return list(command)
    if os.name != "nt":
        return list(command)
    resuelto = shutil.which(command[0])
    if resuelto is None:
        # Que falle en `create_subprocess_exec` con su mensaje habitual: aquí
        # no se sabe si el binario existirá bajo otro nombre en tiempo de
        # ejecución, y tragarse el error dificultaría el diagnóstico.
        return list(command)
    if resuelto.lower().endswith((".cmd", ".bat")):
        comspec = os.environ.get("COMSPEC") or "cmd.exe"
        return [comspec, "/c", resuelto, *command[1:]]
    return [resuelto, *command[1:]]
...
class StdioTransport(MCPTransport):
    ...
    def __init__(
        ...
        *,
        ...
        line_limit_bytes: int = DEFAULT_STDIO_LINE_LIMIT_BYTES,
    ) -> None:
    ...
    async def _ensure_started(self) -> asyncio.subprocess.Process:
        ...
        argv = _argv_ejecutable(self._command)
        try:
            self._process = await asyncio.create_subprocess_exec(
                *argv,
                ...
                env=env,
                # Ver `DEFAULT_STDIO_LINE_LIMIT_BYTES` — sin esto,
                # `create_subprocess_exec` arma el `StreamReader` de stdout
                # con el límite de 64 KiB de asyncio y CUALQUIER respuesta
                # MCP mediana revienta `readline()`.
    ...
    async def send(self, request: MCPRequest) -> MCPResponse:
        ...
        # WP-MCP-AUDIT-01 (medido contra el servidor de referencia oficial de
        # MCP: 0 tools descubiertas, sin error): un servidor stdio puede
        # mandar NOTIFICACIONES por su cuenta en cualquier momento —
        # `notifications/tools/list_changed`, `notifications/message`,
        # progreso — no solo la respuesta de lo que se le pidió. Un solo
        # `readline()` que asume "la próxima línea es MI respuesta" confunde
        # esa notificación con la respuesta real y deja el stream desfasado
        # PARA SIEMPRE (la respuesta real de esta llamada queda huérfana en
        # el pipe y la SIGUIENTE llamada se la atribuye a sí misma). El fix:
        # seguir leyendo líneas, descartando cualquier mensaje sin "id" o con
        # un "id" que no es el de ESTA request, hasta encontrar la respuesta
        # que de verdad corresponde — o agotar el presupuesto de tiempo
        # total (no por-línea: una notificación intercalada no debe resetear
        # el timeout).
        loop = asyncio.get_running_loop()
        limite = loop.time() + self._timeout
        while True:
            ...
            try:
                respuesta_linea = await asyncio.wait_for(
                    process.stdout.readline(), timeout=restante
                )
                ...
                # `readline()` de asyncio cuando una línea (sin el
                # separador) supera `line_limit_bytes` — ver
                # `DEFAULT_STDIO_LINE_LIMIT_BYTES`. Nunca dejar escapar el
                # `ValueError` crudo de asyncio hacia el llamador.
                raise MCPTransportError(
                    f"El servidor MCP «{self._command[0]}» mandó una respuesta que superó el "
                    f"tamaño máximo permitido por línea ({self._line_limit} bytes)."
                ) from exc
            ...
            texto = respuesta_linea.decode("utf-8", errors="replace").strip()
            if not texto:
                continue  # línea en blanco entre mensajes: nada que interpretar
            ...
            if not isinstance(bruto, dict) or "id" not in bruto:
                logger.debug(
                    "mcp stdio (%s): notificación descartada mientras se esperaba id=%r: %.200s",
                    self._command[0],
                    request.id,
                    texto,
                )
                continue
            if bruto.get("id") != request.id:
                # Respuesta a OTRA request (no debería pasar en v1, que es
                # estrictamente secuencial, pero un servidor puede
                # reordenar/duplicar) — se descarta igual, nunca se le
                # atribuye a la request equivocada.
                logger.debug(
                    "mcp stdio (%s): respuesta con id=%r descartada, se esperaba id=%r: %.200s",
                    self._command[0],
                    bruto.get("id"),
                    request.id,
                    texto,
            ...
            try:
                return MCPResponse.from_json(texto)
...
__all__ = [
    ...
    "DEFAULT_STDIO_LINE_LIMIT_BYTES",
