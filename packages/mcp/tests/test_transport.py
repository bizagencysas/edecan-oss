...
# ---------------------------------------------------------------------------
# WP-MCP-AUDIT-01: `StdioTransport.send()` hacía un `readline()` y asumía que
# ESA línea era la respuesta — cualquier notificación del servidor (sin `id`,
# p. ej. `notifications/tools/list_changed`, `notifications/message`) que se
# colara entre medio dejaba el stream desalineado PARA SIEMPRE: la respuesta
# real de la siguiente llamada quedaba huérfana en el pipe y esa llamada
# recibía la notificación ajena como si fuera su respuesta. Medido contra el
# servidor de referencia oficial de MCP: 0 tools descubiertas, sin ningún
# error. El fix correlaciona por `id` y descarta (sin romper el stream)
# cualquier mensaje sin `id` o con un `id` que no es el que se está
# esperando.
# ---------------------------------------------------------------------------

_STDIO_NOTIFICACION_INTERCALADA_SCRIPT = r"""
import json, sys
...
def responder(req):
    ...
    if metodo == "initialize":
        ...
        return {"jsonrpc": "2.0", "id": rid, "result": result}
    if metodo == "tools/list":
        herramienta = {"name": "echo", "description": "", "inputSchema": {"type": "object"}}
        return {"jsonrpc": "2.0", "id": rid, "result": {"tools": [herramienta]}}
...
for linea in sys.stdin:
    ...
    req = json.loads(linea)
    if req.get("id") is None:
        # Notificación DEL CLIENTE (p. ej. notifications/initialized). Un
        # servidor MCP real puede aprovechar justo este momento para mandar
        # SU PROPIA notificación no solicitada — exactamente el escenario
        # medido en la auditoría contra el servidor de referencia.
        if req.get("method") == "notifications/initialized":
            aviso = {"jsonrpc": "2.0", "method": "notifications/tools/list_changed"}
            sys.stdout.write(json.dumps(aviso) + "\n")
            sys.stdout.flush()
        continue
    resp = responder(req)
    sys.stdout.write(json.dumps(resp) + "\n")
...
async def test_stdio_transport_ignora_notificacion_intercalada_y_correlaciona_por_id() -> None:
    transport = StdioTransport([sys.executable, "-c", _STDIO_NOTIFICACION_INTERCALADA_SCRIPT])
    try:
        init = await transport.send(MCPRequest(method="initialize", params={}, id=1))
        assert init.result["protocolVersion"] == "2025-03-26"

        # Dispara al servidor a emitir SU notificación no solicitada.
        await transport.send_notification(
            MCPRequest(method="notifications/initialized", params={}, id=None)
        )
        # Le da chance al subprocess de escribir y flushear esa notificación
        # antes de que la siguiente request real compita por la misma línea
        # — sin esto el test sería correcto solo "por suerte" de scheduling.
...
_STDIO_RESPUESTA_DESORDENADA_SCRIPT = r"""
import json, sys
...
for linea in sys.stdin:
    ...
    if rid is None:
        continue
    # Responde con un `id` que NO es el de la request que acaba de llegar —
    # simula un servidor que reordena o duplica respuestas. El cliente nunca
    # debe atribuirle esta respuesta a la request equivocada.
    resp = {"jsonrpc": "2.0", "id": rid + 100, "result": {"marca": "id-equivocado"}}
    sys.stdout.write(json.dumps(resp) + "\n")
    sys.stdout.flush()
"""


async def test_stdio_transport_descarta_respuesta_con_id_que_no_coincide() -> None:
    transport = StdioTransport(
        [sys.executable, "-c", _STDIO_RESPUESTA_DESORDENADA_SCRIPT], timeout_seconds=0.5
    )
    try:
        with pytest.raises(MCPTransportError):
            # El servidor solo manda respuestas con id+100 — nunca llega una
            # que coincida, así que debe agotarse por timeout, NUNCA devolver
            # silenciosamente la respuesta de otra request.
            await transport.send(MCPRequest(method="initialize", params={}, id=1))
    finally:
        await transport.close()
...
# ---------------------------------------------------------------------------
# WP-MCP-AUDIT-02: `process.stdout.readline()` usa el límite por defecto de
# `asyncio.StreamReader` (64 KiB). Cualquier respuesta MCP más grande que eso
# en una sola línea (framing por línea, sin longitud máxima en la spec)
# reventaba con `ValueError: Separator is found, but chunk is longer than
# limit` — medido contra `server-filesystem` oficial leyendo un archivo de
# 40 KB. El fix agranda el límite del `StreamReader` del subprocess y, si aun
# así se excede, da un `MCPTransportError` legible en vez de un `ValueError`
# crudo.
# ---------------------------------------------------------------------------

_STDIO_RESPUESTA_GRANDE_SCRIPT = r"""
import json, sys
...
for linea in sys.stdin:
    ...
    if rid is None:
        continue
    grande = "x" * 200000  # muy por encima de los 64 KiB por defecto de asyncio
    contenido = [{"type": "text", "text": grande}]
    resp = {"jsonrpc": "2.0", "id": rid, "result": {"content": contenido, "isError": False}}
    sys.stdout.write(json.dumps(resp) + "\n")
    sys.stdout.flush()
"""


async def test_stdio_transport_acepta_respuestas_mas_alla_del_limite_por_defecto_de_asyncio() -> (
    None
):
    transport = StdioTransport([sys.executable, "-c", _STDIO_RESPUESTA_GRANDE_SCRIPT])
    try:
        resp = await transport.send(
            MCPRequest(method="tools/call", params={"name": "x", "arguments": {}}, id=1)
        )
        assert len(resp.result["content"][0]["text"]) == 200000
    finally:
        await transport.close()


async def test_stdio_transport_linea_mas_alla_del_limite_configurado_da_error_legible() -> None:
    # Con un límite chico a propósito, la respuesta de 200000 bytes SIGUE sin
    # entrar — el punto es que el error sea un `MCPTransportError` legible
    # (nunca un `ValueError` crudo de asyncio) y que el transporte quede en
    # un estado desde el que `close()` funciona normal.
    transport = StdioTransport(
        [sys.executable, "-c", _STDIO_RESPUESTA_GRANDE_SCRIPT], line_limit_bytes=1000
    )
    try:
        with pytest.raises(MCPTransportError, match="tamaño máximo"):
