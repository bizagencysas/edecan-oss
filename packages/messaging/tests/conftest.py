"""Fixtures compartidas de `edecan_messaging` (ver `ARCHITECTURE.md` §10.1, §10.15).

El bloque de `sys.path` de abajo se ejecuta primero (por su efecto
secundario: agrega el propio código fuente de este paquete,
`packages/messaging/`, a `sys.path`) para que `import edecan_messaging`
funcione en un `pytest` corrido solo sobre este paquete, sin depender de que
ya se haya registrado como miembro del workspace uv de la raíz
(`[tool.uv.workspace].members` en el `pyproject.toml` raíz — lo agrega
WP-V2-01, ver ROADMAP_V2.md §7.1). Mismo motivo y misma técnica que
`apps/api/tests/_stub_siblings.py`; a diferencia de ese módulo (que expone
paquetes HERMANOS), aquí lo que hace falta exponer es este mismo paquete.
Una vez WP-V2-01 registre `packages/messaging` en el workspace y quede
instalado editable en el entorno compartido, este bloque es un no-op inocuo
(`if ... not in sys.path` ya lo evita duplicar).

Los fakes de abajo (`FakeSession`, `FakeVault`, `make_ctx`) calcan
`packages/toolkit/tests/conftest.py`: ningún test de este paquete importa
`edecan_db` para construir sus dobles — `ctx.session`/`ctx.vault` se
completan con objetos locales que solo implementan lo que `edecan_messaging`
realmente usa, y `ctx` en sí es un `SimpleNamespace` (no
`edecan_core.ToolContext`), así que tampoco hace falta esa importación aquí
(aunque `edecan_core` SÍ debe estar instalado/en `sys.path` para poder
importar el código de PRODUCCIÓN de este paquete, que sí lo usa).
"""
...
class FakeResult:
    ...
    (`.mappings().first()` — ver `edecan_messaging._creds`)."""
