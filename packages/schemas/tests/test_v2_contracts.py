...
def test_accion_def_discrimina_agent_instruction_y_create_linkedin_post():
    ...
    # Segunda variante (paridad Aria, ver docstring del módulo): encola
...
def test_accion_def_kind_faltante_ya_no_infiere_agent_instruction():
    # Antes de que `AccionDef` fuera una unión discriminada real, un dict sin
    # "kind" caía al default de `AgentInstructionAccion.kind`. Una unión
    # discriminada de Pydantic v2 exige el tag EXPLÍCITO en la entrada -- no
    # lo infiere de ningún default de sus miembros (ver docstring de
    # `edecan_automations.engine.validate_accion`).
    with pytest.raises(ValidationError):
        AccionDefAdapter.validate_python({"instruccion": "haz X"})
