...
**Fuentes #2/#3 (WP-V5-05)**: además de `owner/repo`/GitHub/skills.sh, `InstalarSkillTool`
también instala desde una URL http(s) CUALQUIERA (`fuente="url"`, `.installer.install_from_url`
sobre `.sources.UrlSkillSource` — guardrail anti-SSRF propio, ver ese módulo) y desde una
carpeta/archivo `.md` del propio Mac del dueño (`fuente="local"`,
`.installer.install_from_local` sobre `.fuente_local.leer_skill_local` — guardrail
anti-symlink propio, ver ese módulo). Ninguna de las dos vuelve `trust_tier="indexada"`
(no vienen de un índice curado, ver `security.clasificar_trust_tier`) ni afloja el gate
`dangerous=True`/confirmación humana: una URL directa o una carpeta local siguen siendo
contenido de un tercero que el agente seguirá literalmente, ni más ni menos que un
...
from .fuente_local import RutaSkillInvalidaError
...
class InstalarSkillTool(Tool):
    name = "instalar_skill"
    description = (
        "Instala una Agent Skill de terceros. La forma de 'source' depende de 'fuente': "
        "(1) 'fuente=directo' (default) — 'owner/repo', 'owner/repo/sub/path', o una URL "
        "de github.com/skills.sh; (2) 'fuente=skills_sh'/'openclaw'/'hermes' — el mismo "
        "'source' que devolvió un 'buscar_skills' previo en ese índice, para que quede "
        "marcada 'indexada' en vez de 'sin revisar'; (3) 'fuente=url' — CUALQUIER URL "
        "http(s) que el usuario haya pegado apuntando directo a un SKILL.md (no solo "
        "GitHub/skills.sh); (4) 'fuente=local' — una ruta del propio Mac del dueño (una "
        "carpeta con SKILL.md, o un .md suelto), sin subir nada a ningún lado. En los "
        "cuatro casos descarga/lee el SKILL.md y lo deja disponible para usar con "
        "'usar_skill'. Requiere confirmación SIEMPRE: instala instrucciones de un tercero "
        "que el agente seguirá literalmente cuando se active — que 'source' sea una URL "
        "directa o una carpeta local no lo vuelve más seguro."
    )
    input_schema = {
        "type": "object",
        "properties": {
            "source": {
                "type": "string",
                "description": (
                    "Según 'fuente': 'owner/repo' o 'owner/repo/sub/path' o una URL de "
                    "GitHub/skills.sh ('directo'/'skills_sh'/'openclaw'/'hermes'); "
                    "cualquier URL http(s) de un SKILL.md ('url'); o una ruta local del "
                    "Mac, carpeta o .md suelto ('local')."
                ),
            },
            "fuente": {
                "type": "string",
                "enum": ["directo", "url", "local", *sorted(FUENTES_INDEXADAS)],
                "description": (
                    "'directo' (default) si armaste 'source' tú mismo como 'owner/repo'; "
                    "'skills_sh'/'openclaw'/'hermes' si 'source' vino de un resultado de "
                    "'buscar_skills' en ese índice; 'url' si 'source' es una URL http(s) "
                    "que el usuario pegó directo (de cualquier sitio, no solo GitHub/"
                    "skills.sh); 'local' si 'source' es una ruta del Mac del dueño."
                ),
            },
        },
        "required": ["source"],
    ...
    async def run(self, ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
        ...
        if not source:
            return ToolResult(
                content=(
                    "Dime qué skill instalar (ej. 'owner/repo', una URL, o una ruta local "
                    "del Mac)."
                )
            )
        ...
        try:
            if fuente == "local":
                instalada = install_from_local(source)
            elif fuente == "url":
                async with httpx.AsyncClient(timeout=_timeout(ctx)) as http:
                    instalada = await install_from_url(source, http=http)
            else:
                async with httpx.AsyncClient(timeout=_timeout(ctx)) as http:
            ...
            return ToolResult(content=str(exc))
        except RutaSkillInvalidaError as exc:
            return ToolResult(content=str(exc))
