...
`_sin_dns_real` (autouse): reemplaza `edecan_skills.sources.resolve_hostname_ips` por un
resolutor falso que siempre devuelve una IP pública fija — así ningún test que ejercite
`UrlSkillSource` sobre un nombre de dominio (ej. `https://ejemplo.com/SKILL.md`) depende de
DNS real. Mismo patrón, por el mismo motivo, que `_sin_dns_real` en
`packages/browser/tests/conftest.py`; los tests de SSRF por dominio la vuelven a reemplazar
puntualmente con `monkeypatch.setattr` para simular una resolución privada o un DNS caído.
"""
...
from edecan_skills import sources  # noqa: E402
...
def _sin_dns_real(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(sources, "resolve_hostname_ips", _resolver_falso)
