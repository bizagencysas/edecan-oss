...
import pytest
import respx
from edecan_skills.fuente_local import RutaSkillInvalidaError
...
# ---------------------------------------------------------------------------
# install_from_url — Fuente #3: URL http(s) CUALQUIERA (WP-V5-05)
# ---------------------------------------------------------------------------
#
# El guardrail anti-SSRF en sí (esquema, credenciales embebidas, host privado/reservado
# literal o resuelto por DNS, redirects revalidados, tope de tamaño) ya está cubierto a
# fondo en `test_sources.py` sobre `UrlSkillSource` directo — acá solo se cubre que
# `install_from_url` conecta bien esa descarga con `parse_skill_md`/`parse_capabilities`
# (mismo criterio que `install_from_source`) y que sigue propagando los mismos tipos de
# error. `conftest._sin_dns_real` (autouse) hace que un hostname como "ejemplo.com"
# resuelva a una IP pública falsa, sin DNS real.


@respx.mock
async def test_install_from_url_pipeline_completo(http):
    ...
    instalada = await install_from_url("https://ejemplo.com/skills/pdf/SKILL.md", http=http)

    assert instalada.owner == ""
    assert instalada.repo == ""
    assert instalada.subpath is None
    assert instalada.source == "https://ejemplo.com/skills/pdf/SKILL.md"
    assert instalada.nombre == "PDF Helper"
    assert instalada.descripcion == "Ayuda con PDFs."
    assert instalada.contenido == "cuerpo"
    assert instalada.capabilities == ["enviar_correo"]


@respx.mock
async def test_install_from_url_sin_name_usa_el_path_como_nombre(http):
    respx.get("https://ejemplo.com/skills/pdf-helper/SKILL.md").mock(
        return_value=httpx.Response(200, text="Sin frontmatter, solo texto plano.")
    )
    instalada = await install_from_url(
        "https://ejemplo.com/skills/pdf-helper/SKILL.md", http=http
    )
    assert instalada.nombre == "pdf-helper"  # fallback: último segmento del path


async def test_install_from_url_vacia_lanza_fuente_invalida(http):
    with pytest.raises(FuenteInvalidaError):
        await install_from_url("   ", http=http)


async def test_install_from_url_host_privado_lanza_fuente_invalida(http):
    ...
    with pytest.raises(FuenteInvalidaError):
        await install_from_url("http://127.0.0.1/SKILL.md", http=http)


@respx.mock
async def test_install_from_url_404_propaga_no_encontrada(http):
    respx.get("https://ejemplo.com/SKILL.md").mock(return_value=httpx.Response(404))
    with pytest.raises(SkillNoEncontradaError):
        await install_from_url("https://ejemplo.com/SKILL.md", http=http)


@respx.mock
async def test_install_from_url_demasiado_grande(http):
...
@respx.mock
async def test_install_from_url_nombre_solo_simbolos_rechazado(http):
    respx.get("https://ejemplo.com/SKILL.md").mock(
        return_value=httpx.Response(200, text='---\nname: "!!! ??? ..."\n---\ncuerpo')
    )
    with pytest.raises(FuenteInvalidaError):
...
# ---------------------------------------------------------------------------
# install_from_local — Fuente #2: carpeta/archivo del propio Mac (WP-V5-05)
# ---------------------------------------------------------------------------
#
# El guardrail anti-symlink/tope de tamaño en sí ya está cubierto a fondo en
# `test_fuente_local.py` sobre `leer_skill_local` directo — acá solo se cubre que
# `install_from_local` conecta bien esa lectura con `parse_skill_md`/`parse_capabilities`
# y el fallback de nombre (nombre de archivo/carpeta), sin ninguna red de por medio.


def test_install_from_local_pipeline_completo(tmp_path):
    ...
    instalada = install_from_local(str(carpeta))

    assert instalada.owner == ""
    assert instalada.repo == ""
    assert instalada.subpath is None
    assert instalada.source == str(carpeta)
    assert instalada.nombre == "PDF Helper"
    assert instalada.descripcion == "Ayuda con PDFs."
    assert instalada.contenido == "cuerpo"
    assert instalada.capabilities == ["enviar_correo"]
...
def test_install_from_local_sin_name_usa_el_nombre_de_la_carpeta(tmp_path):
    carpeta = tmp_path / "sin-frontmatter"
    carpeta.mkdir()
    (carpeta / "SKILL.md").write_text("solo texto plano, sin frontmatter", encoding="utf-8")

    instalada = install_from_local(str(carpeta))
    assert instalada.nombre == "sin-frontmatter"


def test_install_from_local_md_suelto_usa_el_nombre_sin_extension(tmp_path):
    archivo_md = tmp_path / "mi-skill.md"
    archivo_md.write_text("cuerpo sin frontmatter", encoding="utf-8")

    instalada = install_from_local(str(archivo_md))
    assert instalada.nombre == "mi-skill"


def test_install_from_local_ruta_inexistente_lanza_error_claro(tmp_path):
    with pytest.raises(RutaSkillInvalidaError):
        install_from_local(str(tmp_path / "no-existe"))


def test_install_from_local_nombre_solo_simbolos_rechazado(tmp_path):
    carpeta = tmp_path / "rara"
    carpeta.mkdir()
    (carpeta / "SKILL.md").write_text('---\nname: "!!! ??? ..."\n---\ncuerpo', encoding="utf-8")

    with pytest.raises(FuenteInvalidaError):
        install_from_local(str(carpeta))
