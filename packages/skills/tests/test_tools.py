...
def test_instalar_skill_fuente_acepta_url_y_local():
    # Wiring de la tool spec (WP-V5-05): si el modelo no ve estos valores en el enum,
    # jamás los usa — ver docstring de `tools.py`.
    enum = InstalarSkillTool.input_schema["properties"]["fuente"]["enum"]
    assert {"directo", "url", "local", "skills_sh", "openclaw", "hermes"} <= set(enum)


@respx.mock
async def test_instalar_skill_fuente_url_exito_persiste_y_marca_sin_revisar(make_ctx):
    ...
    resultado = await InstalarSkillTool().run(
        ctx, {"source": "https://ejemplo.com/skills/pdf/SKILL.md", "fuente": "url"}
    )

    assert "PDF Helper" in resultado.content
    assert "https://ejemplo.com/skills/pdf/SKILL.md" in resultado.content
    assert "(sin_revisar)" in resultado.content
    assert resultado.data["trust_tier"] == "sin_revisar"
    fila = next(iter(ctx.session.filas.values()))
    assert fila["contenido"] == "cuerpo"
    assert fila["source"] == "https://ejemplo.com/skills/pdf/SKILL.md"


async def test_instalar_skill_fuente_url_host_privado_no_toca_la_sesion(make_ctx):
    # Sin ningún respx.mock: si el guardrail anti-SSRF no bloqueara esto antes de la red,
    # este test fallaría con un error de conexión real en vez de aprobar por accidente.
    ctx = make_ctx()

    resultado = await InstalarSkillTool().run(
        ctx, {"source": "http://127.0.0.1/SKILL.md", "fuente": "url"}
    )
...
@respx.mock
async def test_instalar_skill_fuente_url_demasiado_grande(make_ctx):
    ...
    resultado = await InstalarSkillTool().run(
        ctx, {"source": "https://ejemplo.com/SKILL.md", "fuente": "url"}
    )
...
async def test_instalar_skill_fuente_local_exito_persiste_y_marca_sin_revisar(make_ctx, tmp_path):
    ...
    resultado = await InstalarSkillTool().run(ctx, {"source": str(carpeta), "fuente": "local"})

    assert "PDF Helper" in resultado.content
    assert "(sin_revisar)" in resultado.content
    assert resultado.data["trust_tier"] == "sin_revisar"
    fila = next(iter(ctx.session.filas.values()))
    assert fila["contenido"] == "cuerpo local"
    assert fila["source"] == str(carpeta)


async def test_instalar_skill_fuente_local_ruta_inexistente_no_toca_la_sesion(make_ctx, tmp_path):
    ctx = make_ctx()

    resultado = await InstalarSkillTool().run(
        ctx, {"source": str(tmp_path / "no-existe"), "fuente": "local"}
    )
...
async def test_instalar_skill_fuente_local_no_hace_ninguna_peticion_de_red(make_ctx, tmp_path):
    # Sin ningún respx.mock activo: si `fuente="local"` intentara abrir una conexión HTTP
    # real, este test fallaría por un fallo de conexión en vez de aprobar por accidente.
    carpeta = tmp_path / "sin-red"
    carpeta.mkdir()
    ...
    assert resultado.data["nombre"] == "X"
