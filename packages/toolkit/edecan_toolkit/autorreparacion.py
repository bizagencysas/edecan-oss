...
def _state_root(ctx: ToolContext, repo: Path) -> Path:
    data_dir = Path(str(getattr(ctx.settings, "DATA_DIR", DEFAULT_DATA_DIR))).expanduser().resolve()
...
def _atomic_json(path: Path, data: dict[str, Any]) -> None:
    ...
    tmp = Path(tmp_name)
    try:
        # newline="" fija el fin de línea a LF tal cual lo escribe json.dump,
        # sin la traducción a CRLF que Windows aplica por defecto en modo texto.
...
class GestionarAutorreparacionLocalTool(Tool):
    ...
    async def _apply(
        ...
        for target, _raw_path, content in prepared:
            ...
            tmp = Path(tmp_name)
            try:
                # newline="" escribe "content" byte a byte tal cual lo mandó el
                # modelo (LF, como el resto del repo). Sin esto, Windows traduce
                # cada "\n" a "\r\n" y el archivo reparado queda con fin de línea
                # distinto al resto del repo -- infla el diff de Git con cambios
                # de línea completa y puede desalinear "rutas_esperadas"/hashes
