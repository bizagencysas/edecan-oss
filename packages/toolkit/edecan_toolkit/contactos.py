        ...
        if not filas:
            if consulta:
                ...
                if respuesta is not None:
                    ...
                    contactos_mac = payload.get("contacts")
                    if isinstance(contactos_mac, list) and contactos_mac:
                        lineas_mac: list[str] = []
                        salida_mac: list[dict[str, Any]] = []
                        for item in contactos_mac:
                            if not isinstance(item, dict):
                                continue
                            salida_mac.append(dict(item))
                            index = len(salida_mac)
                            emails = f" — {item.get('emails')}" if item.get("emails") else ""
                            phones = f" — {item.get('phones')}" if item.get("phones") else ""
                            lineas_mac.append(
                                f"{index}. {item.get('name') or '(sin nombre)'}{emails}{phones}"
                            )
                        if salida_mac:
                            return ToolResult(
                                content="\n".join(lineas_mac),
                                data={"contactos": salida_mac, "origen": "Contactos.app"},
                            )
            mensaje = (
