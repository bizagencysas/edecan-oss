...
async def _kill_pentestgpt_tree(process: asyncio.subprocess.Process, *, force: bool) -> None:
    """Mata el árbol de PentestGPT (arrancó con ``start_new_session=True``).

    POSIX: la sesión completa recibe la señal por ``os.killpg`` — el proceso
    raíz y cualquier hijo que haya lanzado. Windows no tiene ``os.killpg``;
    ``taskkill /T`` recorre el árbol por PID y ``/F`` fuerza en la segunda
    pasada. Sin esto, un timeout en Windows solo detenía la raíz y dejaba
    vivos los procesos que PentestGPT hubiera lanzado (mismo bug documentado
    en docs/edecan-windows.md §3 para el resto de los runners de subproceso).
    """
...
async def _run_pentestgpt(
    ...
    *,
    ...
    try:
        ...
        timed_out = True
        await _kill_pentestgpt_tree(process, force=False)
        try:
            output, _ = await asyncio.wait_for(process.communicate(), timeout=10)
        except TimeoutError:
            await _kill_pentestgpt_tree(process, force=True)
...
class EjecutarPentestGPTAutorizadoTool(Tool):
    ...
    async def run(self, ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
        ...
        data_dir = Path(str(getattr(ctx.settings, "DATA_DIR", DEFAULT_DATA_DIR))).expanduser()
