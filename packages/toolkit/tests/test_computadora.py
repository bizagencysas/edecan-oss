...
from edecan_toolkit.computadora import PARAMS_CAPTURA_PARA_EL_MODELO, UsarComputadoraTool


def test_params_captura_para_el_modelo_son_webp_antes_de_varios_megas():
    assert PARAMS_CAPTURA_PARA_EL_MODELO["format"] == "webp"
    assert PARAMS_CAPTURA_PARA_EL_MODELO["max_width"] == 2560
    assert PARAMS_CAPTURA_PARA_EL_MODELO["quality"] == 90
    assert PARAMS_CAPTURA_PARA_EL_MODELO["crop_frontmost"] is True
...
async def test_usar_computadora_no_deja_que_el_modelo_pida_png_crudo(make_ctx):
    llamadas = []

    async def companion_falso(accion: str, parametros: dict) -> dict:
        llamadas.append((accion, parametros))
        return {"ok": True, "image_b64": "Zm90bw==", "mime": "image/webp"}

    ctx = make_ctx(
        extras={
            "companion": companion_falso,
            "flags": {FLAG_COMPANION_REMOTE_VIEW: True},
        }
    )
    await UsarComputadoraTool().run(
        ...
        {
            "accion": "screenshot",
            "parametros": {"format": "png", "display": 2},
        },
    )
...
async def test_usar_computadora_adjunta_captura_despues_de_un_clic(make_ctx):
    llamadas = []

    async def companion_falso(accion: str, parametros: dict) -> dict:
        llamadas.append(accion)
        if accion == "screenshot":
            return {"ok": True, "image_b64": "Zm90bw==", "mime": "image/jpeg"}
        return {"ok": True}
    ...
    ctx = make_ctx(
        extras={
            ...
            "flags": {
                FLAG_COMPANION_REMOTE_VIEW: True,
                FLAG_COMPANION_REMOTE_INPUT: True,
            },
        }
    )
    resultado = await UsarComputadoraTool().run(
        ctx, {"accion": "input_pointer", "parametros": {"x": 10, "y": 20}}
    )

    assert llamadas == ["input_pointer", "screenshot"]
    assert resultado.data["resultado"]["image_b64"] == "Zm90bw=="
    assert "captura" in resultado.content.lower()
    assert "artifacts" not in resultado.data


async def test_usar_computadora_screenshot_deja_artifact_para_el_iphone(make_ctx):
    ...
    async def companion_falso(accion: str, parametros: dict) -> dict:
        del parametros
        assert accion == "screenshot"
        return {"ok": True, "image_b64": "Zm90bw==", "mime": "image/webp"}

    async def uploader_falso(ctx, *, data, filename, mime):
        del ctx
        subidas.append((data, filename, mime))
    ...
    ctx = make_ctx(
        extras={
            "companion": companion_falso,
            "flags": {FLAG_COMPANION_REMOTE_VIEW: True},
            "subir_archivo": uploader_falso,
        }
    )
    resultado = await UsarComputadoraTool().run(ctx, {"accion": "screenshot"})

    assert subidas == [(b"foto", "mac-pantalla.webp", "image/webp")]
