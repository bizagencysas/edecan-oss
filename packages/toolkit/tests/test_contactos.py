...
async def test_buscar_contactos_sin_resultado_crm_usa_contactos_de_mac(make_ctx, make_session):
    ...
    resultado = await BuscarContactosTool().run(
        make_ctx(
            session=make_session([[]]),
            extras={"companion": companion},
        ),
        {"consulta": "Ana", "limite": 5},
    )

    assert llamadas == [("mac_contacts_search", {"query": "Ana", "limit": 5})]
    assert "Ana Torres" in resultado.content
    assert "ana@example.com" in resultado.content
    assert resultado.data["origen"] == "Contactos.app"


async def test_buscar_contactos_no_devuelve_payload_local_malformado(make_ctx, make_session):
    async def companion(_action, _params):
        return {"ok": True, "result": {"contacts": ["registro inválido"]}}
