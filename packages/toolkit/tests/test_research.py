...
async def test_buscar_web_tool_resume_resultados_del_proveedor(make_ctx, monkeypatch):
    ...
    assert resultado.data["search_provenance"]["provider"] == "stub"
    assert resultado.data["search_provenance"]["mode"] == "demo"
    assert all(citation["retrieved_at"] for citation in resultado.citations)


async def test_buscar_web_cachea_la_misma_consulta_en_sesion(make_ctx, monkeypatch):
    calls = {"n": 0}
    ...
    class _Contador:
        ...
        async def search(self, query: str, k: int = 5) -> list[SearchHit]:
            calls["n"] += 1
            return [SearchHit(title="Hotel", url="https://example.com/hotel", snippet="ok")]
    ...
    monkeypatch.setattr("edecan_toolkit.research.get_tenant_search_provider", proveedor_stub)
    tool = BuscarWebTool()
    args = {"consulta": "cache-sesion-unica-edecan", "k": 2}
    first = await tool.run(make_ctx(), args)
    second = await tool.run(make_ctx(), args)
    assert calls["n"] == 1
    assert first.data.get("cache_hit") is not True
    assert second.data.get("cache_hit") is True
    assert "Hotel" in second.content
...
async def test_buscar_web_tool_sin_consulta(make_ctx):
    ...
    assert "consulta" in resultado.content.lower()
    assert "error" in resultado.content.lower()
