...
def enlace_busqueda_hoteles(ciudad: str, checkin: str, checkout: str) -> str:
    """Google Hotels: abre la búsqueda, no reserva ni cobra."""
...
def _direccion_amadeus(hotel: dict[str, Any]) -> str | None:
    addr = hotel.get("address")
    if isinstance(addr, str) and addr.strip():
        return addr.strip()
    if not isinstance(addr, dict):
        return None
    lineas = addr.get("lines") or []
    partes = [*(lineas if isinstance(lineas, list) else []), addr.get("cityName")]
    texto = ", ".join(str(parte).strip() for parte in partes if parte)
...
def _parse_hotel_oferta(item: dict[str, Any]) -> HotelOferta | None:
    ...
    direccion = _direccion_amadeus(hotel)
    return HotelOferta(
...
class AmadeusClient:
    ...
    async def buscar_hoteles(
        ...
        busqueda = enlace_busqueda_hoteles(ciudad, checkin, checkout)
        return [
            replace(o, booking_url=o.booking_url or busqueda)
            for o in ofertas
            if o is not None
        ]
