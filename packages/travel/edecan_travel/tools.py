...
def _tarjeta_google_hoteles(
    ciudad: str, checkin: str, checkout: str, *, motivo: str
) -> dict[str, Any]:
    url = enlace_busqueda_hoteles(ciudad, checkin, checkout)
    return {
        "type": "hotel",
        "fallback_text": motivo,
        "offer_id": "google-hoteles",
        "name": f"Buscar en Google Hotels — {ciudad}",
        "city": ciudad,
        "checkin": checkin,
        "checkout": checkout,
        "rating": None,
        ...
        "actions": [
            {
                "id": "travel.hotel.fallback.open",
                "label": "Abrir Google Hotels",
                "action": "open_url",
                "url": url,
            }
        ],
...
class BuscarHotelesTool(Tool):
    name = "buscar_hoteles"
    description = (
        "Busca ofertas reales de hotel por ciudad y fechas vía MCP "
        "(Trivago y Skiplagged; el equivalente a Kiwi en vuelos). "
        "Pinta cards con foto, precio y dirección. Usa SIEMPRE esta tool: "
        "nunca buscar_web ni listar hoteles en un párrafo. "
        "Si solo hay una fecha, checkout es el día siguiente. "
        "No abras Booking ni la Mac. No reserva ni paga."
    )
    input_schema = {
        "type": "object",
        "properties": {
            "ciudad": {
                "type": "string",
                "description": "Ciudad o barrio, p. ej. 'Medellín', 'Poblado', 'PAR'.",
            },
            "zona": {
                "type": "string",
                "description": "Barrio opcional (Poblado, Laureles) para afinar la búsqueda.",
            },
            "checkin": {"type": "string", "description": "Fecha de entrada en formato AAAA-MM-DD."},
            "checkout": {
                "type": "string",
                "description": "Fecha de salida AAAA-MM-DD. Si falta, se usa el día siguiente al check-in.",
            },
            "adultos": {
                "type": "integer",
    ...
    async def run(self, ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
        ciudad = _texto(args, "ciudad")
        zona = _texto(args, "zona")
        consulta = " ".join(parte for parte in (ciudad, zona) if parte)
        checkin = _texto(args, "checkin")
        checkout = _texto(args, "checkout")
        if not consulta:
            return ToolResult(content="Necesito la ciudad o el barrio (p. ej. Medellín, Poblado).")
        if not checkin:
            return ToolResult(content="Necesito la fecha de check-in (AAAA-MM-DD).")
        if not checkout:
            try:
        ...
        provider = await self._resolver(ctx)
        try:
            ofertas = await provider.buscar_hoteles(consulta, checkin, checkout, adultos=adultos)
        except Exception as exc:
            motivo = f"No pude buscar hoteles: {exc}"
            return ToolResult(
                content=f"{motivo} Puedes continuar en Google Hotels.",
                presentation=[_tarjeta_google_hoteles(consulta, checkin, checkout, motivo=motivo)],
            )

        if not ofertas:
            motivo = f"No encontré hoteles en {consulta} para esas fechas."
            return ToolResult(
                content=f"{motivo} Puedes continuar en Google Hotels.",
                data={"ofertas": []},
                presentation=[_tarjeta_google_hoteles(consulta, checkin, checkout, motivo=motivo)],
        ...
        presentation = [
            {
                ...
                "currency": o.moneda.upper(),
                "address": o.direccion,
                "image_url": o.imagen_url,
                "source_mode": source_mode,
                "provider": provider_name,
                "observed_at": observed_at,
                "actions": _acciones_oferta(
                    "hotel",
                    o.id,
                    index,
                    o.booking_url or enlace_busqueda_hoteles(consulta, checkin, checkout),
                ),
            }
            for index, o in enumerate(ofertas[:10])
        ]
        return ToolResult(
            content=f"Hoteles en {consulta} ({checkin} → {checkout}):\n" + "\n".join(lineas),
            data={"ofertas": [asdict(o) for o in ofertas]},
            presentation=presentation,
        )
...
class RastrearPaqueteTool(Tool):
    ...
    async def run(self, ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
        ...
        if not tracking_number:
            return ToolResult(
                content="Necesito el número de guía/tracking del envío.", is_error=True
            )
        ...
        try:
            ...
            return ToolResult(
                content=f"No pude rastrear el paquete '{tracking_number}': {exc}", is_error=True
            )

        # Antes, `data=asdict(rastreo)` no llevaba NINGUNA marca de demo/real (a diferencia
        # de `buscar_vuelos`/`buscar_hoteles`, que sí calculan `_fuente_viaje` para sus
        # `presentation`): un `StubTrackingProvider` (sin cuenta de AfterShip conectada)
        # devolvía `estado="InTransit"` — un estado real de transportadora — para CUALQUIER
        # número de guía, con el único aviso enterrado dentro del texto de un checkpoint
        # (`"... (ejemplo, sin cuenta de AfterShip conectada)"`), que una tarjeta que solo
        # pinte `data.estado` nunca llega a mostrar. Mismo criterio que el resto del
        # contrato de veracidad (`edecan_core.veracidad`): la fuente va en `data`, no solo
        # en un texto que alguien puede truncar.
        source_mode, provider_name = _fuente_viaje(provider)
        ...
        if source_mode != "live":
            contenido = (
                "AVISO: no hay cuenta de rastreo (AfterShip) conectada — lo que sigue es un "
                "EJEMPLO de demostración, no el estado real de este paquete.\n\n" + contenido
            )
        datos = asdict(rastreo)
        datos["source_mode"] = source_mode
        datos["provider"] = provider_name
        return ToolResult(content=contenido, data=datos)
