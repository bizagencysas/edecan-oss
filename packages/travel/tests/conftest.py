"""Fixtures compartidas de `edecan_travel` (ver `ARCHITECTURE.md` §10.1, §10.15).

Fakes deliberadamente ligeros, por duck typing: ningún test de este paquete importa
`edecan_db` ni `edecan_api` para construir sus dobles — `ctx.session`/`ctx.vault` se
completan con objetos locales que solo implementan lo que este paquete realmente usa, y
`ctx` en sí es un `SimpleNamespace` (no `edecan_core.ToolContext`). Mismo patrón que
`packages/ads/tests/conftest.py`, duplicado a propósito.
"""
...
@dataclass
class FakeSession:
    """`ctx.session` falso: cada `execute()` consume la siguiente respuesta programada
    (una lista de filas-dict, en el orden exacto en que el código bajo prueba las pide)
    y registra `(sql, params)` en `llamadas`. `flush()` es un no-op registrado en
    `flushes`."""
...
@dataclass
class FakeVault:
    """`ctx.vault` falso (`providers.get_tenant_travel_provider`/
    `get_tenant_tracking_provider`): devuelve siempre el mismo `bundle` (o `None` si no
...
def make_ctx():
    ...
    argumentos, cada dependencia se rellena con un fake vacío (`vault=None` por defecto:
    `get_tenant_travel_provider`/`get_tenant_tracking_provider` lo tratan como "sin
    credencial bring-your-own del tenant" y caen al stub correspondiente)."""
