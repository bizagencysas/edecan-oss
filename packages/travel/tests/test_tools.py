...
_OFERTA_HOTEL = HotelOferta(
    ...
    direccion="Carrera 70 # 24-21, Laureles",
    imagen_url="https://img.trivago.com/hotel/centro.jpg",
)
...
async def test_buscar_hoteles_ok(make_ctx):
    ...
    assert resultado.presentation[0]["actions"][0]["action"] == "open_url"
    assert resultado.presentation[0]["type"] == "hotel"
    assert resultado.presentation[0]["address"] == "Carrera 70 # 24-21, Laureles"
    assert resultado.presentation[0]["image_url"] == "https://img.trivago.com/hotel/centro.jpg"
    assert provider.llamadas_hoteles == [("par", "2026-09-01", "2026-09-05", 1)]


async def test_buscar_hoteles_sin_checkout_usa_noche_siguiente(make_ctx):
    provider = _FakeTravelProvider(hoteles=[_OFERTA_HOTEL])
    tool = BuscarHotelesTool(provider_resolver=_resolver_fijo(provider))

    resultado = await tool.run(
        make_ctx(), {"ciudad": "Laureles Medellín", "checkin": "2026-08-23"}
    )

    assert "Hotel Centro" in resultado.content
    assert provider.llamadas_hoteles == [("Laureles Medellín", "2026-08-23", "2026-08-24", 1)]


async def test_buscar_hoteles_sin_ofertas_abre_google_hotels(make_ctx):
    provider = _FakeTravelProvider(hoteles=[])
    tool = BuscarHotelesTool(provider_resolver=_resolver_fijo(provider))

    resultado = await tool.run(
        make_ctx(), {"ciudad": "MDE", "checkin": "2026-09-01", "checkout": "2026-09-05"}
    )

    assert "no encontré hoteles" in resultado.content.lower()
    assert resultado.presentation[0]["actions"][0]["action"] == "open_url"
    assert "google.com/travel" in resultado.presentation[0]["actions"][0]["url"]
...
async def test_rastrear_paquete_con_stub_marca_data_como_demo_y_avisa_en_el_texto(make_ctx):
    """Antes, `data=asdict(rastreo)` no llevaba NINGUNA marca de demo/real — un
    `StubTrackingProvider` real (`name = "stub"`, `packages/travel/edecan_travel/
    providers.py`) devolvía `estado="InTransit"` para CUALQUIER número de guía sin que
    `data.estado` dijera que es un ejemplo. Este test usa el stub REAL (no el fake local del
    resto de este archivo) para probar la clasificación end-to-end vía `_fuente_viaje`."""
    from edecan_travel.providers import StubTrackingProvider

    tool = RastrearPaqueteTool(provider_resolver=_resolver_fijo(StubTrackingProvider()))
    ...
    assert "no hay cuenta de rastreo" in resultado.content.lower()
