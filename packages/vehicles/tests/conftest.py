"""Fixtures compartidas de `edecan_vehicles` (ver `ARCHITECTURE.md` §10.1, §10.15).

El bloque de `sys.path` de abajo se ejecuta primero (por su efecto
secundario: agrega el propio código fuente de este paquete,
`packages/vehicles/`, a `sys.path`) para que `import edecan_vehicles`
...
misma técnica que `packages/smarthome/tests/conftest.py` y
`packages/messaging/tests/conftest.py`. Una vez `packages/vehicles` queda
...
`packages/smarthome/tests/conftest.py`/`packages/creative/tests/conftest.py`):
ningún test de este paquete importa `edecan_db`/`edecan_schemas` para
construir sus dobles — `ctx.session`/`ctx.vault` se completan con objetos
locales que solo implementan lo que `edecan_vehicles` realmente usa, y `ctx`
en sí es un `SimpleNamespace` (no `edecan_core.ToolContext`), aunque
`edecan_core` SÍ debe estar instalado/en `sys.path` para poder importar el
código de PRODUCCIÓN de este paquete, que sí lo usa.
"""
...
class FakeResult:
    ...
    (`.mappings().first()` — ver `edecan_vehicles.providers.get_tenant_vehicle_provider`)."""
...
@dataclass
class FakeVault:
    """`ctx.vault` falso: `get()` devuelve siempre el mismo `bundle` (o
    `None`); `put()` registra cada escritura en `puts` — usado para verificar
    que la rotación del `refresh_token` de Smartcar se persiste de vuelta."""

    bundle: Any = None
    llamadas_get: list[tuple[Any, Any]] = field(default_factory=list)
    puts: list[tuple[Any, Any, Any]] = field(default_factory=list)

    async def get(self, tenant_id: Any, connector_account_id: Any) -> Any:
        self.llamadas_get.append((tenant_id, connector_account_id))
        return self.bundle
