...
async def test_vehiculo_controlar_modo_demo_avisa_pegado_al_verbo_no_al_final(make_ctx):
    """Antes el aviso de modo demo iba SOLO al final de la frase, después de la
    afirmación completa en pasado ("Bloqueé las puertas del vehículo «...».") — fácil de
    no leer antes de creer que se tocó un vehículo real. Ahora tiene que estar en la
    MISMA cláusula que el verbo, no después del punto."""
    resultado = await VehiculoControlarTool().run(
        make_ctx(), {"vehicle_id": STUB_VEHICLE_ID, "accion": "bloquear"}
    )
    primera_oracion = resultado.content.split(".")[0]
    assert "modo demo" in primera_oracion.lower()
    assert "no toqué ningún vehículo real" in resultado.content.lower()
    assert resultado.data["demo"] is True
    assert resultado.is_error is False


async def test_vehiculo_controlar_sin_vehicle_id_es_error(make_ctx):
    resultado = await VehiculoControlarTool().run(make_ctx(), {"accion": "bloquear"})
    assert resultado.is_error is True


async def test_vehiculo_controlar_accion_invalida_es_error(make_ctx):
    resultado = await VehiculoControlarTool().run(
        make_ctx(), {"vehicle_id": STUB_VEHICLE_ID, "accion": "arrancar"}
    )
    assert resultado.is_error is True
