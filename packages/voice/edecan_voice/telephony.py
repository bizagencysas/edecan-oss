...
def normalize_twilio_status(value: Any) -> str:
    """Mapea el CallStatus de Twilio a los estados internos.

    Vacío o desconocido NO se convierte en failed: el statusCallback del
    <Stream> a menudo llega sin CallStatus (stream-started). Tratarlo como
    failed cerraba la llamada a los 400 ms con un resumen vacío.
    """
    ...
    return status if status in CALL_STATUSES else ""
    ...
    async def create_call(
        ...
        *,
        ...
        try:
            body = response.json()
            sid = str(body["sid"])
            status = normalize_twilio_status(body.get("status", "queued")) or "queued"
...
def stream_twiml(
    *,
    ...
    """TwiML de Media Streams: Twilio abre un WebSocket y manda audio μ-law 8 kHz.

    Es el contrato de REFERENCIA (`<Connect><Stream>`), no el de Gather/Play. El
    `call_id` y el `From` viajan como parámetros custom: Twilio no los mete
    solo en el WebSocket.
    """
    root = Element("Response")
    connect = SubElement(root, "Connect")
    stream_attrs = {"url": stream_url}
    if status_callback_url:
        stream_attrs["statusCallback"] = status_callback_url
        stream_attrs["statusCallbackMethod"] = "POST"
    stream = SubElement(connect, "Stream", stream_attrs)
    SubElement(stream, "Parameter", {"name": "call_id", "value": call_id})
    if inbound:
        SubElement(stream, "Parameter", {"name": "inbound", "value": "1"})
    if from_number:
        SubElement(stream, "Parameter", {"name": "from", "value": from_number})
    return _xml(root)
