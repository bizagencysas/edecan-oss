...
import aioboto3
from edecan_core import Fidelidad, InfoFidelidad, ProveedorDeclarado, Tool, ToolContext, ToolResult
...
def _info_fidelidad_tts(provider: TTSProvider) -> InfoFidelidad:
    """`ToolResult.fidelidad` para `sintetizar_voz` — contrato de veracidad
    (`edecan_core.veracidad`, ver docstring del módulo ahí).

    `StubTTS`/`ElevenLabsTTS`/`PollyTTS` (los tres proveedores reales de este
    paquete) heredan `ProveedorDeclarado` y siempre tienen `info_fidelidad()`.
    El único caso sin ese método es un `TTSProvider` inyectado a mano en un
    test (`FakeTTSProvider` en `packages/voice/tests/test_voice_tools.py`,
    por duck typing, sin heredar el contrato) — fail-closed: se trata como
    SIMULADO en vez de asumir que es real sin haberlo comprobado, mismo
...
class SintetizarVozTool(Tool):
    ...
    async def run(self, ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
        ...
        # Contrato de veracidad (`edecan_core.veracidad`): antes, `es_stub` de
        # arriba se calculaba y solo se usaba para elegir la extensión del
        # archivo — nunca para avisar. `_info_fidelidad_tts` cubre también los
        # proveedores inyectados por tests que no heredan `ProveedorDeclarado`
        # (fail-closed: si no declara, se trata como simulado, nunca como
        # real sin pruebas).
        fidelidad = _info_fidelidad_tts(provider)
        ...
        # Honestidad EN LA FUENTE, no solo en el contrato: aunque
        # `ToolResult.fidelidad` ya lleva este mismo dato a `Agent.run_turn`
        # (que lo antepone para el modelo y lo expone a la app vía
        # `ToolEndEvent.fidelidad`), el texto de `content` no debe decir "ya
        # lo puedes escuchar" sobre 0.5s de silencio ni aunque, algún día,
        # alguna superficie muestre este `content` sin pasar por ese
        # contrato — ver `edecan_core.veracidad` y la medición que motivó
        # este cambio (WAV de silencio reportado como audio real).
        if fidelidad.fidelidad is Fidelidad.SIMULADO:
            mensaje = (
                f"Convertí «{preview}» a un archivo de audio («{filename}»), pero NO es tu "
                f"voz: {fidelidad.motivo_simulado}. Lo que guardé es {fidelidad.fuente}, no una "
                "síntesis real — no lo confundas con lo que pediste."
            )
        else:
            mensaje = (
        ...
        return ToolResult(
            ...
            data={
                ...
                "mime": mime,
                "caption": preview,
                "source_mode": fidelidad.fidelidad.value,
            },
            fidelidad=fidelidad,
        )
...
_ENV_CONFIRMACION_LLAMADA = "EDECAN_PHONE_REQUIRE_CONFIRMATION"


def _requiere_confirmacion_de_llamada() -> bool:
    """Si `llamar_contacto` exige la tarjeta de confirmación antes de marcar.

    Por defecto SÍ: para quien recién instala Edecán, que un modelo marque un teléfono real
    sin un freno explícito es demasiado. Pero cuando el propio dueño ya escribió el número,
    el destinatario, el objetivo y el agente en su mensaje, la tarjeta le pregunta justo lo
    que acaba de decir -- ahí el freno solo estorba, y por eso se puede apagar con
    `EDECAN_PHONE_REQUIRE_CONFIRMATION=0`.

    Apagarlo NO desactiva las otras defensas: la tool sigue exigiendo los cuatro datos
    completos, el número sigue normalizándose a E.164, y el consentimiento por destinatario
    sigue vigente (`repo.has_phone_consent`).
    """
    valor = os.getenv(_ENV_CONFIRMACION_LLAMADA)
    if valor is None:
        return True
...
def get_all_tools() -> list[Tool]:
    ...
    llamar = LlamarContactoTool()
    # `dangerous` se lee por instancia en `Agent.run_turn`, así que basta con bajarlo acá:
    # no hace falta una subclase ni tocar la definición de la tool.
    llamar.dangerous = _requiere_confirmacion_de_llamada()
    return [
        ListarVocesTool(),
        SintetizarVozTool(),
