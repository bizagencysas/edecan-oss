...
def test_twilio_status_vacio_o_desconocido_no_es_failed() -> None:
    assert normalize_twilio_status("") == ""
    assert normalize_twilio_status(None) == ""
    assert normalize_twilio_status("stream-started") == ""
...
def test_stream_twiml_inyecta_call_id_y_from_como_jarvis() -> None:
    xml = stream_twiml(
        ...
        inbound=True,
        status_callback_url="https://edecan.test/v1/phone/twilio/status",
    )
    assert "<Connect>" in xml
    assert 'url="wss://edecan.test/v1/phone/twilio/media"' in xml
    assert 'name="call_id" value="inbound-uuid"' in xml
    assert 'name="from" value="+573001234567"' in xml
    assert 'name="inbound" value="1"' in xml
    assert 'statusCallback="https://edecan.test/v1/phone/twilio/status"' in xml
