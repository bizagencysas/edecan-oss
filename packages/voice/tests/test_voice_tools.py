...
class FakeTTSProvider(ProveedorDeclarado):
    """`TTSProvider` falso: devuelve bytes fijos y registra la última llamada.

    Hereda `ProveedorDeclarado` (`edecan_core.veracidad`) y declara
    `fidelidad=REAL`: en los tests de este archivo representa un proveedor
    bring-your-own YA CONFIGURADO (el escenario "el tenant sí conectó algo"),
    no el fallback stub — ver `test_voice_veracidad.py` para el caso
    contrario (proveedor que NO declara nada, tratado fail-closed como
    simulado)."""
